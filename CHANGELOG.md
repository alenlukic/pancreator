...
- Store the Cursor model catalog's `models` array in a lossless compact encoding: per-model variant grids become `variant_keys` plus one value tuple per variant, with the default variant and divergent variant display names recorded separately (167KB → 47KB). The compressor proves losslessness by expanding its own output and comparing deep-equal against the input — a model the encoding cannot represent stays verbatim — and the loader accepts both forms, so a catalog refresh can paste the raw `Cursor.models.list()` result unchanged ([codec](src/lib/executors/cursor-catalog-codec.ts), [codec test](tests/unit/cursor-catalog-codec.test.ts), [registry](governance/registries/cursor_model_catalog.json)).
- Add `governance/registries/cursor_model_catalog.json`: the verbatim `Cursor.models.list()` result wrapped with retrieval provenance and Cursor's documented spec grammar, as the single authoritative record of Cursor models and per-model parameters. Persona mappings are validated against it at config load and projection — model id or alias, parameter names, and parameter values, all per model — and an unknown identifier fails loudly with a refresh pointer, because Cursor's own failure mode for an unusable spec is silent fallback to a default variant ([registry](governance/registries/cursor_model_catalog.json), [cursor-catalog](src/lib/executors/cursor-catalog.ts), [pipeline-config](src/lib/pipeline-config.ts)).
