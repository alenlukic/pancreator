...
- Print one idempotent `pan cohort integrate <cohort-id>` as the manual command of a failed release continuation. Replace the `init` and `set-stage` pair the harness could never adopt ([cohorts](src/lib/cohorts.ts)).
...
- Record a bare agent-run `pan repository-check <profile>` against every live run of its workspace. Print the exact check command on the evidence worker brief ([repository-checks](src/lib/repository-checks.ts), [state](src/lib/state.ts), [render](src/lib/render.ts)).
