---
title: Command Center Automations list and run history under ten-surface shell Engineering Spec
...
source_inbox_item: lib/inbox/in/172966_06-09-26/63489_0621_command-center-automations-shell-history.md
depends_on:
...
next_stage: plan
ux_spec: lib/memory/features/command-center-ux-philosophy-information-architecture-and-user-stories/ux-spec.md
intake_closure:
...
intake_notes:
  ...
  - The canonical shell dependency slug is command-center-app-shell-navigation-rail-and-operator-theme-tokens rather than the program shorthand command-center-shell-theme-foundation cited in the directive depends_on field.
  - The ten-surface route client/src/app/(command-center)/automations/page.tsx currently renders CommandCenterSurfacePlaceholder; AutomationsModule still mounts inside the superseded three-module DashboardModuleShell tab on DashboardPage.
  - Pause/disable guarded confirm satisfies ux-spec §4.11 in this Feature; Activity Log event emission for pause actions defers to command-center-activity-log per the directive Out of scope section.
...
references:
  - kind: lines
    path: lib/inbox/in/172966_06-09-26/63489_0621_command-center-automations-shell-history.md
    range: [44, 47]
  ...
  - kind: lines
    path: lib/memory/features/command-center-automation-registry-and-management-ui/delivery-report.md
    range: [1, 6]
    note: Shipped registry CRUD, AutomationsModule list, wizard, and API routes under /api/automations.
  - kind: lines
    path: lib/memory/features/command-center-local-scheduler-tick-and-run-history/delivery-report.md
    range: [1, 6]
...
This Feature SHALL re-home Command Center automations list, create wizard, and run
...
APIs from `command-center-automation-registry-and-management-ui` and
`command-center-local-scheduler-tick-and-run-history` without changing tick
execution or JSONL schemas. The Feature SHALL replace the
`CommandCenterSurfacePlaceholder` on the Automations route, SHALL migrate
`AutomationsModule` out of the superseded three-module `DashboardModuleShell` tab, and
...
- When an operator navigates to `/automations`, the ten-surface shell SHALL render
  ...
  inside `client/src/app/(command-center)/automations/page.tsx` instead of
  `CommandCenterSurfacePlaceholder`.
...
- When Automations content mounts in the ten-surface shell, layout chrome SHALL
  use existing `command-center-app-shell-navigation-rail-and-operator-theme-tokens`
...
- When pause or resume succeeds, Activity Log event emission SHALL remain out of
  ...
  `command-center-activity-log` owns durable event recording.
...
- When `pnpm --filter client test` executes, focused tests under
  `client/src/components/command-center/automations/` and Automations route tests
  SHALL pass.
...
- Activity Log event emission for pause, resume, run, or retry actions;
  `command-center-activity-log` owns durable stream recording per source directive.
...
- Scheduler tick dispatch, concurrency lock logic, JSONL append schemas, or
  CLI subcommand changes; `command-center-local-scheduler-tick-and-run-history`
...
- LangGraph wiring, cloud scheduler hosting, or Cursor Automations product API
  coupling.
- Replacing the superseded three-module `DashboardModuleShell` entirely; this Feature
