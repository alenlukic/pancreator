---
title: Command Center active memory, inbox triage, and multi-run view UX Spec
feature_id: command-center-active-memory-inbox-triage-multi-run-view
task_id: 49726_1011_command-center-active-memory-inbox-triage-multi-run-view
program: command-center
stage: plan
owner: design-engineer
status: draft
design_steps: true
depends_on: [command-center-live-run-refresh-and-stage-artifact-drawer, command-center-ux-spec-and-information-architecture]
references:
  - kind: lines
...
**Breakpoints:** inherit parent Command Center rules (≥1024px two-column; 768–1023px stacked; <768px horizontal scroll on tables).

## Visual design tokens

Reuse Command Center tokens from `client/src/app/globals.css`. Add scoped classes under `/* command-center orientation */` without altering existing stage-status or live-refresh semantics.
...
- **Copy path** — `CopyCommandButton` copies full repo-relative path (e.g. `lib/inbox/in/172967_06-08-26/54352_0854_command-center-pipeline-orientation.md`); 2s Copied tooltip with `aria-live="polite"`.
...
```yaml
contract:
  id: command-center-active-memory-inbox-triage-multi-run-view.ux.multi-run-row-selection
  kind: llm-judge
  severity: block
  applies_to:
  ...
  metadata:
    pancreator.contract_id: command-center-active-memory-inbox-triage-multi-run-view.ux.multi-run-row-selection
    pancreator.applies_to: artifact-symbol:/lib/memory/features/command-center-active-memory-inbox-triage-multi-run-view/ux-spec.md#Multi-run-table
    pancreator.wcag-criteria: ["2.1.1", "4.1.2"]
```

```yaml
contract:
  id: command-center-active-memory-inbox-triage-multi-run-view.ux.execute-confirm-modal
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: command-center-active-memory-inbox-triage-multi-run-view.ux.execute-confirm-modal
    pancreator.applies_to: artifact-symbol:/lib/memory/features/command-center-active-memory-inbox-triage-multi-run-view/ux-spec.md#Guarded-execute-affordances
    pancreator.wcag-criteria: ["2.1.1", "2.4.3"]
```

```yaml
contract:
  id: command-center-active-memory-inbox-triage-multi-run-view.ux.active-memory-fields
  kind: llm-judge
  severity: warn
  applies_to:
    kind: artifact-symbol
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: command-center-active-memory-inbox-triage-multi-run-view.ux.active-memory-fields
    pancreator.applies_to: artifact-symbol:/lib/memory/features/command-center-active-memory-inbox-triage-multi-run-view/ux-spec.md#Active-memory-header
