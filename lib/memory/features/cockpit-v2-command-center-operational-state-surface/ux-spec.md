---
title: Cockpit v2 Command Center operational state surface UX Spec
...
references:
  - kind: lines
    path: lib/memory/features/cockpit-v2-command-center-operational-state-surface/spec.md
    range: [106, 214]
...
Reuse Cockpit v2 tokens from the ratified parent ux-spec and `client/src/app/globals.css`. Do not introduce per-card one-off spacing.
...
- **Extract** — refactor `HumanGateBanner.tsx` gate listing and `NextActionPanel.tsx` overflow affordances into `client/src/components/cockpit/command-center/` (or shared helpers consumed by Command Center rows). Gate dismiss from the prior banner is out of scope; all gates remain visible in **Needs you** until remediated.
...
```yaml
contract:
  id: cockpit-v2-command-center-operational-state-surface.ux.command-center-operational-state
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
    path: /lib/memory/features/cockpit-v2-command-center-operational-state-surface/ux-spec.md
  ...
  metadata:
    pancreator.contract_id: cockpit-v2-command-center-operational-state-surface.ux.command-center-operational-state
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-command-center-operational-state-surface/ux-spec.md#Interaction-requirements
    pancreator.wcag-criteria: ["1.4.3", "2.1.1", "4.1.2"]
```

```yaml
contract:
  id: cockpit-v2-command-center-operational-state-surface.ux.loading-error-empty
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: cockpit-v2-command-center-operational-state-surface.ux.loading-error-empty
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-command-center-operational-state-surface/ux-spec.md#Interaction-requirements
