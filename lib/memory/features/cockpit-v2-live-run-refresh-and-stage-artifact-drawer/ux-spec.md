---
title: Cockpit v2 live run refresh and stage artifact drawer UX Spec
feature_id: cockpit-v2-live-run-refresh-and-stage-artifact-drawer
task_id: 51057_0949_cockpit-v2-live-run-refresh-and-stage-artifact-drawer
program: cockpit-v2
stage: plan
...
references:
  ...
  - kind: lines
    ...
    range: [96, 136]
    note: Ratified Cockpit v2 tokens, artifact drawer intent, and P10 viewing guards.
  - kind: lines
    path: lib/memory/features/cockpit-v2-pipeline-command-center-and-human-gate-queue/ux-spec.md
    range: [48, 69]
...
Reuse Cockpit v2 tokens from `client/src/app/globals.css`. Add scoped classes under `/* cockpit-v2 live refresh */` without altering existing P9 stage-status semantics.
...
sequenceDiagram
  ...
  Op->>Pipe: Open Cockpit while SDK run active
...
```yaml
contract:
  id: cockpit-v2-live-run-refresh-and-stage-artifact-drawer.ux.active-stage-live-indicator
  kind: llm-judge
  severity: block
  applies_to:
  ...
  metadata:
    pancreator.contract_id: cockpit-v2-live-run-refresh-and-stage-artifact-drawer.ux.active-stage-live-indicator
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-live-run-refresh-and-stage-artifact-drawer/ux-spec.md#Running-stage-indicator
    pancreator.wcag-criteria: ["1.4.3", "1.4.11"]
```

```yaml
contract:
  id: cockpit-v2-live-run-refresh-and-stage-artifact-drawer.ux.artifact-drawer-p10-readonly
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: cockpit-v2-live-run-refresh-and-stage-artifact-drawer.ux.artifact-drawer-p10-readonly
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-live-run-refresh-and-stage-artifact-drawer/ux-spec.md#Stage-artifact-drawer
    pancreator.wcag-criteria: ["2.1.1", "4.1.2"]
```

```yaml
contract:
  id: cockpit-v2-live-run-refresh-and-stage-artifact-drawer.ux.timeline-telemetry-badges
  kind: llm-judge
  severity: warn
  applies_to:
    kind: artifact-symbol
      ...
      examples:
        ...
        bad:
          - text: "Escalation records render as plain text only; active cell has no telemetry chip."
            rationale: Telemetry requirements are invisible in the cockpit surface.
    panel:
      quorum: 2-of-3
      judges: [haiku, haiku, sonnet]
      seed: 42
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: cockpit-v2-live-run-refresh-and-stage-artifact-drawer.ux.timeline-telemetry-badges
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-live-run-refresh-and-stage-artifact-drawer/ux-spec.md#Run-event-timeline-merge
```
