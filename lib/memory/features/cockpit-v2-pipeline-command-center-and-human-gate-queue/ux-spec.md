---
title: Cockpit v2 Pipeline command center and human gate queue UX Spec
...
references:
  ...
  - kind: lines
    ...
    range: [44, 136]
    note: Ratified Cockpit v2 shell, tokens, shared affordances, and Pipeline module baseline.
  - kind: lines
    path: lib/memory/features/cockpit-v2-ux-spec-and-information-architecture/ux-spec.md
    range: [168, 183]
    note: Component inventory paths under client/src/components/cockpit/.
  - kind: lines
...
**Automations / Maintenance placeholders** — single dashed empty state with module name and “Coming in a later Cockpit v2 item”; no wizard or audit UI in this feature.

## Visual design tokens

Reuse Cockpit v2 tokens from `client/src/app/globals.css` (`--surface-primary`, `--surface-elevated`, `--surface-attention`, `--text-primary`, `--text-muted`, `--accent`, `--space-*`). Do not alter existing P9 stage-status classes (`.stage-cell-pending`, `.stage-cell-active`, `.stage-cell-complete`, `.stage-cell-failed`).
...
- **Extracted components** — `layout/CockpitShell`, `pipeline/NextActionPanel`, `pipeline/HumanGateBanner`, `pipeline/ConfigReadOnlyPanel`, and `shared/CopyCommandButton`, `shared/AttentionBanner`, `shared/LoadingState`, `shared/EmptyState`, `shared/ErrorState`.
...
```yaml
contract:
  id: cockpit-v2-pipeline-command-center-and-human-gate-queue.ux.copy-command-feedback
  kind: llm-judge
  severity: block
  applies_to:
  ...
  metadata:
    pancreator.contract_id: cockpit-v2-pipeline-command-center-and-human-gate-queue.ux.copy-command-feedback
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-pipeline-command-center-and-human-gate-queue/ux-spec.md#Interaction-requirements
    pancreator.wcag-criteria: ["4.1.2"]
```

```yaml
contract:
  id: cockpit-v2-pipeline-command-center-and-human-gate-queue.ux.human-gate-banner
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: cockpit-v2-pipeline-command-center-and-human-gate-queue.ux.human-gate-banner
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-pipeline-command-center-and-human-gate-queue/ux-spec.md#Interaction-requirements
...
```yaml
contract:
  id: cockpit-v2-pipeline-command-center-and-human-gate-queue.ux.config-readonly
  kind: llm-judge
  severity: warn
  applies_to:
    kind: artifact-symbol
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: cockpit-v2-pipeline-command-center-and-human-gate-queue.ux.config-readonly
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-pipeline-command-center-and-human-gate-queue/ux-spec.md#Interaction-requirements
```
