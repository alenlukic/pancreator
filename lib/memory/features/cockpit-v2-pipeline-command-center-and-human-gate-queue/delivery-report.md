# Delivery Report — Cockpit v2 Pipeline command center and human gate queue

## Summary

This feature delivery ships the Cockpit v2 Pipeline command center: run-state field split, enriched task envelopes, a read-only config API, CockpitShell extraction, and P9 grid and timeline migration. The server exposes `humanAttention` and `nextCommand` as separate `StageCell` fields, adds `runDir` and optional `inboxSource` to each `TaskRunStateEnvelope`, and serves live runtime policy through `GET /api/config`. The client renders `CockpitShell` with Pipeline as the default module, a sticky Next Action panel, a global human gate queue banner, and a read-only config panel. Review passed with `review_passes: true`, test passed with `qa_passes: true`, and 33 focused client tests pass after relocating gate helpers into client-safe `run-state-shared.ts`.
...
- The client SHALL render `CockpitShell` with Pipeline as the default module, compose the sticky Next Action panel, global human gate queue, read-only config panel, and migrated P9 grid and timeline, and SHALL preserve P10 read-only defaults and write guards on Files-tab actions.
...
- Gate-queue helpers and the `HumanGateQueueEntry` type SHALL reside in `run-state-shared.ts` so `"use client"` cockpit components import only client-safe modules and production build avoids transitive Node built-in loading from `run-state.ts`.
...
- `CockpitShell` exposes module tabs (`pipeline`, `automations`, `maintenance`, `files`); pipeline surfaces include `NextActionPanel`, `HumanGateBanner`, `ConfigReadOnlyPanel`, `StageMachineGrid`, and `RunEventTimeline`; shared affordances include `CopyCommandButton`, `AttentionBanner`, `LoadingState`, `EmptyState`, and `ErrorState`.
...
{
  "kind": "lines",
  "path": "client/src/components/cockpit/pipeline/ConfigReadOnlyPanel.tsx",
  "range": [8, 8],
  "contentHash": "b4b6ccc"
}
```
...
- Gate artifact links in `HumanGateBanner` open `/api/file?path=…` anchor hrefs instead of routing through the Files-tab artifact modal; operators may leave the cockpit shell context when following gate links.
...
- To render the default Pipeline module with shell tabs, mount `DashboardPage` and assert `cockpit-shell`, `module-tab-pipeline`, and `pipeline-module` test ids after `GET /api/run-state` resolves.
