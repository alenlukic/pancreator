---
title: Command Center Command Center operational state surface Engineering Spec
...
source_inbox_item: lib/inbox/in/172966_06-09-26/63491_0621_command-center-command-center.md
depends_on:
  - command-center-shell-theme-foundation
  - command-center-ux-philosophy-information-architecture-and-user-stories
blocks:
  - command-center-compliance-recovery
...
intake_notes:
  ...
  - The canonical ux-spec authority path is lib/memory/features/command-center-ux-philosophy-information-architecture-and-user-stories/ux-spec.md; contract command-center-ux-philosophy-information-architecture-and-user-stories.ux.command-center-operational-state applies at design review.
  - Prior command-center-pipeline-command-center-and-human-gate-queue shipped Next Action panel and human gate queue inside the superseded three-module Pipeline shell; this Feature migrates those patterns into the Command Center default landing without restoring Pipeline as the default module tab.
references:
  - kind: lines
    path: lib/inbox/in/172966_06-09-26/63491_0621_command-center-command-center.md
    range: [38, 42]
  ...
  - kind: lines
    path: lib/memory/features/command-center-pipeline-command-center-and-human-gate-queue/delivery-report.md
    range: [34, 67]
...
This Feature SHALL deliver the Command Center Command Center as the default operator
...
`command-center-ux-philosophy-information-architecture-and-user-stories.ux.command-center-operational-state`,
compose atop `GET /api/run-state` without replacing P9 aggregation, mount inside
the ten-surface shell from `command-center-shell-theme-foundation`, and migrate
...
- When the latest compliance audit exposes open findings, the **Compliance
  ...
  `command-center-quick-fix` ships invocation.
...
- When implement stage completes, human gate queue logic from
  ...
  `client/src/components/command-center/command-center/` or shared client-safe helpers
...
- When the ten-surface shell renders, Command Center SHALL NOT require the
  ...
  `client/src/components/command-center/pipeline/` for legacy routes but SHALL NOT
  duplicate the Command Center card regions as the primary orientation surface.
- When `DashboardPage.tsx` or the `(command-center)` app routes mount Command Center,
...
- When `pnpm --filter client test` executes, Command Center component and data
  tests under `client/src/components/command-center/command-center/` SHALL pass.

## Out of scope

- FD Mission Control run detail, stage rail, retry-limit banner, and artifact
  drawer; the `command-center-fd-mission-control` inbox item owns that surface per
  source directive.
- Quick fix invocation backend and diff acceptance flow; Command Center CTAs
  MAY stub or deep-link until `command-center-quick-fix` ships per source directive.
- Full Compliance + Recovery grouped list view; the
  `command-center-compliance-recovery` inbox item owns that surface per source
  directive.
- Real-time Activity Log stream, filters, and virtualization; the
  `command-center-activity-log` inbox item owns that surface per source directive.
- Replacing `GET /api/run-state`, mutating pipeline APIs, or changing
  `state.json` and `run.log.jsonl` schemas.
- Restoring the superseded three-module DashboardModuleShell tabs as the default operator
