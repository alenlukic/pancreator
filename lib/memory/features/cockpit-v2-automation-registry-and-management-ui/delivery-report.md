# Delivery Report — Cockpit v2 automation registry and management UI

## Summary

This feature delivery ships a Pancreator-native automation registry and Cockpit Automations management UI. The `@pancreator/scheduler` package validates schema v1 YAML, enforces path guards under `.pan/automations/`, and exposes registry CRUD plus enabled-only due filtering. Next.js routes at `/api/automations` wrap the primitive server-side so the client never reads `.pan/` directly. `CockpitShell` renders `AutomationsModule` with a two-column list, run-history stub, inline enabled toggles, and a four-step create/edit wizard. Run now and run-history population remain disabled until `cockpit-v2-automations-scheduler` ships. Review passed with `review_passes: true`, test passed with `qa_passes: true`, and 54 focused tests pass across scheduler, API, UI, and browser-safe client helpers.
...
- The Automations UI SHALL render a two-column list plus stub run-history panel, inline enabled toggles, and a four-step Schedule → Persona → Prompt → Review wizard; Run now SHALL render disabled with helper text citing `cockpit-v2-automations-scheduler`.
...
- Cockpit UI surfaces include `AutomationsModule`, `AutomationListView`, `AutomationWizardShell` with Schedule/Persona/Prompt/Review steps, `AutomationRunHistory` empty state, `StatusBadge`, and shared `useFocusTrap` for delete-confirm dialogs.
...
- Scheduler tick execution and run-log population SHALL remain deferred to `cockpit-v2-automations-scheduler`; Run now renders disabled and run history shows a dashed empty state.
