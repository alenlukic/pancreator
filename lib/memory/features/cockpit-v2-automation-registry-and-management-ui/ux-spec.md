---
title: Cockpit v2 automation registry and management UI UX Spec
...
references:
  ...
  - kind: lines
    ...
    range: [50, 81]
    note: Ratified Cockpit v2 shell, Automations wireframe, and module tab hierarchy.
  - kind: lines
    path: lib/memory/features/cockpit-v2-ux-spec-and-information-architecture/ux-spec.md
    range: [96, 141]
  ...
  - kind: lines
    path: lib/memory/features/cockpit-v2-ux-spec-and-information-architecture/ux-spec.md
    range: [168, 183]
    note: Component inventory paths under client/src/components/cockpit/automations/.
  - kind: lines
...
# Cockpit v2 automation registry and management UI UX Spec

## Overview

This feature replaces the Cockpit Automations placeholder with a full registry management surface so operators create, edit, pause, and delete recurring agent automations without OS cron or ad-hoc scripts. The Automations module loads validated records from `GET /api/automations`, persists changes through CRUD routes to `.pan/automations/<id>.yaml`, and presents a list view with inline enable toggles plus a four-step create/edit wizard (Schedule → Persona → Prompt → Review). Run now and run-history population remain stubbed until `cockpit-v2-automations-scheduler` ships. Shell layout, module tabs, design tokens, and shared loading/empty/error affordances inherit from the ratified Cockpit v2 UX spec.

## Layout and navigation

- **Shell authority** — `CockpitShell` renders `AutomationsModule` when the operator selects the Automations tab; Pipeline and Maintenance modules remain unchanged.
...
**Breakpoints:** inherit parent Cockpit v2 rules (≥1024px two-column; 768–1023px stacked; <768px horizontal scroll on list table).

## Visual design tokens

Reuse Cockpit v2 tokens from `client/src/app/globals.css` (`--surface-primary`, `--surface-elevated`, `--surface-attention`, `--text-primary`, `--text-muted`, `--accent`, `--space-*`). Add scoped classes under `/* cockpit-v2 automations */` without altering Pipeline or Maintenance semantics.
...
- **Run now** — every Run now control SHALL render disabled (`aria-disabled="true"`) with helper text: "Run now ships with cockpit-v2-automations-scheduler." Helper text uses `--text-muted` and appears on hover/focus of the disabled control.
...
Extract under `client/src/components/cockpit/automations/`:
...
```yaml
contract:
  id: cockpit-v2-automation-registry-and-management-ui.ux.list-enabled-toggle
  kind: llm-judge
  severity: block
  applies_to:
  ...
  metadata:
    pancreator.contract_id: cockpit-v2-automation-registry-and-management-ui.ux.list-enabled-toggle
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-automation-registry-and-management-ui/ux-spec.md#Interaction-requirements
    pancreator.wcag-criteria: ["2.1.1", "4.1.2"]
```

```yaml
contract:
  id: cockpit-v2-automation-registry-and-management-ui.ux.wizard-four-steps
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: cockpit-v2-automation-registry-and-management-ui.ux.wizard-four-steps
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-automation-registry-and-management-ui/ux-spec.md#Interaction-requirements
    pancreator.wcag-criteria: ["2.1.1", "4.1.2"]
```
...
```yaml
contract:
  id: cockpit-v2-automation-registry-and-management-ui.ux.run-now-stub
  kind: llm-judge
  severity: warn
  applies_to:
    kind: artifact-symbol
  ...
  description: |
    ...
    cockpit-v2-automations-scheduler; the run-history panel SHALL render
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: cockpit-v2-automation-registry-and-management-ui.ux.run-now-stub
    pancreator.applies_to: artifact-symbol:/lib/memory/features/cockpit-v2-automation-registry-and-management-ui/ux-spec.md#Interaction-requirements
```
