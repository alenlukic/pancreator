---
title: Command Center Feature Delivery Mission Control run detail Engineering Spec
...
source_inbox_item: lib/inbox/in/172966_06-09-26/63490_0621_command-center-fd-mission-control.md
depends_on:
...
intake_notes:
  ...
  - P9 `StageMachineGrid` and `RunEventTimeline` SHALL migrate into `client/src/components/command-center/mission-control/` while `client/src/components/command-center/pipeline/` retains thin re-exports for the Pipeline module regression surface.
  - Remediation CTAs Retry stage, Retry with config, Run quick fix, Mark issue resolved, and Cancel run MAY stub with operator-visible not-implemented toasts until `command-center-quick-fix` lands; the banner and strip MUST still render per ux-spec §4.4.
  - Contract `command-center-ux-philosophy-information-architecture-and-user-stories.ux.fd-mission-control-stage-rail` is the blocking llm-judge gate at threshold 0.75.
references:
  - kind: lines
    path: lib/inbox/in/172966_06-09-26/63490_0621_command-center-fd-mission-control.md
    range: [38, 42]
  ...
  - kind: lines
    path: lib/memory/features/command-center-live-run-refresh-and-stage-artifact-drawer/delivery-report.md
    range: [34, 76]
...
This Feature SHALL deliver the Command Center FD Mission Control surface at route
...
`client/src/components/command-center/mission-control/`, SHALL compose
`GET /api/run-state` with the live polling patterns from
`command-center-live-run-refresh-and-stage-artifact-drawer`, and SHALL satisfy
contract
`command-center-ux-philosophy-information-architecture-and-user-stories.ux.fd-mission-control-stage-rail`
...
- When the Pipeline module imports `StageMachineGrid` or `RunEventTimeline` from
  `client/src/components/command-center/pipeline/`, those modules SHALL re-export the
  canonical implementations from `client/src/components/command-center/mission-control/`
...
- Command Center hub cards and operational-state rows; the
  `command-center-command-center` Feature owns that surface per source directive.
- Out-of-band quick fix persona invocation and diff review flow; remediation
  CTAs MAY stub until `command-center-quick-fix` lands per source directive.
- Global compliance queue grouping and re-run compliance check actions; the
  `command-center-compliance-recovery` program item owns that surface per source
  directive.
- Work Intake kickoff stepper, model presets, and Launch feature delivery flow;
  the `command-center-work-intake-kickoff` Feature owns that surface per source
  directive.
