...
next_stage: plan
source_inbox_item: lib/inbox/in/172979_05-27-26/72021_0359_feature-delivery-cursor-runner-harness-wiring/72021_0359_feature-delivery-cursor-runner-harness-wiring.md
intake_round: 0
...
references:
  - kind: lines
    path: lib/inbox/in/172979_05-27-26/72021_0359_feature-delivery-cursor-runner-harness-wiring/72021_0359_feature-delivery-cursor-runner-harness-wiring.md
    range: [69, 95]
    contentHash: 6eeaa2f
...
|---|---|---|
| Stage inventory and persona ownership | `lib/pipelines/feature-delivery.yaml` lines 25–67 | Reference only; no edits in this Feature. |
| Manual `pan run` / `pan advance` semantics | `OPERATION.md` lines 26–94 | Reference only; manual remains the default. |
| Implement-stage obligations after loopback | `lib/personas/coder.md` lines 81–167 | Reference only; consumed when automation routes to `implement`. |
| Review-stage gate semantics | `lib/personas/reviewer.md` lines 90–145 | Reference only; consumed when automation routes back from `review`. |
| Test-stage routing and severity | `lib/personas/qa-tester.md` lines 89–180 | Reference only; consumed when automation routes back from `test`. |
| Outbox naming conventions | `lib/memory/features/timestamp-naming-conventions/spec.md` lines 77–82 | Reference only; halt artifact and report-gate artifact MUST conform. |
| Inbox path layout | `lib/memory/handbook/inbox-lifecycle.md` lines 69–75 | Reference only; halt and gate artifacts target `lib/inbox/out/<day-bucket>/`. |
...
- When the implementation lands, `pancreator-defaults.yaml`, `pancreator.yaml`, or the handbook entry at `lib/memory/handbook/pancreator-config.md` SHALL document at least one example block showing both `manual` and `sdk` values; the implementor SHALL select the documentation surface in the plan stage.
...
- When a run introduces or modifies logic, the run SHALL include automated test outcomes and manual QA evidence in the test report; when QA fails, `qa-tester` SHALL select the re-entry target (`plan` or `implement`) per the severity rules already defined in `lib/personas/qa-tester.md` lines 89–180.
...
1. **What:** Read the canonical Engineering Spec and decide whether the intake artifact is acceptable for plan-stage delegation.
   **How:** Read-only: open `lib/memory/features/feature-delivery-harness-wire-cursorrunner-through-run-and-advance/spec.md` and confirm the seven work-package acceptance blocks, the documentation-impact record, and the empty `## Open questions` section reflect the directive at `lib/inbox/in/172979_05-27-26/72021_0359_feature-delivery-cursor-runner-harness-wiring/72021_0359_feature-delivery-cursor-runner-harness-wiring.md`.
