---
title: Command Center Pipeline command center and human gate queue Engineering Spec
...
source_inbox_item: lib/inbox/in/172967_06-08-26/54353_0854_command-center-pipeline-command-center.md
depends_on: [command-center-ux-spec-and-information-architecture]
blocks: [command-center-pipeline-live-artifacts]
...
intake_notes:
  ...
  - The canonical ux-spec authority path is lib/memory/features/command-center-ux-spec-and-information-architecture/ux-spec.md rather than the program shorthand command-center/ cited in the directive goal.
  - Inbox triage, multi-run table, live polling, and Automations or Maintenance modules remain out of scope for this Feature per the directive and downstream command-center inbox items.
references:
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54353_0854_command-center-pipeline-command-center.md
    range: [24, 27]
  ...
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54353_0854_command-center-pipeline-command-center.md
    range: [59, 69]
    note: Source directive Touch set lists Command Center components, run-state services, API routes, and globals.css tokens.
  - kind: lines
  ...
  - kind: lines
    path: lib/memory/features/command-center-ux-spec-and-information-architecture/ux-spec.md
    range: [168, 183]
    note: Ratified component inventory under client/src/components/command-center/.
  - kind: lines
  ...
  - kind: lines
    ...
    range: [1, 80]
    note: Monolithic dashboard surface to refactor into DashboardModuleShell and pipeline components.
  - kind: lines
...
This Feature SHALL deliver the Command Center Pipeline command center so operators
...
and SHALL begin extracting `client/src/components/command-center/` from
...
- When the dashboard loads, `DashboardPage` SHALL render through
  `DashboardModuleShell` with Pipeline as the default module tab per the ux-spec
  three-module shell.
- When implement stage completes, the touch-set paths under
  `client/src/components/command-center/pipeline/`, `client/src/components/command-center/layout/`,
  and `client/src/components/command-center/shared/` SHALL contain extracted
  `NextActionPanel`, `HumanGateBanner`, and `ConfigReadOnlyPanel` components.
- When Command Center styles apply, `client/src/app/globals.css` SHALL extend
  command-center design tokens from the ux-spec without breaking existing P9 stage
  status classes.
...
- Live polling or websocket refresh of run state; the
  `command-center-pipeline-live-artifacts` inbox item owns that capability per
  source directive.
...
- Automations and Maintenance module implementation beyond placeholder module
  tabs required by `DashboardModuleShell`; those modules ship in separate Command Center
  inbox items.
