---
title: Command Center local scheduler tick and run history Engineering Spec
...
source_inbox_item: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-scheduler.md
...
intake_closure:
  ...
  note: Spec emitted for human ratification. No clarifying rounds were required because the source directive defines CLI tick behavior, JSONL run history, Command Center Run now and run-history UI outcomes, intervention compatibility, OPERATION.md OS scheduling guidance, four acceptance checks, explicit out-of-scope boundaries, a touch set, and a ratified registry dependency without unresolved scope, acceptance, constraint, or prior-art gaps.
intake_notes:
  - The intake-analyst did not enter the canonicalize-spec clarifying-question loop because the source directive enumerates tick dispatch, run-log append, Command Center execution surfaces, and external scheduler documentation with machine-checkable acceptance criteria.
  ...
  - The canonical registry dependency slug is `command-center-automation-registry-and-management-ui` rather than the program shorthand `command-center-automations-registry` cited in the directive depends_on field.
  - The Command Center run-history component path is `AutomationRunHistory.tsx` per the ratified ux-spec component inventory, not the `RunHistory.tsx` shorthand in the directive touch set.
  - Run now from Command Center SHALL invoke tick for one automation id through a dedicated API route rather than shelling out from the browser.
references:
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-scheduler.md
    range: [24, 27]
    note: Source directive Problem section states automations without execution and history are incomplete.
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-scheduler.md
    range: [28, 31]
    note: Source directive Goal section names tick CLI, JSONL run history, Command Center UI, and OPERATION.md OS scheduling guidance.
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-scheduler.md
    range: [32, 38]
    note: Source directive Required outcomes enumerate CLI subcommand, run history, Command Center UI, intervention compatibility, and documentation.
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-scheduler.md
    range: [40, 45]
    note: Source directive Acceptance criteria anchor due execution, Command Center history visibility, concurrency lock, and OPERATION.md external scheduler setup.
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-scheduler.md
    range: [47, 51]
  ...
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-scheduler.md
    range: [57, 65]
    note: Source directive Touch set lists CLI run module, scheduler package, run-log directory, Command Center components, optional run API route, and OPERATION.md.
  - kind: lines
  ...
  - kind: lines
    path: lib/memory/features/command-center-ux-spec-and-information-architecture/ux-spec.md
    range: [176, 178]
    note: Ratified component inventory names AutomationRunHistory under client/src/components/command-center/automations/.
  - kind: lines
...
I/O; SHALL add `pnpm -w exec pan scheduler tick` to the CLI; SHALL wire Command Center
...
### Command Center Run now and run history UI
...
- When an operator runs `pnpm -w exec pan scheduler tick` against an enabled
  ...
  `.pan/scheduler/runs/<automation-id>.jsonl` and the Command Center run-history panel
...
- Automation registry schema, CRUD API, and wizard UI; the
  `command-center-automation-registry-and-management-ui` Feature owns those surfaces.
- Replacing OS cron or launchd with an always-on Pancreator daemon in Command Center.
- Maintenance module changes beyond any shared Command Center styling tokens already
