---
title: Cockpit v2 active memory operator craft revalidation Engineering Spec
...
source_inbox_item: lib/inbox/in/172966_06-09-26/81282_0125_cockpit-v2-active-memory-operator-craft-revalidation.md
depends_on:
...
intake_notes:
  ...
  - F-05 and F-10 are regressions from cockpit-v2-active-memory-operator-readability; plan stage SHALL restore those remediations without reopening scope.
references:
  - kind: lines
    path: lib/inbox/in/172966_06-09-26/81282_0125_cockpit-v2-active-memory-operator-craft-revalidation.md
    range: [55, 66]
...
This Feature SHALL close all P0 and P1 design-craft gate failures on the Cockpit
...
`cockpit-v2-active-memory-operator-readability`, replace dashed wireframe
...
- When `client/src/app/page.test.tsx` runs Active memory header tests, the suite
  ...
  regressing unrelated cockpit modules.
...
- Module tab WAI-ARIA pattern (Feature C,
  `cockpit-v2-module-tab-accessibility`) per source directive.
...
- `client/` mutations performed by the cockpit-design-audit-validation audit
