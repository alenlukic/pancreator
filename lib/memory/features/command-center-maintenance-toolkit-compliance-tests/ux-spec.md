---
title: Command Center Maintenance toolkit (compliance and tests) UX Spec
...
references:
  ...
  - kind: lines
    ...
    range: [44, 82]
    note: Ratified Command Center shell, Maintenance module wireframe, and shared affordances.
  - kind: lines
    path: lib/memory/features/command-center-ux-spec-and-information-architecture/ux-spec.md
    range: [96, 120]
...
**Breakpoints:** inherit parent Command Center rules (≥1024px two-column; 768–1023px stacked; <768px horizontal scroll on descriptor table and history list).

## Visual design tokens

Reuse Command Center tokens from `client/src/app/globals.css` (`--surface-primary`, `--surface-elevated`, `--surface-attention`, `--text-primary`, `--text-muted`, `--accent`, `--space-*`). Add scoped classes under `/* command-center maintenance */` without altering Pipeline or Automations semantics.
...
```yaml
contract:
  id: command-center-maintenance-toolkit-compliance-tests.ux.output-stream-exit-code
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
    path: /lib/memory/features/command-center-maintenance-toolkit-compliance-tests/ux-spec.md
  ...
  metadata:
    pancreator.contract_id: command-center-maintenance-toolkit-compliance-tests.ux.output-stream-exit-code
    pancreator.applies_to: artifact-symbol:/lib/memory/features/command-center-maintenance-toolkit-compliance-tests/ux-spec.md#Shared-OutputStream
    pancreator.wcag-criteria: ["4.1.3"]
```

```yaml
contract:
  id: command-center-maintenance-toolkit-compliance-tests.ux.pre-close-eligibility
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: command-center-maintenance-toolkit-compliance-tests.ux.pre-close-eligibility
    pancreator.applies_to: artifact-symbol:/lib/memory/features/command-center-maintenance-toolkit-compliance-tests/ux-spec.md#Pre-close-validation-panel
    pancreator.wcag-criteria: ["2.1.1", "4.1.2"]
```

```yaml
contract:
  id: command-center-maintenance-toolkit-compliance-tests.ux.compliance-findings-expand
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
      ...
      examples:
        ...
        bad:
          - text: "Failed rows show only a red badge with no detail affordance."
            rationale: Operator cannot diagnose failure in Command Center.
    panel:
      quorum: 2-of-3
      judges: [haiku, haiku, sonnet]
      seed: 42
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: command-center-maintenance-toolkit-compliance-tests.ux.compliance-findings-expand
    pancreator.applies_to: artifact-symbol:/lib/memory/features/command-center-maintenance-toolkit-compliance-tests/ux-spec.md#Compliance-audit-panel
    pancreator.wcag-criteria: ["2.1.1", "4.1.2"]
```
