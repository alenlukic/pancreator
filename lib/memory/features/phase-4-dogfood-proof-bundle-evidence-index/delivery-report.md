...
- `lib/memory/features/phase-4-dogfood-proof-bundle-evidence-index/spec.md` defines the nested pipeline contract, the out-of-scope boundaries, and the ratification gate. 
...
- `lib/memory/features/us-1-dogfood-phase-4-exit/phase-4-proof-bundle.md` is the parent proof-bundle surface that receives the nested task id and immutable run-log path. 
...
- To start the proof-bundle flow, use `lib/inbox/in/phase-4-dogfood-proof-bundle-index.md` and run the nested pipeline through `intake`, `plan`, `implement`, `review`, `report`, `ship`, and `index`. 
