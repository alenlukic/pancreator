---
title: Command Center UX spec and information architecture Engineering Spec
...
source_inbox_item: lib/inbox/in/172967_06-08-26/54353_0854_command-center-ux-spec.md
next_owner: reviewer
next_stage: review
...
intake_notes:
  - The intake-analyst did not enter the canonicalize-spec clarifying-question loop because the source directive enumerates three Command Center modules, eight required outcomes, five acceptance checks, and explicit out-of-scope boundaries.
  - The canonical ux-spec artifact path follows feature-delivery pipeline convention at lib/memory/features/command-center-ux-spec-and-information-architecture/ux-spec.md rather than the program shorthand lib/memory/features/command-center/ cited in the directive touch set.
  - This Feature delivers spec and ux-spec documentation only; production React and API implementation routes to downstream Command Center inbox items.
references:
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54353_0854_command-center-ux-spec.md
    range: [27, 29]
    note: Source directive Problem section states P9/P10 Command Center gaps and operator context-switching pain.
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54353_0854_command-center-ux-spec.md
    range: [31, 33]
  ...
  - kind: lines
    ...
    range: [1, 6]
    note: P9 shipped the 9-stage grid and run-event timeline that Command Center Pipeline module evolves.
  - kind: lines
    path: lib/memory/features/surface-opt-p10-dashboard-safe-editing/spec.md
    range: [42, 46]
    note: P10 read-only default and diff confirmation carry forward into Command Center artifact modals.
  - kind: lines
...
This Feature SHALL ratify Command Center operator-dashboard UX authority before any
...
a proposed client/src/components/command-center/ component inventory, and globals.css
design-token guidance. Downstream Command Center implementation inbox items SHALL
...
- When intake completes, the Feature SHALL emit
  lib/memory/features/command-center-ux-spec-and-information-architecture/spec.md
...
- When plan completes with design_steps enabled, the design-engineer companion
  SHALL emit
  lib/memory/features/command-center-ux-spec-and-information-architecture/ux-spec.md
...
- When an operator opens Command Center, the primary navigation SHALL expose exactly
  three module tabs: Pipeline, Automations, and Maintenance.
- When an operator opens Command Center, the Files browser SHALL appear only on a
  secondary tab and SHALL NOT occupy the default module view.
- When Command Center renders the default landing surface, domain cards SHALL NOT
...
- When the ux-spec documents components, the ux-spec SHALL map surfaces to a
  proposed client/src/components/command-center/ tree with layout, pipeline,
...
- Where wireframe assets aid comprehension, the Feature MAY add optional assets
  under lib/memory/features/command-center-ux-spec-and-information-architecture/
...
- Implementing API routes, React components, or client/src/components/command-center/
  source files; downstream Command Center implementation inbox items own code per
...
- Replacing P9 run-state aggregation or P10 write-guard logic; Command Center UX
  SHALL compose atop those deliverables.
- Creating lib/memory/features/command-center/ as a separate program folder; the
