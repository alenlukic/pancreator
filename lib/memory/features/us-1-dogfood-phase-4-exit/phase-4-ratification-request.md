...
1. Nested proof-bundle-index task ledger and preserved `run.log.jsonl` (canonical
   ...
   `.pan/archive/work/<day>/<nested-task-id>/`; the **librarian** SHALL
...
3. `pause-resume-abort-evidence.json` — this file is **not** produced by the
   ...
   `.pan/archive/inbox/in/172988_05-18-26/71040_0416_intake-phase-4-intervention-probe-pause-resume-abort-ratification.md`.
...
5. `lib/memory/features/us-1-dogfood-phase-4-exit/delivery-report.md` and, when
   the parent US-1 run has progressed through **ship** / **index**, a matching
   `lib/inbox/out/<timestamp>-us-1-dogfood-phase-4-exit-delivery-report.md`. The
   ...
   **`pan advance` never requires that file for the report stage** — only
   `delivery-report.md` under `lib/memory/features/…/` is the report artifact
...
1. Open `lib/memory/features/us-1-dogfood-phase-4-exit/pause-resume-abort-evidence.json`.
...
author `lib/memory/features/us-1-dogfood-phase-4-exit/delivery-report.md` and
advance with `--artifact` pointing at that path. Nothing in the CLI enforces
writing `lib/inbox/out/…-us-1-dogfood-phase-4-exit-delivery-report.md` at report
advance.
...
2. **For the outbox copy:** expect **supervisor** (ship) and/or **librarian**
   (index) to add `lib/inbox/out/<timestamp>-us-1-dogfood-phase-4-exit-delivery-report.md`
...
| Outcome | Notes |
| --- | --- |
| Ratified Phase 4 exit | 1. .pan/archive/work/172988_05-18-26/77373_0230_phase-4-dogfood-proof-bundle-evidence-index/run.log.jsonl ; 2. Item 2 deferred per bootstrap; no Phoenix importer; engineering backlog remains - lib/memory/features/us-1-dogfood-phase-4-exit/phoenix-trace-evidence.md ; 3. Item 3 pass — pause, resume, abort captured - lib/memory/features/us-1-dogfood-phase-4-exit/pause-resume-abort-evidence.json ; 4. .pan/archive/work/172990_05-16-26/20004_1826_us-1-dogfood-phase-4-exit/policy-compliance.json ; 5. lib/inbox/out/172987_05-19-26/77614_0226_us-1-dogfood-phase-4-exit-delivery-report.md + lib/memory/features/us-1-dogfood-phase-4-exit/delivery-report.md |

Post-ratification updates to `pancreator.yaml`, `AGENTS.md`, `.docs/M1.index.md`, and
`lib/memory/active/current.md` remain gated until humans approve per parent feature spec.
