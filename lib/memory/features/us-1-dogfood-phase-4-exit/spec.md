...
source_inbox_item: lib/inbox/in/us-1-dogfood-phase-4-exit.md
...
intake_notes:
  - The source inbox item frontmatter at line 1 carries a leading "f"
    ...
    `lib/inbox/in/us-1-dogfood-phase-4-exit.md` per the persona contract.
  ...
  - The directive references `pan pause`, `pan resume`, and `pan abort`.
    `AGENTS.md` lines 248 to 253 record that the `@pancreator/cli` runtime
    exposes those verbs through `pan run` and `pan advance` orchestration
    today. The plan stage owns the choice of dogfood scenario that exercises
    each verb.
references:
  - kind: lines
    path: lib/inbox/in/us-1-dogfood-phase-4-exit.md
    range: [1, 18]
    contentHash: 414a8f4
  ...
  - kind: lines
    ...
    range: [79, 84]
    contentHash: 414a8f4
    note: "Source directive section 3 specifies a second controlled dogfood exercise that empirically exercises `pan pause`, `pan resume`, and `pan abort`."
  - kind: lines
    path: lib/inbox/in/us-1-dogfood-phase-4-exit.md
    range: [86, 95]
    contentHash: 414a8f4
    note: "Source directive section 4 enumerates the Phase 4 proof bundle: staged PR outcome, delivery report under `lib/inbox/out/`, clean external run trace, intervention evidence, and residual-gap statement."
  - kind: lines
    path: lib/inbox/in/us-1-dogfood-phase-4-exit.md
    range: [97, 101]
    contentHash: 414a8f4
...
directive at
`{kind: lines, path: lib/inbox/in/us-1-dogfood-phase-4-exit.md, range: [86, 95], contentHash: 414a8f4}`.
...
- When the Feature opens the Phase 4 exit slice, the Feature MUST stage one
  new real intake item under `lib/inbox/in/` and MUST NOT reuse a previously
  archived inbox item.
...
- When the second run reaches a live stage, the operator MUST invoke `pan
...
- When the paused run is ready to continue, the operator MUST invoke `pan
...
- When the second run is ready to abort, the operator MUST invoke `pan
...
- When the human reviews the proof bundle, the human MUST inspect the
  ...
  `{kind: lines, path: lib/inbox/in/us-1-dogfood-phase-4-exit.md, range: [122, 132], contentHash: 414a8f4}`.
...
- When the ratification record marks the exit as ratified, the Feature
  MUST update `pancreator.yaml` to advance the bootstrap phase tracker
...
- This Feature SHALL NOT read, write, traverse, or cite any path under
  `/lib/inbox/notes/` per the operator-sandbox rule.
...
- D1. Choose the dogfood directive content for the new
  `lib/inbox/in/<entry>.md` intake item. The plan stage MUST pick a slice
...
- D2. Choose the external observability tool used for the run-log
  ...
  `{kind: lines, path: lib/inbox/in/us-1-dogfood-phase-4-exit.md, range: [74, 77], contentHash: 414a8f4}`.
- D3. Choose the second-run scenario that exercises `pan pause`, `pan
  resume`, and `pan abort` in a live stage context. The plan stage MUST
...
- D4. Choose the proof-bundle storage path: either the per-feature
  directory at `lib/memory/features/us-1-dogfood-phase-4-exit/` or a
...
- D5. Choose whether to seed the Phase 5 readiness ADR under
  `lib/memory/adr/` inside this slice or to defer that ADR to the first
