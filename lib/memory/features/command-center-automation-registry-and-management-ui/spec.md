---
title: Command Center automation registry and management UI Engineering Spec
...
source_inbox_item: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-registry.md
depends_on: [command-center-ux-spec-and-information-architecture]
blocks: [command-center-automations-scheduler]
...
intake_closure:
  ...
  note: Spec emitted for human ratification. No clarifying rounds were required because the source directive defines schema v1, validation primitive, CRUD API, Command Center UI outcomes, four acceptance checks, explicit out-of-scope boundaries, a touch set, and a ratified ux-spec dependency without unresolved scope, acceptance, constraint, or prior-art gaps.
intake_notes:
  - The intake-analyst did not enter the canonicalize-spec clarifying-question loop because the source directive enumerates registry schema, validation package, CRUD API, and Command Center Automations module outcomes with machine-checkable acceptance criteria.
  ...
  - The canonical ux-spec authority path is lib/memory/features/command-center-ux-spec-and-information-architecture/ux-spec.md rather than the program shorthand command-center-ux-spec cited in the directive depends_on field.
  - Run history population and Run now execution remain stubbed or disabled in this Feature; command-center-automations-scheduler owns tick execution and run-log append per the directive Out of scope section.
references:
  - kind: lines
  ...
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-registry.md
    range: [28, 31]
    note: Source directive Goal section names Pancreator-native registry, validation, CRUD API, and Command Center UI.
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-registry.md
    range: [32, 38]
    note: Source directive Required outcomes enumerate schema v1, validation, CRUD API, and Command Center UI.
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-registry.md
    range: [39, 45]
  ...
  - kind: lines
    path: lib/inbox/in/172967_06-08-26/54351_0854_command-center-automations-registry.md
    range: [59, 65]
    note: Source directive Touch set lists automations directory, scheduler package, API route, and Command Center components.
  - kind: lines
  ...
  - kind: lines
    path: lib/memory/features/command-center-ux-spec-and-information-architecture/ux-spec.md
    range: [176, 178]
    note: Ratified component inventory under client/src/components/command-center/automations/.
  - kind: lines
...
This Feature SHALL deliver a Pancreator-native automation registry and Command Center
...
`DashboardModuleShell` Automations placeholder with list and create/edit wizard surfaces
per the ratified ux-spec. Tick execution, run-history population, and Run now
invocation remain out of scope for `command-center-automations-scheduler`.
...
### Command Center Automations module

- When an operator opens the Automations module tab, `DashboardModuleShell` SHALL render
...
- When an operator creates an hourly agent automation through the Command Center UI,
...
- Scheduler tick execution, due-run dispatch, and run-log append; the
  `command-center-automations-scheduler` inbox item owns those capabilities per
  source directive.
...
- Maintenance module implementation beyond the existing `DashboardModuleShell` placeholder.
- Pipeline module changes beyond wiring `AutomationsModule` into `DashboardModuleShell`.
