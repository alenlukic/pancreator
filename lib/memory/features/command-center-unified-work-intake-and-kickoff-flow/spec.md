---
title: Command Center unified Work Intake and kickoff flow Engineering Spec
...
source_inbox_item: lib/inbox/in/172966_06-09-26/63490_0621_command-center-work-intake-kickoff.md
depends_on:
...
parallel_with:
  ...
  - command-center-automations-shell-history
...
intake_notes:
  ...
  - Route `/work-intake` currently renders `CommandCenterSurfacePlaceholder` with `isKickoffStub`; this Feature replaces that stub with the full kickoff stepper under `client/src/components/command-center/kickoff/`.
  ...
  - Global shell header **Start feature delivery** CTA per ux-spec layout §53 is not yet mounted in `(command-center)/layout.tsx`; this Feature SHALL add header wiring alongside kickoff route integration.
  - Contract `command-center-ux-philosophy-information-architecture-and-user-stories.ux.kickoff-flow-inputs` is the blocking llm-judge gate at threshold 0.75.
references:
  - kind: lines
    path: lib/inbox/in/172966_06-09-26/63490_0621_command-center-work-intake-kickoff.md
    range: [37, 39]
...
This Feature SHALL deliver the Command Center Work Intake / Kickoff surface at route
...
contract
`command-center-ux-philosophy-information-architecture-and-user-stories.ux.kickoff-flow-inputs`,
mount inside the ten-surface shell from `command-center-shell-theme-foundation`,
...
- When `(command-center)/work-intake/page.tsx` mounts, the page SHALL render
  `KickoffSurface` from `client/src/components/command-center/kickoff/` and SHALL NOT
  render `CommandCenterSurfacePlaceholder` with `isKickoffStub`.
...
- When the operator selects **Feature backlog** before
  `command-center-feature-backlog-ux` ships, the kickoff flow SHALL render a guided
...
- When `pnpm --filter client test` executes, kickoff component tests and kickoff
  API route tests under `client/src/components/command-center/kickoff/` and
...
- Full Feature Backlog browse UX with search, filters, and detail drawer; the
  `command-center-feature-backlog-ux` inbox item owns that surface per source
  directive.
- Sandbox target selection beyond a stub empty state; advanced sandbox comparison
  and scoped run targeting belong to `command-center-sandbox-manager-advanced` per
  source directive.
- FD Mission Control display of a newly launched run; post-launch navigation
  ...
  `command-center-fd-mission-control` per source directive.
