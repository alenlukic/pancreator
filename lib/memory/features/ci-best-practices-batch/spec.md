...
next_stage: plan
source_inbox_item: lib/inbox/in/172981_05-25-26/71701_0613_ci-best-practices-batch.md
intake_round: 0
work_packages:
...
references:
  - kind: lines
    path: lib/inbox/in/172981_05-25-26/71701_0613_ci-best-practices-batch.md
    range: [1, 43]
    contentHash: 41e9308
...
`node lib/internal/tools/run-compliance.mjs` as an executable node script that
...
`node lib/internal/tools/refresh-citations.mjs` as an idempotent tool that
...
- When an operator invokes
  `node lib/internal/tools/run-compliance.mjs [<descriptor>]`, the runner SHALL
...
- When an operator invokes
  `node lib/internal/tools/refresh-citations.mjs [--dry-run] [<glob>...]`,
...
- The tool SHALL compute abbreviated hashes using the utilities in
  `lib/internal/tools/canonical-json-format.mjs`, producing 7-character SHA
...
- The tool SHALL refuse to write any path under `lib/inbox/notes/` and SHALL
  treat `lib/inbox/{in,out,threads}/` as semantically immutable, skipping those
...
- A `node --test` suite under `tests/` SHALL cover all three citation surfaces,
  the idempotency invariant, and the `lib/inbox/notes/` refusal.
...
- When an MCP client calls `pan.feature.list`, the handler SHALL return a typed
  array of feature summaries derived from `lib/memory/features/*/index.json`
...
- Remaining write MCP tools SHALL remain stubbed using the structured deferral
  protocol per `lib/internal/packages/@pancreator/cli/lib/run.ts` when that
...
- Inbox archival for any file touched by WP-C; semantic immutability per
  `lib/memory/handbook/inbox-lifecycle.md` §3b prohibits mutation of
  `lib/inbox/{in,out,threads}/`.
