...
source_inbox_item: lib/inbox/in/token-economy-enhanced.md
...
intake_notes:
  ...
  - The intake-analyst MUST NOT modify the inbox source item per the
    ...
    at `lib/inbox/in/token-economy-enhanced.md`.
...
references:
  - kind: lines
    path: lib/inbox/in/token-economy-enhanced.md
    range: [1, 33]
    contentHash: fb1ac76
...
the directive at
`{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [99, 478], contentHash: fb1ac76}`.
...
- When the Feature lands the memory-tier model, the Feature MUST create one
  new handbook page at `lib/memory/handbook/memory-tiers.md`.
- When the Feature authors `lib/memory/handbook/memory-tiers.md`, the page MUST
  ...
  directive at
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [101, 229], contentHash: fb1ac76}`.
...
- When the Feature creates the active-memory tier, the Feature MUST create the
  directory `lib/memory/active/`.
- When the Feature creates the active-memory tier, the Feature MUST add one
  file at `lib/memory/active/README.md` that explains tier scope and exclusions.
- When the Feature creates the active-memory tier, the Feature MUST add one
  file at `lib/memory/active/current.md` that contains short summaries and
  pointers without copied artifacts.
- When the Feature creates the active-memory tier, the Feature MUST add one
  file at `lib/memory/active/runs.md` that stores active-run pointers without
...
- When the Feature documents the active-memory soft budgets, the page MUST
  state that the entire `lib/memory/active/` tier SHOULD remain at most 5000
...
- When the Feature evaluates physical migration of existing directory trees,
  ...
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [254, 276], contentHash: fb1ac76}`.
...
- When the Feature authors the new ADR, the ADR MUST record the problem
  ...
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [278, 294], contentHash: fb1ac76}`.

### Context-economy and handbook index updates

- When the Feature updates the context-economy handbook page, the Feature
  MUST extend `lib/memory/handbook/context-economy.md` with the memory-tier
  routing rule.
- When the Feature updates the context-economy page, the page MUST route
  active-memory questions to `lib/memory/active/current.md`.
- When the Feature updates the context-economy page, the page MUST route
  memory-tier questions to `lib/memory/handbook/memory-tiers.md`.
...
- When the Feature updates the handbook routing index, the Feature MUST add
  ...
  to `lib/memory/active/current.md`.
- When the Feature updates the handbook routing index, the Feature MUST add
  one row to `lib/memory/handbook/index.md` that routes memory-tier questions to
...
- When the Feature defines `simple task mode`, the Feature MUST publish one
  ...
  `lib/memory/handbook/context-economy.md`, or in a new dedicated handbook page
  per the directive at
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [313, 352], contentHash: fb1ac76}`.
...
- When the Feature defines `simple task mode` escalation, the page MUST list
  ...
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [344, 351], contentHash: fb1ac76}`.
...
- When the Feature audits `.cursor/rules/*.mdc`, the audit MUST cover at least
  ...
  at
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [378, 402], contentHash: fb1ac76}`.
- When the Feature narrows `.cursor/rules/pancreator-engineer.mdc`, the
  Feature MUST remove `lib/personas/**/*.md` from the default activation set
...
- When the Feature extends `lib/internal/tools/context-budget-report.mjs`, the report MUST
...
- When the Feature emits a token estimate, the report MUST label the
  ...
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [422, 426], contentHash: fb1ac76}`.
...
- When the Feature audits `.cursorindexingignore`, the policy MUST keep
  `lib/memory/active/**` indexable by default.
...
- When the Feature documents escalation guidance, the Feature MUST publish
  one section in either `lib/memory/handbook/context-economy.md` or `AGENTS.md`
  per the directive at
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [446, 457], contentHash: fb1ac76}`.
...
- When the Feature applies the documentation-impact contract, the Feature
  ...
  `.cursorindexingignore`, `lib/memory/handbook/index.md`,
  `lib/memory/handbook/context-economy.md`, `lib/memory/handbook/memory-tiers.md`,
  the three new `lib/memory/active/*.md` files, the new active-versus-archival
  ADR, `README.md`, and `lib/memory/backlog/index.yaml`.
...
- When the Feature stages the delivery report, the report MUST request the
  ...
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [505, 515], contentHash: fb1ac76}`.
- When the Feature stages the delivery report, the report MUST cite the
  directional cache-read token bands declared at
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [517, 535], contentHash: fb1ac76}`
...
- D1. Choose the single canonical surface that publishes `simple task mode`:
  an `AGENTS.md` section, a new `lib/memory/handbook/context-economy.md` section,
  or a new dedicated handbook page. The directive authorizes the
  implementation to choose at
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [315, 318], contentHash: fb1ac76}`.
- D2. Choose the next available ADR sequence number for
  `lib/memory/adr/000X-active-vs-archival-memory.md`. The directive authorizes the
  implementation to choose at
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [282, 284], contentHash: fb1ac76}`.
- D3. Choose whether to ship a lightweight active-memory budget warning
  ...
  choose at
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [248, 252], contentHash: fb1ac76}`.
- D4. Choose the canonical surface for model-and-context escalation guidance:
  `lib/memory/handbook/context-economy.md` or `AGENTS.md`. The directive
  authorizes the implementation to choose at
  `{kind: lines, path: lib/inbox/in/token-economy-enhanced.md, range: [446, 449], contentHash: fb1ac76}`.
...
- The Feature MAY defer the memory-tier addition to
  `lib/memory/handbook/glossary.md` to a documentation-impact backlog item when
...
- This Feature extends `lib/memory/features/cursor-token-economy/`. The
  relationship is `extends-deferred-scope`, recorded in
  `lib/memory/features/active-memory-context-economy-pass-2/index.json`.
