---
title: Command Center active memory operator craft revalidation UX Spec
...
references:
  - kind: lines
    path: lib/memory/features/command-center-active-memory-operator-craft-revalidation/spec.md
    range: [99, 185]
...
# Command Center active memory operator craft revalidation UX Spec
...
- **Shell authority** — inherit Command Center Pipeline sidebar stack: `ActiveMemoryHeader` remains the topmost right-column panel at ≥1024px and in stacked mobile order.
...
│ Command Center active memory craft revalidation │  ← primary label (semibold)
...
Reuse Command Center tokens from `client/src/app/globals.css`. Refine scoped classes under the orientation block only; do not alter inbox triage, multi-run, or stage-status tokens.
...
| `.active-memory-refresh-link` | `.command-center-action-button`, `margin-top: var(--space-3)`; sole accent CTA in panel | Open OPERATION.md |
...
```yaml
contract:
  id: command-center-active-memory-operator-craft-revalidation.ux.no-raw-path-default-view
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
    path: /lib/memory/features/command-center-active-memory-operator-craft-revalidation/ux-spec.md
  ...
  runtime:
    rubric:
      ...
      examples:
        good:
          - text: "Primary label reads command-center-active-memory-operator-craft-revalidation; copy icon present; DOM probe containsLibInbox false."
  ...
  metadata:
    pancreator.contract_id: command-center-active-memory-operator-craft-revalidation.ux.no-raw-path-default-view
    pancreator.applies_to: artifact-symbol:/lib/memory/features/command-center-active-memory-operator-craft-revalidation/ux-spec.md#Active-memory-header
    pancreator.wcag-criteria: ["2.4.6"]
```

```yaml
contract:
  id: command-center-active-memory-operator-craft-revalidation.ux.solid-chrome-and-containment
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: command-center-active-memory-operator-craft-revalidation.ux.solid-chrome-and-containment
    pancreator.applies_to: artifact-symbol:/lib/memory/features/command-center-active-memory-operator-craft-revalidation/ux-spec.md#Visual-design-tokens
    pancreator.wcag-criteria: ["1.4.11"]
```

```yaml
contract:
  id: command-center-active-memory-operator-craft-revalidation.ux.blockers-chips-cta-timestamp
  kind: llm-judge
  severity: block
  applies_to:
    kind: artifact-symbol
  ...
  runtime:
    ...
    panel:
      ...
      cost_ceiling_usd: 0.50
  metadata:
    pancreator.contract_id: command-center-active-memory-operator-craft-revalidation.ux.blockers-chips-cta-timestamp
    pancreator.applies_to: artifact-symbol:/lib/memory/features/command-center-active-memory-operator-craft-revalidation/ux-spec.md#Active-memory-header
