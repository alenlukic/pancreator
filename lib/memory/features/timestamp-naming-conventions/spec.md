...
- When the Feature creates a system-produced artifact under `lib/inbox/out/`, the
  ...
  shape.
- When the Feature creates a system-produced artifact under `lib/inbox/threads/`,
  the Feature MUST use `{SID-prefix}_{HHMM}_{semantic-suffix}` as the basename
  shape.
- When the Feature migrates artifacts under `lib/inbox/threads/`, the Feature MUST
  preserve the existing per-feature parent folder.
- When the Feature processes a non-conforming human-generated artifact in
  `lib/inbox/in/`, the agent performing that work MUST append the two time
  prefixes before downstream processing continues.
- When the Feature processes a non-conforming human-generated artifact in
  `lib/inbox/threads/`, the agent performing that work MUST append the two time
  prefixes before downstream processing continues.
...
- When the Feature migrates existing artifacts, the Feature MUST migrate
  already-existing round files under `lib/inbox/threads/`.
