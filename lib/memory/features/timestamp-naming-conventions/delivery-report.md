...
{
  "kind": "lines",
  "path": "lib/inbox/threads/173009_04-27-26/timestamp-naming-conventions/51237_0946_round-01-clarify.md",
  "range": [1, 97],
  "contentHash": "ec8f770"
}
```
...
- `timestamp-naming-conventions-migration-write` MUST wait for the `human_approval` gate, MUST require `PANCREATOR_MIGRATION_GO=1`, and MUST use the manifest as the rollback map if a partial write occurs. 
...
- The script MUST keep the `PANCREATOR_MIGRATION_GO=1` guard, `--strict-references` audit, and manifest-backed rollback path intact so the later write commit stays auditable. 
...
3. `migrateTargetForInboxPath()` MUST preserve the `lib/inbox/threads/<feature>/` parent while it computes the new basename for thread files. 
...
- `node lib/internal/tools/migrate-timestamp-naming.mjs --dry-run --strict-references` - pass, with `renames=35`, `referenceUpdates=451`, and `collisionWarnings=0`. 
