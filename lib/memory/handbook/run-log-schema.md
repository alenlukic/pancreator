...
`metadata.run_log_offset` in `/lib/memory/checkpoints/<task-id>/<seq>.json`.
...
4. Checkpoint records MUST include `checkpoint_seq` and enough metadata to map
   to `/lib/memory/checkpoints/<task-id>/<seq>.json`.
