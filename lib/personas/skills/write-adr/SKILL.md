---
name: write-adr
description: Authors one immutable Architecture Decision Record in the Nygard format. Picks the next sequence number, fills the four-section template (context, decision, status, consequences), wires `supersedes:` and dual-anchor citations, runs Layer 1 lint, then files under `/lib/memory/adr/<seq>-<slug>.md` for human ratification.
license: Apache-2.0
metadata:
  pancreator-stability: experimental
  pancreator-bootstrap-only: false
  pancreator-pipeline-stages: [plan, review, knowledge-curation]
...
canonical callers are `lib/personas/tech-lead.md` (drafts the ADR at the `plan`
stage), `lib/personas/reviewer.md` (drafts a corrective ADR when the `review`
stage uncovers an undocumented decision), and `lib/personas/librarian.md` (drafts
...
- `/lib/memory/adr/` SHALL exist and contain the prior ADR sequence; the next ADR
  sequence number is `max(seq) + 1` zero-padded to 4 digits.
- `/lib/memory/handbook/glossary.md` SHALL exist; every domain noun in the ADR
  body resolves through this file.
- `/lib/memory/handbook/contract-style.md` SHALL exist; the ADR body satisfies
...
Read `/lib/memory/adr/` and pick `<seq> = max(existing seq) + 1`, zero-padded to
...
`/lib/memory/handbook/contract-style.md` to every normative clause in the ADR
