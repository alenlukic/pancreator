---
name: author-persona
description: Authors a Pancreator subagent persona spec to the Anthropic Claude Agent SDK 16-field YAML frontmatter spec, emits the matching Cursor `.cursor/rules/<name>.mdc` shim, and self-checks against the Layer 1 lint discipline before submission.
license: Apache-2.0
metadata:
  pancreator-stability: experimental
  pancreator-bootstrap-only: false
  pancreator-pipeline-stages: [bootstrap-phase-1, sme-spawn, ad-hoc-persona-new]
...
A reusable 7-step procedure for authoring one Pancreator subagent persona spec. The
canonical caller is `lib/personas/persona-designer.md`; the Librarian MAY invoke it
directly when proposing an SME draft (M4+).

## Prerequisites

- `/lib/memory/handbook/persona-spec.md` SHALL exist and define the Anthropic Claude
  Agent SDK 16-field YAML reference plus the Cursor `.mdc` shim recipe.
- `/lib/memory/handbook/glossary.md` SHALL exist; the persona's `description:` and body
...
If the proposed scope overlaps an existing persona's `pancreator-pipeline-stages` by
...
Load `/lib/memory/handbook/persona-spec.md` into context. The 16 fields are: `name`,
...
Load `/lib/memory/handbook/glossary.md` so domain nouns resolve at lint time without a
re-read.
...
- `name` MUST be lowercase-kebab-case and unique across `lib/personas/`.
...
- `model` defaults to `inherit` unless the persona's `pancreator-risk-tier` is
...
- `memory` defaults to `project`; `private` for SMEs whose notes belong under
  `/lib/memory/smes/<name>/`.
- `effort` ∈ `{low, medium, high}`; corresponds to the model's reasoning budget.
- `color` is a UX hint only; pick from the unused palette in `/lib/memory/handbook/persona-colors.md`.
- `hooks`, `initialPrompt`, `background` MAY be omitted when no value applies.

### Step 4 — Populate the `metadata` map with Pancreator extensions

Required extensions:

- `pancreator-risk-tier` ∈ `{low, medium, high, any}` — drives default contract
  bundle per PRD §7.
- `pancreator-pipeline-stages` — array of stage IDs the persona is invoked in.
- `pancreator-bootstrap-only` — boolean; `true` means the persona retires after the
...
Write `.cursor/rules/<name>.mdc` per the recipe in `/lib/memory/handbook/persona-spec.md`.
...
`@`-include of the canonical persona file (`@lib/personas/<name>.md`).

The shim MUST round-trip through the `@pancreator/persona` parser identically once
