---
name: write-rfc
description: Authors one Request for Comments in the Rust-RFC-process shape. Picks a slug, fills the seven-section template (motivation, guide-level, reference-level, drawbacks, alternatives, prior art, unresolved questions), wires dual-anchor citations, runs Layer 1 lint, then files the draft under `/lib/memory/rfc/draft/<slug>.md` for ensemble debate or human ratification.
license: Apache-2.0
metadata:
...
references:
  ...
  - kind: lines
    ...
    range: [154, 167]
    contentHash: 2ce8e5c
    note: "PRD §3.5 US-5 — Scouts emit RFC drafts under `/lib/memory/rfc/draft/`; ensembles + SMEs deliberate; PM folds accepted RFCs into the backlog."
  - kind: lines
    path: docs/PRD.md
    range: [708, 715]
    contentHash: 2ce8e5c
    note: "PRD §7 — `rfc-debate`, `proactive-research-scan`, and `debt-grooming` pipelines all emit or consume RFCs under `/lib/memory/rfc/draft/`."
  - kind: lines
    path: docs/PRD.md
    range: [916, 919]
    contentHash: 2ce8e5c
    note: "PRD §8 — Memory architecture: `/lib/memory/rfc/{draft,accepted,rejected}/` directory layout for in-flight, accepted, and rejected RFCs."
  - kind: lines
    path: docs/PRD.md
    range: [1004, 1010]
    contentHash: 2ce8e5c
...
canonical caller is `lib/personas/tech-lead.md` (drafts an RFC when a `plan`
...
- `/lib/memory/rfc/draft/`, `/lib/memory/rfc/accepted/`, and `/lib/memory/rfc/rejected/`
  SHALL exist per PRD §8 lines 916 through 919.
- `/lib/memory/handbook/glossary.md` SHALL exist; every domain noun in the RFC
  body resolves through this file.
- `/lib/memory/handbook/contract-style.md` SHALL exist; the RFC body satisfies
...
proposal title. Filename is `/lib/memory/rfc/draft/<slug>.md`. When a draft RFC
...
to `/lib/memory/rfc/rejected/<slug>.md` per PRD §8 line 919.

### Step 5 — Author the ratification rubric

When `pipeline-binding:` is non-null, the RFC MUST include an inline
contract block per `/lib/memory/handbook/contract-format.md` declaring the
...
`/lib/memory/handbook/contract-style.md` to every normative clause in the RFC
...
1. Stage the new file at `/lib/memory/rfc/draft/<slug>.md` with `status:
   draft`. You MUST NOT commit; you MUST NOT push.
2. Open one inbox item at `lib/inbox/in/<timestamp>-rfc-<slug>-debate.md`
