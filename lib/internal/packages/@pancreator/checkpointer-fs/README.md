# @pancreator/checkpointer-fs

File-backed checkpoint envelopes under `<checkpoints-root>/<task_id>/<seq>.json` with a required `metadata.run_log_offset` that ties to the JSONL run log (see `/lib/memory/handbook/run-log-schema.md`).

## Quickstart

```sh
pnpm install
pnpm --filter @pancreator/checkpointer-fs run build
pnpm --filter @pancreator/checkpointer-fs test
pnpm --filter @pancreator/checkpointer-fs run typecheck
```
