...
export function assertPersonaSpec(data: Record<string, unknown>): PersonaSpec {
  ...
  const tier = metadata["pancreator-risk-tier"];
  if (typeof tier !== "string" || !RISK.has(tier)) {
    throw err("`metadata.pancreator-risk-tier` MUST be `low`, `medium`, `high`, or `any`.");
  }

  const stages = metadata["pancreator-pipeline-stages"];
  if (!Array.isArray(stages) || !stages.every((s) => typeof s === "string")) {
    throw err("`metadata.pancreator-pipeline-stages` MUST be a string array.");
  }

  const bootstrap = metadata["pancreator-bootstrap-only"];
  if (typeof bootstrap !== "boolean") {
    throw err("`metadata.pancreator-bootstrap-only` MUST be a boolean.");
  }

  const stability = metadata["pancreator-stability"];
