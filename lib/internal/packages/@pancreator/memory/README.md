...
`FileMemoryStore` reads and writes UTF-8 under a `lib/memory/` root. `MemoryRouter` parses `/lib/memory/handbook/index.md` route rows. `verifyDualAnchor` checks `contentHash` values on `DualAnchorCitation` objects.
