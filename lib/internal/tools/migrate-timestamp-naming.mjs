 ...
 * `work/timestamp-naming-conventions/migration-manifest.dry-run.json`.
 *
...
/**
 * Computes planned relocation for `work/<task>/` (flat layout).
...
export function migrateTargetForWorkPath(absPath, ctx) {
  ...
  if (parts.length < 2 || parts[0] !== "work") {
    throw new Error(`migrateTargetForWorkPath: expected work/ path, got ${rel}`);
  }
  const slug = parts[1];
  if (parts.length !== 2) {
    throw new Error(
      `migrateTargetForWorkPath: expected flat work/<slug>, got ${rel}`,
    );
  }
  ...
  const targetRel = `work/${dayDir}/${taskBasename}`;
...
const EXCLUDED_SOURCES = new Set([
  "work/timestamp-naming-conventions/plan.md",
  "work/timestamp-naming-conventions/adr-draft.md",
  "work/timestamp-naming-conventions/touch-set.json",
...
function listFlatWorkTasks(repoRoot) {
  const w = path.join(repoRoot, "work");
  if (!existsSync(w)) {
    return [];
  }
  const out = [];
...
function inboxConventionRunDir(repoRoot) {
  ...
  return (
    findTaskRunDir(repoRoot, "archive/work", taskId) ??
    findTaskRunDir(repoRoot, "work", taskId)
  );
}
