...
function runNodeScript(relScript, args = []) {
  const abs = path.join(REPO_ROOT, relScript);
  try {
    execFileSync(process.execPath, [abs, ...args], {
      cwd: REPO_ROOT,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
    });
...
const ASSERTION_ADAPTERS = {
  async "governance-enforcement"(descriptor) {
    ...
    for (const step of [
      ...
      () => runNodeScript("lib/internal/tools/check-cursor-projection-drift.mjs"),
      () => runNodeScript("lib/internal/tools/check-token-telemetry.mjs"),
      () => runNodeScript("lib/internal/tools/audit-governance-usage.mjs", ["--stdout-only"]),
    ]) {
      const result = step();
      if (result.pass instanceof Promise) {
        const awaited = await result;
        if (!awaited.pass) return awaited;
        continue;
      }
  ...
  async "feature-index-artifact-index"(descriptor) {
    void descriptor;
    return runNodeTestFile("tests/feature-index-artifact-index.test.mjs");
  },
...
export function resolveRunOutputDir(runId) {
  ...
  throw new Error(`No .pan/work/<day>/${runId} directory found for --run-id.`);
}
...
export async function runCompliance(filterId, options = {}) {
  ...
  if (options.runId !== undefined) {
    ...
    writeFileSync(outPath, stringifyRepoJson(report, REPO_ROOT), "utf8");
