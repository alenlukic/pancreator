 ...
 * @see lib/memory/features/bootstrap-repo-ops/repository-layout-restructure-and-archive-migration/index.json
 */
import {
  existsSync,
  mkdirSync,
...
function gitMove(repoRoot, fromRel, toRel, dryRun) {
  ...
  const cmd = `git mv ${quoteJsonString(fromRel)} ${quoteJsonString(toRel)}`;
  if (dryRun) {
  ...
  try {
    ...
    execSync(`git add ${quoteJsonString(toRel)}`, { cwd: repoRoot, stdio: "pipe" });
  }
}
