 ...
 *
 * Usage:
 *   node lib/internal/tools/evaluate-greenfield-evidence.mjs --input <artifact.json>
 */
...
export function validateEvidenceStructure(artifact) {
  ...
  if (!isObject(metadata)) {
    ...
    for (const key of ["feature_id", "task_id", "target_repo", "pipeline_id", "captured_at_iso"]) {
      if (!(key in metadata)) {
...
export function evaluateGreenfieldEvidence(artifact) {
  const structureErrors = validateEvidenceStructure(artifact);
