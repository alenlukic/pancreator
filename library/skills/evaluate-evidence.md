...
- Looked for: before you record that a repository-check profile has no current
  ...
  profile, status, and `workspace_fingerprint` match the current workspace.
  A card cites gate evidence only, so a clean agent-recorded pass from an
  earlier worker in the same run does not appear there, and a gap you report
  without that read costs a later stage the work of disproving it.
- Complete: it covers the whole criterion, not a convenient slice of it.
...
Your card allows the `fast` profile once and forbids a second run.
That limit exists for cost: the profile is the most expensive thing an evidence
worker can do, and a second run at an unchanged fingerprint buys nothing.

It does not protect a gate or a record, so the one thing you owe it is a
...
- Pass only when every hard criterion has direct, current, complete evidence.
