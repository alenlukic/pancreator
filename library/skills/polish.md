...
- A non-empty prompt is the design task itself. Perform that task under the
...
1. Capture the surface, score it against the handbook heuristics and the
   ...
   `BROWSER-001` governs how captures are taken; follow
   `library/skills/browser-inspection.md`. Iteration produces craft feedback,
   so the disclosed capture fallback that skill names is permitted.
2. Ensure cross-surface consistency through the same loop: two surfaces that
...
- An empty working-tree diff with an empty prompt ends the session: report
  that there is nothing to polish.
