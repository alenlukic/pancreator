...
10. Run `npm{{PANCREATOR_NPM_PREFIX}} run format:check`, `npm{{PANCREATOR_NPM_PREFIX}} run typecheck`, and `npm{{PANCREATOR_NPM_PREFIX}} run validate`, inside the resolved worktree path when the operator named one. If validation fails, provide the exact failures to the release steward for one correction pass and rerun the failed checks.
