...
3. For every chunk run that is not terminal, run these commands in this order per run. `{{PANCREATOR_PAN_COMMAND}} prepare` and `{{PANCREATOR_PAN_COMMAND}} submit` refuse with `SUPERVISOR_CARD_UNATTESTED` or `REDLINE_MISSING` until the card and the redline are done for that run.
   - `{{PANCREATOR_PAN_COMMAND}} status <run-id> --json`. Keep its managed worktree name.
   - `{{PANCREATOR_PAN_COMMAND}} governance card --mode supervisor --run <run-id>`. Read the card it writes in full.
   - `{{PANCREATOR_PAN_COMMAND}} governance attest-supervisor <run-id> --sha256 <digest>` with the digest it reported.
   - `{{PANCREATOR_PAN_COMMAND}} status <run-id> --redline --occasion pan-cohort`. Quote each redline record path in your first report.
