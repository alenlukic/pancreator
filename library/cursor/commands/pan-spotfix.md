...
3. Run `./bin/pan governance card --mode spotfix --request <harness-relative-input>` and read the card it writes. It resolves the complete spotfix governance, including `SPOT-001` and the unrolled spotfix procedure; do not assemble policy text by hand.
