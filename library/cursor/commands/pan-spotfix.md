...
3. Run `./bin/pan governance card --mode spotfix --request <harness-relative-input>` and read the card it writes. It resolves the complete spotfix governance, including `SPOT-001` and its reference to the spotfix procedure. Read that reference. Do not assemble policy text by hand. When the operator names a worktree, add `--worktree <name>` to create or resolve it. The card then binds the session workspace to that worktree.
