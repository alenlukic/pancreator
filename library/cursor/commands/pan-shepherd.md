...
3. Run `./bin/pan governance card --mode shepherd --request <harness-relative-input>` and read the card it writes. It resolves the complete shepherd governance, including `SHEPHERD-001` and its reference to the shepherd procedure. Do not assemble policy text by hand.
