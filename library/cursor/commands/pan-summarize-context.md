...
2. Capture the goal, relevant background, current repository or artifact state,
   decisions already made, work completed, validation done, unresolved
...
4. Produce exactly one fenced Markdown block containing a standalone handoff.
   Use concise headings and bullets so another agent can copy the entire block
   into a new agent conversation as-is.
5. Do not create or change files, run workflow transitions, delegate to another
