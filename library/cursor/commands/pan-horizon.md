...
2. For each task run, the fresh driver process runs `{{PANCREATOR_PAN_COMMAND}} governance card --mode supervisor --run <run-id>` as its first run action, reads the card, and attests it under the preflight authorization. Do not perform that per-run step in this chat.
