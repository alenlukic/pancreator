...
7. Arm one watch per launched run in the launch turn before any other action, under `DELEGATE-001`: `{{PANCREATOR_PAN_COMMAND}} watch <run-id> --mark-background` when the platform backgrounds a launch, `--foreground-returned` when a launch returns with its declared output present, a plain `watch` otherwise. Run each watch as a foreground blocking call (one-hour default bound). When the platform detaches a blocking watch call, run `{{PANCREATOR_PAN_COMMAND}} watch --attach <ledger>` at once. Never call `AwaitShell`. Observe every launched process at least every 60 seconds and never end your turn while one is unfinished.
