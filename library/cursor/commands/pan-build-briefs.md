...
4. Invoke the `pan-librarian` subagent with the target workspace root and those two output paths. Require it to read `docs/operator-brief-system.md`, `library/operator-briefs/primitives.json`, both current project files, target-owned documentation, and a bounded sample of recurring operator-facing artifacts or workflows.
