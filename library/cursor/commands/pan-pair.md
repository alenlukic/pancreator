...
2. If this conversation already established a pair card, reuse that card and skip to step 4. Otherwise run `{{PANCREATOR_PAN_COMMAND}} governance card --mode pair`, read the card it writes, and keep its path. It contains the complete resolved governance for this mode. Do not assemble policy text by hand and do not generate a second card for the same session. When the operator names a worktree, add `--worktree <name>` to create or resolve it. The card then binds the session workspace to that worktree.
...
5. For each operator directive, including every directive after the first:
   - If the conversation was summarized or compacted and you no longer hold the card's contents, re-read the card file before acting. Re-read it, do not regenerate it, and do not proceed on a remembered summary of its governance.
...
7. Do not create or advance a workflow run, and do not write workflow state. Do not produce stage outputs or operator briefs, and do not invoke `pan submit`, `pan set-stage`, or `pan decide`.
