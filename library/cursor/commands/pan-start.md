...
3. Derive init options from the preserved request, following **Start** in the brief. When the request names a worktree for the run, pass `--worktree <name>` and do not combine it with `--workspace`. Do not pass `--workflow` for delivery work. The default is `planning`, and approval of its ratified plan routes the work itself. A single-chunk plan starts one `delivery` run, and a wider plan starts cohort 1. Pass `--workflow prototype` or `--workflow design` only when the brief's **Start** rules select them. Pass `--no-autostart` only when the operator explicitly asked to stop at the ratified plan.
4. Run `{{PANCREATOR_PAN_COMMAND}} init --request <preserved-request-path>` with those options. The request path is always the `--request` option. `init` refuses a positional path with `REQUEST_REQUIRED`.
