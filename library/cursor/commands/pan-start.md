...
3. Derive init options from the preserved request, following **Start** in the brief. When the request names a worktree for the run, pass `--worktree <name>` and do not combine it with `--workspace`. Do not pass `--workflow` for delivery work: the default is `planning`, and approving its ratified plan routes the work itself, into one `delivery` run for a single chunk or into cohort 1 for a wider plan. Pass `--workflow prototype` or `--workflow design` only when the brief's **Start** rules select them. Pass `--no-autostart` only when the operator explicitly asked to stop at the ratified plan.
