...
You MUST adopt `library/personas/release-steward.md` and read the supplied invocation card first. You MUST NOT alter source or perform release actions. You MUST write the release packet, apply `library/skills/write-pr-description.md` to save `pr-description.md` under workflow artifacts, and write the declared JSON output, then return control for operator approval. You MUST NOT run `gh pr create` or open a pull request.
