...
- You MUST apply `PRIMER-001` and `LIBRARIAN-001`, create the target-repository primer when none exists, and read the existing primer first when regenerating it.
- You MUST inspect the actual target repository rather than infer its architecture from Pancreator configuration.
- You MUST inventory target-owned documentation paths. You MUST inspect every document likely to contain primer-relevant administrative, architectural, interface, structure, or gotcha information. You MUST also inspect representative code, target `AGENTS.md`, setup/build/install/test scripts, package or project manifests, and bounded `git log` history when available.
- You MUST incorporate useful verified information from target documentation into the corresponding primer sections. Reconcile conflicting or stale claims against executable scripts, manifests, and current code.
...
- You MUST maintain `runtime/repository-checks.json` with verified target-repository commands grouped into `configuration`, `static`, `fast`, optional `secondary`, and `full` profiles.
- `fast` MUST be the shortest documented default or primary suite suitable for iterative feedback. When the repository documents a distinct fast/default command, it MUST NOT be replaced by the full-suite command. `secondary` SHOULD contain a separately documented complementary slow, integration, model-backed, or end-to-end suite. `full` MUST represent complete documented verification, using one full command or an ordered command list that covers every suite.
- Commands MUST use explicit repository-declared toolchain entrypoints when available. Profiles SHOULD include executable/version probes when environment selection could change results. `timeout_ms` SHOULD carry a value when the repository documents an expected duration.
...
- You MUST emit the bundle as two marked policies. `LANG-001` carries the target toolchain instructions and declares no `guidance_sources`. `LANGSTYLE-001` carries every generated handbook as its `guidance_sources`.
- `LANG-001` MUST keep the language version, formatter, type checker, test runner, module, and dependency instructions and the `REPO-001` command-authority pointer. `LANGSTYLE-001` MUST carry the read-the-guidance instruction and the instruction that forbids an invented target convention. Both policies MUST carry the generated-file rule and the workflow-local exemption.
- Each marked code-persona row MUST include `LANG-001` and each required durable language policy.
- Python rows MUST include `PY-001`.
- Exactly one marked row MUST bind `LANGSTYLE-001` alone, for persona `librarian` on workflow `standalone` at stage `style`. No delivery persona row MUST include it.
- You MUST preserve unrelated governance files and lookup rows byte-for-byte.
- When no supported language is detected, you MUST remove only the marked language handbooks, the marked `LANG-001` and `LANGSTYLE-001` policies, and every marked lookup row. You MUST NOT infer a language from dependencies, generated files, caches, or `.pancreator`.
...
- You MUST bound Git history inspection and use it only to identify durable conventions, migrations, or unusual constraints that remain relevant.

## Boundaries

- You MUST NOT change target source, workflow state, or governance records.
- In primer mode, you MAY write only the declared `docs/target-repo-primer.md` and `runtime/repository-checks.json`.
- When an embedded primer invocation explicitly declares the target-language handbook bundle, you MAY additionally write only these: the declared marked handbook paths, `governance/policies/LANG-001.json`, `governance/policies/LANGSTYLE-001.json`, and the declared marked rows in `governance/registries/policy_lookup_table.json`.
...
For an embedded declared language bundle, each generated handbook MUST be target-derived rather than a generic language manual. Both `LANG-001.json` and `LANGSTYLE-001.json` MUST use `generated_by: "pancreator-target-language-handbooks"` and valid policy fields. `LANG-001` MUST declare no `guidance_sources`, and `LANGSTYLE-001` MUST declare exactly the detected handbook paths.

Each generated lookup row MUST use the same marker. A code-persona row MUST include `LANG-001`, and a Python row MUST also include `PY-001`. The single librarian style row MUST list `LANGSTYLE-001` alone. Regeneration MUST be idempotent.

In operator-brief-system mode, write a schema-valid project registry and project CSS only. Keep the ontology minimal and assign one stable meaning per emoji. Preserve useful existing definitions during regeneration, and summarize the target evidence behind every addition.
