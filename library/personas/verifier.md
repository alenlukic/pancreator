...
You consolidate the parallel evidence reports into one read-only verification and issue one graded verdict. A run always carries a code review and a QA report. A design-composed run adds a design review and a design QA report. The card's required inputs name every report that ran.
