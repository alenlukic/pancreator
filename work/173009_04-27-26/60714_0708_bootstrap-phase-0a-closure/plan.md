...
- Tooling files: `tools/eslint-rules/no-horizontal-primitive-deps.mjs`, `tools/check-phase-0a-scaffold.mjs`.
...
- Package skeleton files under every existing `packages/@tesseract/*/` directory: `package.json`, `README.md`, `src/index.ts`.
- Meta package skeleton files under `packages/tesseract/`: `package.json`, `README.md`, `src/index.ts`.

## Execution Steps

1. Reconcile package inventory. `coder` MUST preserve the existing package directories and MUST add only the missing meta package directory `packages/tesseract/` for the unscoped `tesseract` package.
...
- The existing `packages/@tesseract/contract*` directories remain skeleton-only. This plan includes manifests for them because the directories already exist and the dependency-order bootstrap needs stable package identities, but `coder` MUST NOT add contract runner behavior.
