...
references:
  ...
  - kind: lines
    path: work/173009_04-27-26/50991_0950_timestamp-naming-conventions/plan.md
    range: [1, 36]
    contentHash: TBD-on-commit
    note: "Plan stage architecture summary, touch set, execution steps, and verification plan."
  - kind: lines
    path: work/173009_04-27-26/50991_0950_timestamp-naming-conventions/touch-set.json
    range: [1, 130]
    contentHash: TBD-on-commit
  ...
  - kind: lines
    path: work/173009_04-27-26/50991_0950_timestamp-naming-conventions/migration-manifest.dry-run.json
    range: [1, 40]
    contentHash: TBD-on-commit
...
| ----------------------------------------------------------------------------------------------------- | -- | ------------- |
| `tests/compliance/timestamp-naming-conventions.yaml`                                                  | A  | yes           |
...
| `tools/migrate-timestamp-naming.mjs`                                                                  | A  | yes           |
| `memory/handbook/inbox-lifecycle.md`                                                                  | M  | yes           |
| `memory/features/timestamp-naming-conventions/index.json`                                             | M  | yes           |
| `tools/migrate-timestamp-naming.test.mjs`                                                             | A  | parent-ratified add #1 |
| `work/173009_04-27-26/50991_0950_timestamp-naming-conventions/migration-manifest.dry-run.json`                                   | A  | parent-ratified add #2 |
| `memory/adr/0005-timestamp-naming-conventions.md`                                                     | A  | parent-ratified add #3 |
| `package.json`                                                                                        | M  | parent-ratified add #4 |
| `work/173009_04-27-26/50991_0950_timestamp-naming-conventions/adr-draft.md`                                                      | M  | annotated promotion stub (in feature workspace) |
| `work/173009_04-27-26/50991_0950_timestamp-naming-conventions/policy-compliance.json`                                            | M  | required policy-compliance gate artifact (AGENTS.md §5) |

The four parent-ratified additions match the coder's return message exactly.
Citation: `{kind: lines, path: work/173009_04-27-26/50991_0950_timestamp-naming-conventions/touch-set.json, range: [1, 130], contentHash: TBD-on-commit}`.
...
| L3 | info    | `{kind: lines, path: tools/migrate-timestamp-naming.mjs, range: [305, 409], contentHash: TBD-on-commit}` | `inventoryReferences` scans `work/` including the dry-run manifest itself, so re-running with `--strict-references` inflates `referenceUpdateRows` (committed: 451; second run: 1700) because the manifest text contains every legacy path token it plans to rewrite. | Either exclude `work/173009_04-27-26/50991_0950_timestamp-naming-conventions/migration-manifest*.json` from `inventoryReferences` scan roots, or regenerate the dry-run manifest immediately before the write-mode commit so counters track reality. |
...
| --------------------------------------------- | ----- | -------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `tesseract.features.timestamp_naming.policy_descriptor`     | tests | block    | deferred-runner-not-wired (structural pass) | Compliance descriptor parses and lists `structure-change` + `operator-on-demand`; no rego/llm-judge runner panel wired in this slice. Citation: `{kind: lines, path: tests/compliance/timestamp-naming-conventions.yaml, range: [1, 18], contentHash: TBD-on-commit}`. |
| `tesseract.features.timestamp_naming.migration_precedence` | tests | block    | deferred-runner-not-wired (test pass)        | `pnpm migration:test` exercises four `chooseTimestamp` precedence cases (git, frontmatter, mtime, override) and exits 18/18 green. Citation: `{kind: lines, path: tools/migrate-timestamp-naming.test.mjs, range: [115, 152], contentHash: TBD-on-commit}`. |
| `tesseract.features.timestamp_naming.reference_update_audit` | tests | block    | deferred-runner-not-wired (script pass)      | `node tools/migrate-timestamp-naming.mjs --dry-run --strict-references` returns exit 0 with `strictReferenceAudit.staleRows: []`. Citation: `{kind: lines, path: tools/migrate-timestamp-naming.mjs, range: [789, 818], contentHash: TBD-on-commit}`. |
...
| - | ----------------------------------------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------- |
| P1 | The intake directive and operator response NEVER appear as rename candidates.                        | pass   | `{kind: lines, path: tools/migrate-timestamp-naming.mjs, range: [433, 457], contentHash: TBD-on-commit}` (`EXCLUDED_SOURCES` set + `isExcludedFromMigration`)                                                                                                                                                                                                                                                                                       |
| P2 | `work/*/run.log.jsonl` NEVER receives a rename plan; basename SHALL stay `run.log.jsonl`.            | pass   | `{kind: lines, path: tools/migrate-timestamp-naming.mjs, range: [449, 456], contentHash: TBD-on-commit}` (`/run.log.jsonl` short-circuits inclusion); manifest spot-check confirms zero file-level rename rows for `run.log.jsonl`.                                                                                                                                                                                                                  |
| P3 | Write mode requires `TESSERACT_MIGRATION_GO=1` AND `--write`.                                        | pass   | `{kind: lines, path: tools/migrate-timestamp-naming.mjs, range: [896, 921], contentHash: TBD-on-commit}` (env-var guard with stderr message and non-zero exit when missing)                                                                                                                                                                                                                                                                          |
| P4 | `--strict-references` audit fails loudly (non-zero exit) when uncovered legacy hits exist.           | pass   | `{kind: lines, path: tools/migrate-timestamp-naming.mjs, range: [789, 938], contentHash: TBD-on-commit}` (`stale.length > 0` -> `console.error` + `process.exitCode = 1`)                                                                                                                                                                                                                                                                            |
| P5 | Existing day-directory paths matching `^\d{6}_\d{2}-\d{2}-\d{2}$` are skipped on re-run.             | pass   | `{kind: lines, path: tools/migrate-timestamp-naming.mjs, range: [463, 483], contentHash: TBD-on-commit}` (`isDayDirectoryName` guard inside `listFlatWorkTasks`)                                                                                                                                                                                                                                                                                     |
...
| - | ----------------------------------------------------------------- | -------------------------- | -------- | --------- | ---------- | ---- | ------------------------------------------------- | ------------------------------------------------------------------------------------------ | ------ |
| 1 | `work/173010_04-26-26/23777_1723_policy-compliance-gate-2026-04-26/`                         | 2026-04-26T17:23:43Z      | 173010   | 04-26-26  | 23777      | 1723 | `policy-compliance-gate-2026-04-26`               | `work/173010_04-26-26/23777_1723_policy-compliance-gate-2026-04-26/`                       | pass   |
| 2 | `work/173009_04-27-26/51356_0944_bootstrap-phase-4-pipeline-roster/`                         | 2026-04-27T09:44:04Z      | 173009   | 04-27-26  | 51356      | 0944 | `bootstrap-phase-4-pipeline-roster`               | `work/173009_04-27-26/51356_0944_bootstrap-phase-4-pipeline-roster/`                       | pass   |
| 3 | `work/173009_04-27-26/55075_0842_tesseract-worktree-env-isolation-impl/`                     | 2026-04-27T08:42:05Z      | 173009   | 04-27-26  | 55075      | 0842 | `tesseract-worktree-env-isolation-impl`           | `work/173009_04-27-26/55075_0842_tesseract-worktree-env-isolation-impl/`                   | pass   |
| 4 | `work/173009_04-27-26/55309_0838_tesseract-persona-pipeline-runner-impl/`                    | 2026-04-27T08:38:11Z      | 173009   | 04-27-26  | 55309      | 0838 | `tesseract-persona-pipeline-runner-impl`          | `work/173009_04-27-26/55309_0838_tesseract-persona-pipeline-runner-impl/`                  | pass   |
| 5 | `work/173009_04-27-26/55884_0828_tesseract-observability-impl/`                              | 2026-04-27T08:28:36Z      | 173009   | 04-27-26  | 55884      | 0828 | `tesseract-observability-impl`                    | `work/173009_04-27-26/55884_0828_tesseract-observability-impl/`                            | pass   |
...
`{kind: lines, path: work/173009_04-27-26/50991_0950_timestamp-naming-conventions/migration-manifest.dry-run.json, range: [1, 11], contentHash: TBD-on-commit}`.

## Reference-update audit

`node tools/migrate-timestamp-naming.mjs --dry-run --strict-references` exits
...
| phase-0a scaffold          | `pnpm run check:phase0a`                                             | pass   | `node tools/check-phase-0a-scaffold.mjs` exits 0.                                                                                                                |
| migration tests            | `pnpm migration:test`                                                | pass   | `node --test tools/migrate-timestamp-naming.test.mjs` reports `# pass 18 / # fail 0`.                                                                            |
| package tests              | `pnpm -r run test`                                                   | pass   | All workspaces (vitest + node:test) report green; no regressions vs implement-stage commit.                                                                      |
| migration dry-run + strict | `node tools/migrate-timestamp-naming.mjs --dry-run --strict-references` | pass   | Exit 0; `staleRows: []`; basename derivations match independent re-derivation for all 5 sampled rows and the 35-row structural sweep.                            |

Coverage delta: the implement slice introduces 18 new node:test cases under
`tools/migrate-timestamp-naming.test.mjs` covering FDS rollover, SID
