...
| tesseract-engineer.mdc | yes | Broad list including `memory/**/*`, `work/**/*`, `.cursor/agents/**/*` | Scoped packages, tools, tests, configs, `memory/handbook`, `personas`, `skills/SKILL.md`, `.cursor/rules/*.mdc`, `inbox/in` | Remove default `work/**/*` and bulk memory activation per Engineering Spec. |
| intake-analyst.mdc | no | `memory/features/**/spec.md`, inbox threads and in | unchanged | Already narrow for intake work. |
| tech-writer.mdc | no | Feature delivery reports, work delivery-report, inbox/out | unchanged | Scoped to reporting artifacts. |
| supervisor.mdc | no | pipelines, run logs, checkpoints, postmortems | unchanged | Supervisor needs run-control paths; not broadened. |
| tech-lead.mdc | no | plan trio, memory/adr | unchanged | Scoped to planning surfaces. |
...
| persona-designer.mdc | no | `personas/**/*.md`, `.cursor/rules/**/*.mdc` | unchanged | Touches mirror parity; defer narrowing until human ratifies persona-designer scope. |
