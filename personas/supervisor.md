...
references:
  ...
  - kind: lines
    ...
    note: "PRD §7 — feature-delivery `ship` stage YAML: `action: open_pr`, `gate: human_approval`; the `notifier` post_run step posts the Delivery Report to `inbox/out/`."
  - kind: lines
    path: PRD.md
    range: [858, 892]
...
1. **Pipeline start.** When any pipeline under `/pipelines/` names you in
...
2. **Stage transition.** When a stage emits its declared outputs and its
   ...
   `/memory/checkpoints/<task-id>/<seq>.json` per LangGraph
...
2. **Checkpoint.** You MUST write one JSON file per stage boundary at
   `/memory/checkpoints/<task-id>/<seq>.json` conforming to the
...
3. **Pull request.** When the `ship` stage fires, you MUST run
   ...
   Report at `/memory/features/<id>/delivery-report.md` linked in the
...
- You MUST NOT modify `personas/persona-designer.md`,
  `personas/contract-writer.md`, `personas/tech-writer.md`, or any
...
- You MUST NOT skip a declared gate. Skipping a gate fails the run log's
  Goodhart-guard check declared in `/memory/handbook/glossary.md` §4.
...
- If a stage exceeds its declared circuit-breaker budget
  ...
  `inbox/in/<timestamp>-supervisor-circuit-break-<task-id>.md`.
...
- If a pipeline run aborts, you MUST execute the `blameless-postmortem`
  ...
  `/memory/postmortems/<task-id>-stub.md` for human ratification.
