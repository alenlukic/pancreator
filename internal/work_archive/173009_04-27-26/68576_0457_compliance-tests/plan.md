...
The implement slice SHALL keep compliance descriptors and schemas in the canonical `internal/tests/compliance/` tree, SHALL remove rejected duplicate files under `memory/features/compliance-tests/contracts/`, SHALL preserve operator and agent manual invocation for slice one, SHALL keep scheduler automation deferred to backlog, and SHALL preserve AGENTS trigger guidance for structure-change compliance runs.

## Implementation Tasks

1. Verify `internal/tests/compliance/schemas/latest.yaml` and `internal/tests/compliance/schemas/v1.yaml`; each schema MUST validate descriptor fields `schema_ref`, `id`, `severity`, `trigger_modes`, `scope`, and `assertion`.
2. Verify the three canonical seed descriptors in `internal/tests/compliance/`; each descriptor MUST use `schema_ref: "internal/tests/compliance/schemas/latest.yaml"` and one accepted severity value.
...
- Stay inside `internal/work_archive/173009_04-27-26/68576_0457_compliance-tests/touch-set.json`.
- Treat the `internal/tests/compliance/` files as the canonical assets.
...
- Verify canonical descriptors and schemas live only under `internal/tests/compliance/`.
