...
target_path: memory/adr/0006-active-vs-archival-memory.md
references:
  - kind: lines
...
When routine orientation starts, Tesseract SHALL load `memory/active/**` as the only memory tier intended for default orientation.
...
When Tesseract classifies `work/**`, `inbox/out/**`, and `inbox/threads/**`, Tesseract SHALL treat those paths as archival memory and explicit-read surfaces.

When Tesseract classifies `memory/features/**`, `memory/adr/**`, and `memory/backlog/**`, Tesseract SHALL treat those paths as durable memory.

When Tesseract classifies `memory/handbook/**`, `personas/**`, `skills/**`, `.cursor/rules/**`, and `.cursor/agents/**`, Tesseract SHALL treat those paths as internal operating content loaded by route.
...
When Tesseract evaluates physical migration of `work/**` or existing `memory/**` trees, Tesseract SHALL defer migration until one slice ships reference updates, compatibility shims, tests, a migration manifest, and rollback notes.

## Status

Status is proposed on 2026-05-09 for implement-stage review and later promotion to `memory/adr/0006-active-vs-archival-memory.md`.
...
- Negative: Operators must maintain active summaries so `memory/active/**` does not become another archival surface.
