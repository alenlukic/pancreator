...
landed in one orchestrated pass because `personas/coder.md` forbids writes
under `/memory/` and `/.cursor/rules/`, while the Engineering Spec requires
those surfaces.
...
- Added `memory/handbook/context-economy.md` and routed it from `memory/handbook/index.md`.
- Added `PRD.summary.md` and `PRD.index.md`; updated `AGENTS.md` and `README.md` for summary-first retrieval.
- Added `internal/tools/context-budget-report.mjs` plus `pnpm run context:budget` and `context:budget:test`.
- Staged `documentation-impact.json` and `policy-compliance.json`; extended `memory/backlog/index.yaml` with deferral follow-ups.
...
1. Confirm human-ratification items in `memory/features/cursor-token-economy/spec.md` are honored before merge.
