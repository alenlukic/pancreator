 ...
 * Files under `internal/packages/@tesseract/<name>/` SHALL NOT import or re-export
 * another primitive package except `core` and the current package.
 *
 * Carveout: `internal/packages/@tesseract/cli` and `internal/packages/@tesseract/mcp-server` are
 ...
 * @returns {string | null} Directory name of the primitive, or `null` if the file is not under `internal/packages/@tesseract/<id>/`.
 */
function primitiveNameFromFile(filePath) {
  const rel = path.relative(REPO_ROOT, filePath).split(path.sep).join("/");
  const m = /^internal/packages\/@tesseract\/([^/]+)\//.exec(rel);
  if (!m) {
    return null;
  }
  return m[1] ?? null;
