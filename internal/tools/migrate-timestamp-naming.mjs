 ...
 * `eslint.config.mjs` ignore globs for `internal/tools` plus `*.mjs` so helper-focused
 * scripts stay outside TypeScript-eslint parser scope.
 *
 * @see memory/features/timestamp-naming-conventions/spec.md (acceptance criteria, TBD-on-commit)
 */
...
/**
 * Computes planned rename for inbox files; preserves `inbox/threads/<feature>/` parent.
...
export function migrateTargetForInboxPath(absPath, ctx) {
  ...
  const parts = rel.split("/");
  if (parts[0] !== "inbox") {
    throw new Error(`migrateTargetForInboxPath: expected inbox/ path, got ${rel}`);
  }
  const d = ctx.chosenDate;
  const sid = secondsRemainingInDay(d);
  const hm = hhmm(d);
  ...
  if (parts[1] === "threads" && parts.length >= 4) {
...
const EXCLUDED_SOURCES = new Set([
  ...
  "inbox/in/timestamp_naming_conventions.md",
...
function listInboxFiles(repoRoot) {
  ...
  const threads = path.join(repoRoot, "inbox", "threads");
  if (existsSync(threads)) {
    for (const feat of readdirSync(threads, { withFileTypes: true })) {
      ...
      for (const e of readdirSync(d, { withFileTypes: true })) {
        if (e.isFile()) {
          const rel = `inbox/threads/${feat.name}/${e.name}`;
          if (!isExcludedFromMigration(rel)) {
