...
test("classifyExclusiveTier separates handbook, active work, archival, durable JSON, internal source", () => {
  ...
  assert.equal(classifyExclusiveTier("internal/work_archive/172997_05-09-26/example/plan.md"), "archival_memory");
  assert.equal(classifyExclusiveTier("internal/packages/@tesseract/core/src/index.ts"), "source_code");
  assert.equal(
    classifyExclusiveTier("internal/tests/compliance/schemas/latest.yaml"),
    "source_code",
  );
});
...
test("indexing policy excludes archival, full specs, and agent projections while keeping compact routes", () => {
  ...
  assert.ok(isIndexingExcluded("internal/work_archive/172997_05-09-26/run/plan.md", matchers));
