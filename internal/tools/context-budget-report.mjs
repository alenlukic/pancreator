...
export function classifyExclusiveTier(rel) {
  ...
  const activeWork = /^work(?:\/|$)/.test(r);
  if (activeWork) return "active_work";

  const archival =
    /^internal\/work_archive(?:\/|$)/.test(r) ||
    /^inbox\/out(?:\/|$)/.test(r) ||
  ...
  const src =
    /^internal\/packages(?:\/|$)/.test(r) ||
    /^internal\/tools(?:\/|$)/.test(r) ||
    /^internal\/tests(?:\/|$)/.test(r);
...
const TIER_META = /** @type {const} */ ({
  ...
  active_work: { label: "2 active work", dir: "`work/**`" },
  durable_memory: {
  ...
  archival_memory: {
    ...
    dir: "`internal/work_archive/**`, `inbox/out/**`, `inbox/archive/**`, `inbox/threads/**`",
  },
  internal_operating: {
  ...
  source_code: { label: "7 source code & tests", dir: "`internal/packages/**`, `internal/tools/**`, `internal/tests/**`" },
  generated_machine: {
...
function printReport() {
  ...
  const legacyScopes = [
    ...
    { id: "active_work", label: "Active work slice: work/**", pred: (rel) => /^work(?:\/|$)/.test(posixRel(rel)) },
    { id: "archived_work", label: "Archived work slice: internal/work_archive/**", pred: (rel) => /^internal\/work_archive(?:\/|$)/.test(posixRel(rel)) },
