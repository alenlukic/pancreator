...
4. Call `tess.inbox` with `tools/call` and `{}` arguments while the process `cwd` is your Tesseract repository root. The result SHALL list file names under `inbox/in/` as JSON.
