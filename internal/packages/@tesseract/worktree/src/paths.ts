...
/** Returns the canonical `.tess/worktrees` directory for a repository root. */
export function defaultWorktreesRoot(repoRoot: string): string {
  return path.resolve(repoRoot, ".tess", "worktrees");
}

/** Ensures `worktreesRoot` resolves to `repoRoot/.tess/worktrees`. */
