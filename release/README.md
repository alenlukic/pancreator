...
1. create the release commit, including the final `VERSION` and installable files;
2. add the resulting `version -> commit` entry to `index.json` in a later metadata commit.

The current `0.1` worktree is intentionally unindexed until an operator creates
