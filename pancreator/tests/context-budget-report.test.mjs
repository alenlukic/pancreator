...
import {
  ...
  posixRel,
} from "../lib/internal/tools/context/context-budget-report.mjs";

const HARNESS_ROOT = path.resolve(import.meta.dirname, "..", "..");
const PROJECT_ROOT = path.join(HARNESS_ROOT, "pancreator");
...
function eachIndexingPatternLine(cb) {
  const text = fs.readFileSync(path.join(HARNESS_ROOT, ".cursorindexingignore"), "utf8");
  text
    .split(/\r?\n/)
...
test("tier char sums reconcile with total corpus and indexable partitioning", () => {
  const s = computeTierAndIndexingStats(HARNESS_ROOT);
  let sumChars = 0;
  let sumFiles = 0;
...
test("indexing policy excludes internal product, archival, and agent projections while keeping external routes", () => {
  const matchers = indexingMatchersFromRoot(HARNESS_ROOT);
  assert.ok(isIndexingExcluded(".cursor/agents/tech-lead.md", matchers));
  assert.ok(isIndexingExcluded("pancreator/.pan/archive/inbox/in/example.md", matchers));
  assert.ok(isIndexingExcluded("pancreator/.pan/work/active-run/plan.md", matchers));
  assert.ok(isIndexingExcluded("pancreator/.pan/archive/work/172997_05-09-26/run/plan.md", matchers));
  assert.ok(isIndexingExcluded("pancreator/lib/inbox/notes/private.md", matchers));
  assert.ok(isIndexingExcluded("pancreator/.docs/PRD.md", matchers));
  assert.ok(isIndexingExcluded("pancreator/.docs/BOOTSTRAP.md", matchers));
  assert.ok(isIndexingExcluded("pancreator/.docs/README.md", matchers));
  assert.ok(isIndexingExcluded("pancreator/.docs/PRD.summary.md", matchers));
  assert.ok(isIndexingExcluded("pancreator/.docs/M1.index.md", matchers));
  assert.ok(isIndexingExcluded("AGENTS.md", matchers));
  assert.ok(!isIndexingExcluded("pancreator/lib/memory/active/README.md", matchers));
...
test("CLI exits zero with nine tiers and aggregate labels", () => {
  ...
  const r = spawnSync(process.execPath, [script, "--root", HARNESS_ROOT], {
    encoding: "utf8",
    cwd: HARNESS_ROOT,
  });
...
test("Cursor persona projections use one canonical file per persona", () => {
  const dir = path.join(HARNESS_ROOT, ".cursor", "agents");
  const names = fs
    .readdirSync(dir)
  ...
  for (const base of bases) {
    ...
    assert.ok(fs.existsSync(path.join(PROJECT_ROOT, "lib/personas", `${base}.md`)), `${base} persona source must exist`);
  }
});
...
test("persona and Cursor agent frontmatter avoids inline maxTurns comments", () => {
  const files = [
    ...fs
      .readdirSync(path.join(PROJECT_ROOT, "lib", "personas"))
      .map((f) => path.join(PROJECT_ROOT, "lib", "personas", f)),
    ...fs
      .readdirSync(path.join(HARNESS_ROOT, ".cursor", "agents"))
      .map((f) => path.join(HARNESS_ROOT, ".cursor", "agents", f)),
  ].filter((f) => f.endsWith(".md"));
  for (const file of files) {
    const rel = posixRel(path.relative(HARNESS_ROOT, file));
