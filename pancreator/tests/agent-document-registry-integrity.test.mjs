...
const REGISTRY_REL = "pancreator/lib/memory/handbook/agent-document-registry.md";
...
/** @param {string} rel */
function exists(rel) {
  return fs.existsSync(path.join(ROOT, rel)) || fs.existsSync(path.join(PROJECT_ROOT, rel));
}
...
function listPersonaFiles() {
  const dir = path.join(PROJECT_ROOT, PERSONA_DIR_REL);
  return fs
...
test("the deleted persona-execution-contract is not referenced anywhere live", () => {
  ...
  assert.equal(
    exists("pancreator/lib/memory/handbook/persona-execution-contract.md"),
    false,
