...
function runStatusBadge(status: RunRecord["status"], executionMode?: RunRecord["executionMode"]): RunHistoryStatus {
...
function RunHistoryRow({
  ...
  const badgeStatus = runStatusBadge(run.status, run.executionMode);
  const skipped = run.status === "skipped";
  const dryRun = run.executionMode === "dry-run";
  ...
  return (
    ...
    >
      <div className="automation-run-row-header">
        <RunStatusBadge status={badgeStatus} label={dryRun ? "dry run" : undefined} />
        ...
        <div className="automation-run-log-excerpt" data-testid={`automation-run-logs-${run.runId}`}>
          {run.status === "skipped" ? (
            <p className="automation-run-skipped-reason">
              {run.stderrSummary || (dryRun ? "Dry-run mode; nothing executed." : "Skipped due to concurrency lock.")}
            </p>
          ) : null}
          {run.stdoutSummary ? (
