...
function argvForSuiteStep(suiteId: SuiteId, stepIndex: number, repoRoot: string): string[] {
  ...
  if (suiteId === "compliance") {
    return ["node", path.join(repoRoot, "lib/internal/tools/compliance/run-compliance.mjs")];
  }
  ...
  if (suiteId === "repo-structure") {
    if (stepIndex === 0) {
      return ["node", path.join(repoRoot, "lib/internal/tools/checks/check-workspace-contracts.mjs")];
    }
