...
- You MUST limit execution to invocation stages: `compliance-audit, compliance`.
...
1. **Compliance-stage exit bundle.** When you run the `compliance` stage, you
   ...
   bundle MUST NOT include `node lib/internal/tools/compliance/run-compliance.mjs`.
2. **Compliance descriptor harness.** When `DOC.COMPLIANCE_RUNS`
   ...
   through `runCompliance` in `lib/internal/tools/compliance/run-compliance.mjs` against
...
| `CA-DESC`   | Compliance descriptor assertions                    | Compliance descriptor harness | `node lib/internal/tools/compliance/run-compliance.mjs` over `tests/compliance/*.yaml`                                                                  | severity-mapped           |
