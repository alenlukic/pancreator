...
- You MUST limit execution to invocation stages: `plan`.
...
- You MUST baseline and record pre-existing repo-wide lint/test/compliance failures in `handoff.md` before coder handoff so downstream stages treat them as visibility-only unless the active run causes them.
