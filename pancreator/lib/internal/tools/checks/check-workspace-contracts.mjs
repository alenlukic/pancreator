#!/usr/bin/env node
/**
 * @file Workspace contract check: package dependency boundaries, required
 * files, and work directory naming hygiene.
 *
 * Source imports are covered by @pancreator/no-horizontal-primitive-deps in
 * ESLint.
 *
...
const PRIMITIVE_DIRS = readdirSync(
  path.join(PROJECT_ROOT, "lib", "internal", "packages", "@pancreator"),
  { withFileTypes: true },
)
...
function err(msg) {
  failed += 1;
  console.error(`[check-workspace-contracts] ${msg}`);
}
...
const workspacePath = path.join(WORKSPACE_ROOT, "pnpm-workspace.yaml");
if (!existsSync(workspacePath)) {
  err("pnpm-workspace.yaml is missing.");
} else {
  const ws = readFileSync(workspacePath, "utf8");
  const pancreatorScopeCovered =
    ws.includes("pancreator/lib/internal/packages/@pancreator/*") ||
    ws.includes("lib/internal/packages/@pancreator/*");
  if (!pancreatorScopeCovered) {
    for (const name of PRIMITIVE_DIRS) {
      if (!ws.includes(name)) {
        err(
          `Workspace file must include a glob for scoped primitives (e.g. pancreator/lib/internal/packages/@pancreator/*) or list @pancreator/${name} explicitly.`,
        );
        break;
      }
    }
...
for (const name of PRIMITIVE_DIRS) {
  ...
  for (const f of [
    ...
    if (sec && typeof sec === "object" && !Array.isArray(sec)) {
      checkDepSection(/** @type {Record<string, string>} */ (sec), pkgPath, f, name);
    }
  }
}
...
if (process.argv.includes("--list-packages")) {
  const all = new Set(PRIMITIVE_DIRS.map((d) => `@pancreator/${d}`));
  for (const id of Array.from(all).sort()) {
    console.log(id);
  }
}

const workRoot = path.join(PROJECT_ROOT, ".pan/work");
