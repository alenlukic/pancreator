 ...
 *
 * @see lib/memory/features/delivery-pipeline/inbox-convention-migration/index.json
 */
 ...
 * `.pan/work/<day-dir>/` reference in the same frontmatter block.
 * @param {string} text
 * @returns {string | null} ISO-8601 instant
 */
/**
...
export function resolveWorkDayDirForPipelineTask(repoRoot, taskId) {
  for (const rootRel of ["lib/.pan/work", "lib/internal/work_archive"]) {
...
export function findFeatureIntakeIso(repoRoot, featureSlug) {
  const roots = [
    path.join(repoRoot, "lib", "inbox", "in"),
    path.join(repoRoot, ".pan/archive", "inbox", "in"),
  ];
...
export function readInboxSourceText(repoRoot, sourceRel) {
  ...
  const archiveRoot = path.join(repoRoot, ".pan/archive", "inbox", "in");
...
export function listInboxArtifactRows(repoRoot) {
  ...
  const roots = /** @type {const} */ ([
    { queue: "in", segs: ["lib", "inbox", "in"] },
    { queue: "out", segs: ["lib", "inbox", "out"] },
    { queue: "archive-in", segs: [".pan/archive", "inbox", "in"] },
    { queue: "threads", segs: ["lib", "inbox", "threads"] },
  ]);
...
/**
 * Target queue prefix (POSIX segments, no `lib/inbox` prefix).
 * @param {'in' | 'out' | 'archive-in' | 'threads'} queue
 */
function queuePrefixSegments(queue) {
  switch (queue) {
    case "in":
      return ["lib", "inbox", "in"];
    case "out":
      return ["lib", "inbox", "out"];
    case "archive-in":
      return [".pan/archive", "inbox", "in"];
    case "threads":
      return ["lib", "inbox", "threads"];
    default: {
  ...
  for (const e of enriched) {
    ...
    if (e.queue === "threads" && sourceParts.length > 4) {
      ...
      if (e.threadFeatureId && !DAY_DIR_RE.test(e.threadFeatureId)) {
        dirsToDrop.add(path.posix.join("lib/inbox/threads", e.threadFeatureId) + "/");
      }
...
function directoryIsEffectivelyEmpty(dirAbs) {
  try {
    ...
    return entries.every((name) => isIgnorableDirEntry(name));
  } catch {
    return false;
  }
}
...
function main() {
  ...
  if (!args.write) {
    ...
    writeFileSync(outPath, stringifyRepoJson(manifest, args.root), "utf8");
    console.log(
