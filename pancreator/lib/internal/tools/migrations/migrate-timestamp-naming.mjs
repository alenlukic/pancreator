 ...
 * `.pan/work/timestamp-naming-conventions/migration-manifest.dry-run.json`.
 *
 * Carveout: this file is intentionally excluded from ESLint via
 * `eslint.config.mjs` ignore globs for `lib/internal/tools` plus `*.mjs` so helper-focused
 * scripts stay outside TypeScript-eslint parser scope.
 *
 * @see lib/memory/features/quality-governance/timestamp-naming-conventions/index.json (acceptance criteria, TBD-on-commit)
 */
...
/**
 * Computes planned relocation for `.pan/work/<task>/` (flat layout).
...
export function migrateTargetForWorkPath(absPath, ctx) {
  ...
  if (parts.length < 3 || parts[0] !== ".pan" || parts[1] !== "work") {
    throw new Error(`migrateTargetForWorkPath: expected .pan/work/ path, got ${rel}`);
  }
  const slug = parts[2];
  if (parts.length !== 3) {
    throw new Error(
      `migrateTargetForWorkPath: expected flat .pan/work/<slug>, got ${rel}`,
    );
  }
  ...
  const targetRel = `.pan/work/${dayDir}/${taskBasename}`;
  return { sourceRel: `${rel}/`, targetRel: `${targetRel}/`, dayDir, taskBasename };
}

/**
 * Computes planned rename for inbox files; preserves `lib/inbox/threads/<feature>/` parent.
...
export function migrateTargetForInboxPath(absPath, ctx) {
  const rel = path.relative(ctx.repoRoot, absPath).split(path.sep).join("/");
  const parts = rel.split("/");
  if (parts.length < 3 || parts[0] !== "lib" || parts[1] !== "inbox") {
    throw new Error(`migrateTargetForInboxPath: expected lib/inbox/ path, got ${rel}`);
  }
  const d = ctx.chosenDate;
  const sid = secondsRemainingInDay(d);
  const hm = hhmm(d);
...
const EXCLUDED_SOURCES = new Set([
  ".pan/work/timestamp-naming-conventions/plan.md",
  ".pan/work/timestamp-naming-conventions/adr-draft.md",
  ".pan/work/timestamp-naming-conventions/touch-set.json",
  "lib/inbox/in/timestamp_naming_conventions.md",
]);
...
function listFlatWorkTasks(repoRoot) {
  const w = path.join(repoRoot, ".pan/work");
  if (!existsSync(w)) {
    return [];
  }
  const out = [];
...
function inboxConventionRunDir(repoRoot) {
  ...
  return (
    findTaskRunDir(repoRoot, ".pan/archive/work", taskId) ??
    findTaskRunDir(repoRoot, ".pan/work", taskId)
  );
}
 ...
 * @param {string} dest
 */
export function writeManifest(manifest, dest, repoRoot = REPO_ROOT) {
  mkdirSync(path.dirname(dest), { recursive: true });
  writeFileSync(dest, stringifyRepoJson(manifest, repoRoot), "utf8");
}
...
async function main() {
  ...
  if (args.rollback) {
    ...
    console.log(
      stringifyRepoJson({ rollbackPlanned: back.length, steps: back }, repoRoot).trimEnd(),
    );
    return;
  }
  ...
  const outPath = args.manifestPath
    ? path.resolve(args.manifestPath)
    : defaultManifest;
  writeManifest(manifest, outPath, repoRoot);
  console.log(
