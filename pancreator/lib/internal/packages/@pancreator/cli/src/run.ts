...
export async function parseAndRun(
  ...
  program
    ...
    .description(
      "Archive all non-active feature-delivery runs, orphan work directories, and related inbox surfaces",
    )
    .action(async () => {
      const result = await runArchiveSweep(repoRoot, { clock: options?.clock });
      emit(writeOut, repoRoot, result);
