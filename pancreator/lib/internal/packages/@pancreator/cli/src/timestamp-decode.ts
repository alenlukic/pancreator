/** UTC instant for FDS — mirrors `lib/internal/tools/migrations/migrate-timestamp-naming.mjs`. */
...
export function formatCountdownIdLine(dayBucket: string, taskOrEntryId: string): string {
  const decoded = decodeCountdownTimestamp(dayBucket, taskOrEntryId);
  if (!decoded.ok) {
    return `${dayBucket}/${taskOrEntryId} (${decoded.diagnostic})`;
  }
  return `${dayBucket}/${taskOrEntryId} (${decoded.utcLabel})`;
}
...
/** @deprecated Prefer {@link parseRunDirParts}; kept for existing imports. */
export function parseWorkRunDir(runDirRel: string): { dayBucket: string; taskId: string } | null {
  return parseRunDirParts(runDirRel);
}

export function parseInboxEntryPath(entryPath: string): { dayBucket: string; taskId: string } | null {
  const normalized = entryPath.replace(/\\/gu, "/").replace(/^\/+/, "");
  ...
  return { dayBucket: parts[0]!, taskId };
}
