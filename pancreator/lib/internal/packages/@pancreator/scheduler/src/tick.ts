...
export type TickExecutionMode = "dry-run" | "live";
...
async function dispatchAutomation(
  ...
  const status: RunStatus =
    dispatch.executionMode === "dry-run" ? "skipped" : dispatch.ok ? "success" : "error";
  await updateRunRecord(repoRoot, automation.id, runId, {
    ...
    ...(dispatch.executionMode ? { executionMode: dispatch.executionMode } : {}),
  });
