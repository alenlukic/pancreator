...
at `lib/memory/adr/0001-q-ratification.md`.
...
| Q10 | `/lib/memory/backlog/` is canonical; M3 adds sync adapters | ratified |
...
| Q17 | `lib/personas/*.md` is canonical; emit `.cursor/agents` mirror; keep `.mdc` only where rule-loading still requires it | ratified |
