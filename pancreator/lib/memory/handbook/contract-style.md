...
schema in `/lib/memory/handbook/contract-format.md`, and the prose passes the
...
Adding a word to the ban list requires an RFC under `/lib/memory/rfc/draft/`.
Removing a word requires the same.

### Rule 1.7 — Glossary discipline

Every domain noun MUST resolve to an entry in `/lib/memory/handbook/glossary.md`
...
table per clause. Exceedances MUST carry a `pancreator.lint-debt` entry with
...
the dual-anchor form defined in `/lib/memory/handbook/glossary.md` (kind:
...
# custom:
#   pancreator.contract_id: <wrapper.id>
#   pancreator.applies_to: <serialized applies_to>
package pancreator.<area>
```
...
- The package name MUST start with the `pancreator.` namespace for clauses
  authored under `/lib/memory/`. Vendor-authored modules MUST use the
...
example per assertion. The OpenAPI extension `x-pancreator.contract_id` MUST
...
`/lib/memory/handbook/contract-templates/<name>.template.md`. The MVP set is
...
the author MUST open an RFC under `/lib/memory/rfc/draft/` proposing the
...
Every `wrapper.owner` MUST resolve to a persona file under `lib/personas/`. The
runner refuses orphan clauses.

### Rule 4.2 — Owner is in scope

Every `wrapper.owner`'s `metadata.pancreator-pipeline-stages` MUST include the
...
authoring persona's `metadata.pancreator-allowed-kinds-<milestone>` field.
...
A template change requires an RFC under `/lib/memory/rfc/draft/`. The RFC SHALL
...
`/lib/memory/adr/`. The ADR SHALL declare the milestone the kind is allowed in
and SHALL update the allowlist on `lib/personas/contract-writer.md`.
...
The aggregate `pancreator.lint-debt` count across the repo MUST be at most:
...
From Phase 3 step 2 onward, `pan lint contracts --explain` automates steps 1
