...
`/lib/memory/debt/` tree. When an item records groomer or tech-debt-scan work,
the writer MUST add:
...
metadata:
  ...
  owners: [tech-lead, librarian]
  source_of_truth: /lib/memory/backlog/index.yaml

items:
  - id: bootstrap-prd-hash-refresh
    title: Refresh TBD-on-commit content hashes
    source: lib/memory/adr/0001-backlog-tracking.md
    owner: librarian
    milestone: phase-1
    status: open
    priority: high
