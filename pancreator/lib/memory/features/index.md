...
| `bootstrap-phase-0a-closure` | ready-for-review | Phase 0a delivered the M1 primitive package skeleton under lib/internal/packages/@pancreator/ plus workspace checks; the temporary unscoped facade is deferred to M9. | `lib/memory/features/bootstrap-repo-ops/bootstrap-phase-0a-closure/index.json` |
...
Package-by-package substrate entries remain in historical artifacts, but live routing no longer enumerates each package row.
