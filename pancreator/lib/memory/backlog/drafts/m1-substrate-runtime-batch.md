...
This Feature SHALL close substrate and runtime gaps around checkpointer conformance (WP-A), Cursor SDK runner invocation (WP-B), pipeline StateGraph compiler (WP-C), run-logger Phoenix conformance (WP-D), and install paths (WP-F) so that the `feature-delivery` harness loop reaches a runnable end-to-end state with checkpoint resume and run-log observability conformance either proven or formally deferred. Library-distribution script examples (WP-E) are deferred to milestone M9 and are not part of the live M1 routing surface.
...
### WP-E — Library-mode script example (deferred to M9)
