...
const PRIMITIVE_DIRS = readdirSync(
  path.join(REPO_ROOT, "packages", "@tesseract"),
  { withFileTypes: true },
)
  .filter((d) => d.isDirectory())
  .map((d) => d.name);

const META_ROOT = "packages/tesseract";
...
if (!existsSync(workspacePath)) {
  ...
  const tesseractScopeCovered = ws.includes("packages/@tesseract/*");
  if (!tesseractScopeCovered) {
    for (const name of PRIMITIVE_DIRS) {
      if (!ws.includes(name)) {
        err(
          `Workspace file must include a glob for scoped primitives (e.g. packages/@tesseract/*) or list @tesseract/${name} explicitly.`,
        );
        break;
      }
    }
  }
  const metaCovered = ws.includes("packages/*");
  if (!metaCovered && !ws.includes("packages/tesseract")) {
    err(
      "Workspace file must include `packages/*` (so packages/tesseract is a member) or add `packages/tesseract` explicitly.",
    );
  }
  if (!existsSync(path.join(REPO_ROOT, "packages", "tesseract"))) {
    err("Meta package path packages/tesseract is missing on disk.");
  }
}
...
if (!existsSync(tesseractSrc)) {
  err("Missing packages/tesseract/src/index.ts.");
}
