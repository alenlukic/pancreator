...
export function StageMachineGrid({
  ...
  return (
    <div className="command-center-grids">
      {tasks.map((task) => {
        const isSelected = task.taskId === selectedTaskId;
        return (
          <section
            key={task.taskId}
            className="task-command-center"
            aria-label={`Pipeline ${taskDisplayLabel(task)}`}
            aria-selected={isSelected}
            data-testid={`task-command-center-${task.taskId}`}
          >
            <button
              type="button"
              className="task-command-center-select"
              onClick={() => onSelectTask(task.taskId)}
            >
              <h3 className="task-command-center-title">{taskDisplayLabel(task)}</h3>
            </button>
