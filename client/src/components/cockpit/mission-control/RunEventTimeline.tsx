...
export function RunEventTimeline({
  ...
  function matchesFilters(event: RunLogEvent): boolean {
    if (severityFilter === "retry" && !event.retryBadge) {
      return false;
    }
    if (severityFilter === "escalation" && event.escalationLabel === undefined) {
      return false;
    }
    if (severityFilter === "deferral" && !event.deferralBadge) {
      return false;
    }
    if (eventTypeFilter !== "all" && event.event !== eventTypeFilter && event.name !== eventTypeFilter) {
      return false;
    }
    return true;
  }
  ...
  return (
    <div className="run-timeline" data-testid="run-timeline">
      {visibleTasks.map((task) => {
        const events = (mergedRef.current[task.taskId] ?? task.runEvents).filter(matchesFilters);
        return (
          <section key={task.taskId} className="run-timeline-task">
            <h3>Run log · {taskDisplayLabel(task)}</h3>
            <div className="run-timeline-events">
              {events.map((event) => (
                <RunEventItem key={`${task.taskId}-${runEventKey(event)}`} event={event} />
              ))}
            </div>
          </section>
        );
