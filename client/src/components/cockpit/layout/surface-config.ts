...
export type CockpitSurfaceConfig = {
  id: CockpitSurfaceId;
  label: string;
  shortLabel: string;
  route: string;
...
export const COCKPIT_SURFACES: CockpitSurfaceConfig[] = [
  {
    id: "command-center",
...
export const FIRST_SLICE_SURFACES = COCKPIT_SURFACES.filter((surface) => surface.firstSlice);

/** Mobile tab bar order per ux-spec (differs from rail enumeration order). */
const MOBILE_TAB_ORDER: CockpitSurfaceId[] = [
  "command-center",
  "mission-control",
  "work-intake",
  "compliance",
...
export const MOBILE_TAB_SURFACES = MOBILE_TAB_ORDER.map(
  (id) => COCKPIT_SURFACES.find((surface) => surface.id === id)!,
);

export function getSurfaceByRoute(pathname: string): CockpitSurfaceConfig | undefined {
  const normalized = pathname === "/" ? "/command-center" : pathname;
  return COCKPIT_SURFACES.find((surface) => surface.route === normalized);
}
