...
const PROMPT_REL_PATH = "tests/compliance/context-usage/prompt.md";
...
export function buildLiveEnvelopeFromCalibration(calibration, modelCalibration) {
  ...
  return {
    ...
    source_report:
      calibration.source_raw ?? "tests/compliance/context-usage/baselines/overhead-calibration.json",
    metrics,
    tolerance,
    maxima,
 ...
 *   runtime?: { sdk_version?: string; node_version?: string; platform?: string };
 * }} envelopeInput
 */
export function buildLiveEnvelope(baseline, envelopeInput) {
  ...
  return {
    ...
    tolerance,
    maxima,
    runtime: envelopeInput.runtime ?? null,
  };
}
 ...
 * @param {{ expectedModelId?: string; expectedRuntimeSdkVersion?: string }} [options]
 */
export function compareToBudget(observed, baseline, options = {}) {
  const errors = [];
  /** @type {string[]} */
  const warnings = [];
  const expectedModelId = options.expectedModelId
    ? resolveSdkModelId(options.expectedModelId)
    : undefined;
  const baselineModelId = baseline.model ? resolveSdkModelId(String(baseline.model)) : undefined;
  if (expectedModelId && baselineModelId && expectedModelId !== baselineModelId) {
    errors.push(
      `baseline model mismatch: baseline.model=${baseline.model} expected_model=${expectedModelId}. Re-run pnpm run context:usage:baseline -- --model ${expectedModelId}`,
    );
  }
  if (!baseline.calibration_overhead) {
    errors.push(
      "missing calibration_overhead in baseline — run pnpm run context:usage:calibrate and pnpm run context:usage:calibrate:summary, then re-run pnpm run context:usage:baseline",
    );
  } else {
    const repoSamples = Number(
      baseline.calibration_overhead?.calibration_method?.repo_root_samples ?? 0,
    );
    const emptySamples = Number(
      baseline.calibration_overhead?.calibration_method?.empty_dir_samples ?? 0,
    );
    if (!Number.isFinite(repoSamples) || !Number.isFinite(emptySamples) || repoSamples < 1 || emptySamples < 1) {
      errors.push(
        "insufficient calibration sample counts in calibration_overhead — re-run context:usage:calibrate and context:usage:calibrate:summary",
      );
    }
  }
  ...
  if (!liveEnvelope?.maxima) {
    ...
    if (!liveEnvelope.runtime?.sdk_version) {
      warnings.push(
        "live_envelope missing runtime metadata — re-ratify with pnpm run context:usage:ratify -- <report.json>",
      );
    } else if (
      options.expectedRuntimeSdkVersion &&
      liveEnvelope.runtime.sdk_version !== options.expectedRuntimeSdkVersion
    ) {
      errors.push(
        `live_envelope runtime mismatch: sdk_version=${liveEnvelope.runtime.sdk_version} expected=${options.expectedRuntimeSdkVersion}. Re-ratify live envelope for current runtime.`,
      );
    }
    for (const key of LIVE_ENVELOPE_METRIC_KEYS) {
