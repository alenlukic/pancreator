...
test("model: model baseline helpers parse and resolve paths", () => {
  assert.equal(parseModelArg(["--model", "composer-2.5"]), "composer-2.5");
  assert.equal(parseModelArg(["--model=gpt-5.4-mini"]), "gpt-5.4-mini");
  assert.equal(baselineFileNameForModel("composer-2.5"), "composer-2.5.json");
  assert.equal(
    resolveModelBaselinePath("/tmp/context-usage", "composer-2.5"),
    "/tmp/context-usage/baselines/composer-2.5.json",
  );
});
...
test("collect-usage: ignores non-concrete and non-read tool paths", () => {
  const metrics = createEmptyMetrics();
  const toolPaths = [];
  processStreamEvent(
    {
      type: "tool_call",
      name: "glob",
      args: { glob: "**/*.md", target_directory: "/tmp" },
    },
    metrics,
    toolPaths,
  );
  ...
  assert.equal(metrics.tool_read_count, 1);
  assert.deepEqual(toolPaths, ["/tmp/allowed.md"]);
});

test("collect-usage: ignores usage on non-turn-ended events", () => {
  const metrics = createEmptyMetrics();
  processStreamEvent(
    {
      type: "assistant",
      usage: { inputTokens: 999, outputTokens: 888, cacheReadTokens: 777, cacheWriteTokens: 666 },
    },
    metrics,
    [],
  );
  assert.equal(metrics.input_tokens, 0);
  assert.equal(metrics.output_tokens, 0);
...
test("live-env: resolveHarnessRepoRoot points at repository root", () => {
  ...
  assert.ok(fs.existsSync(path.join(repoRoot, "tests/compliance/context-usage/prompt.md")));
});
...
function baselineWithLiveEnvelope() {
  ...
  baseline.live_envelope = buildLiveEnvelope(baseline, {
    ...
    source_report: "tests/compliance/context-usage/output/example-report.json",
    metrics: {
      input_tokens: 34_429,
      output_tokens: 769,
...
test("compareToBudget: model mismatch fails closed", () => {
  const baseline = baselineWithLiveEnvelope();
  const observed = createEmptyMetrics();
  observed.tool_read_count = baseline.budgets.tool_read_count.min;
  const cmp = compareToBudget(observed, baseline, { expectedModelId: "gpt-5.4-mini" });
  assert.equal(cmp.ok, false);
  assert.ok(cmp.errors.some((e) => e.includes("baseline model mismatch")));
});
