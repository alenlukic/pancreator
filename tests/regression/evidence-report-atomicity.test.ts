...
test('a report truncated part way through keeps every case recorded before it', () => {
  const prepared = checkpoint('delivery@verify-prepared')
  ...
  let report = `# ${worker.role} evidence\n\n`

  report += caseBlock('QA-001', 'pass')
  report += caseBlock('QA-002', 'fail')

  // The worker dies here, part way through its third case. Nothing else is
  // written, and no completion marker ever lands.
  const survived = readEvidenceReportState(report)
  ...
  report += `${EVIDENCE_REPORT_COMPLETE_MARKER}\n`

  const finished = readEvidenceReportState(report)
