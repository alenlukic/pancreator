...
import { listWorkflowSlugs, loadWorkflow, stageBySlug, validateWorkflow } from "../../src/lib/workflow.mjs";
...
test("loader fails when an indexed stage file is missing", () => {
  ...
  assert.throws(() => loadWorkflow(root, "dev"), /missing stage file stages\/ship\.json/);
});
...
test("workflow validation rejects unknown transition targets", () => {
  ...
  assert.throws(() => validateWorkflow(root, workflow, "fixture"), /unknown 'missing'/);
});
