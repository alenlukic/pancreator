...
test('the scan separates recorded use from silence inside the window', () => {
  ...
  assert.equal(tier('workflow:delivery'), 'execution')
  assert.equal(tier('persona:coder'), 'execution')
  assert.equal(tier('policy:ENG-001'), 'execution')
  assert.equal(tier('mode:spotfix'), 'execution')

  // The session names the mode its command resolves, which is the only
  // runtime trace a card-backed slash command leaves behind.
  assert.equal(tier('command:pan-spotfix'), 'execution')
...
test('a facility nothing names or references becomes a candidate', () => {
  const { root, transcripts } = createScannedFixture()
...
test('an operator selection cannot be widened past the candidate list', () => {
  const { root, transcripts } = createScannedFixture()
  ...
  assert.throws(
    () => selectDebloatFacilities(root, SESSION, ['persona:coder'], NOW),
    (error: unknown) =>
...
test('impact splits deletions from repairs and records the adjudication', () => {
  const { root, transcripts } = createScannedFixture()
  ...
  const summary = computeDebloatImpact(root, SESSION, NOW)
...
test('verify fails on a surviving path and on a dangling reference', () => {
  const { root, transcripts } = createScannedFixture()
  ...
  const before = verifyDebloat(root, SESSION, NOW)
  ...
  const dangling = verifyDebloat(root, SESSION, NOW)
  ...
  assert.equal(verifyDebloat(root, SESSION, NOW).status, 'clean')
})

test('a step run out of order names the command that produces its input', () => {
