...
test('spend card resolves librarian secret and reporting boundaries', () => {
  const root = createFixture()
  const card = buildGovernanceCard(root, {
    mode: 'spend',
    outputPath: 'runtime/inbox/spend-card.md',
  })
  const ids = card.policies.map((policy) => policy.id)

  assert.equal(STANDALONE_MODES.spend?.persona, 'librarian')
  assert.ok(ids.includes('ASK-001'))
  assert.match(card.markdown, /pan spend/u)
  assert.match(card.markdown, /raw usage events/u)
})
...
test('a missing operator input is reported rather than silently omitted', () => {
  ...
  assert.throws(
    ...
    /Available: author, best-of-n, build-briefs, build-docs, cleanup, conform, debloat, decomposition, harden, pair, polish, qa-workflow, release, repair, research, review, shepherd, spend, spotfix, style, supervisor, target, trace, tune-harness, unbound, write-pr/u,
  )
})
...
test('the card-less command modes resolve their persona governance', () => {
  ...
  const expectations: Record<string, string[]> = {
    ...
    research: ['STE-001', 'GLOBAL-001', 'GLOBAL-002'],
  }
