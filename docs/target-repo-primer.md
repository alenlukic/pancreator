...
- Release publication is a two-commit protocol: prepare synchronized version metadata, create the immutable release commit, then map that hash in `release/index.json`. The current `2.15.0` checkout is intentionally unindexed, so automatic embedded updates remain unavailable until publication.
