...
- `package.json` still names the project `pancreator-v2-prototype` while `VERSION` reports `5.3.0`. Treat `VERSION` as authoritative for the version and the npm `name` field as legacy metadata.
...
- Release publication is a two-commit protocol: prepare synchronized version metadata, create the immutable release commit, then map that hash in `release/index.json`. `VERSION`, `package.json`, and `package-lock.json` agree on `5.3.0`. `release/index.json` maps only 4.0.0, 5.0.0, and 5.0.1 to their release commits. The 5.1.0 mapping (release commit `733ad2eb`) and the 5.2.0 mapping (release commit `17a698ab`) still wait for the index commit. A 5.3.0 mapping can be added only after its release commit exists, so an embedded `bin/update` cannot fast-forward to those versions yet.
