...
- `./bin/pan governance card --mode <best-of-n|decomposition|investigation|pair|repair|review|shepherd|spotfix|unbound>` generates the governance card every non-workflow mode requires; `./bin/pan governance audit-directives` reports normative directives that no policy owns.
...
- `runtime/`: `inbox/`, `logs/`, `pr-descriptions/`, and `repository-checks.json`. Generated workflow records here are harness-owned.
...
- `config.json` is the public workspace and persona-model configuration surface. Shared `defaults` merge with the selected `active_config`, `operator_involvement` declares gate profiles, `verification` declares levels, and a persona value MAY carry the `claude-code:` executor prefix.
...
- `package.json` still names the project `pancreator-v2-prototype` while `VERSION` reports `4.10.0`. Treat `VERSION` as authoritative for the version and the npm `name` field as legacy metadata.
...
- Every non-workflow mode (`best-of-n`, `decomposition`, `investigation`, `pair`, `repair`, `review`, `shepherd`, `spotfix`, `unbound`) takes its governance from `./bin/pan governance card --mode <mode>`. Do not hand-assemble policy text from `governance/policies/`.
