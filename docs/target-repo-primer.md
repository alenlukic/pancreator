...
- `./bin/pan governance review-scope --target <ref> [--base <ref>] [--json]` reports the three-dot changed-path count for a review target and which of those paths belong to the review machinery itself, so a squad review never grades the lineup, charters, coordinator, or entry points that define how it judges.
