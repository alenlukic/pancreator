...
- `governance/policies/BROWSER-001.json` is the single canonical browser-inspection contract; `library/cursor/mcp.json` declares the servers it uses. The policy is unrolled into invocation cards and generated into `.cursor/rules/pan-browser-isolation.mdc` for work that runs outside a card.
...
- Browser inspection rules live only in `BROWSER-001`. Read the unrolled policy on the active card rather than a restatement; personas, prompts, and docs deliberately no longer duplicate it.
...
- The latest merged work consolidated browser-inspection guidance into `BROWSER-001` and unrolled the supervisor delegation contract onto every worker invocation card. Prefer current executable sources and manifests when older narrative text conflicts.
