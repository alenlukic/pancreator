...
- `governance/policies/BROWSER-001.json` is the single canonical browser-inspection contract; `library/cursor/mcp.json` declares the servers it uses. The policy text is inlined on invocation cards, its procedure skill is referenced there, and both are generated into `.cursor/rules/pan-browser-isolation.mdc` for work that runs outside a card.
...
- Run state, `events.jsonl`, snapshots, invocation records, and generated artifacts are harness-owned. New runs keep them under `runtime/logs/workflows/<run-id>/agent/` and keep operator-read files under `operator/`; runs created before that split keep both at the run root. Resolve a path from the active card rather than a fixed literal, and run lifecycle and maintenance through `./bin/pan`, not direct edits.
...
- Browser inspection rules live only in `BROWSER-001`. Read the policy on the active card and the procedure it references rather than a restatement; personas, prompts, and docs deliberately no longer duplicate it.
- Release publication is a two-commit protocol: prepare synchronized version metadata, create the immutable release commit, then map that hash in `release/index.json`. The current `2.19.0` checkout is unindexed (`release/index.json` has no releases), so automatic target updates remain unavailable until publication.
...
- `docs/workflow-authoring.md` still names a `fable` model configuration, while `config.json` declares `simple`, `default`, `complex`, and `auto`. Per-checkout model preferences live in the untracked `config.local.json`, which merges over `config.json`.
- Invocation cards use progressive policy disclosure: policy summaries, instructions, requirements, inputs, outputs, and boundaries are inline, while handbook and skill guidance arrives as an audited reference carrying source path, selected range, content digest, and read trigger. The exact selected content stays in the invocation JSON snapshot, digests cover the selection with surrounding whitespace trimmed, and the stage output attests each referenced read (or the reason it was skipped). Cards prepared before this change keep inline guidance bodies, and validation accepts both shapes.
- The latest merged work consolidated browser-inspection guidance into `BROWSER-001` and put the supervisor delegation contract onto every worker invocation card. Prefer current executable sources and manifests when older narrative text conflicts.
