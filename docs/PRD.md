...
todos:
  - id: ratify
    ...
    status: pending
  - id: monorepo-layout
    content: Set up @pancreator/* monorepo per §5.5 — pnpm workspace + catalogs + Turborepo + Changesets + @arethetypeswrong/cli + publint + sub-path exports + no-horizontal-deps ESLint rule + CI conformance check (US-8 + SOTA tooling foundation)
    status: pending
  - id: scaffold
    content: "Scaffold repo: AGENTS.md (primary contract), .cursor/rules/ (auto-shimmed), lib/personas/ (Claude-Code 16-field YAML), lib/pipelines/ (LangGraph-compiled), lib/ensembles/ (AutoGen-runtime), lib/memory/{handbook,adr,rfc,prd,features,smes,backlog,debt,checkpoints,adoption,runbooks,postmortems,research}/ (Spec-Kit paths), lib/inbox/, lib/personas/skills/ (Agent-Skills-spec), work/, .pan/{worktrees,sandboxes,scheduler}/"
    status: pending
  - id: handbook-seed
    content: Seed /lib/memory/handbook with constitution, glossary, /lib/memory/handbook/index.md routing table (Claude-Code MEMORY.md model), OpenInference run-log schema, dual-anchor citation conventions, contract format, threshold-policy schema (Conftest+Rego), intervention semantics (LangGraph primitives), AGENTS.md authoring guide
    status: pending
  - id: personas-mvp
  ...
  - id: skills-mvp
    ...
    status: pending
  - id: policy-schema
    content: Implement threshold-policy schema (pancreator.yaml + pancreator-defaults.yaml) compiled to Conftest + OPA Rego with SonarQube QualityGate field shape, one working default per risk tier, commands.test|build|lint hooks (US-4 + US-9 coexistence)
    status: pending
  - id: pipelines-mvp
  ...
  - id: control-plane
    ...
    status: pending
  - id: intervention-mvp
    content: Implement stage-boundary LangGraph checkpointing + pan pause/resume/abort mapped to LangGraph interrupt()/Command(goto)/checkpoint_id time-travel, with safety invariants (US-10 MVP)
    status: pending
  - id: feature-index
    content: "Implement Feature first-class entity using Spec-Kit path conventions: /lib/memory/features/<slug>/{spec.md,plan.md,tasks.md,index.json}, librarian-driven indexing, contract extraction"
    status: pending
  - id: mcp-server-skeleton
    content: "Author @pancreator/mcp-server skeleton: publish all CLI verbs as MCP Tools with shared handler core; expose /lib/memory/, /lib/inbox/, /work/run-log as MCP Resources; expose human.respond as MCP Elicitation (per Q18 1:1 verb surface with mcp.disabled opt-out)"
    status: pending
  - id: cli
    content: "Implement pan CLI MVP subcommands (Claude-Code-aligned grammar): init (greenfield + adopt), mcp [list|add|remove|status], inbox, run, chat, feature, status, approve, memory query, worktree, policy, pause, resume, abort"
    status: pending
  - id: run-logger-mvp
    content: Implement @pancreator/run-logger emitting OTLP-encoded spans with OpenInference + OTel GenAI semantic conventions; default JSONL local sink to /work/<day>/<task-id>/run.log.jsonl; conformance-tested against Phoenix/Langfuse importers
    status: pending
  - id: examples
    ...
    status: deferred
  - id: research-dossier
    content: (Optional) Persist all six research reports (3 founding + 3 SOTA-alignment) as /lib/memory/research/ founding dossier with dual-anchor citations
    status: pending
  - id: dogfood-us1
    content: "Dogfood US-1 end-to-end: drop a semi-formal spec into lib/inbox/in/, complete the canonicalization dialogue, ship a staged PR with delivery report in lib/inbox/out/, and verify OpenInference run-log renders cleanly in Phoenix/Langfuse (proves A20 ecosystem-reuse KPI from day 1)"
    status: pending
isProject: false
---

# Pancreator — Simulated Product-Org Agentic Development Ecosystem

> **Status:** Draft v0.4 — SOTA-alignment pass. Cross-agent research found high cross-confidence on ~22 substrate-adoption recommendations. v0.4 swaps internals to battle-tested SOTA libraries while preserving every Pancreator-novel abstraction. **Key adoptions (all high-confidence):** LangGraph.js `BaseCheckpointSaver` v1 schema + `StateGraph` runtime; OpenInference + OTel GenAI semconv for run-log; Claude Code `.claude/agents/*.md` 16-field persona spec (auto-shimmed to Cursor `.mdc`); AGENTS.md as primary cross-tool contract; Agent Skills open spec (`agentskills.io`); GitHub Spec Kit file conventions for `/lib/memory/features/<id>/`; Mergiraf 0.16.x as default merge driver; Conftest + OPA Rego under threshold-policy YAML; Schemathesis v4.8 at M2 (not M3); AutoGen/AG2 GroupChat as Persona Ensemble runtime (with operationalized sycophancy parameters per 2025–26 MAD literature); GEPA + Voyager-style metadata for M8 self-improvement; Mem0-shaped + Letta-tier `MemoryStore` interface; AST-symbol + content-hash citations (not bare `path:line`); Cedar `AuthorizationEngine` shape for `Authorizer`; MCP server (`@pancreator/mcp-server`) + MCP elicitations for human interfaces; A2A v1.x bring-forward to M5; Mastra-style monorepo tooling (pnpm catalogs + Turborepo + Changesets + `arethetypeswrong` + `publint`); Claude-Code-aligned CLI flag vocabulary + JSON key schema + `gh`-style exit codes. **Key new primitives:** `EnvIsolation` (single biggest US-6 gap), `SandboxPool` (E2B/Daytona/Modal for untrusted code), `@pancreator/mcp-server`, `@pancreator/a2a`, `@pancreator/adopter-scan`. **Defended novelties:** touch-set + Conflict Planner, no-silent-merges resolver composition, per-artifact `contract:` blocks as gates, named-character Persona Ensemble dissent surface, long-lived SMEs, inbox-as-org-interface, 7-lever Intervention spectrum, adopter non-destructive scan, Watchdog suggest-don't-auto-apply, Conversation-Mode promotion, library-mode + framework-mode duality, run-log → handbook diff loop with mandatory ensemble ratification. v0.4 changes: §2 G7 hardened; §3.5 unchanged; §4 adds 10 glossary terms; §5 architecture redrawn with substrate adoptions; §5.5 rewritten with concrete Mastra-style tooling + 5 new packages; §6 persona format swapped to Claude Code spec + ensemble runtime adoption; §7 pipelines + intervention semantics map to LangGraph primitives, threshold policy → Conftest, run-log → OpenInference + OTel GenAI, behavior-preservation contract layered; §8 memory adopts dual-anchor citations + Spec Kit paths + Mem0/Letta interface; §9 CLI gains Claude-Code-aligned flag vocabulary + MCP verbs; §10 observability rewritten; §11 MVP scope substantially expanded (no scope creep — concrete substrates replace hand-rolled ones); §12 milestones rebalanced (Mergiraf/Schemathesis/runner-claude pulled forward; A2A pulled to M5; SandboxPool added at M3; GEPA/Voyager named at M8); §13 adds Q16-Q22 + R20-R26; §14 adds A19-A21.
> **Codename:** `pancreator` (matches repo name; the 4D motif fits the four orthogonal axes: *roles × pipelines × memory × process*).
...
- **G3.** Provide durable, citation-backed institutional memory (`/lib/memory/`) that survives context-window compression and is the system's source of truth — a "GitLab handbook for agents."
...
- **G6.** Be installable into both **greenfield** repos (`npx create-pancreator my-app`) and **existing** repos (`cd my-repo && npx pancreator init`) with zero infrastructure beyond a git repo and a Cursor/Claude Code install. Adoption into existing repos is *non-destructive* and seeded by an `adopter` persona that scans the codebase.
- **G7.** Ship as both a **framework** (opinionated, full org, conventions everywhere — Rails-like) *and* a **library** (cherry-pick individual primitives — a single persona, the worktree pool, the conflict planner — into another toolchain). Every primitive is independently importable, configurable without depending on a global `pancreator.yaml`, and documented as a standalone how-to. Where SOTA OSS standards exist (LangGraph `Checkpoint v1`, OpenInference + OTel GenAI semconv, Claude Code agent frontmatter, Agent Skills open spec, AGENTS.md, MCP, A2A, Cedar `AuthorizationEngine`, Conftest/Rego), `@pancreator/<primitive>` MUST be a *conformant implementation*, not a parallel design.
...
> *As an engineer, I want to implement the backend for complex feature A. I write out a semi-formal specification and feed it to Pancreator's software delivery pipeline. The coordinating agent interacts with me to clear up any ambiguities, ensures my spec is transformed into the system's internal representation of eng specs, and then kicks off the pipeline. The backend is implemented, tested, reviewed, and after a few such cycles, delivered. I receive a report in my inbox with a high-signal summary about the architecture, interfaces, tradeoffs, usage guidelines, etc.*

Implies:

- An `**intake-analyst`** persona that runs a *spec-canonicalization* sub-pipeline: ingests informal markdown → asks clarifying questions back through the inbox → emits a canonical *Engineering Spec* artifact (`/lib/memory/features/<id>/eng-spec.md`).
- The pipeline is **multi-cycle**: implement → review → fix → review → ship, with bounded loop count.
- A `**tech-writer`** post-pipeline step produces a high-signal *Delivery Report* (`/lib/memory/features/<id>/delivery-report.md`) covering architecture, interfaces, tradeoffs, usage guidelines — explicitly *not* a changelog. Delivered to `lib/inbox/out/`.
...
- A first-class **Conversation Mode** alongside Pipeline Mode: `pan chat <persona> [--feature <id>]` opens a long-running thread with a single persona.
...
- A first-class **Feature** entity with its own folder (`/lib/memory/features/<id>/`) and `index.json` linking PRD, UX Spec, ADR(s), code paths, tests, runbook, postmortems.
...
- **Long-lived SMEs** — *persistent* personas (vs. ephemeral task agents) that own a domain (`sme-postgres`, `sme-stripe-billing`, `sme-frontend-perf`, `sme-our-auth-stack`). Each has a private memory folder (`/lib/memory/smes/<name>/`) that accumulates across sessions; they are first-citizens of the inbox and can be DM'd or invited into ensemble reviews. Onboarding a new dependency creates a new SME.
- **Backlog as live entity** — `/lib/memory/backlog/` is a ranked, structured list of opportunities/features/RFC-derived work. The `pm` persona owns re-prioritization. The backlog is *the* product roadmap, computed not declared.
...
- **Worktree Pool** — pipelines execute in isolated `git worktree` directories under `.pan/worktrees/<task-id>/` (Cursor's `best-of-n-runner` already pioneers this pattern). The Worktree Pool is managed by the control plane: allocate, reserve, dispose, GC.
...
- A new `**tech-debt-scan`** pipeline (cron) — produces a backlog-ranked debt inventory (`lib/memory/backlog/index.yaml` items with `tags: [debt]`) ranked by smell density × change frequency × business value.
...
- **Configurable mode**: `pancreator.yaml: grooming.mode: propose | auto-pr`. Default = **propose-only** (drafts refactor RFCs into `/lib/memory/rfc/draft/` for human/ensemble ratification before any code change). Per-repo opt-in to `auto-pr` for fast-moving solo teams.
- A `pan debt` CLI surface for human visibility (US-7b extension): inventory listing, weekly debt-budget burn-down posted to inbox.
- Memory: backlog items with `tags: [debt]` in `/lib/memory/backlog/index.yaml` (inventory, history, deferred-restructure notes linked from backlog entries).
...
- **Two distribution modes**, both first-class:
  - **Framework mode** — `npx create-pancreator` (greenfield) or `npx pancreator init` (existing); installs the full opinionated org with conventions, persona roster, pipelines, scheduler, watchdog. Rails-/Next.js-style.
  - **Library mode** — `npm install @pancreator/<primitive>` for any single primitive (e.g., `@pancreator/worktree`, `@pancreator/conflict-planner`, `@pancreator/inbox`, `@pancreator/persona-runner`, `@pancreator/spec-contract`). Each primitive is importable into any other TS toolchain (LangGraph, OpenAI Agents SDK, plain Node script) without depending on Pancreator's CLI, conventions, or `pancreator.yaml`.
- **Module boundaries from day 1**: monorepo with `@pancreator/core` (shared types, no logic), `@pancreator/persona`, `@pancreator/pipeline`, `@pancreator/memory`, `@pancreator/inbox`, `@pancreator/worktree`, `@pancreator/conflict-planner`, `@pancreator/ensemble`, `@pancreator/policy`, `@pancreator/scout`, `@pancreator/intervention`, `@pancreator/cli`, plus a meta package `pancreator` that bundles them. **No circular dependencies between primitives.**
- **Every primitive ships with**: (a) a TypeScript public API, (b) a sensible-defaults inline config (works with zero external config files), (c) a standalone "Use this primitive in 5 minutes" how-to in its README, (d) at least one example using it *outside* Pancreator (e.g., the worktree pool used as a library by a non-Pancreator LangGraph script).
- **No primitive may require Pancreator's filesystem layout to function** — `MemoryStore`, `Inbox`, `WorktreePool` etc. accept paths as parameters, with framework-mode defaults that match `/lib/memory/`, `/lib/inbox/`, `.pan/worktrees/`.
...
> *As a developer, I want Pancreator to work for both greenfield repos and existing projects.*

Implies:

- Two install paths, both MVP-tier:
  - **Greenfield**: `npx create-pancreator my-app` scaffolds a complete repo with sample app, full Pancreator install, seeded handbook, working `feature-delivery` example.
  - **Existing**: `cd my-existing-repo && npx pancreator init` runs **non-destructively**: never overwrites an existing file; everything Pancreator owns lives under prefixed paths (`/lib/memory/`, `/lib/personas/`, `/lib/personas/skills/`, `/lib/pipelines/`, `/lib/inbox/`, `/.pan/`, `pancreator.yaml`); existing `AGENTS.md`, `.cursor/rules/`, `.github/agents/` are *augmented*, not replaced.
- A new `**adopter`** persona (and `adopt` sub-pipeline) runs at first install on existing repos and:
  ...
  - Detects existing `AGENTS.md` / `.cursor/rules` and writes a *merge plan* (additive only, with explicit conflicts surfaced to inbox).
  ...
  - Writes its findings to `/lib/memory/adoption/scan-<date>.md` — a citation-bearing artifact, ratifiable by the human, replayable on `pan re-adopt`.
- **No-conflict guarantees**: `pan init` performs a dry-run and exits non-zero if it would touch any existing file; `--force` prints a diff and requires explicit confirmation per file.
- **Coexistence with existing CI/CD**: Pancreator pipelines invoke (not replace) the user's existing test/build commands via a configurable `pancreator.yaml: commands.test | build | lint`.
...
- `**Intervention` interface** in the control plane (one of the §5 primitives), with a CLI surface (`pan steer | pause | reroute | snapshot | rollback | abort | quarantine`) and a programmatic API (library-mode usable per US-8).
- **Checkpointing** built into the supervisor: every stage boundary writes a checkpoint to `/lib/memory/checkpoints/<task-id>/<seq>.json` (state + worktree commit hash + run-log offset). Borrows from LangGraph's checkpointer pattern.
...
- **SME (Subject-Matter Expert)** — a *long-lived* persona that owns a knowledge domain and accumulates a private memory under `/lib/memory/smes/<name>/`. SMEs persist across pipelines and sessions and can be invited into ensembles or DM'd directly.
...
- **Feature** — a first-class entity (`/lib/memory/features/<id>/`) binding all artifacts for a single product capability: PRD, RFC(s), ADR(s), UX Spec, code references, tests, runbooks, postmortems, delivery report. Has a unique slug ID and a generated `index.json`.
- **Backlog** — `/lib/memory/backlog/` — the live, ranked product roadmap. Owned by the `pm` persona; computed from features, RFCs, opportunity-tree branches, and inbox asks. Re-prioritized continuously, never just at planning rituals.
...
- **Worktree Pool** — managed pool of git worktrees under `.pan/worktrees/` for parallel pipeline execution; allocated/disposed by the control plane.
...
- **Memory** — the union of `/lib/memory/` (semantic + procedural), `/lib/memory/smes/` (per-SME), `/lib/memory/features/` (per-feature), `/lib/memory/backlog/`, `/lib/inbox/` (episodic), and the codebase itself.
- **Inbox** — bidirectional message queue between humans and the org (`lib/inbox/in/`, `lib/inbox/out/`, `lib/inbox/threads/`). `lib/inbox/notes/` is a human-operator-only sandbox excluded from agent traversal (canonical rule in `/lib/memory/handbook/inbox-lifecycle.md` §1a).
...
- **Library Mode** — independent import of one or more primitives (`@pancreator/<primitive>`) into another toolchain, with no dependency on Pancreator's CLI, conventions, or `pancreator.yaml`. Each primitive is independently versioned and documented. (US-8)
- **Primitive** — a single, independently importable piece of Pancreator: persona-runner, pipeline, memory-store, inbox, worktree-pool, conflict-planner, ensemble, policy, scout, intervention. Each lives in its own `@pancreator/<name>` package.
- **Adoption (`adopter` / `adopt` pipeline)** — non-destructive first-run process for existing repos: scans codebase, detects conventions, proposes initial SMEs, seeds threshold policy from existing baselines, writes `/lib/memory/adoption/scan-<date>.md`. (US-9)
...
- **Checkpoint** — a serialized snapshot of pipeline state (stage, run-log offset, worktree commit hash, persona context summary) written at every stage boundary into `/lib/memory/checkpoints/<task-id>/<seq>.json`. The substrate that makes `rollback` and `pause/resume` cheap.
...
- **Persona Spec Format** — Markdown with **Anthropic Claude Agent SDK 16-field YAML frontmatter** (`name`, `description`, `model`, `permissionMode`, `tools`, `disallowedTools`, `mcpServers`, `maxTurns`, `skills`, `isolation`, `memory`, `effort`, `color`, `hooks`, `initialPrompt`, `background`, plus Pancreator-specific extensions in a `metadata` map). Authored once at `lib/personas/<name>.md`; a build step auto-emits a `.cursor/rules/<persona>.mdc` shim for Cursor users (the only Cursor-stable fields are `description`, `globs`, `alwaysApply`, so the shim is a 5-line transformer).
- **Skill Format** — Markdown conforming to the **Agent Skills open spec** (`agentskills.io`, Anthropic-stewarded, consumed verbatim by Cursor `SKILL.md`, Claude Code Skills, OpenAI Codex Skills); validated by `skills-ref` in CI. Folder convention: `lib/personas/skills/<name>/SKILL.md` plus optional `scripts/`, `references/`, `assets/`. Pancreator extensions go in the `metadata` map (`pancreator-stability`, `pancreator-pipeline-stages`, `pancreator-risk-tier`).
- **AGENTS.md** — repo-level Markdown briefing per the Linux Foundation Agentic AI Foundation standard; Pancreator's *primary* cross-tool contract. The repository does not maintain a separate GitHub Copilot instruction file at `.github/copilot-instructions.md`; tools that read that path SHOULD be pointed at `AGENTS.md` when supported. Cursor loads `AGENTS.md` natively via its built-in AGENTS.md support.
- **EnvIsolation** — pluggable allocator for *non-filesystem* environment state per worktree (PORT, DB_NAME, COMPOSE_PROJECT_NAME, `.env.pan` overrides). Default `PortRegistryEnvIsolation` uses a file-backed registry under `.pan/scheduler/ports.json`. **Without this primitive, parallel pipelines that bind ports / write to a DB / use Docker compose collide silently** — the single most-important gap finding from the SOTA pass.
- **SandboxPool** — alternative to `WorktreePool` for untrusted code (`coder` on new dependencies, `pen-tester` always). Container/VM-per-task via E2B (Firecracker microVM, ~125–150ms cold start), Daytona (Docker/Kata/Sysbox, 27–90ms cold start), Modal (gVisor + GPU), or Devin Cloud/VPC. Selected per-persona via `pancreator.yaml: worktrees.per_persona.<name>`. M3.
...
- **MCP Server (`@pancreator/mcp-server`)** — Pancreator publishes its own primitives (`inbox`, `memory`, `feature`, `pipeline`, `intervention`, `backlog`, `debt`) as MCP Tools + Resources so any MCP client (Cursor, Claude desktop, Codex CLI, ChatGPT desktop, Jules) can drive a Pancreator org without `pan`. Uses **MCP elicitations spec (2025-11-25)** for intake-analyst clarification + ship-stage approval (collapses three of four human interfaces). Uses **MCP authorization (OAuth 2.1 + PKCE + RFC 8707 Resource Indicators + RFC 9728)** for cross-process auth. Stdio transport in MVP; HTTP at M5.
- **A2A (Agent-to-Agent) v1.x** — Linux-Foundation-hosted protocol (March 2026; 150+ orgs in production; SDKs in TS/Py/Java/Go/.NET; Signed Agent Cards; multi-tenancy; JSON-RPC + gRPC). Each Pancreator org serves `/.well-known/agent-card.json` exposing public personas + pipelines as A2A skills. Supervisor speaks A2A as both server and client. **Pulled forward from M9 to M5.**
- **Spec Kit Alignment** — `/lib/memory/features/<id>/` paths align with GitHub Spec Kit v0.8 conventions (`spec.md`, `plan.md`, `tasks.md`, `constitution.md` at `/lib/memory/handbook/constitution.md`). Pancreator's intake → tech-lead → coder pipeline maps 1:1 onto Spec Kit's `/speckit.specify → /speckit.plan → /speckit.implement`. Per-artifact `contract:` blocks remain Pancreator's novel addition.
- **Pipeline Runtime** — YAML pipelines (`/lib/pipelines/*.yaml`) are the *human-facing* DSL; at runtime they compile to **LangGraph.js v1.2.x `StateGraph`** (chosen runtime, see ADR-001). Mastra is the credible TS-native alternative if M0 ratifies a swap. Hand-rolled supervisor execution is explicitly *not* the runtime.
...
flowchart TB
    ...
    Human -->|pan chat| Conversation[Conversation Channel]
    ...
    subgraph Ecosystem [Ecosystem Endpoints]
        MCPServer["@pancreator/mcp-server (stdio MVP, HTTP M5)"] --> ExtMCP["Cursor / Claude / Codex / ChatGPT clients"]
    ...
    MemoryStore --> Files["/lib/memory/handbook, /adr, /rfc, /prd, /features, /smes, /runbooks"]
...
The four orthogonal layers (the "pancreator"):

- **Roles layer** — persona specs (`lib/personas/<name>.md`, **Anthropic Claude Agent SDK frontmatter spec**, auto-shimmed to Cursor `.cursor/rules/<persona>.mdc`), with two lifecycles (ephemeral task agents and long-lived SMEs); ensemble configurations (`/lib/ensembles/*.yaml`, AutoGen/AG2 GroupChat runtime with sycophancy parameters); reusable procedures (`/lib/personas/skills/<name>/SKILL.md`, **Agent Skills open spec** validated by `skills-ref`).
- **Process layer** — pipelines (YAML compiled to **LangGraph.js `StateGraph`** at runtime), gates (Spec Contracts pulled from feature memory), ceremonies, threshold policies (`pancreator.yaml`, lowered to **Conftest + OPA Rego** under the hood), Conversation Mode threads, Intervention bus (mapped to LangGraph `interrupt()` / `Command` / Cancellation primitives).
- **Memory layer** — institutional memory (`/lib/memory/`, AGENTS.md as primary cross-tool contract), feature index (`/lib/memory/features/<id>/` aligned with **GitHub Spec Kit v0.8** file conventions: `spec.md`, `plan.md`, `tasks.md`), backlog (`/lib/memory/backlog/`, including debt-tagged items per `tags: [debt]`), per-SME stores (`/lib/memory/smes/`), checkpoints (`/lib/memory/checkpoints/<task-id>/<seq>.json` per **LangGraph `Checkpoint v1`** schema with worktree-commit + run-log-offset in `metadata`), adoption scans (`/lib/memory/adoption/`), inbox (`/lib/inbox/`). All cross-references use **dual-anchor citations** (AST symbol + content hash; line-range fallback).
...
- **Ecosystem endpoints** — `@pancreator/mcp-server` exposes lib/inbox/lib/memory/feature/intervention/backlog/debt as MCP Tools + Resources (any MCP client drives Pancreator); `@pancreator/a2a` serves `/.well-known/agent-card.json` exposing public personas + pipelines as A2A v1.x skills (cross-org collaboration).
...
lib/internal/packages/
  core/                  # @pancreator/core — shared types only, no logic, zero deps
  persona/               # @pancreator/persona — persona spec parser (Claude Code agent spec) + runner adapter; auto-emits Cursor .mdc shim
  pipeline/              # @pancreator/pipeline — YAML DSL parser + LangGraph.js StateGraph compiler
  checkpointer/          # @pancreator/checkpointer — FileCheckpointSaver (conformant LangGraph BaseCheckpointSaver v1)
  lib/memory/                # @pancreator/memory — MemoryStore (Mem0-shaped CRUD + Letta-tier overlay) + FileMemoryStore
  lib/inbox/                 # @pancreator/inbox — Inbox interface + FileInbox + MCP-elicitation transport
  worktree/              # @pancreator/worktree — WorktreePool interface + GitWorktreePool
  env-isolation/         # @pancreator/env-isolation — EnvIsolation interface + PortRegistryEnvIsolation (NEW; closes the US-6 gap)
  sandbox-e2b/           # @pancreator/sandbox-e2b — SandboxPool E2B adapter (M3)
  sandbox-daytona/       # @pancreator/sandbox-daytona — SandboxPool Daytona adapter (M3)
  conflict-planner/      # @pancreator/conflict-planner — touch-set + interference graph (LSP-derived M3, Bazel-query M4)
  merge-resolver/        # @pancreator/merge-resolver — Mergiraf driver registration + LLM+tests residual re-resolution
  ensemble/              # @pancreator/ensemble — Persona Ensemble runner (AutoGen/AG2 GroupChat under the hood; sycophancy parameters)
  policy/                # @pancreator/policy — threshold-policy YAML schema + Conftest/Rego transpiler + evaluator
  scout/                 # @pancreator/scout — scheduler + scout runtime + per-week cost ceiling
  intervention/          # @pancreator/intervention — 7-lever spectrum mapped to LangGraph interrupt/Command + Cedar Authorizer interface shape
  run-logger/            # @pancreator/run-logger — OTLP / OpenInference + OTel GenAI exporter (File / OTLP / LangSmith)
  adopter-scan/          # @pancreator/adopter-scan — wraps github-linguist/enry, licensee, Renovate manager-extractors, CI-detector (NEW)
  mcp-server/            # @pancreator/mcp-server — publishes lib/inbox/lib/memory/feature/intervention/backlog/debt as MCP Tools+Resources (stdio MVP, HTTP M5) (NEW)
  a2a/                   # @pancreator/a2a — A2A v1.x server + client; Agent Card emitter (M5) (NEW)
  notifier/              # @pancreator/notifier — Notifier interface + console/lib/inbox/Slack/Notion adapters
  cli/                   # @pancreator/cli — `pan` CLI (Claude-Code-aligned flag vocabulary + JSON keys + gh-style exit codes)
  runner-cursor/         # @pancreator/runner-cursor — Cursor AgentRunner adapter
  runner-claude/         # @pancreator/runner-claude — Claude Code AgentRunner adapter (pulled from M7 to M3 to validate substrate abstraction)
  self-improvement/      # @pancreator/self-improvement — GEPA + Voyager-style skill metadata (M8; optional Python sidecar)
  meta/                  # @pancreator — meta-package that re-exports everything
...
- **Changesets** with `linked: [["@pancreator/*"]]` for coordinated lockstep versioning + `@changesets/changelog-github` for auto-changelogs.
- **Public-API enforcement in CI**: `@arethetypeswrong/cli` + `publint` + `tsup --dts` (or `api-extractor`).
- **Sub-path exports** per primitive (`@pancreator/persona/parser`, `@pancreator/pipeline/runtime`, `@pancreator/intervention/types`).

### Cross-cutting design rules (enforced by lint + CI)

- **Each `@pancreator/<primitive>` package depends only on `@pancreator/core` and external libs** — no horizontal dependencies between primitives. Custom ESLint rule `@pancreator/no-horizontal-primitive-deps`.
- **Where a SOTA OSS standard exists, `@pancreator/<primitive>` MUST be a conformant implementation, not a parallel design.** Hard list of conformance targets: LangGraph `BaseCheckpointSaver` v1 (`@pancreator/checkpointer`), OpenInference + OTel GenAI semconv (`@pancreator/run-logger`), Anthropic Claude Agent SDK persona frontmatter (`@pancreator/persona`), Agent Skills open spec validated by `skills-ref` (`/lib/personas/skills/`), AGENTS.md (repo root), MCP 2025-11-25 spec (`@pancreator/mcp-server`), A2A v1.x (`@pancreator/a2a`), Cedar `AuthorizationEngine` interface shape (`@pancreator/intervention`), Conftest/Rego policy schema (`@pancreator/policy`).
- **Each primitive exports a `defaults` object** with sensible inline values that work without any external config.
- **Each primitive exports a `Pancreator*Options` type** so library users get full TypeScript autocompletion.
- **Each primitive's README includes** (a) a 5-line minimum example, (b) a "use this without Pancreator" example, (c) a stability tier badge, (d) the SOTA conformance statement (if applicable).
- **Public API is the only documented surface**; internal helpers live in `lib/internal/` and are not re-exported.
- **No package may read the filesystem unless given an explicit base path** — no implicit dependence on `/lib/memory/`, `/lib/inbox/`, `.pan/`. The CLI/framework binds these defaults; the library lets you provide your own.
- **Conformance test suite** *(ratcheting through M3 → M7)* — for each conformance target above, the test suite is *the SOTA project's own test suite where it exists* (e.g., LangGraph's `BaseCheckpointSaver` test suite for `@pancreator/checkpointer`).

### Two consumption flows

```mermaid
flowchart LR
    Greenfield[Greenfield user] -->|"npx create-pancreator"| Framework["Framework Mode: full org"]
    Existing[Existing-repo user] -->|"npx pancreator init"| Adopter[adopter persona scan]
    Adopter --> Framework
    Library[Library-mode user] -->|"npm install @pancreator/worktree"| Primitive[Single primitive]
    Library -->|"npm install @pancreator/intervention @pancreator/persona"| Subset[Subset of primitives]
...
mcpServers: [github, "pancreator-memory"]
maxTurns: 30
...
Auto-generated shim (`lib/personas/.generated/reviewer.mdc`):
...
- **adopter** *(MVP — added for US-9)* — runs at first install on existing repos via the `adopt` sub-pipeline. Scans codebase for languages/frameworks/test infra/CI/dependencies/conventions; detects existing `AGENTS.md`/`.cursor/rules`; proposes initial SMEs; seeds threshold policy from existing baselines; writes `/lib/memory/adoption/scan-<date>.md`. Read-only on existing files; write-only to Pancreator-prefixed paths.
- **intake-analyst** — runs the *spec-canonicalization* sub-pipeline (US-1): ingests informal markdown, conducts a clarifying-question dialogue through the inbox (≤N rounds), emits a canonical Engineering Spec into `/lib/memory/features/<id>/eng-spec.md`. Strong model; permission scoped to the feature folder.
...
- **coder** — implements one task at a time against an ADR/RFC/eng-spec. Write-scoped to its declared touch-set + tests; cannot touch CI/CD, `/lib/memory/`, or files outside the touch-set. Default-deny on production secrets. (M2 splits into `backend-eng` / `frontend-eng` specializations.)
...
- **librarian** — continuous memory curator. Detects stale ADRs, dedupes notes, **maintains the Feature index** (`/lib/memory/features/<id>/index.json`), generates the "handbook diff" on every merge.
...
- A persona file (`lib/personas/sme-<name>.md`) declaring scope and sources of truth.
- A private memory folder (`/lib/memory/smes/<name>/`) for accumulated notes, FAQs, gotchas, change-logs of its domain.
...
Ensembles live in `lib/ensembles/<name>.yaml` and configure a cohort of *named, opinionated* personas with explicit dispositions. Inspired by [BoN-MAV multi-judge verification](https://openreview.net/forum?id=LriQ3NY9uL), but operationalized for *named characters* not anonymous judges.
...
stages:
  - id: intake
    ...
    outputs: [/lib/memory/features/<id>/eng-spec.md]
    loop:
  ...
  - id: plan
    persona: tech-lead
    inputs:
      - /lib/memory/features/<id>/eng-spec.md
      - /lib/memory/handbook/
      - /lib/memory/adr/
    outputs:
      - /work/<day>/<id>/plan.md
  ...
  - id: report
    ...
    outputs: [/lib/memory/features/<id>/delivery-report.md]  # US-1's "high-signal summary"
  - id: ship
    persona: supervisor
    action: open_pr
    gate: human_approval
post_run:
  ...
  - persona: notifier
    action: post_to_inbox  # delivery report → lib/inbox/out/
```

### Other pipelines

- `**init-greenfield**` *(M1, US-9)* — `npx create-pancreator <name>` scaffolds a fresh repo: full Pancreator install + sample app + seeded handbook + working `feature-delivery` example. Idempotent.
- `**adopt`** *(M1, US-9)* — `npx pancreator init` in an existing repo. Drives the `adopter` persona through the codebase scan, conflict-checks every write, surfaces a per-file diff before applying, writes the scan report to `/lib/memory/adoption/scan-<date>.md`, opens an inbox item with proposed initial SMEs and threshold-policy for human ratification.
- `**chat-with-persona`** *(M2+, backlog)* — Conversation Mode (US-2). `pan chat <persona> [--feature <id>]` opens a multi-turn thread under `/lib/inbox/threads/<thread-id>/`. Any turn may end with `/end --emit <artifact-type>` to promote the dialogue into a canonical artifact. Used by `design-engineer` for US-2 and by SMEs for ad-hoc consults. Bootstrap registers `lib/pipelines/chat-with-persona.yaml` as a skeleton only; M1 does not execute this pipeline.
...
gates:
  ...
  ui:
    drift:
      ...
      visual_baseline: ".pan/visual-baselines/"
    a11y:
...
Generated Rego (`@pancreator/policy` transpiles YAML to Rego at build time; users may also hand-write Rego under `/policies/*.rego` for advanced overrides):

```rego
package pancreator.coverage
...
Evaluation pipeline: `conftest test --policy lib/internal/packages/policy/rego/ ./run-output.json`. Standard exit codes; `gates_on_failure` map routes failures back into the pipeline. `**severity: warn` vs `deny**` lets the audit→soft-gate→hard-gate rollout pattern ship without code changes.

### Worktree Parallelism (covers US-6)

Pipelines run inside isolated `git worktree` directories under `.pan/worktrees/<task-id>/`, modeled on Cursor's `best-of-n-runner` pattern (confirmed 2026-04: native git worktrees on host with `.cursor/worktrees.json` setup commands; Claude Code's `isolation: worktree` frontmatter is the substrate for Claude users). Two execution-substrate implementations:
...
- `**SandboxPool**` *(M3)* — container/VM-per-task adapter. `**@pancreator/sandbox-e2b`** (Firecracker microVM, ~125–150ms cold start), `**@pancreator/sandbox-daytona`** (Docker/Kata/Sysbox, 27–90ms cold start), `**@pancreator/sandbox-modal`** (gVisor + GPU). Required for `pen-tester` always; required for `coder` when `on_new_deps` is true.

Per-persona substrate selection in `pancreator.yaml`:
...
`PortRegistryEnvIsolation` (default) allocates a unique PORT from a pool (3000-3100, file-backed registry under `.pan/scheduler/ports.json`), generates `DB_NAME = <baseDb>_<taskIdShort>` (runs `CREATE DATABASE`), sets `COMPOSE_PROJECT_NAME = pan-<taskIdShort>`, writes a per-worktree `.env.pan` with these overrides, and registers cleanup hooks (drop DB, stop compose project). `pan worktree gc --prune` enforces a `MAX_DISK_GB` policy.
...
2. The `conflict-planner` builds an interference graph across the cohort. Three-layer derivation: (1) LLM-declared (MVP), (2) **LSP-reference-graph closure** via `@pancreator/lsp-bridge` over `tree-sitter` (M3), (3) **Bazel/Buck `query` rdeps** when a build graph is present (M4). Each upgrade tightens the guarantee.
...
- Worktrees share `/lib/memory/` read-only; any `/lib/memory/` write is funneled through the supervisor and serialized.

**On reconvergence (Merge Resolver) — Mergiraf as default driver:**

`pan init` registers **Mergiraf 0.16.x** ([mergiraf.org](https://mergiraf.org), tree-sitter-based syntax-aware Git merge driver, 25+ languages, GPL-3.0 invoked as external binary) as the global merge driver:
...
```text
# .gitattributes (repo-level so it's tracked, opt-out via pancreator.yaml: merge.mergiraf=false)
* merge=mergiraf
```
...
3. **If any test fails** → write to `/lib/inbox/in/merge-conflict-<id>.md` with the failure context; do *not* push. Humans (or a follow-up pipeline) decide.
4. **If all pass** → fast-forward to `main` (or open a single consolidated PR, per `pancreator.yaml`).
...
- **Run log** — every pipeline writes `/work/<day>/<id>/run.log.jsonl` of OTLP-encoded spans carrying **OpenInference** primary attributes (`openinference.span.kind` ∈ {LLM, AGENT, TOOL, RETRIEVER, RERANKER, EMBEDDING, GUARDRAIL, EVALUATOR, CHAIN, PROMPT}; `input.value`/`output.value`; `llm.input_messages.<i>.message.role|content`; `tool.name|parameters|result`) and **OTel GenAI semconv** parallel layer (`gen_ai.operation.name`, `gen_ai.provider.name`, `gen_ai.request.model`, `gen_ai.usage.input_tokens|output_tokens`). Pancreator-specific extensions namespaced as `pancreator.task_id|pipeline|persona|stage_id|checkpoint_seq|intervention.lever|contract.id`. `RunLogger` ships three exporters in MVP: `FileExporter` (default), `OTLPExporter` (Phoenix/Langfuse/Datadog/Honeycomb/New Relic), `LangSmithExporter` (M2). One file format = free integration with the entire OSS LLM-observability stack.
- **Spec contracts are gates, not suggestions** — any artifact under a feature can declare a `contract:` block; downstream pipelines load and enforce them automatically. **Per-artifact `contract:` is Pancreator's deliberate addition over Spec Kit's repo-level `constitution.md`** (Spec Kit has no per-artifact machine-checkable contract concept). M2 contract runners: **Schemathesis v4.8** ([schemathesis.io](https://schemathesis.io), `uvx schemathesis run` — single command, standard exit codes, 1.4–4.5× more bugs than competitors) for API contracts; **Playwright + `@axe-core/playwright`** for UX contracts; **Hypothesis (Python) / fast-check (TS)** for invariant assertions. LLM-judged fallback only when no harness applies.
- **Stage-boundary checkpointing** *(US-10)* — every supervisor writes a checkpoint at every stage boundary using **LangGraph's `BaseCheckpointSaver` interface and `Checkpoint v1` schema verbatim** (`{v, ts, id, channel_values, channel_versions, versions_seen, pending_sends}`, plus Pancreator-specific `metadata.worktree_commit` and `metadata.run_log_offset`). MVP `FileCheckpointSaver` is a conformant `BaseCheckpointSaver` implementation persisting to `/lib/memory/checkpoints/<task-id>/<seq>.json`. M5+ adapters (`SqliteSaver`, `PostgresSaver`) are LangGraph's own saver classes.
...
Each Pancreator org publishes an **MCP server (`@pancreator/mcp-server`, stdio in MVP, HTTP at M5)** exposing:
...
- **MCP Resources:** `lib/inbox/threads/`*, `lib/memory/handbook/`*, `lib/memory/adr/*`, `lib/memory/features/*`, `lib/memory/backlog/*` (including debt-tagged items).
- **MCP Elicitations** (spec `2025-11-25`, both `form` and `url` modes): used by the intake-analyst clarification dialogue (form-mode for structured input, url-mode for human approval gates). Replaces three of the four file-based human interfaces (file-based `lib/inbox/in/` remains as fallback transport through M5).
...
Any MCP client (Cursor, Claude desktop, Codex CLI, ChatGPT desktop, Jules, custom) can drive a Pancreator org without `pan` CLI. The `pan` CLI is one MCP client among equals.

### A2A v1.x server interface (M5)

Each Pancreator org serves `/.well-known/agent-card.json` per **A2A v1.x** ([a2a-protocol.org](https://a2a-protocol.org), Linux-Foundation-hosted, 150+ orgs in production as of March 2026, SDKs in TS/Py/Java/Go/.NET, Signed Agent Cards, multi-tenancy, JSON-RPC + gRPC). Public personas + pipelines are exposed as A2A skills. `supervisor` speaks A2A as both server and client. AP2 (Agent Payments Protocol) discoverable via Agent Card `extensions` field.
...
| Pancreator lever        | Closest existing primitive                                                                                                                               | Implementation                                                                                                   |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
...
| `abort`                | `disposeWorktree(taskId)` + `checkpointer.deleteThread(taskId)` + emit `run-summary.md` (analog: Cloudflare `terminate()`, Temporal `Workflow.cancel()`) | Implemented in `@pancreator/intervention`                                                                         |
| `quarantine`           | **No SOTA equivalent — Pancreator original.**                                                                                                             | Mark thread `quarantined: true` in metadata; supervisor refuses to advance until `release`                       |
...
- `pan steer <task-id> "consider X instead"` — inject a hint for the next turn.
- `pan pause <task-id>` — halt at next checkpoint.
- `pan resume <task-id>` — continue from paused state.
- `pan reroute <task-id> --to <stage|persona> [--reason "..."]` — redirect.
- `pan snapshot <task-id>` — fork a side-worktree at HEAD for inspection.
- `pan rollback <task-id> --to <checkpoint-seq>` — revert to checkpoint.
- `pan abort <task-id> [--reason "..."]` — kill + dispose + summarize.
- `pan quarantine <task-id> [--reason "..."]` — freeze for review.
- `pan release <task-id>` — release from quarantine.
...
/lib/memory/
  ...
  adoption/         # adopter scan reports for existing-repo onboarding (US-9)
    scan-<date>.md  # citation-bearing artifact, replayable on `pan re-adopt`
...
- **Procedural memory** — `/lib/memory/handbook/`, `/lib/personas/`, `/lib/personas/skills/` — *the rules*. Survives context compression because it's read on demand.
- **Semantic memory** — `/lib/memory/adr|rfc|prd|runbooks/` — *the facts and decisions*. Each artifact has a citation block (file/line links into code).
- **Episodic memory** — `/lib/inbox/threads/`, `/work/*/*/run.log.jsonl` — *the experiences*. Append-only, periodically distilled into semantic memory by the Librarian.

### `MemoryStore` interface (Mem0-shaped CRUD + Letta-tier overlay + Pancreator-native citation)

```typescript
export interface MemoryStore {
  // Mem0-shaped CRUD (so @pancreator/memory-mem0 adapter is a structural cast)
  ...
  // Letta-shaped tiers (so @pancreator/memory-letta adapter is trivial)
  tier(name: 'core' | 'recall' | 'archival'): TieredView;

  // Pancreator-native: filesystem-grounded, dual-anchor citation-bearing
...
export interface MemoryRouter {
  // Reads /lib/memory/handbook/index.md routing table; loads top-K topic files for an intent
...
- **MVP:** filesystem + grep + simple JSON index maintained by Librarian. `**MemoryRouter` ships in MVP** with a `/lib/memory/handbook/index.md` 200-line routing table (intent-dispatch pattern, modeled on Claude Code's `MEMORY.md` + topic-files lazy-load model — prevents context-window overload as the handbook grows past ~100 files).
- **M5+:** add pluggable `MemoryStore` adapters: `@pancreator/memory-mem0` (Mem0 cloud or self-hosted, ~48k★ Apache-2.0); `@pancreator/memory-letta` (Letta agent runtime, ~21k★ Apache-2.0, three-tier `core/recall/archival` model); `@pancreator/memory-graphiti` (Zep / temporal KG); `@pancreator/memory-cognee` (poly-store local-first); `**@pancreator/memory-hipporag2`** as the default M5 multi-hop retriever (10–30× cheaper than IRCoT/GraphRAG; matches Pancreator's local-lookup query pattern far better than GraphRAG's global-thematic strength). Self-RAG / CRAG patterns applied by Librarian on every retrieval (relevance + support + utility check; web/repo re-search on low quality).
...
- Every ADR/RFC/PRD has a `references:` block of **dual-anchor citations**:
  - **Symbol anchor** (preferred): `{ kind: 'symbol', path: 'lib/billing/charge.ts', symbol: 'applyDiscount', contentHash: 'sha256:abc...' }` — resolved via `tree-sitter` / `ast-grep`. **Survives moves and renames cleanly.**
...
- `lib/inbox/in/2026-04-24-add-stripe-billing.md` — human writes a request, supervisor picks it up.
- `lib/inbox/out/2026-04-24-billing-plan-ready-for-review.md` — agent posts an ask back; human edits the file with a decision and the supervisor resumes.
- Threads accumulate in `lib/inbox/threads/<id>/`.
- Convention: YAML frontmatter with `from`, `to`, `tier`, `pipeline`, `status`.

### MVP: CLI (Claude-Code-aligned grammar; MCP-verb compatible)

The CLI grammar is intentionally **isomorphic to `claude-code` and `gemini` CLIs** (verb-first, slash-commands inside REPL, `pan mcp` subcommand for MCP server management) so any operator who already knows one of those tools is productive in Pancreator on day one. **Every CLI verb is also a pan MCP tool with the same name** (e.g. `pan.run`, `pan.steer`, `pan.feature.show`) so external agents drive Pancreator over MCP without a parallel API surface.

Install / adoption (US-9):

- `npx create-pancreator <name>` — greenfield scaffold (full sample app + Pancreator install).
- `pan init` — adopt into existing repo (non-destructive; runs `adopter` persona; emits `/lib/memory/adoption/scan-<date>.md`; opens inbox item for ratification).
- `pan init --dry-run` — print every file that would be created/modified; exit non-zero if any conflict.
- `pan re-adopt` — re-run the adopter scan (e.g. after major refactor / new dependencies).
- `pan mcp [list|add <name> <transport>|remove <name>|status]` — manage MCP servers Pancreator personas may call (mirrors `claude mcp` exactly).

Daily workflow:

- `pan inbox [list|read <id>|reply <id>]` — manage the inbox.
- `pan run <pipeline> <inbox-id|--feature <id>>` — kick off a pipeline.
- `pan chat <persona> [--feature <id>]` — Conversation Mode (US-2). Opens a thread; `/end --emit <artifact>` to promote.
- `pan feature [list|new <slug>|show <slug>|attach <slug> <file>]` — manage features (US-1, US-2, US-3).
- `pan backlog [list|show <id>|prioritize]` — interact with the live backlog (US-5).
- `pan sme [list|new <name>|chat <name>]` — manage long-lived SMEs (US-5).
- `pan ensemble run <ensemble-name> --on <artifact-path>` — invoke a Persona Ensemble against an artifact (US-5).
- `pan scout [list|run <name>|schedule <name> <cron>]` — manage scouts (US-5).
- `pan worktree [list|gc|inspect <id>]` — manage the worktree pool (US-6).
- `pan plan-cohort <feature-ids...>` — run the conflict-planner over a cohort, dry-run (US-6).
- `pan policy [show|validate|diff <feature>]` — inspect threshold policies (US-4).
- `pan status` — what's in-flight, blocked, awaiting approval.
- `pan approve <task-id>` — clear a gate.
- `pan memory query "..."` — Librarian-mediated search.

Intervention (US-10):

- `pan pause <task-id>` / `pan resume <task-id>` — *(MVP)* halt at next checkpoint / continue.
- `pan abort <task-id> [--reason "..."]` — *(MVP)* kill + dispose worktree + emit summary.
- `pan steer <task-id> "consider X instead"` — *(M2)* inject a hint into the next turn.
- `pan reroute <task-id> --to <stage|persona> [--reason "..."]` — *(M2)* redirect.
- `pan snapshot <task-id>` — *(M2)* fork a side-worktree at HEAD for inspection.
- `pan rollback <task-id> --to <checkpoint-seq>` — *(M2)* revert state + worktree.
- `pan quarantine <task-id> [--reason "..."]` / `pan release <task-id>` — *(M2)* freeze for ombudsperson review.

Grooming (US-7, M3):

- `pan debt [list|show <id>|inventory]` — view the tech-debt inventory.
- `pan debt budget` — current week's grooming PR-budget burn-down.
- `pan debt mode [propose|auto-pr]` — switch grooming mode.

### M2+: Slack adapter

- Notifier writes to `#pancreator-<repo>` via existing Slack MCP; replies route back into the inbox.

### M3+: Lightweight web admin UI

- Static site (Next.js/Astro) bundled with the framework; renders `/lib/inbox/`, `/work/`, `/lib/memory/index.json`, run-log timelines.
...
Every persona turn, tool call, ensemble vote, intervention, gate evaluation, checkpoint, and worktree action is emitted as an **OTLP-encoded span** that conforms to **both** the [OpenTelemetry GenAI semantic conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/) (`gen_ai.system`, `gen_ai.request.model`, `gen_ai.usage.input_tokens`, etc.) **and** the [OpenInference attribute set](https://github.com/Arize-ai/openinference) (`llm.input_messages`, `llm.output_messages`, `tool.name`, `tool.parameters`, etc.). This single emit makes every Pancreator run viewable in **Phoenix, Langfuse, LangSmith, Helicone, OpenLLMetry, OTel-native backends (Jaeger, Tempo, Honeycomb)** with zero adapter code, and lets users plug in their existing observability stack instead of paying a Pancreator tax. Local default exporter writes JSONL to `/work/<day>/<task-id>/run.log.jsonl` for grep-ability and offline replay.
...
Every stage boundary writes a checkpoint via `BaseCheckpointSaver.put()`. `**@pancreator/checkpointer-fs`** is the MVP implementation (writes to `/lib/memory/checkpoints/<task-id>/<seq>.json`); `**@pancreator/checkpointer-sqlite`** uses `@langchain/langgraph-checkpoint-sqlite` directly; `**@pancreator/checkpointer-postgres`** wraps the official Postgres saver. `pan pause | resume | rollback | snapshot` translate directly to LangGraph `interrupt()`, `Command(goto=...)`, `checkpoint_id` time-travel calls — **same primitives as the rest of the LangGraph ecosystem, no Pancreator-specific persistence to maintain.**
...
- `lib/personas/`: supervisor, **adopter**, intake-analyst, tech-lead, coder, reviewer, librarian, tech-writer (8 markdown files; `adopter` added in v0.3 for US-9; intake-analyst and tech-writer added in v0.2 for US-1).
- `lib/pipelines/`: `feature-delivery.yaml` (flagship), `knowledge-curation.yaml`, `**init-greenfield.yaml`** (US-9), `**adopt.yaml`** (US-9). `chat-with-persona.yaml` is registered as a bootstrap skeleton only; runtime is backlog-deferred to M2.
- `lib/memory/`: scaffolds for handbook, adr, rfc, prd, **features**, **adoption** (US-9), **checkpoints** (US-10), runbooks, postmortems, research; handbook seed (constitution, glossary, run-log schema, citation conventions, contract format, threshold-policy schema, intervention semantics).
- `lib/inbox/`: scaffolds + conventions doc.
- `lib/personas/skills/`: write-adr, write-rfc, modern-code-review, blameless-postmortem, **canonicalize-spec**, **adopt-existing-repo** (US-9) (6 reusable skills, Cursor SKILL.md-compatible).
- `pancreator.yaml` + `pancreator-defaults.yaml`: threshold-policy schema with one working default per risk tier (US-4 foundation; full UI-drift / regression suites land in M2/M3); `commands.test|build|lint` hooks (US-9 coexistence).
...
- **Monorepo package layout per §5.5 from day 1** (US-8): `@pancreator/core`, `@pancreator/persona`, `@pancreator/pipeline`, `@pancreator/memory`, `@pancreator/inbox`, `@pancreator/worktree`, `@pancreator/env-isolation`, `@pancreator/intervention`, `@pancreator/notifier`, `@pancreator/policy`, `@pancreator/checkpointer-fs`, `@pancreator/run-logger` (OpenInference + OTel GenAI exporter), `@pancreator/runner-cursor`, `@pancreator/adopter-scan`, `@pancreator/cli`, `pancreator` (meta). pnpm workspace + Turborepo + Changesets + `@arethetypeswrong/cli` + `publint` per §5.5. ESLint rule + CI check enforcing no horizontal deps between primitives. *Library-mode primitives don't all need rich features in MVP — the boundaries do.*
- `AgentRunner` interface + `CursorRunner` implementation (in `@pancreator/runner-cursor`); `ClaudeCodeRunner` is M3.
- `MemoryStore` interface (Mem0-shaped CRUD + Letta-tier overlay + dual-anchor citation methods) + `FileMemoryStore` implementation (with Feature-index support + `MemoryRouter` + `/lib/memory/handbook/index.md` routing table).
...
- `Checkpointer` interface = LangGraph `BaseCheckpointSaver` v1 directly (no Pancreator wrapper); `@pancreator/checkpointer-fs` writes to `/lib/memory/checkpoints/<task-id>/<seq>.json`.
- `RunLogger` (`@pancreator/run-logger`): OTLP/JSONL exporter with OpenInference + OTel GenAI semconv attributes; default local file sink.
- `**Intervention` interface + checkpointer** (US-10): supervisor writes a LangGraph checkpoint at every stage boundary; CLI surfaces `pan pause | resume | abort` (full lever spectrum lands in M2 mapped to LangGraph `interrupt()` / `Command(goto)` / `checkpoint_id` time-travel).
...
- `**@pancreator/mcp-server`** (MVP-skeleton, fully wired in M2): publishes `pan.*` verbs as MCP Tools and `/lib/memory/`, `/lib/inbox/`, `/work/run-log` as MCP Resources; `human.respond` exposed as MCP Elicitation.
- **Install-path proof (US-9, M1):** `pnpm -w exec pan create-pancreator <name>` scaffolds greenfield; `pnpm -w exec pan init` runs adopter scan and emits a ratification inbox item on an existing repo. Standalone `examples/` apps are backlog-deferred past M1.
...
- **M7 — Portability + Conformance. Locks in US-8.** `ClaudeCodeRunner`, `OpenAIAgentsSDKRunner`, `LangGraphRunner` adapters; **conformance test suite** (single test suite that runs against any `AgentRunner`/`MemoryStore`/`WorktreePool`/`Intervention` implementation); A2A protocol support so a pancreator org can talk to a non-pancreator org. Each primitive promoted to `stability: stable` once conformance tests are green.
- **M8 — Self-improvement.** Ombudsperson promoted from watchdog-only to full audit role; pipelines that mine their own run logs to propose handbook/persona/skill diffs (Bugbot's hill-climb-on-resolution-rate pattern). Scout-emitted RFCs that propose changes to *Pancreator itself* (the literal US-5 scenario, applied recursively).
- **M9 — Multi-repo / multi-org.** A team of teams: pancreator orgs that share a meta-handbook (the "engineering organization" tier); cross-repo conflict planning.
...
- **Q1.** Implementation language for `pan` CLI and adapters — TS/Node (rich MCP/Mastra/Inngest ecosystem) vs Python (richer LLM-eval tooling)? Default proposed: **TS**.
...
- **Q4.** Branding/codename: keep `pancreator`, or rename for distribution?
...
- **Q10 (new).** *Backlog source-of-truth.* Should `/lib/memory/backlog/` be the system's only backlog, or should it sync to GitHub Issues / Linear? Default proposed: `**/lib/memory/backlog/` as canonical source; M3+ adds bidirectional sync adapters**.
- **Q11 (new).** *Conversation Mode persistence.* Threads in `/lib/inbox/threads/` — cap on retained turns before summarization? Default proposed: **last 100 turns verbatim + Librarian rolling summary**.
...
- **Q13 (new, US-8).** *Library-mode minimum API surface for MVP.* All primitives importable from M1, or only `@pancreator/persona` + `@pancreator/worktree` + `@pancreator/intervention` (the three with the highest standalone value)? Default proposed: **package boundaries from M1; rich library APIs ratchet up by milestone, with full conformance only at M7**.
...
- **Q17 (SOTA, persona spec).** *Cursor frontmatter compatibility.* The Cursor `.mdc` rule format diverges from the Claude Code 16-field agent frontmatter on ~3 fields (`alwaysApply`, `globs`, `description`-as-trigger). Auto-shim + warn, or require dual-authoring? Default proposed: **`lib/personas/*.md` canonical; emit `.cursor/agents` mirror; keep `.mdc` only where rule loading is still required**.
- **Q18 (SOTA, MCP-vs-CLI).** *MCP server scope.* Should `@pancreator/mcp-server` expose every CLI verb 1:1 (broad surface, easier discoverability), or curate a smaller set of high-value verbs (smaller blast radius)? Default proposed: **phased surface: curated MVP set, then expansion toward parity**.
...
- **Q20 (SOTA, retrieval at M5).** *Default M5 vector index.* HippoRAG2 (research-leading on multi-hop), Mem0 (largest user base), Letta (richest tier model), or all three behind a unified API with profile-based defaults? Default proposed: **HippoRAG2 default for retrieval, Mem0 CRUD-shape as the canonical interface, Letta tiers as overlay; users choose via `pancreator.yaml memory.profile`**.
...
- **R10 (new).** *Scope creep via SMEs and scouts.* A repo could accumulate 50 SMEs and 10 scouts each costing tokens. Mitigated by Librarian-driven SME demotion (idle-decay) and explicit budget visibility in `pan status`.
...
- **R13 (new, US-8).** *Library boundaries are easier to design in than to retrofit.* If MVP ships a tightly-coupled monolith, splitting later costs months. Mitigated by enforcing the `@pancreator/<primitive>` layout + no-horizontal-deps lint rule + CI check from day 1, even if some primitives are skeletons until later milestones.
- **R14 (new, US-9).** *Adopter persona makes wrong assumptions about an existing repo.* Misclassified framework, missed conventions, or "helpful" rewrites of `.cursor/rules` could destroy trust on first contact. Mitigated by `pan init --dry-run` defaulting to dry-run with explicit per-file confirmation, and by writing scan output as a *proposal* in `/lib/memory/adoption/scan-<date>.md` for ratification rather than auto-applying.
...
- **R18 (SOTA, dependency depth).** *Reliance on the LangGraph / OpenInference / Mergiraf / Conftest ecosystem.* Adopting SOTA means we now ship a transitive footprint that can break independently of us. Mitigated by (a) pinning to LTS minor versions, (b) the conformance test suite from M3, (c) keeping each adapter behind a `@pancreator/`* package boundary so a breaking upstream change is one package's blast radius, not the whole framework's.
- **R19 (SOTA, schema drift).** *OpenInference and OTel GenAI semconv are still evolving.* Field renames or attribute restructures could orphan our existing run logs. Mitigated by version-stamping every span (`pancreator.run_logger.version`) and shipping forward-migration scripts in `@pancreator/run-logger` major releases.
- **R20 (SOTA, LangGraph runtime lock-in).** *We compile pipelines to LangGraph `StateGraph`, but Cursor / Claude Code agents may evolve their own native graph runtime that conflicts.* Mitigated by Q16 deferral discipline + the `Pipeline → StateGraph` compiler being one isolated package (`@pancreator/pipeline`) so swapping runtimes is one PR, not a cross-cutting refactor.
- **R21 (SOTA, MCP attack surface).** *Publishing all CLI verbs as MCP tools opens automated remote agent control of Pancreator.* Mitigated by per-verb `mcp.disabled` opt-out (Q18), Cedar `Authorizer` gating every verb invocation in library mode, and OpenInference logging every MCP-originated call with caller identity.
...
- **R23 (SOTA, sycophancy by configuration).** *A misconfigured ensemble (`sycophancy: 1.0` on every persona) silently regresses to agreement theater.* Mitigated by `pan lint ensembles` warning on configurations with median sycophancy > 0.5 and zero `dispositions`, and runtime ombudsperson alerts when measured SS deviates > 0.3 from configured target.
...
- **A2.** ≥ 90% of merged PRs from pancreator pipelines reference a PRD/RFC/ADR.
...
- **A5.** Inbox SLA: every `/lib/inbox/in/` message acknowledged within 1 agent invocation; status posted within 1h of pipeline start.
...
- **A16 (US-8).** Every primitive in `lib/internal/packages/` (a) passes the conformance test suite at M7, (b) declares an explicit `stability` tier, (c) has zero horizontal package dependencies (CI-enforced from M1). Non-Pancreator usage examples in `examples/` are backlog-deferred past M1 and become required by M7.
- **A17 (US-9).** `pan init` on 5 representative existing repos (TS monorepo, Python service, Rust binary, Go API, Next.js app) completes non-destructively (zero overwritten files) and produces an `adoption/scan-*.md` rated ≥ 4/5 by repo owners on accuracy.
- **A18 (US-10).** Median time from operator-issued `pan pause` to actual pause-acknowledgment ≤ 1 stage boundary; zero rollback/abort actions destroy uncommitted human edits; from M5+, watchdog detects ≥ 80% of stuck-state cases that humans would have flagged manually within 5 turns of the human.
...
- **A20 (SOTA, ecosystem reuse).** ≥ 1 third-party LangGraph checkpointer / OpenInference exporter / Mem0 memory backend / MCP client / A2A peer integrated end-to-end without writing Pancreator-specific glue, by M7. (Validates that "we conform, not parallel-design" is real.)
...
- **A13.** Onboarding time for a new human collaborator on a pancreator repo: < 1 day to ship first PR (vs. industry baseline of 3.3× monthly salary in lost productivity per [developeronboardingcost.com](https://www.developeronboardingcost.com/)).
- **A14.** Cost per feature-delivery (median) reported in `pan status`; flat or decreasing over time as Librarian curation reduces context retrieval cost.
...
If you'd like, I can persist all six reports to `/lib/memory/research/` as the org's founding "research dossier" so future agents can cite them — proposed on first scaffold pass.