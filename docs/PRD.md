...
- **adopter** *(MVP — added for US-9)* — runs at first install on existing repos via the `adopt` sub-pipeline. Scans codebase for languages/frameworks/test infra/CI/dependencies/conventions; detects existing `AGENTS.md`/`.cursor/rules`; proposes initial SMEs; seeds threshold policy from existing baselines; writes `/src/memory/adoption/scan-<date>.md`. Read-only on existing files; write-only to Tesseract-prefixed paths.
