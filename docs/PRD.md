...
todos:
  - id: ratify
    ...
    status: pending
  - id: monorepo-layout
    content: Set up @daedaline/* monorepo per §5.5 — pnpm workspace + catalogs + Turborepo + Changesets + @arethetypeswrong/cli + publint + sub-path exports + no-horizontal-deps ESLint rule + CI conformance check (US-8 + SOTA tooling foundation)
    status: pending
  - id: scaffold
    content: "Scaffold repo: AGENTS.md (primary contract), .cursor/rules/ (auto-shimmed), src/personas/ (Claude-Code 16-field YAML), src/pipelines/ (LangGraph-compiled), src/ensembles/ (AutoGen-runtime), src/memory/{handbook,adr,rfc,prd,features,smes,backlog,debt,checkpoints,adoption,runbooks,postmortems,research}/ (Spec-Kit paths), src/inbox/, src/skills/ (Agent-Skills-spec), src/work/, .ddl/{worktrees,sandboxes,scheduler}/"
    status: pending
  - id: handbook-seed
  ...
  - id: skills-mvp
    ...
    status: pending
  - id: policy-schema
    content: Implement threshold-policy schema (daedaline.yaml + daedaline-defaults.yaml) compiled to Conftest + OPA Rego with SonarQube QualityGate field shape, one working default per risk tier, commands.test|build|lint hooks (US-4 + US-9 coexistence)
    status: pending
  - id: pipelines-mvp
  ...
  - id: control-plane
    ...
    status: pending
  - id: intervention-mvp
    content: Implement stage-boundary LangGraph checkpointing + ddl pause/resume/abort mapped to LangGraph interrupt()/Command(goto)/checkpoint_id time-travel, with safety invariants (US-10 MVP)
    status: pending
  - id: feature-index
  ...
  - id: mcp-server-skeleton
    content: "Author @daedaline/mcp-server skeleton: publish all CLI verbs as MCP Tools with shared handler core; expose /src/memory/, /src/inbox/, /src/work/run-log as MCP Resources; expose human.respond as MCP Elicitation (per Q18 1:1 verb surface with mcp.disabled opt-out)"
    status: pending
  - id: cli
    content: "Implement ddl CLI MVP subcommands (Claude-Code-aligned grammar): init (greenfield + adopt), mcp [list|add|remove|status], inbox, run, chat, feature, status, approve, memory query, worktree, policy, pause, resume, abort"
    status: pending
  - id: run-logger-mvp
    content: Implement @daedaline/run-logger emitting OTLP-encoded spans with OpenInference + OTel GenAI semantic conventions; default JSONL local sink to /src/work/<day>/<task-id>/run.log.jsonl; conformance-tested against Phoenix/Langfuse importers
    status: pending
  - id: examples
...
- **G6.** Be installable into both **greenfield** repos (`npx create-daedaline my-app`) and **existing** repos (`cd my-repo && npx daedaline init`) with zero infrastructure beyond a git repo and a Cursor/Claude Code install. Adoption into existing repos is *non-destructive* and seeded by an `adopter` persona that scans the codebase.
- **G7.** Ship as both a **framework** (opinionated, full org, conventions everywhere — Rails-like) *and* a **library** (cherry-pick individual primitives — a single persona, the worktree pool, the conflict planner — into another toolchain). Every primitive is independently importable, configurable without depending on a global `daedaline.yaml`, and documented as a standalone how-to. Where SOTA OSS standards exist (LangGraph `Checkpoint v1`, OpenInference + OTel GenAI semconv, Claude Code agent frontmatter, Agent Skills open spec, AGENTS.md, MCP, A2A, Cedar `AuthorizationEngine`, Conftest/Rego), `@daedaline/<primitive>` MUST be a *conformant implementation*, not a parallel design.
...
- A first-class **Conversation Mode** alongside Pipeline Mode: `ddl chat <persona> [--feature <id>]` opens a long-running thread with a single persona.
...
- **Worktree Pool** — pipelines execute in isolated `git worktree` directories under `.ddl/worktrees/<task-id>/` (Cursor's `best-of-n-runner` already pioneers this pattern). The Worktree Pool is managed by the control plane: allocate, reserve, dispose, GC.
...
- **Configurable mode**: `daedaline.yaml: grooming.mode: propose | auto-pr`. Default = **propose-only** (drafts refactor RFCs into `/src/memory/rfc/draft/` for human/ensemble ratification before any code change). Per-repo opt-in to `auto-pr` for fast-moving solo teams.
- A `ddl debt` CLI surface for human visibility (US-7b extension): inventory listing, weekly debt-budget burn-down posted to inbox.
...
- **Two distribution modes**, both first-class:
  - **Framework mode** — `npx create-daedaline` (greenfield) or `npx daedaline init` (existing); installs the full opinionated org with conventions, persona roster, pipelines, scheduler, watchdog. Rails-/Next.js-style.
  - **Library mode** — `npm install @daedaline/<primitive>` for any single primitive (e.g., `@daedaline/worktree`, `@daedaline/conflict-planner`, `@daedaline/inbox`, `@daedaline/persona-runner`, `@daedaline/spec-contract`). Each primitive is importable into any other TS toolchain (LangGraph, OpenAI Agents SDK, plain Node script) without depending on Daedaline's CLI, conventions, or `daedaline.yaml`.
- **Module boundaries from day 1**: monorepo with `@daedaline/core` (shared types, no logic), `@daedaline/persona`, `@daedaline/pipeline`, `@daedaline/memory`, `@daedaline/inbox`, `@daedaline/worktree`, `@daedaline/conflict-planner`, `@daedaline/ensemble`, `@daedaline/policy`, `@daedaline/scout`, `@daedaline/intervention`, `@daedaline/cli`, plus a meta package `daedaline` that bundles them. **No circular dependencies between primitives.**
- **Every primitive ships with**: (a) a TypeScript public API, (b) a sensible-defaults inline config (works with zero external config files), (c) a standalone "Use this primitive in 5 minutes" how-to in its README, (d) at least one example using it *outside* Daedaline (e.g., the worktree pool used as a library by a non-Daedaline LangGraph script).
- **No primitive may require Daedaline's filesystem layout to function** — `MemoryStore`, `Inbox`, `WorktreePool` etc. accept paths as parameters, with framework-mode defaults that match `/src/memory/`, `/src/inbox/`, `.ddl/worktrees/`.
...
> *As a developer, I want Daedaline to work for both greenfield repos and existing projects.*

Implies:

- Two install paths, both MVP-tier:
  - **Greenfield**: `npx create-daedaline my-app` scaffolds a complete repo with sample app, full Daedaline install, seeded handbook, working `feature-delivery` example.
  - **Existing**: `cd my-existing-repo && npx daedaline init` runs **non-destructively**: never overwrites an existing file; everything Daedaline owns lives under prefixed paths (`/src/memory/`, `/src/personas/`, `/src/skills/`, `/src/pipelines/`, `/src/inbox/`, `/.ddl/`, `daedaline.yaml`); existing `AGENTS.md`, `.cursor/rules/`, `.github/agents/` are *augmented*, not replaced.
- A new `**adopter`** persona (and `adopt` sub-pipeline) runs at first install on existing repos and:
  ...
  - Writes its findings to `/src/memory/adoption/scan-<date>.md` — a citation-bearing artifact, ratifiable by the human, replayable on `ddl re-adopt`.
- **No-conflict guarantees**: `ddl init` performs a dry-run and exits non-zero if it would touch any existing file; `--force` prints a diff and requires explicit confirmation per file.
- **Coexistence with existing CI/CD**: Daedaline pipelines invoke (not replace) the user's existing test/build commands via a configurable `daedaline.yaml: commands.test | build | lint`.
...
- `**Intervention` interface** in the control plane (one of the §5 primitives), with a CLI surface (`ddl steer | pause | reroute | snapshot | rollback | abort | quarantine`) and a programmatic API (library-mode usable per US-8).
...
- **Worktree Pool** — managed pool of git worktrees under `.ddl/worktrees/` for parallel pipeline execution; allocated/disposed by the control plane.
...
- **Library Mode** — independent import of one or more primitives (`@daedaline/<primitive>`) into another toolchain, with no dependency on Daedaline's CLI, conventions, or `daedaline.yaml`. Each primitive is independently versioned and documented. (US-8)
- **Primitive** — a single, independently importable piece of Daedaline: persona-runner, pipeline, memory-store, inbox, worktree-pool, conflict-planner, ensemble, policy, scout, intervention. Each lives in its own `@daedaline/<name>` package.
...
- **MCP Server (`@daedaline/mcp-server`)** — Daedaline publishes its own primitives (`inbox`, `memory`, `feature`, `pipeline`, `intervention`, `backlog`, `debt`) as MCP Tools + Resources so any MCP client (Cursor, Claude desktop, Codex CLI, ChatGPT desktop, Jules) can drive a Daedaline org without `ddl`. Uses **MCP elicitations spec (2025-11-25)** for intake-analyst clarification + ship-stage approval (collapses three of four human interfaces). Uses **MCP authorization (OAuth 2.1 + PKCE + RFC 8707 Resource Indicators + RFC 9728)** for cross-process auth. Stdio transport in MVP; HTTP at M5.
- **A2A (Agent-to-Agent) v1.x** — Linux-Foundation-hosted protocol (March 2026; 150+ orgs in production; SDKs in TS/Py/Java/Go/.NET; Signed Agent Cards; multi-tenancy; JSON-RPC + gRPC). Each Daedaline org serves `/.well-known/agent-card.json` exposing public personas + pipelines as A2A skills. Supervisor speaks A2A as both server and client. **Pulled forward from M9 to M5.**
- **Spec Kit Alignment** — `/src/memory/features/<id>/` paths align with GitHub Spec Kit v0.8 conventions (`spec.md`, `plan.md`, `tasks.md`, `constitution.md` at `/src/memory/handbook/constitution.md`). Daedaline's intake → tech-lead → coder pipeline maps 1:1 onto Spec Kit's `/speckit.specify → /speckit.plan → /speckit.implement`. Per-artifact `contract:` blocks remain Daedaline's novel addition.
...
flowchart TB
    ...
    Human -->|ddl chat| Conversation[Conversation Channel]
    ...
    subgraph Ecosystem [Ecosystem Endpoints]
        MCPServer["@daedaline/mcp-server (stdio MVP, HTTP M5)"] --> ExtMCP["Cursor / Claude / Codex / ChatGPT clients"]
...
The four orthogonal layers (the "daedaline"):
...
- **Process layer** — pipelines (YAML compiled to **LangGraph.js `StateGraph`** at runtime), gates (Spec Contracts pulled from feature memory), ceremonies, threshold policies (`daedaline.yaml`, lowered to **Conftest + OPA Rego** under the hood), Conversation Mode threads, Intervention bus (mapped to LangGraph `interrupt()` / `Command` / Cancellation primitives).
...
- **Ecosystem endpoints** — `@daedaline/mcp-server` exposes src/inbox/src/memory/feature/intervention/backlog/debt as MCP Tools + Resources (any MCP client drives Daedaline); `@daedaline/a2a` serves `/.well-known/agent-card.json` exposing public personas + pipelines as A2A v1.x skills (cross-org collaboration).
...
src/internal/packages/
  core/                  # @daedaline/core — shared types only, no logic, zero deps
  persona/               # @daedaline/persona — persona spec parser (Claude Code agent spec) + runner adapter; auto-emits Cursor .mdc shim
  pipeline/              # @daedaline/pipeline — YAML DSL parser + LangGraph.js StateGraph compiler
  checkpointer/          # @daedaline/checkpointer — FileCheckpointSaver (conformant LangGraph BaseCheckpointSaver v1)
  src/memory/                # @daedaline/memory — MemoryStore (Mem0-shaped CRUD + Letta-tier overlay) + FileMemoryStore
  src/inbox/                 # @daedaline/inbox — Inbox interface + FileInbox + MCP-elicitation transport
  worktree/              # @daedaline/worktree — WorktreePool interface + GitWorktreePool
  env-isolation/         # @daedaline/env-isolation — EnvIsolation interface + PortRegistryEnvIsolation (NEW; closes the US-6 gap)
  sandbox-e2b/           # @daedaline/sandbox-e2b — SandboxPool E2B adapter (M3)
  sandbox-daytona/       # @daedaline/sandbox-daytona — SandboxPool Daytona adapter (M3)
  conflict-planner/      # @daedaline/conflict-planner — touch-set + interference graph (LSP-derived M3, Bazel-query M4)
  merge-resolver/        # @daedaline/merge-resolver — Mergiraf driver registration + LLM+tests residual re-resolution
  ensemble/              # @daedaline/ensemble — Persona Ensemble runner (AutoGen/AG2 GroupChat under the hood; sycophancy parameters)
  policy/                # @daedaline/policy — threshold-policy YAML schema + Conftest/Rego transpiler + evaluator
  scout/                 # @daedaline/scout — scheduler + scout runtime + per-week cost ceiling
  intervention/          # @daedaline/intervention — 7-lever spectrum mapped to LangGraph interrupt/Command + Cedar Authorizer interface shape
  run-logger/            # @daedaline/run-logger — OTLP / OpenInference + OTel GenAI exporter (File / OTLP / LangSmith)
  adopter-scan/          # @daedaline/adopter-scan — wraps github-linguist/enry, licensee, Renovate manager-extractors, CI-detector (NEW)
  mcp-server/            # @daedaline/mcp-server — publishes src/inbox/src/memory/feature/intervention/backlog/debt as MCP Tools+Resources (stdio MVP, HTTP M5) (NEW)
  a2a/                   # @daedaline/a2a — A2A v1.x server + client; Agent Card emitter (M5) (NEW)
  notifier/              # @daedaline/notifier — Notifier interface + console/src/inbox/Slack/Notion adapters
  cli/                   # @daedaline/cli — `ddl` CLI (Claude-Code-aligned flag vocabulary + JSON keys + gh-style exit codes)
  runner-cursor/         # @daedaline/runner-cursor — Cursor AgentRunner adapter
  runner-claude/         # @daedaline/runner-claude — Claude Code AgentRunner adapter (pulled from M7 to M3 to validate substrate abstraction)
  self-improvement/      # @daedaline/self-improvement — GEPA + Voyager-style skill metadata (M8; optional Python sidecar)
  meta/                  # @daedaline — meta-package that re-exports everything
...
- **Changesets** with `linked: [["@daedaline/*"]]` for coordinated lockstep versioning + `@changesets/changelog-github` for auto-changelogs.
- **Public-API enforcement in CI**: `@arethetypeswrong/cli` + `publint` + `tsup --dts` (or `api-extractor`).
- **Sub-path exports** per primitive (`@daedaline/persona/parser`, `@daedaline/pipeline/runtime`, `@daedaline/intervention/types`).

### Cross-cutting design rules (enforced by lint + CI)

- **Each `@daedaline/<primitive>` package depends only on `@daedaline/core` and external libs** — no horizontal dependencies between primitives. Custom ESLint rule `@daedaline/no-horizontal-primitive-deps`.
- **Where a SOTA OSS standard exists, `@daedaline/<primitive>` MUST be a conformant implementation, not a parallel design.** Hard list of conformance targets: LangGraph `BaseCheckpointSaver` v1 (`@daedaline/checkpointer`), OpenInference + OTel GenAI semconv (`@daedaline/run-logger`), Anthropic Claude Agent SDK persona frontmatter (`@daedaline/persona`), Agent Skills open spec validated by `skills-ref` (`/src/skills/`), AGENTS.md (repo root), MCP 2025-11-25 spec (`@daedaline/mcp-server`), A2A v1.x (`@daedaline/a2a`), Cedar `AuthorizationEngine` interface shape (`@daedaline/intervention`), Conftest/Rego policy schema (`@daedaline/policy`).
- **Each primitive exports a `defaults` object** with sensible inline values that work without any external config.
- **Each primitive exports a `Daedaline*Options` type** so library users get full TypeScript autocompletion.
- **Each primitive's README includes** (a) a 5-line minimum example, (b) a "use this without Daedaline" example, (c) a stability tier badge, (d) the SOTA conformance statement (if applicable).
- **Public API is the only documented surface**; internal helpers live in `src/internal/` and are not re-exported.
- **No package may read the filesystem unless given an explicit base path** — no implicit dependence on `/src/memory/`, `/src/inbox/`, `.ddl/`. The CLI/framework binds these defaults; the library lets you provide your own.
- **Conformance test suite** *(ratcheting through M3 → M7)* — for each conformance target above, the test suite is *the SOTA project's own test suite where it exists* (e.g., LangGraph's `BaseCheckpointSaver` test suite for `@daedaline/checkpointer`).

### Two consumption flows

```mermaid
flowchart LR
    Greenfield[Greenfield user] -->|"npx create-daedaline"| Framework["Framework Mode: full org"]
    Existing[Existing-repo user] -->|"npx daedaline init"| Adopter[adopter persona scan]
    Adopter --> Framework
    Library[Library-mode user] -->|"npm install @daedaline/worktree"| Primitive[Single primitive]
    Library -->|"npm install @daedaline/intervention @daedaline/persona"| Subset[Subset of primitives]
...
mcpServers: [github, "daedaline-memory"]
maxTurns: 30
...
- **adopter** *(MVP — added for US-9)* — runs at first install on existing repos via the `adopt` sub-pipeline. Scans codebase for languages/frameworks/test infra/CI/dependencies/conventions; detects existing `AGENTS.md`/`.cursor/rules`; proposes initial SMEs; seeds threshold policy from existing baselines; writes `/src/memory/adoption/scan-<date>.md`. Read-only on existing files; write-only to Daedaline-prefixed paths.
...
A schema-validated config in `daedaline.yaml` (repo root), with overrides at `/src/memory/features/<id>/policy.yaml` and per-run flags. Defaults shipped in `daedaline-defaults.yaml`. **The user-facing layer is YAML; the evaluator is `Conftest` over `OPA Rego*`* ([conftest.dev](https://www.conftest.dev), [openpolicyagent.org](https://www.openpolicyagent.org)). Field shape mirrors **SonarQube QualityGate** for familiarity.
...
gates:
  ...
  ui:
    drift:
      ...
      visual_baseline: ".ddl/visual-baselines/"
    a11y:
...
Generated Rego (`@daedaline/policy` transpiles YAML to Rego at build time; users may also hand-write Rego under `/policies/*.rego` for advanced overrides):

```rego
package daedaline.coverage
...
Pipelines run inside isolated `git worktree` directories under `.ddl/worktrees/<task-id>/`, modeled on Cursor's `best-of-n-runner` pattern (confirmed 2026-04: native git worktrees on host with `.cursor/worktrees.json` setup commands; Claude Code's `isolation: worktree` frontmatter is the substrate for Claude users). Two execution-substrate implementations:
...
- `**SandboxPool**` *(M3)* — container/VM-per-task adapter. `**@daedaline/sandbox-e2b`** (Firecracker microVM, ~125–150ms cold start), `**@daedaline/sandbox-daytona`** (Docker/Kata/Sysbox, 27–90ms cold start), `**@daedaline/sandbox-modal`** (gVisor + GPU). Required for `pen-tester` always; required for `coder` when `on_new_deps` is true.

Per-persona substrate selection in `daedaline.yaml`:
...
`PortRegistryEnvIsolation` (default) allocates a unique PORT from a pool (3000-3100, file-backed registry under `.ddl/scheduler/ports.json`), generates `DB_NAME = <baseDb>_<taskIdShort>` (runs `CREATE DATABASE`), sets `COMPOSE_PROJECT_NAME = ddl-<taskIdShort>`, writes a per-worktree `.env.ddl` with these overrides, and registers cleanup hooks (drop DB, stop compose project). `ddl worktree gc --prune` enforces a `MAX_DISK_GB` policy.
...
2. The `conflict-planner` builds an interference graph across the cohort. Three-layer derivation: (1) LLM-declared (MVP), (2) **LSP-reference-graph closure** via `@daedaline/lsp-bridge` over `tree-sitter` (M3), (3) **Bazel/Buck `query` rdeps** when a build graph is present (M4). Each upgrade tightens the guarantee.
...
`ddl init` registers **Mergiraf 0.16.x** ([mergiraf.org](https://mergiraf.org), tree-sitter-based syntax-aware Git merge driver, 25+ languages, GPL-3.0 invoked as external binary) as the global merge driver:
...
```text
# .gitattributes (repo-level so it's tracked, opt-out via daedaline.yaml: merge.mergiraf=false)
* merge=mergiraf
```
...
4. **If all pass** → fast-forward to `main` (or open a single consolidated PR, per `daedaline.yaml`).
...
- **Run log** — every pipeline writes `/src/work/<day>/<id>/run.log.jsonl` of OTLP-encoded spans carrying **OpenInference** primary attributes (`openinference.span.kind` ∈ {LLM, AGENT, TOOL, RETRIEVER, RERANKER, EMBEDDING, GUARDRAIL, EVALUATOR, CHAIN, PROMPT}; `input.value`/`output.value`; `llm.input_messages.<i>.message.role|content`; `tool.name|parameters|result`) and **OTel GenAI semconv** parallel layer (`gen_ai.operation.name`, `gen_ai.provider.name`, `gen_ai.request.model`, `gen_ai.usage.input_tokens|output_tokens`). Daedaline-specific extensions namespaced as `daedaline.task_id|pipeline|persona|stage_id|checkpoint_seq|intervention.lever|contract.id`. `RunLogger` ships three exporters in MVP: `FileExporter` (default), `OTLPExporter` (Phoenix/Langfuse/Datadog/Honeycomb/New Relic), `LangSmithExporter` (M2). One file format = free integration with the entire OSS LLM-observability stack.
- **Spec contracts are gates, not suggestions** — any artifact under a feature can declare a `contract:` block; downstream pipelines load and enforce them automatically. **Per-artifact `contract:` is Daedaline's deliberate addition over Spec Kit's repo-level `constitution.md`** (Spec Kit has no per-artifact machine-checkable contract concept). M2 contract runners: **Schemathesis v4.8** ([schemathesis.io](https://schemathesis.io), `uvx schemathesis run` — single command, standard exit codes, 1.4–4.5× more bugs than competitors) for API contracts; **Playwright + `@axe-core/playwright`** for UX contracts; **Hypothesis (Python) / fast-check (TS)** for invariant assertions. LLM-judged fallback only when no harness applies.
- **Stage-boundary checkpointing** *(US-10)* — every supervisor writes a checkpoint at every stage boundary using **LangGraph's `BaseCheckpointSaver` interface and `Checkpoint v1` schema verbatim** (`{v, ts, id, channel_values, channel_versions, versions_seen, pending_sends}`, plus Daedaline-specific `metadata.worktree_commit` and `metadata.run_log_offset`). MVP `FileCheckpointSaver` is a conformant `BaseCheckpointSaver` implementation persisting to `/src/memory/checkpoints/<task-id>/<seq>.json`. M5+ adapters (`SqliteSaver`, `PostgresSaver`) are LangGraph's own saver classes.
...
Each Daedaline org publishes an **MCP server (`@daedaline/mcp-server`, stdio in MVP, HTTP at M5)** exposing:
...
Any MCP client (Cursor, Claude desktop, Codex CLI, ChatGPT desktop, Jules, custom) can drive a Daedaline org without `ddl` CLI. The `ddl` CLI is one MCP client among equals.

### A2A v1.x server interface (M5)

Each Daedaline org serves `/.well-known/agent-card.json` per **A2A v1.x** ([a2a-protocol.org](https://a2a-protocol.org), Linux-Foundation-hosted, 150+ orgs in production as of March 2026, SDKs in TS/Py/Java/Go/.NET, Signed Agent Cards, multi-tenancy, JSON-RPC + gRPC). Public personas + pipelines are exposed as A2A skills. `supervisor` speaks A2A as both server and client. AP2 (Agent Payments Protocol) discoverable via Agent Card `extensions` field.
...
| Daedaline lever        | Closest existing primitive                                                                                                                               | Implementation                                                                                                   |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
...
| `abort`                | `disposeWorktree(taskId)` + `checkpointer.deleteThread(taskId)` + emit `run-summary.md` (analog: Cloudflare `terminate()`, Temporal `Workflow.cancel()`) | Implemented in `@daedaline/intervention`                                                                         |
| `quarantine`           | **No SOTA equivalent — Daedaline original.**                                                                                                             | Mark thread `quarantined: true` in metadata; supervisor refuses to advance until `release`                       |
...
- `ddl steer <task-id> "consider X instead"` — inject a hint for the next turn.
- `ddl pause <task-id>` — halt at next checkpoint.
- `ddl resume <task-id>` — continue from paused state.
- `ddl reroute <task-id> --to <stage|persona> [--reason "..."]` — redirect.
- `ddl snapshot <task-id>` — fork a side-worktree at HEAD for inspection.
- `ddl rollback <task-id> --to <checkpoint-seq>` — revert to checkpoint.
- `ddl abort <task-id> [--reason "..."]` — kill + dispose + summarize.
- `ddl quarantine <task-id> [--reason "..."]` — freeze for review.
- `ddl release <task-id>` — release from quarantine.
...
/src/memory/
  ...
  adoption/         # adopter scan reports for existing-repo onboarding (US-9)
    scan-<date>.md  # citation-bearing artifact, replayable on `ddl re-adopt`
...
### `MemoryStore` interface (Mem0-shaped CRUD + Letta-tier overlay + Daedaline-native citation)

```typescript
export interface MemoryStore {
  // Mem0-shaped CRUD (so @daedaline/memory-mem0 adapter is a structural cast)
  ...
  // Letta-shaped tiers (so @daedaline/memory-letta adapter is trivial)
  tier(name: 'core' | 'recall' | 'archival'): TieredView;

  // Daedaline-native: filesystem-grounded, dual-anchor citation-bearing
...
- **M5+:** add pluggable `MemoryStore` adapters: `@daedaline/memory-mem0` (Mem0 cloud or self-hosted, ~48k★ Apache-2.0); `@daedaline/memory-letta` (Letta agent runtime, ~21k★ Apache-2.0, three-tier `core/recall/archival` model); `@daedaline/memory-graphiti` (Zep / temporal KG); `@daedaline/memory-cognee` (poly-store local-first); `**@daedaline/memory-hipporag2`** as the default M5 multi-hop retriever (10–30× cheaper than IRCoT/GraphRAG; matches Daedaline's local-lookup query pattern far better than GraphRAG's global-thematic strength). Self-RAG / CRAG patterns applied by Librarian on every retrieval (relevance + support + utility check; web/repo re-search on low quality).
...
The CLI grammar is intentionally **isomorphic to `claude-code` and `gemini` CLIs** (verb-first, slash-commands inside REPL, `ddl mcp` subcommand for MCP server management) so any operator who already knows one of those tools is productive in Daedaline on day one. **Every CLI verb is also a ddl MCP tool with the same name** (e.g. `ddl.run`, `ddl.steer`, `ddl.feature.show`) so external agents drive Daedaline over MCP without a parallel API surface.

Install / adoption (US-9):

- `npx create-daedaline <name>` — greenfield scaffold (full sample app + Daedaline install).
- `ddl init` — adopt into existing repo (non-destructive; runs `adopter` persona; emits `/src/memory/adoption/scan-<date>.md`; opens inbox item for ratification).
- `ddl init --dry-run` — print every file that would be created/modified; exit non-zero if any conflict.
- `ddl re-adopt` — re-run the adopter scan (e.g. after major refactor / new dependencies).
- `ddl mcp [list|add <name> <transport>|remove <name>|status]` — manage MCP servers Daedaline personas may call (mirrors `claude mcp` exactly).

Daily workflow:

- `ddl inbox [list|read <id>|reply <id>]` — manage the inbox.
- `ddl run <pipeline> <inbox-id|--feature <id>>` — kick off a pipeline.
- `ddl chat <persona> [--feature <id>]` — Conversation Mode (US-2). Opens a thread; `/end --emit <artifact>` to promote.
- `ddl feature [list|new <slug>|show <slug>|attach <slug> <file>]` — manage features (US-1, US-2, US-3).
- `ddl backlog [list|show <id>|prioritize]` — interact with the live backlog (US-5).
- `ddl sme [list|new <name>|chat <name>]` — manage long-lived SMEs (US-5).
- `ddl ensemble run <ensemble-name> --on <artifact-path>` — invoke a Persona Ensemble against an artifact (US-5).
- `ddl scout [list|run <name>|schedule <name> <cron>]` — manage scouts (US-5).
- `ddl worktree [list|gc|inspect <id>]` — manage the worktree pool (US-6).
- `ddl plan-cohort <feature-ids...>` — run the conflict-planner over a cohort, dry-run (US-6).
- `ddl policy [show|validate|diff <feature>]` — inspect threshold policies (US-4).
- `ddl status` — what's in-flight, blocked, awaiting approval.
- `ddl approve <task-id>` — clear a gate.
- `ddl memory query "..."` — Librarian-mediated search.

Intervention (US-10):

- `ddl pause <task-id>` / `ddl resume <task-id>` — *(MVP)* halt at next checkpoint / continue.
- `ddl abort <task-id> [--reason "..."]` — *(MVP)* kill + dispose worktree + emit summary.
- `ddl steer <task-id> "consider X instead"` — *(M2)* inject a hint into the next turn.
- `ddl reroute <task-id> --to <stage|persona> [--reason "..."]` — *(M2)* redirect.
- `ddl snapshot <task-id>` — *(M2)* fork a side-worktree at HEAD for inspection.
- `ddl rollback <task-id> --to <checkpoint-seq>` — *(M2)* revert state + worktree.
- `ddl quarantine <task-id> [--reason "..."]` / `ddl release <task-id>` — *(M2)* freeze for ombudsperson review.

Grooming (US-7, M3):

- `ddl debt [list|show <id>|inventory]` — view the tech-debt inventory.
- `ddl debt budget` — current week's grooming PR-budget burn-down.
- `ddl debt mode [propose|auto-pr]` — switch grooming mode.

### M2+: Slack adapter

- Notifier writes to `#daedaline-<repo>` via existing Slack MCP; replies route back into the inbox.
...
Every persona turn, tool call, ensemble vote, intervention, gate evaluation, checkpoint, and worktree action is emitted as an **OTLP-encoded span** that conforms to **both** the [OpenTelemetry GenAI semantic conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/) (`gen_ai.system`, `gen_ai.request.model`, `gen_ai.usage.input_tokens`, etc.) **and** the [OpenInference attribute set](https://github.com/Arize-ai/openinference) (`llm.input_messages`, `llm.output_messages`, `tool.name`, `tool.parameters`, etc.). This single emit makes every Daedaline run viewable in **Phoenix, Langfuse, LangSmith, Helicone, OpenLLMetry, OTel-native backends (Jaeger, Tempo, Honeycomb)** with zero adapter code, and lets users plug in their existing observability stack instead of paying a Daedaline tax. Local default exporter writes JSONL to `/src/work/<day>/<task-id>/run.log.jsonl` for grep-ability and offline replay.
...
Every stage boundary writes a checkpoint via `BaseCheckpointSaver.put()`. `**@daedaline/checkpointer-fs`** is the MVP implementation (writes to `/src/memory/checkpoints/<task-id>/<seq>.json`); `**@daedaline/checkpointer-sqlite`** uses `@langchain/langgraph-checkpoint-sqlite` directly; `**@daedaline/checkpointer-postgres`** wraps the official Postgres saver. `ddl pause | resume | rollback | snapshot` translate directly to LangGraph `interrupt()`, `Command(goto=...)`, `checkpoint_id` time-travel calls — **same primitives as the rest of the LangGraph ecosystem, no Daedaline-specific persistence to maintain.**
...
- `daedaline.yaml` + `daedaline-defaults.yaml`: threshold-policy schema with one working default per risk tier (US-4 foundation; full UI-drift / regression suites land in M2/M3); `commands.test|build|lint` hooks (US-9 coexistence).
...
- **Monorepo package layout per §5.5 from day 1** (US-8): `@daedaline/core`, `@daedaline/persona`, `@daedaline/pipeline`, `@daedaline/memory`, `@daedaline/inbox`, `@daedaline/worktree`, `@daedaline/env-isolation`, `@daedaline/intervention`, `@daedaline/notifier`, `@daedaline/policy`, `@daedaline/checkpointer-fs`, `@daedaline/run-logger` (OpenInference + OTel GenAI exporter), `@daedaline/runner-cursor`, `@daedaline/adopter-scan`, `@daedaline/cli`, `daedaline` (meta). pnpm workspace + Turborepo + Changesets + `@arethetypeswrong/cli` + `publint` per §5.5. ESLint rule + CI check enforcing no horizontal deps between primitives. *Library-mode primitives don't all need rich features in MVP — the boundaries do.*
- `AgentRunner` interface + `CursorRunner` implementation (in `@daedaline/runner-cursor`); `ClaudeCodeRunner` is M3.
...
- `Checkpointer` interface = LangGraph `BaseCheckpointSaver` v1 directly (no Daedaline wrapper); `@daedaline/checkpointer-fs` writes to `/src/memory/checkpoints/<task-id>/<seq>.json`.
- `RunLogger` (`@daedaline/run-logger`): OTLP/JSONL exporter with OpenInference + OTel GenAI semconv attributes; default local file sink.
- `**Intervention` interface + checkpointer** (US-10): supervisor writes a LangGraph checkpoint at every stage boundary; CLI surfaces `ddl pause | resume | abort` (full lever spectrum lands in M2 mapped to LangGraph `interrupt()` / `Command(goto)` / `checkpoint_id` time-travel).
...
- `**@daedaline/mcp-server`** (MVP-skeleton, fully wired in M2): publishes `ddl.*` verbs as MCP Tools and `/src/memory/`, `/src/inbox/`, `/src/work/run-log` as MCP Resources; `human.respond` exposed as MCP Elicitation.
- **Install-path proof (US-9, M1):** `pnpm -w exec ddl create-daedaline <name>` scaffolds greenfield; `pnpm -w exec ddl init` runs adopter scan and emits a ratification inbox item on an existing repo. Standalone `examples/` apps are backlog-deferred past M1.
...
- **M7 — Portability + Conformance. Locks in US-8.** `ClaudeCodeRunner`, `OpenAIAgentsSDKRunner`, `LangGraphRunner` adapters; **conformance test suite** (single test suite that runs against any `AgentRunner`/`MemoryStore`/`WorktreePool`/`Intervention` implementation); A2A protocol support so a daedaline org can talk to a non-daedaline org. Each primitive promoted to `stability: stable` once conformance tests are green.
- **M8 — Self-improvement.** Ombudsperson promoted from watchdog-only to full audit role; pipelines that mine their own run logs to propose handbook/persona/skill diffs (Bugbot's hill-climb-on-resolution-rate pattern). Scout-emitted RFCs that propose changes to *Daedaline itself* (the literal US-5 scenario, applied recursively).
- **M9 — Multi-repo / multi-org.** A team of teams: daedaline orgs that share a meta-handbook (the "engineering organization" tier); cross-repo conflict planning.
...
- **Q1.** Implementation language for `ddl` CLI and adapters — TS/Node (rich MCP/Mastra/Inngest ecosystem) vs Python (richer LLM-eval tooling)? Default proposed: **TS**.
...
- **Q4.** Branding/codename: keep `daedaline`, or rename for distribution?
...
- **Q13 (new, US-8).** *Library-mode minimum API surface for MVP.* All primitives importable from M1, or only `@daedaline/persona` + `@daedaline/worktree` + `@daedaline/intervention` (the three with the highest standalone value)? Default proposed: **package boundaries from M1; rich library APIs ratchet up by milestone, with full conformance only at M7**.
...
- **Q18 (SOTA, MCP-vs-CLI).** *MCP server scope.* Should `@daedaline/mcp-server` expose every CLI verb 1:1 (broad surface, easier discoverability), or curate a smaller set of high-value verbs (smaller blast radius)? Default proposed: **phased surface: curated MVP set, then expansion toward parity**.
...
- **Q20 (SOTA, retrieval at M5).** *Default M5 vector index.* HippoRAG2 (research-leading on multi-hop), Mem0 (largest user base), Letta (richest tier model), or all three behind a unified API with profile-based defaults? Default proposed: **HippoRAG2 default for retrieval, Mem0 CRUD-shape as the canonical interface, Letta tiers as overlay; users choose via `daedaline.yaml memory.profile`**.
...
- **R10 (new).** *Scope creep via SMEs and scouts.* A repo could accumulate 50 SMEs and 10 scouts each costing tokens. Mitigated by Librarian-driven SME demotion (idle-decay) and explicit budget visibility in `ddl status`.
...
- **R18 (SOTA, dependency depth).** *Reliance on the LangGraph / OpenInference / Mergiraf / Conftest ecosystem.* Adopting SOTA means we now ship a transitive footprint that can break independently of us. Mitigated by (a) pinning to LTS minor versions, (b) the conformance test suite from M3, (c) keeping each adapter behind a `@daedaline/`* package boundary so a breaking upstream change is one package's blast radius, not the whole framework's.
- **R19 (SOTA, schema drift).** *OpenInference and OTel GenAI semconv are still evolving.* Field renames or attribute restructures could orphan our existing run logs. Mitigated by version-stamping every span (`daedaline.run_logger.version`) and shipping forward-migration scripts in `@daedaline/run-logger` major releases.
- **R20 (SOTA, LangGraph runtime lock-in).** *We compile pipelines to LangGraph `StateGraph`, but Cursor / Claude Code agents may evolve their own native graph runtime that conflicts.* Mitigated by Q16 deferral discipline + the `Pipeline → StateGraph` compiler being one isolated package (`@daedaline/pipeline`) so swapping runtimes is one PR, not a cross-cutting refactor.
- **R21 (SOTA, MCP attack surface).** *Publishing all CLI verbs as MCP tools opens automated remote agent control of Daedaline.* Mitigated by per-verb `mcp.disabled` opt-out (Q18), Cedar `Authorizer` gating every verb invocation in library mode, and OpenInference logging every MCP-originated call with caller identity.
...
- **R23 (SOTA, sycophancy by configuration).** *A misconfigured ensemble (`sycophancy: 1.0` on every persona) silently regresses to agreement theater.* Mitigated by `ddl lint ensembles` warning on configurations with median sycophancy > 0.5 and zero `dispositions`, and runtime ombudsperson alerts when measured SS deviates > 0.3 from configured target.
...
- **A2.** ≥ 90% of merged PRs from daedaline pipelines reference a PRD/RFC/ADR.
...
- **A16 (US-8).** Every primitive in `src/internal/packages/` (a) passes the conformance test suite at M7, (b) declares an explicit `stability` tier, (c) has zero horizontal package dependencies (CI-enforced from M1). Non-Daedaline usage examples in `examples/` are backlog-deferred past M1 and become required by M7.
- **A17 (US-9).** `ddl init` on 5 representative existing repos (TS monorepo, Python service, Rust binary, Go API, Next.js app) completes non-destructively (zero overwritten files) and produces an `adoption/scan-*.md` rated ≥ 4/5 by repo owners on accuracy.
- **A18 (US-10).** Median time from operator-issued `ddl pause` to actual pause-acknowledgment ≤ 1 stage boundary; zero rollback/abort actions destroy uncommitted human edits; from M5+, watchdog detects ≥ 80% of stuck-state cases that humans would have flagged manually within 5 turns of the human.
...
- **A20 (SOTA, ecosystem reuse).** ≥ 1 third-party LangGraph checkpointer / OpenInference exporter / Mem0 memory backend / MCP client / A2A peer integrated end-to-end without writing Daedaline-specific glue, by M7. (Validates that "we conform, not parallel-design" is real.)
...
- **A13.** Onboarding time for a new human collaborator on a daedaline repo: < 1 day to ship first PR (vs. industry baseline of 3.3× monthly salary in lost productivity per [developeronboardingcost.com](https://www.developeronboardingcost.com/)).
- **A14.** Cost per feature-delivery (median) reported in `ddl status`; flat or decreasing over time as Librarian curation reduces context retrieval cost.
