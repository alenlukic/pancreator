...
`VERSION` is the operator-facing harness version and MUST use complete Semantic Versioning. `VERSION`, `package.json`, and the root package in `package-lock.json` currently agree on `4.2.0`. `CHANGELOG.md` records curated release history in Common Changelog format.
