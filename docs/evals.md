...
- `prepare_invocation` runs `prepareInvocation`, which runs the workspace setup commands once per run at the start stage, recorded on the run state as `workspace_setup`, and for a source-allowed stage captures the baselines in the toy workspace.
