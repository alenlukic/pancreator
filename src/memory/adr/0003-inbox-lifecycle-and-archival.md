...
- `responded`: the required response artifact is present in `/src/inbox/out/`, and
...
`/src/inbox/in/` as the active queue and `/src/inbox/archive/in/` as the canonical
...
- negative: Existing tools that only read `/src/inbox/in/` MUST be updated before
