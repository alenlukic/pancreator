...
- [x] T1: Package scaffold: `bin.pan` → `dist/cli.js`, `src/cli.ts` + `src/run.ts`, exports `parseAndRun`, `commander` runtime dependency, workspace deps on `@pancreator/inbox` and `@pancreator/intervention`.
