...
next_stage: plan
source_inbox_item: src/inbox/in/172981_05-25-26/71700_0612_bootstrap-cruft-cleanup-batch.md
intake_round: 1
work_packages:
  - id: resolve-package-stub-feature-folders
    label: "Resolve stub @tesseract/* feature folders"
  - id: consolidate-cursor-agent-variants
    label: "Consolidate Cursor agent variants"
references:
  - kind: lines
    path: src/inbox/in/172981_05-25-26/71700_0612_bootstrap-cruft-cleanup-batch.md
    range: [1, 137]
    contentHash: ec6a02d
  ...
  - kind: lines
    ...
    range: [1, 23]
    contentHash: 26b1233
    note: "Representative package engineering spec after frontmatter addition; sibling tesseract-* folders follow the same layered contract pattern."
  - kind: lines
...
This Feature removes two classes of premature surface area from the Tesseract
...
inventory of **20** `src/memory/features/tesseract-*/` folders to Phase-2
...
`src/memory/handbook/subagent-model-tiers.md`; filename suffixes duplicated that
...
- When `tech-lead` authors the disposition table, the table SHALL classify
  each of the 20 `src/memory/features/tesseract-*/` stub folders into exactly
...
- When the disposition table classifies a folder as `author`, `tech-lead`
  ...
  `src/memory/features/json-formatting/` — `spec.md` meeting Layer-1 lint,
...
- When the cleanup ships, every `src/memory/features/tesseract-*/` folder covered
...
- When `persona-designer` records the standard-vs-complex tier distinction,
  ...
  `metadata.tesseract-subagent-tiers` and SHALL be resolved at invocation time
...
- When `tech-writer` updates the handbook, `src/memory/handbook/
...
- **Disposition table authoring.** The per-folder classification (`author` /
  `consolidate` / `delete`) for each of the 20 `tesseract-*` stub folders is
...
- **Invocation grammar schema.** The exact YAML schema for
  `metadata.tesseract-subagent-tiers` and the companion Cursor invocation
