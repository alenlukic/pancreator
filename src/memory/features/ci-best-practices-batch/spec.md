...
Tesseract bootstrap. Each improvement corresponds to a named work package (WP)
...
read-only MCP tools — `tess.feature.list`, `tess.feature.show`, `tess.status`,
and `tess.memory.query` — so they return typed results rather than
...
- The `package.json` root SHALL carry a `"test"` script that runs
  ...
  invocation.
- When `turbo run test` executes, each `@tesseract/*` package SHALL resolve a
...
- When an MCP client calls `tess.feature.list`, the handler SHALL return a typed
  array of feature summaries derived from `src/memory/features/*/index.json`
  entries, not a `{"status":"stub"}` envelope.
- When an MCP client calls `tess.feature.show` with a `feature_id` argument, the
...
- When an MCP client calls `tess.status`, the handler SHALL return a typed
  pipeline-status summary derived from active work state and `tesseract.yaml`.
- When an MCP client calls `tess.memory.query` with a `query` argument, the
...
- The `@tesseract/mcp-server` README SHALL document the wired read surface and
...
- Remaining write MCP tools SHALL remain stubbed using the structured deferral
  protocol per `src/internal/packages/@tesseract/cli/src/run.ts` when that
...
- A `tess compliance run` CLI verb; the node script is sufficient until M2.
...
|---|---|
| WP-A — `pnpm test` script and package `test` entries | `tesseract-engineer` (aggregator, package scripts); `coder` (CI workflow); `reviewer` (runtime budget) |
| WP-B — Compliance runner and CI gate | `tesseract-engineer` (runner, CI); `compliance-auditor` (adapter contract, finding m-02 closure); `reviewer` (exit-code audit) |
| WP-C — Citation refresh tool | `tesseract-engineer` (tool, node-test suite); `librarian` (post-merge sweep); `compliance-auditor` (m-02 closure verification) |
| WP-D — MCP read handlers | `tech-lead` (shared result envelope design); `tesseract-engineer` (handlers, vitest suite); `reviewer` (no-write audit) |
