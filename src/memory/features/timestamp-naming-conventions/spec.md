...
This Feature SHALL define one UTC-based naming convention for in-scope temporal artifacts in `src/work/` and `src/inbox/`, SHALL migrate existing in-scope artifacts to that convention, and SHALL improve reverse-chronological navigability without changing the broader repository organization philosophy that the directive defers to a future run.
...
- When the Feature evaluates naming-policy scope, the Feature MUST include day
  directories and task subdirectories under `src/work/`.
...
- When the Feature creates a task subdirectory under `src/work/`, the Feature MUST
...
- When the Feature creates a system-produced artifact under `src/inbox/out/`, the
  Feature MUST use `{SID-prefix}_{HHMM}_{semantic-suffix}` as the basename
  shape.
- When the Feature creates a system-produced artifact under `src/inbox/threads/`,
  the Feature MUST use `{SID-prefix}_{HHMM}_{semantic-suffix}` as the basename
  shape.
- When the Feature migrates artifacts under `src/inbox/threads/`, the Feature MUST
  preserve the existing per-feature parent folder.
- When the Feature processes a non-conforming human-generated artifact in
  `src/inbox/in/`, the agent performing that work MUST append the two time
  prefixes before downstream processing continues.
- When the Feature processes a non-conforming human-generated artifact in
  `src/inbox/threads/`, the agent performing that work MUST append the two time
  prefixes before downstream processing continues.
...
- When the Feature migrates existing artifacts, the Feature MUST migrate
  already-existing round files under `src/inbox/threads/`.
