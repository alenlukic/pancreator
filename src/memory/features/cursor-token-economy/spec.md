...
- When the Feature stages the indexing policy, the Feature MUST exclude
  `src/work/**`, `src/inbox/out/**`, and `src/inbox/threads/**` from default Cursor
...
- When the Feature stages the indexing policy, the Feature MUST exclude
  ...
  semantic indexing through the `.cursorindexingignore` file.
- When the Feature stages the indexing policy, the Feature MUST exclude
  `src/memory/**/*.json`, `src/memory/**/delivery-report.md`, and
  `src/memory/**/index.json` from default Cursor semantic indexing through the
...
- When the Feature authors `src/memory/handbook/context-economy.md`, the
...
- When the Feature ships the context-budget tool, the Feature MUST add
  one command at `src/internal/tools/context-budget-report.mjs`.
...
- This Feature SHALL NOT read, write, traverse, or cite any file under
  `/src/inbox/notes/`, which the inbox-lifecycle handbook reserves as a
