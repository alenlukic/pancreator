...
vendored paths, and tooling-regenerated files. `tesseract-engineer` SHALL
...
- When an agent or automated script writes a JSON field that contains a SHA-256
  ...
  HEAD` at write time.
- When `tesseract-engineer` executes the bulk migration script, the script
...
- When an agent writes a dual-anchor citation JSON object inside Markdown prose
  ...
  in `src/internal/tools/migrate-json-formatting.mjs`.
...
- When this Feature is delivered, the **Content hash** / `contentHash` entry in
  `src/memory/handbook/glossary.md` SHALL be updated so that its normative
...
- **Citation-verifier implementation.** The operator confirmed that the
  ...
  `src/memory/handbook/glossary.md` to specify the abbreviated form as the
...
Artifacts under `src/work/172983_05-23-26/67055_0522_json-formatting/` —
including `plan.md`, `adr-draft.md`, `touch-set.json`, `implementation-report.md`,
`review.md`, and the delivery report at
`src/memory/features/json-formatting/delivery-report.md` — were produced against
