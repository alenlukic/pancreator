...
`tess`, and agent-chat JSON sketches. A shared canonical formatter module
...
{
  "kind": "lines",
  "path": "src/work/172983_05-23-26/67055_0522_json-formatting/implementation-report.md",
  "range": [13, 16],
  "contentHash": "514098b"
}
```
...
{
  "kind": "lines",
  "path": "src/work/172983_05-23-26/67055_0522_json-formatting/review.md",
  "range": [3, 4],
  "contentHash": "bd00fb1"
}
```
...
Markdown/terminal/agent-chat surfaces beyond the prior repo-file-only scope.
  ...
  {
    "kind": "lines",
    "path": "src/work/172983_05-23-26/67055_0522_json-formatting/adr-draft.md",
    "range": [47, 58],
    "contentHash": "61ffd6b"
  }
  ```
...
extension, test and fixture additions.
  ...
  ```
- Slice B (`tesseract-engineer`) executes the one-shot `.json` bulk migration evidence loop.
...
the `TESS_JSON_FORMAT_ABBREV_LEN` env override (decimal digits, 4–255) or from
...
second pass on already-canonical output returns `changed: false`.
  ...
  ```
- CLI `stringifyCliJson(repoRoot, value)` in `@tesseract/cli/src/canonical-json-io.ts` is the
thin bridge that calls `resolveAbbrevLen`, `abbreviateHashes`, and `formatCanonicalJson` to
serialize all `tess` terminal output and `state.json` writes.
  ```json
  {
    "kind": "lines",
    "path": "src/internal/packages/@tesseract/cli/src/canonical-json-io.ts",
    "range": [7, 12],
    "contentHash": "0ea7b62"
  }
  ```
...
`formatCanonicalJson`.
  ...
  {
    "kind": "lines",
    "path": "src/internal/tools/migrate-json-formatting.mjs",
    "range": [126, 153],
    "contentHash": "081c2c1"
  }
  ```
...
stays bounded while a follow-on feature remains queued.
  ...
  ```
- The `--write` path requires `TESSERACT_MIGRATION_GO=1`, preventing accidental mutation in
...
2. Call `stringifyCliJson(repoRoot, value)` from `@tesseract/cli` for all `tess` terminal
...
3. Set `TESS_JSON_FORMAT_ABBREV_LEN=7` in test environments that operate without a `.git`
...
| `TESSERACT_MIGRATION_GO=1 node src/internal/tools/migrate-json-formatting.mjs --write` (pass 1) | 0         | `files rewritten=0`                |
| `node src/internal/tools/migrate-json-formatting.mjs --dry-run` (post-write)                    | 0         | `wouldRewrite=0`                   |
| `TESSERACT_MIGRATION_GO=1 node src/internal/tools/migrate-json-formatting.mjs --write` (pass 2) | 0         | `files rewritten=0`                |
