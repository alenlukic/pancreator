...
**Done:** `Inbox` + `FileInbox` with canonical `src/inbox/in`, `src/inbox/out`, `src/inbox/threads` paths; `listIn`, `readInFile`, `writeOutFile`; Vitest + README Quickstart; `typecheck`/`test` scripts. Deprecated `TESSERACT_INBOX_STUB` / `inboxStubVersion` retained for the `tesseract` meta package.
