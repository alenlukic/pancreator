# Task List - @pancreator/worktree

- [x] T1: `GitWorktreePool` + `WorktreePool` interface; path validation under `.pan/worktrees/`; `acquire` / `release` / `list`; persisted `pool-state.json`; Q7 single-pipeline lease; injectable `GitOps` with `createNodeGitOps` and `createMemoryGitOps`.
