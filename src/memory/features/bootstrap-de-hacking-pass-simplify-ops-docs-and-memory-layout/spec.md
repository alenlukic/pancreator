...
- When WP-4 ships, every prior reference to `src/memory/debt/` across
  ...
  `.cursor/rules/`, and `src/internal/packages/@pancreator/mcp-server/`
  SHALL redirect to `src/memory/backlog/index.yaml` or SHALL be removed.

- When WP-4 ships, the MCP resource list under
  `src/internal/packages/@pancreator/mcp-server/` SHALL NOT advertise
...
| WP-4 | `AGENTS.md` § 7, `src/memory/handbook/backlog-format.md`, `src/memory/handbook/memory-tiers.md`, `src/internal/packages/@pancreator/mcp-server/`, `docs/PRD.index.md`, `src/memory/backlog/index.yaml` |
