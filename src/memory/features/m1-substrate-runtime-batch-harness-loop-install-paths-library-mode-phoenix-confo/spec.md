...
next_stage: plan
source_inbox_item: src/inbox/in/172980_05-26-26/2597_2316_m1-substrate-runtime-batch.md
intake_round: 0
...
- When `@tesseract/intervention` calls pause, resume, or abort on a running pipeline, the intervention manager SHALL use the `BaseCheckpointSaver` official methods and SHALL NOT maintain a parallel persistence path.
- When `Checkpoint` carries Tesseract-specific fields (`worktree_commit`, `run_log_offset`), those fields SHALL reside in `Checkpoint.metadata` per `src/memory/handbook/run-log-schema.md`.
...
- When `tesseract.yaml: runner.cursor.invocation` is set to `sdk`, the runner SHALL call the Cursor SDK and SHALL respect persona `model`, `tools`, `disallowedTools`, and `maxTurns` fields.
- When `tesseract.yaml: runner.cursor.invocation` is set to `manual` (default), the runner SHALL preserve existing behavior and SHALL NOT call the Cursor SDK.
...
- When intervention levers (`pause`, `reroute`, `abort`) are compiled, they SHALL compile as a single side-channel node so `@tesseract/intervention` does not maintain parallel state.
...
- When `pnpm --filter @tesseract/pipeline test` runs, the test suite SHALL pass compiler tests, intervention-node injection tests, and parse → compile → serialize → re-parse identity tests.
...
- When a pull request touches `@tesseract/run-logger`, CI SHALL run the Phoenix smoke test using a path-filtered trigger.
...
- When the script runs, it SHALL NOT read any path under `src/memory/`, `src/inbox/`, or `tesseract.yaml`.
- When the script resolves its imports, it SHALL import only `@tesseract/persona` and declared external peers.
...
- When an operator runs `pnpm -w exec tess init --dry-run` against a standard example repository, the command SHALL complete without writes outside `src/memory/adoption/` and SHALL surface per-file diffs on stdout.
- When an operator runs `pnpm -w exec tess init --apply` against an empty directory, the command SHALL install the M1 scaffold.
- When `tess init` detects a conflicting file, the command SHALL refuse the write and SHALL exit non-zero unless `--force` is supplied.
- When `tess init` runs, the scan report SHALL write to `src/memory/adoption/scan-<UTC-day>.md` and SHALL open an inbox ratification item listing detected languages, frameworks, and proposed threshold-policy seeds.
- When `npx create-tesseract <name>` is run, the command SHALL create a complete M1 scaffold (handbook seed pointers, `tesseract.yaml`, `AGENTS.md`, a sample inbox directive, and a runnable `feature-delivery` walkthrough) independent of this repository's bootstrap layout.
- When `npx create-tesseract demo` completes, `pnpm -w exec tess inbox` and `pnpm -w exec tess run feature-delivery` SHALL succeed on the generated scaffold.
...
- When delivery is complete, a `compliance-auditor` broad sweep SHALL report zero `block` findings tied to stub runner behavior (`dryRun: true`), stub `tess init` behavior (`{"status":"stub"}`), or missing library-mode proof.
...
| WP-F — CLI + create-tesseract | `coder` |
