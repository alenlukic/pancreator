...
`src/memory/handbook/` and exits non-zero when a line invokes bare `pan …`
...
`/src/memory/handbook/pancreator-config.md` §“CLI invocation in this workspace”.
...
```bash
pnpm -w exec pan advance 67055_0522_json-formatting --artifact src/work/172983_05-23-26/67055_0522_json-formatting/review.md
```
...
See `/src/memory/handbook/pancreator-config.md` §“`feature-delivery` inbox entry
argument”.
...
```markdown
**How:** Stage `AGENTS.md`, `src/memory/handbook/pancreator-config.md`,
...
1. **What:** Record the accepted review artifact and advance the feature-delivery run to the report stage.
   **How:** Run `pnpm -w exec pan advance 67055_0522_json-formatting --artifact src/work/172983_05-23-26/67055_0522_json-formatting/review.md`. Confirm `src/work/172983_05-23-26/67055_0522_json-formatting/state.json` shows `current_stage: report` before delegating `tech-writer`.
```
...
2. **What:** Send the plan back for revision without advancing the run.
   **How:** Add a new thread round under `src/inbox/threads/<day>/<feature-slug>/` with required edits; do not run `pnpm -w exec pan advance …` until a superseding plan artifact exists.
...
| Pick up inbox work | Start next feature | Read-only: `ls src/inbox/in/`; mutating: `pnpm -w exec pan run feature-delivery <day-bucket>/<file>.md` |
