...
schema in `/src/memory/handbook/contract-format.md`, and the prose passes the
...
Adding a word to the ban list requires an RFC under `/src/memory/rfc/draft/`.
Removing a word requires the same.

### Rule 1.7 — Glossary discipline

Every domain noun MUST resolve to an entry in `/src/memory/handbook/glossary.md`
...
the dual-anchor form defined in `/src/memory/handbook/glossary.md` (kind:
...
`/src/memory/handbook/contract-templates/<name>.template.md`. The MVP set is
...
the author MUST open an RFC under `/src/memory/rfc/draft/` proposing the
...
Every `wrapper.owner` MUST resolve to a persona file under `src/personas/`. The
...
A template change requires an RFC under `/src/memory/rfc/draft/`. The RFC SHALL
...
`/src/memory/adr/`. The ADR SHALL declare the milestone the kind is allowed in
and SHALL update the allowlist on `src/personas/contract-writer.md`.
