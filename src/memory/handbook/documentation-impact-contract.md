...
references:
  ...
  - kind: lines
    path: src/memory/handbook/backlog-format.md
    range: [56, 78]
...
- create or update a backlog item in `/src/memory/backlog/index.yaml`, and
...
be routed through `/src/memory/handbook/policy-compliance-contract.md`, which
binds documentation-impact decisions to machine-checkable
`/src/work/<day>/<task-id>/policy-compliance.json` artifacts.
