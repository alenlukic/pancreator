...
| Format agent completion output for the operator | `/src/memory/handbook/operator-output-contract.md` | `/AGENTS.md`, `/src/memory/handbook/persona-spec.md` | Defines the mandatory `## Next operator steps` block (what/how, read-only labels, multi-option when/impact). |
...
| Active-memory pointers and current coordination | `/src/memory/active/current.md` | `/src/memory/active/handoffs.md`, `/src/memory/handbook/context-economy.md`, `/src/memory/handbook/memory-tiers.md` | Active-memory holds summaries and handoff pointers only; durable, active-work, and archival artifacts stay on their own tiers. |
...
| Interpret or update `pancreator.yaml` | `/src/memory/handbook/pancreator-config.md` | `/pancreator.yaml`, `/pancreator-defaults.yaml`, `/src/personas/adopter.md` | Defines `project_root`, live bootstrap tracking, and the boundary between live policy and defaults. |
| Run or document `pan` CLI commands for operators | `/src/memory/handbook/pancreator-config.md` §“CLI invocation in this workspace” | `/src/memory/handbook/operator-output-contract.md`, `/AGENTS.md` | Use `pnpm -w exec pan …` from repo root; bare `pan` is not on PATH. |
