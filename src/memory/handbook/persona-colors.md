...
purpose: |
  ...
  dangling pointer in `src/skills/author-persona/SKILL.md` Step 3 and projects
  the palette table from `/src/memory/handbook/persona-spec.md` §6 into a
...
unknown colors as `slate` and emit a Layer 1 warning during `tess persona
validate`.

When two personas need the same color (e.g., a long-lived SME family),
namespace the color in the persona file via `metadata.tesseract-color-suffix`
...
table in `/src/memory/handbook/persona-spec.md` §6 verbatim. Source of truth is
