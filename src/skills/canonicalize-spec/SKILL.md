...
can act on. The canonical caller is `src/personas/intake-analyst.md`. The
...
`/src/memory/handbook/glossary.md` §5.

## Prerequisites

- `src/inbox/in/` SHALL contain exactly one Markdown file the human posted to
  trigger the `intake` stage; an empty queue MUST halt and route an inbox
  item per the Failure-handling section.
- `/src/memory/features/` SHALL exist; the skill scaffolds
  `/src/memory/features/<id>/` on first invocation.
- `/src/memory/handbook/glossary.md` SHALL exist; every domain noun in the
  canonical spec resolves through this file.
- `/src/memory/handbook/contract-style.md` SHALL exist; the spec body satisfies
...
`/src/memory/features/`, you MUST append a numeric suffix (`-v2`, `-v3`, ...)
rather than overwrite.

Scaffold the directory layout per PRD §8 lines 921 through 931:

```
/src/memory/features/<id>/
...
Read the informal spec at `src/inbox/in/<file>.md`. For each ambiguity, record
...
`src/inbox/threads/<thread-id>/<round-NN>.md`. The file MUST list at most 7
...
4. Append the next round's clarifying file under
   `src/inbox/threads/<thread-id>/<round-NN>.md`.
...
`/src/memory/features/<id>/spec.md` with the four `##` sections below in this
...
2. **Acceptance criteria.** A bulleted list. Each criterion MUST follow an
   ...
   alternates per `/src/memory/handbook/contract-style.md` Rule 1.5) and MUST be
   machine-checkable.
...
`/src/memory/handbook/contract-style.md` to every normative clause. Each rule
MUST hold:
...
1. Stage `/src/memory/features/<id>/spec.md`. You MUST NOT commit; you MUST
   NOT push.
2. Open one inbox item at
   `src/inbox/in/<timestamp>-intake-<id>-ratification.md` summarizing the
...
- Halt when `src/inbox/in/` carries no Markdown file at intake start; route an
...
- Halt when the human does not reply within 7 calendar days of a
  ...
  frontmatter, post one reminder under `src/inbox/out/`, and stop. The Feature
...
- If `src/inbox/in/` is empty at intake start, the skill MUST halt and open an
  inbox item at `src/inbox/in/<timestamp>-intake-empty-queue.md` to the
