...
- `/src/memory/adoption/` SHALL exist as a writable directory; the skill
...
`/src/skills/`, `/src/pipelines/`, `/src/inbox/`, `/.pan/`, and `pancreator.yaml`.
...
`src/inbox/in/<timestamp>-adopter-conflicts.md` carrying the per-file diff per
...
Overwrite `/src/memory/adoption/scan-<date>.md` with the eight `##` sections
declared in `src/personas/adopter.md` in this order. Every fact in every
...
proposal at `src/inbox/in/<timestamp>-adopter-thresholds.md`. Each inbox file
MUST link the relevant section of the scan report.

Apply the Layer 1 lint discipline declared in
`/src/memory/handbook/contract-style.md` to the scan report. Each rule MUST
hold:
