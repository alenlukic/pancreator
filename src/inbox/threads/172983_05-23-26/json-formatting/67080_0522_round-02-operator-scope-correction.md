...
supersedes-partial: src/memory/features/json-formatting/spec.md
...
fold this round into `src/memory/features/json-formatting/spec.md` and treat
...
The informal directive at `src/inbox/in/json_formatting.md` line 1 requires
...
`src/memory/features/json-formatting/spec.md` incorrectly limited enforcement
...
in `src/internal/tools/migrate-json-formatting.mjs` (`formatCanonicalJson`):
...
4. Add an explicit **Superseded work** note: artifacts under
   `src/work/172983_05-23-26/67055_0522_json-formatting/` from plan through
...
1. Read this file, `src/inbox/in/json_formatting.md`, and the current
...
4. Leave `pnpm -w exec pan advance 67055_0522_json-formatting --artifact src/memory/features/json-formatting/spec.md` to the human operator after spec review.
