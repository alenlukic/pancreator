...
- Q1. Single-file leaf placement vs per-artifact HHMM subdirectory. The
  ...
  folder-per-item parity with `src/work/`?
...
- Q4. Compliance descriptor surface. Extend
  `src/internal/tests/compliance/timestamp-naming-conventions.yaml` with hierarchy
  clauses, or land a sibling
  `src/internal/tests/compliance/inbox-directory-hierarchy.yaml`? Assumption A4 picks
