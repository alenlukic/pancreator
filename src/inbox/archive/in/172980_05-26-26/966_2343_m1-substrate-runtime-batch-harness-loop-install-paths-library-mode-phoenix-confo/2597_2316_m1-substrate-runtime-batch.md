...
| WP-F — Install paths (`tess init` + `create-tesseract`) | `tess-install-paths` | `64500_0605_tess-init-and-create-tesseract-install-paths.md` |
...
1. **Checkpointer gap (WP-A).** `@tesseract/checkpointer-fs` is a
   Tesseract-specific file store, not a conformant LangGraph
   `BaseCheckpointSaver` v1. KPI A19 and `tess pause | resume | rollback |
   snapshot` cannot map to LangGraph time-travel without it.

2. **Runner stub (WP-B).** `@tesseract/runner-cursor` returns `{dryRun: true}`
   without calling the Cursor SDK. Operators still paste `next-prompt.md`
   manually and call `tess advance` — the substrate gap BR4 anticipated.

3. **Pipeline walker (WP-C).** `@tesseract/pipeline` executes YAML as a
...
6. **Install paths unwired (WP-F).** US-9 promises `tess init` and
   `npx create-tesseract` in M1. `tess init` returns `{"status":"stub"}` and
   `create-tesseract` does not exist despite the adopter persona, adopter-scan
...
1. `@tesseract/checkpointer-fs` exports a class satisfying LangGraph
   `BaseCheckpointSaver` v1 without adapter glue.
2. LangGraph's checkpoint-saver test suite runs as a vitest harness in CI.
3. Tesseract-specific fields (`worktree_commit`, `run_log_offset`) live in
...
3. Invocation respects `simple task mode` and never reads outside the declared
   touch-set.
4. Feature flag `tesseract.yaml: runner.cursor.invocation: manual | sdk` selects
...
3. Intervention nodes (`pause`, `reroute`, `abort`) compile as a single
   side-channel node so `@tesseract/intervention` does not maintain parallel
   state.
...
2. CI runs the smoke test on PRs touching `@tesseract/run-logger` (path-filtered).
...
2. `tesseract.yaml` bootstrap evidence points at the ADR instead of a free-standing
   deferred note.
...
3. Example uses only `@tesseract/persona` plus declared external peers.
...
### WP-F — `tess-install-paths`

1. `tess init` runs `@tesseract/adopter-scan` in dry-run by default, surfaces
...
3. `tess init --dry-run` is default; `--apply` required for writes outside
   adoption memory.
4. `npx create-tesseract <name>` creates a complete M1 scaffold (handbook seed
   pointers, `tesseract.yaml`, AGENTS.md, sample inbox directive, runnable
...
- A compliance-auditor broad sweep validates zero stale references to removed
  stub behavior (`dryRun`, `{"status":"stub"}` for `tess init`) after the batch
  ships.
...
- `pnpm --filter @tesseract/checkpointer-fs test` passes LangGraph conformance
  plus Tesseract metadata-extension tests.
...
- `pnpm --filter @tesseract/pipeline test` covers compiler, intervention-node
...
- Either smoke tests pass in CI (option A), or ADR is ratified by
  LocalUserAuthorizer and `tesseract.yaml` updated (option B).
...
- Example never reads `src/memory/`, `src/inbox/`, or `tesseract.yaml`.
- README referenced from `docs/PRD.summary.md` as library-mode proof.

### WP-F — Install paths

- `pnpm -w exec tess init --dry-run` against standard example repos completes
  without writes outside `src/memory/adoption/`.
- `pnpm -w exec tess init --apply` against empty directory installs M1 scaffold.
- `npx create-tesseract demo` produces runnable greenfield walkthrough whose
  `tess inbox` and `tess run feature-delivery` flow succeeds.
...
|---|---|
| WP-A — checkpointer | `tech-lead` (conformance contract); `tesseract-engineer` (implementation + vitest); `reviewer` (intervention integration) |
| WP-B — runner-cursor | `tech-lead` (feature-flag contract); `tesseract-engineer` (SDK wiring); `reviewer` (touch-set + disallowed-tool enforcement) |
| WP-C — pipeline compiler | `tech-lead` (YAML-to-graph ADR); `tesseract-engineer` (compiler + migration); `reviewer` (LangGraph pin discipline) |
| WP-D — run-logger | `tech-lead` (option A vs B); `tesseract-engineer` (smoke tests if A); `reviewer` (ratification + active-memory hygiene) |
| WP-E — library-script | `tech-lead` (primitive choice); `coder` (script + README); `reviewer` (no-horizontal-deps audit) |
| WP-F — install paths | `tech-lead` (adoption-report contract); `coder` (CLI + create-tesseract); `reviewer` (non-destructive guarantees) |
