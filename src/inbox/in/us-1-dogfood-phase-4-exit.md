...
- `tess pause`, `tess resume`, and `tess abort` have not been exercised
  empirically;
...
Run a controlled second dogfood exercise that uses `tess pause`, `tess resume`,
and `tess abort` in a realistic stage context. The exercise SHALL demonstrate
...
4. A second controlled run has exercised `tess pause`, `tess resume`, and
   `tess abort` successfully.
...
3. inspect the external run trace and confirm it is clean and attributable to
   the dogfood run;
4. inspect the intervention evidence for `tess pause`, `tess resume`, and
   `tess abort`; and
