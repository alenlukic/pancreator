...
export const HELP_BODY = `Usage:
  ...
  pan prepare <run-id> [--worktree <name>] [--operator-artifacts] [--agent <name>]
      --agent names the next Cursor agent this invocation delegates to. For an evidence-role agent, prepare returns that role's already allocated prompt and report paths before launch. For the stage agent, the harness writes the labeled delegation artifact and starts the detached model probe.
