...
export const HELP_BODY = `Usage:
  ...
  pan prepare <run-id> [--worktree <name>] [--operator-artifacts] [--agent <name>]
      A Cursor-executor stage defaults to the projected agent resolved from its persona, writes the labeled delegation artifact beside the delivery prompt, and starts the detached worker model probe. --agent overrides that resolved label. An external-executor or orchestrator stage writes no artifact at prepare and reports why.
