...
export const HELP_BODY = `Usage:
  ...
  pan prepare <run-id> [--worktree <name>] [--operator-artifacts] [--agent <name>]
      --agent names the next Cursor agent this invocation delegates to. For an evidence-role agent, prepare returns that role's already allocated prompt and report paths before launch. For the stage agent, the harness writes the labeled delegation artifact and starts the detached model probe. A prepare that enters a gated stage can run for the gate's configured timeout; it reports the criterion and timeout before execution.
  ...
  pan watch --targets <run-id>:<invocation-id>[,<run-id>:<invocation-id>...] [--cadence-seconds <n>] [--stall-wakes <n>] [--timeout-seconds <n>] [--mark-background] [--json]
      Multiplexed watch: arms and writes the ordinary per-invocation ledgers, then returns when the first target changes. Every target keeps the focused watch's guarantees: weak evidence of completion holds that target for one confirming wake, and a target unchanged for --stall-wakes wakes ends the wait. Exit 0 when a target moved, 2 on a stall, 3 at the timeout, 4 when a scaffold could not be verified; the result names the targets that moved and the targets that need inspection. --mark-background marks every target in the group. --agent-state is refused here because it speaks for one inspected agent; re-arm the focused watch on that target instead.
