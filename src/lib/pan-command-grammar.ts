...
export const HELP_BODY = `Usage:
  ...
  pan watch --targets <run-id>:<invocation-id>[,<run-id>:<invocation-id>...] [--cadence-seconds <n>] [--stall-timeout-seconds <n>] [--timeout-seconds <n>] [--mark-background] [--json]
      Multiplexed watch: arms and writes the ordinary per-invocation ledgers, then returns when the first target changes. Every target keeps the focused watch's guarantees: weak evidence of completion holds that target for one confirming wake, and a target unchanged for --stall-timeout-seconds (five minutes by default) ends the wait. Exit 0 when a target moved, 2 on a stall, 3 at the timeout, 4 when a scaffold could not be verified; the result names the targets that moved and the targets that need inspection. --mark-background marks every target in the group. --agent-state is refused here because it speaks for one inspected agent; re-arm the focused watch on that target instead.
  ...
  pan spend [--days <1..365>] [--json]
      Fetch team usage events from Cursor's Admin API and report aggregate token volume, charged spend, time series, command, persona and model, tool, Fast mode, governance, workflow role, stage, and remediation slices. The default window is the last 14 days. CURSOR_ADMIN_API_KEY must exist in the process environment or the installation/workspace .env file. The report never includes raw events, emails, or conversation identifiers.
