...
export const HELP_BODY = `Usage:
  ...
  pan watch --attach <ledger>[,<ledger>...] [--timeout-seconds <n>] [--json]
      Attach to one or more existing watch sessions by their ledger paths (within runtime/logs). Blocks until all named sessions reach a terminal state, the watcher process of a session exits without a terminal entry (orphaned, exit 5), or the bound arrives (exit 3, state attach_timed_out). Returns immediately when all sessions are already terminal. Appends nothing to the followed ledgers. The default bound is one hour (3600 seconds).
