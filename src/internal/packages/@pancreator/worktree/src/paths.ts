...
/** Returns the canonical `.pan/worktrees` directory for a repository root. */
export function defaultWorktreesRoot(repoRoot: string): string {
  return path.resolve(repoRoot, ".pan", "worktrees");
}

/** Ensures `worktreesRoot` resolves to `repoRoot/.pan/worktrees`. */
