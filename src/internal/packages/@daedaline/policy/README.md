...
Compatibility helpers for legacy threshold-policy JSON/YAML files and the versioned upgrade path used by `ddl upgrade --apply`.

## Quickstart

```bash
pnpm add @daedaline/policy
pnpm --filter @daedaline/policy run build
pnpm --filter @daedaline/policy run test
```

```typescript
import { loadLegacyPolicyConfig, upgradePolicyConfig } from "@daedaline/policy";

const legacy = await loadLegacyPolicyConfig("./daedaline.yaml");
const v1 = await upgradePolicyConfig("./daedaline.yaml");
```
