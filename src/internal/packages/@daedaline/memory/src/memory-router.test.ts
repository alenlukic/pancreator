...
describe("parseDocList", () => {
  it("splits two comma-separated backtick paths", () => {
    const cell = "`/src/memory/a.md`, `/src/memory/b.md`";
    expect(parseDocList(cell)).toEqual(["/src/memory/a.md", "/src/memory/b.md"]);
  });
});
...
describe("parseHandbookIndexTable", () => {
  it("extracts the routing table from the real handbook index", async () => {
    ...
    expect(glossary?.primaryPaths[0]).toBe("/src/memory/handbook/glossary.md");
  });
});
