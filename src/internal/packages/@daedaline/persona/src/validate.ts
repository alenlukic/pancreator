...
export function assertPersonaSpec(data: Record<string, unknown>): PersonaSpec {
  ...
  const tier = metadata["daedaline-risk-tier"];
  if (typeof tier !== "string" || !RISK.has(tier)) {
    throw err("`metadata.daedaline-risk-tier` MUST be `low`, `medium`, `high`, or `any`.");
  }

  const stages = metadata["daedaline-pipeline-stages"];
  if (!Array.isArray(stages) || !stages.every((s) => typeof s === "string")) {
    throw err("`metadata.daedaline-pipeline-stages` MUST be a string array.");
  }

  const bootstrap = metadata["daedaline-bootstrap-only"];
  if (typeof bootstrap !== "boolean") {
    throw err("`metadata.daedaline-bootstrap-only` MUST be a boolean.");
  }

  const stability = metadata["daedaline-stability"];
  if (typeof stability !== "string" || !STABILITY.has(stability)) {
    throw err("`metadata.daedaline-stability` MUST be `experimental`, `stable`, or `deprecated`.");
  }

  const checklist = metadata["daedaline-checklist"];
  if (!Array.isArray(checklist) || !checklist.every((c) => typeof c === "string")) {
    throw err("`metadata.daedaline-checklist` MUST be a string array.");
  }

  const anchors = metadata["daedaline-handbook-anchors"];
  if (anchors !== undefined) {
    if (!Array.isArray(anchors) || !anchors.every((a) => typeof a === "string")) {
      throw err("`metadata.daedaline-handbook-anchors` MUST be a string array when present.");
    }
  }
