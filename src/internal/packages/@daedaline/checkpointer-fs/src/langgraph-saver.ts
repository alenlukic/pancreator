...
export interface DaedalineCheckpointMetadata extends CheckpointMetadata {
...
function envelopeFromPut(
  ...
  metadata: DaedalineCheckpointMetadata,
...
function metadataFromEnvelope(envelope: CheckpointEnvelopeV1): DaedalineCheckpointMetadata {
  const cv = envelope.channel_values;
  if (cv !== null && typeof cv === "object") {
    const stored = (cv as Record<string, unknown>).langgraph_metadata;
    if (stored !== null && typeof stored === "object") {
      return stored as DaedalineCheckpointMetadata;
    }
  }
  return {
    source: "update",
...
export class FsLangGraphCheckpointSaver extends BaseCheckpointSaver<number> {
  ...
  async put(
    ...
    const ddlMeta = metadata as DaedalineCheckpointMetadata;
