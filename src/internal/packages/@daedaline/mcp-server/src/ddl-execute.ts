...
import { FileInbox } from "@daedaline/inbox";
import {
  FsInterventionStore,
  InterventionManager,
  type CheckpointId,
} from "@daedaline/intervention";
import { FileMemoryStore } from "@daedaline/memory";
import { isRunLogRecord } from "@daedaline/run-logger";
...
/**
 * MCP deferred-tool `tracking_intake` routing: mirror `@daedaline/cli/src/run.ts`
...
export function deferredToolTrackingIntake(name: DdlToolName): string {
  if (name === "ddl.init") {
    return "src/inbox/in/172981_05-25-26/64500_0605_ddl-init-and-create-daedaline-install-paths.md";
  }
...
export function ddlToolEnvelope(name: DdlToolName, milestone: DdlDeferredEnvelope["milestone"]): DdlDeferredEnvelope {
  const workaroundByTool: Record<string, string> = {
    "ddl.init":
      "Bootstrap `ddl init` remains deferred pending install-path convergence; scaffold the substrate manually via `docs/M1.index.md` and adopt flows until the wired command lands.",
    "ddl.run":
      "Only the `feature-delivery` pipeline is executable in bootstrap Phase 4; start runs with `ddl feature new <inbox-path>` followed by persona-driven `ddl advance` staging.",
    "ddl.feature":
      "Expose only `feature new`/`run feature-delivery` via the ddl CLI shell today; MCP `ddl.feature` stays deferred until the tool schema carries the richer argument surface.",
    "ddl.status":
      "Provide a Phase-4 task id to `ddl status <task-id>` until aggregate workspace summaries are modeled for the MCP tool.",
    "ddl.approve":
      "`ddl approve` stays gated on `LocalUserAuthorizer` wiring in Milestone 3 ratification; approve phase exits manually inside the supervising operator session documented in docs/PRD.md.",
    "ddl.memory":
...
/**
 * Runs the same handlers that the `ddl` CLI uses for `inbox`, `pause`, `resume`, and `abort`.
...
export async function executeDdlTool(
  ...
  switch (name) {
    ...
    case "ddl.init":
      throw new DdlDeferredToolError(ddlToolEnvelope("ddl.init", "M3"));
    case "ddl.run":
      throw new DdlDeferredToolError(ddlToolEnvelope("ddl.run", "M2"));
    case "ddl.feature": {
      ...
      throw new Error(`Unsupported ddl.feature action: ${action}`);
    }
    case "ddl.status": {
      const taskId = getString(args, "taskId");
    ...
    case "ddl.approve":
      throw new DdlDeferredToolError(ddlToolEnvelope("ddl.approve", "M3"));
    case "ddl.memory": {
    ...
    case "ddl.contracts":
      throw new DdlDeferredToolError(ddlToolEnvelope("ddl.contracts", "M2"));
    case "ddl.lint":
      throw new DdlDeferredToolError(ddlToolEnvelope("ddl.lint", "M1"));
    default: {
 ...
 */
export async function readDaedalineResource(
  uri: string,
  ctx: DdlExecutionContext,
