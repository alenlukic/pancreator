...
import { MemoryRouter } from "@daedaline/memory";
import { parse as parseYaml } from "yaml";

import { type DdlExecutionContext } from "./ddl-execute.js";
...
export async function readWorkspaceStatus(
  ...
  const daedalineYamlPath = path.join(ctx.repoRoot, "daedaline.yaml");
  let bootstrap: Record<string, unknown> = {};
  try {
    bootstrap = parseYaml(await readFile(daedalineYamlPath, "utf8")) as Record<string, unknown>;
  } catch {
    bootstrap = {};
  }
