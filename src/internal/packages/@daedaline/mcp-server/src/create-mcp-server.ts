...
import { executeDdlTool, readDaedalineResource, type DdlExecutionContext } from "./ddl-execute.js";
...
/**
 * Wires a `Server` and `StdioServerTransport` for the Daedaline MCP surface.
 */
export function createMcpServer(
  ...
  const server = new Server(
    { name: "daedaline-mcp-server", version: DAEDALINE_CORE_VERSION },
    {
      capabilities: {
  ...
  server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    try {
      const { mimeType, text } = await readDaedalineResource(uri, ctx);
      return {
        contents: [
          {
            uri,
...
/**
 * Exposes the same `readDaedalineResource` path the `resources/read` handler uses.
 */
export async function readDaedalineResourceMcp(
  uri: string,
  options?: CreateMcpServerOptions,
): Promise<{ mimeType: string; text: string }> {
  const repoRoot = pathResolveRepoRoot(options?.repoRoot);
  return readDaedalineResource(uri, { repoRoot });
}
