...
const dependencyPackageRoots = [
  path.join(packageRoot, "node_modules", "@daedaline", "core"),
  path.join(packageRoot, "node_modules", "@daedaline", "inbox"),
  path.join(packageRoot, "node_modules", "@daedaline", "intervention"),
  path.join(packageRoot, "node_modules", "@daedaline", "pipeline"),
  path.join(packageRoot, "node_modules", "@daedaline", "run-logger"),
];
...
if (needsBuild) {
  const result = spawnSync("pnpm", ["--dir", repoRoot, "--filter", "@daedaline/cli...", "run", "build"], {
    cwd: repoRoot,
    stdio: "inherit",
  });
  if (result.status !== 0) {
