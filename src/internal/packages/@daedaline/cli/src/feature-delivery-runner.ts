...
} from "@daedaline/pipeline";
import { CursorRunner } from "@daedaline/runner-cursor";
import type { CursorSdkTransport } from "@daedaline/runner-cursor";
import type { RunnerInvocationEnvelope, RunnerPersonaInput } from "@daedaline/runner-cursor";
import {
  appendRunLogRecord,
  newSpanId,
  newTraceId,
...
export function runLogRecordFromRunnerEnvelope(
  ...
  const stageId =
    ...
    (typeof fragment?.attributes["daedaline.stage_id"] === "string"
      ? fragment.attributes["daedaline.stage_id"]
      : "unknown");
  return {
    ts: rfc3339UtcMs(now),
    ...
    attributes: {
      ...
      "daedaline.runner.request_id": envelope.requestId,
    },
...
export function expectedArtifactForEnteringStage(
  ...
  switch (stageId) {
    case "intake":
      return path.posix.join("src", "memory", "features", state.featureId, "spec.md");
    case "plan":
    ...
    case "report":
      return path.posix.join("src", "memory", "features", state.featureId, "delivery-report.md");
    case "ship":
      return path.posix.join(state.artifacts.runDir, "policy-compliance.json");
    case "index":
      return path.posix.join("src", "memory", "features", state.featureId, "index.json");
    default:
...
function buildStageRunLogFragment(
  ...
  return {
    ...
    attributes: {
      ...
      "gen_ai.operation.name": `daedaline.pipeline.stage.${stageId}`,
      "gen_ai.provider.name": "cursor",
      "daedaline.persona": persona.name,
      "daedaline.stage_id": stageId,
      "daedaline.stage_prompt_path": stagePromptPath,
      "daedaline.artifact_path": artifactPath,
    },
  };
}
...
export async function writeRetryLimitHaltArtifact(input: {
  ...
  const rel = path.posix.join("src", "inbox", "out", dayBucket, `${basename}.md`);
...
export async function writeReportApprovalArtifact(input: {
  ...
  const body = [
    ...
    "",
    "```bash",
    `pnpm -w exec ddl advance ${input.state.taskId} --artifact ${rel}`,
    "```",
    "",
  ].join("\n");
  await writeFile(abs, body, "utf8");
...
export async function maybePauseForReportApproval(input: {
  ...
  const reportRel = path.posix.join("src", "memory", "features", input.state.featureId, "delivery-report.md");
