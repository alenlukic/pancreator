...
import { runCreateDaedaline, runDdlInit } from "./ddl-init.js";
...
export { DDL_ACTIVE_MEMORY_CONFLICT_EXIT_CODE };

/** Stable exit code for deferred stub verbs (`ddl init`, MCP deferrals surfaced through the CLI shim, etc.). */
export const DDL_DEFERRED_EXIT_CODE = 125;

/** Fallback when a deferred verb lacks a dedicated intake — mirror `deferredToolTrackingIntake` in `@daedaline/mcp-server/src/ddl-execute.ts`. */
const BATCH_DEFERRAL_TRACKING_INTAKE =
  "src/inbox/in/172981_05-25-26/64488_0605_cli-operator-tooling-batch.md";
/** `ddl init` / MCP `ddl.init` deferral tracker — mirror `deferredToolTrackingIntake` in `@daedaline/mcp-server/src/ddl-execute.ts`. */
const DDL_INIT_DEFERRAL_TRACKING_INTAKE =
...
function defaultDeferredTrackingIntake(cliVerb: string): string {
  if (cliVerb === "ddl init") {
    return DDL_INIT_DEFERRAL_TRACKING_INTAKE;
  }
...
function deferredVerbAction(
  ...
  return async () => {
    ...
    exit.code = DDL_DEFERRED_EXIT_CODE;
  };
}
...
export async function parseAndRun(
  ...
  program.name("ddl");
  program.description("Daedaline workspace CLI (bootstrap Phase 4).");
  ...
  program
    .command("init")
    .description("Initialize a Daedaline workspace in the current repository (dry-run by default)")
  ...
  program
    ...
    .description("Scaffold a new greenfield Daedaline workspace directory")
    ...
    .action(async (pipeline: string, inboxEntry: string | undefined, opts: { feature?: string; task?: string }) => {
      if (pipeline !== "feature-delivery") {
        emitDeferredEnvelope(writeOut, repoRoot, {
          ...
          milestone: "M2",
          manual_workaround:
            "Bootstrap Phase 4 exposes only `ddl feature new`/`ddl run feature-delivery <inbox-entry>` orchestration until additional pipelines compile end-to-end per docs/PRD.md.",
        });
        exit.code = DDL_DEFERRED_EXIT_CODE;
        return;
      }
  ...
  program
    ...
    .action(
      deferredVerbAction(repoRoot, writeOut, exit, {
        ...
        milestone: "M1",
        manual_workaround:
          "Use `pnpm lint`, `pnpm run check:phase0a`, and `bash -n .cursor/hooks/enforce-policy-compliance.sh` locally until ddl wraps the bundles.",
      }),
    );
  ...
  intakeCmd
    ...
    .action(
      async (
        ...
        if (!existsSync(path.join(repoRoot, "daedaline.yaml"))) {
          throw new Error("Missing daedaline.yaml at repository root; run from an initialized Daedaline workspace.");
        }
