...
/** Appends a run-log row for ddl pause/resume/abort when a feature-delivery ledger exists. Returns false when no state file matches the task id. */
...
export async function closeFeatureDeliveryArtifacts(
  ...
  const activeMemoryRel = path.posix.join("src", "memory", "active", "current.md");
  ...
  switch (stage.id) {
    ...
    case "implement":
      return {
        ...
        humanAttention: "Use ddl pause/resume/abort if implementation drifts, loops, or exceeds scope.",
      };
    case "review":
      return {
...
function featureDeliveryTransitions(): FeatureDeliveryTransition[] {
  return [
    ...
    },
    {
      ...
      on: "pause",
      to: "paused",
      humanAttention: "Use ddl pause <task-id> to stop at the next safe boundary.",
    },
    {
      from: "paused",
      on: "resume",
      to: "implement",
      humanAttention: "Use ddl resume <task-id> after resolving the blocker.",
    },
    {
      from: "implement",
      on: "abort",
      to: "aborted",
      humanAttention: "Use ddl abort <task-id> --reason <text> for failed or superseded runs.",
    },
    {
      from: "implement",
...
function stageContractMarkdown(state: FeatureDeliveryState, stage: string): string {
  switch (stage) {
    case "intake":
      return `Input: ${state.source.inboxPath}\nOutput: src/memory/features/${state.featureId}/spec.md\nAdvance after human ratification: pnpm -w exec ddl advance ${state.taskId} --artifact src/memory/features/${state.featureId}/spec.md`;
    case "plan":
      return `Input: src/memory/features/${state.featureId}/spec.md\nOutputs: ${state.artifacts.runDir}/plan.md, ${state.artifacts.runDir}/adr-draft.md, ${state.artifacts.runDir}/touch-set.json, ${state.artifacts.handoffFile}\nAdvance after human ratification: pnpm -w exec ddl advance ${state.taskId} --artifact ${state.artifacts.runDir}/touch-set.json`;
    case "implement": {
      const base = `Inputs: ${state.artifacts.handoffFile}, ${state.artifacts.runDir}/touch-set.json\nOutput: ${state.artifacts.runDir}/implementation-report.md\nAdvance after implementation is accepted: pnpm -w exec ddl advance ${state.taskId} --artifact ${state.artifacts.runDir}/implementation-report.md`;
      const lastAdvance = lastAdvanceEntry(state);
      if (lastAdvance?.event === "must_fix" && lastAdvance.from === "review") {
        return `${base}\nAfter must_fix fixes, when ${state.artifacts.runDir}/review.md already records review_passes: true, chain to test in one step: pnpm -w exec ddl advance ${state.taskId} --artifact ${state.artifacts.runDir}/review.md`;
      }
      return base;
    }
    case "review":
      return `Inputs: ${state.artifacts.handoffFile}, ${state.artifacts.runDir}/touch-set.json, current local diff, validation output\nOutput: ${state.artifacts.runDir}/review.md\nAdvance on pass: pnpm -w exec ddl advance ${state.taskId} --artifact ${state.artifacts.runDir}/review.md\nReturn to implement on must-fix: pnpm -w exec ddl advance ${state.taskId} --event must_fix --artifact ${state.artifacts.runDir}/review.md`;
    case "test":
      return `Inputs: ${state.artifacts.runDir}/review.md, ${state.artifacts.runDir}/touch-set.json, current local diff, validation output\nOutput: ${state.artifacts.runDir}/test-report.md\nAdvance on pass: pnpm -w exec ddl advance ${state.taskId} --artifact ${state.artifacts.runDir}/test-report.md\nReturn to implement on qa-fail: pnpm -w exec ddl advance ${state.taskId} --event qa_fails --artifact ${state.artifacts.runDir}/test-report.md`;
    case "report":
      return `Inputs: ${state.artifacts.runDir}/implementation-report.md, ${state.artifacts.runDir}/review.md, ${state.artifacts.runDir}/test-report.md\nOutput: src/memory/features/${state.featureId}/delivery-report.md\nAdvance after report is accepted: pnpm -w exec ddl advance ${state.taskId} --artifact src/memory/features/${state.featureId}/delivery-report.md`;
    case "ship":
      return `Inputs: local diff, validation output, src/memory/features/${state.featureId}/delivery-report.md\nOutput: ${state.artifacts.runDir}/policy-compliance.json\nAdvance only after human ratifies local diff: pnpm -w exec ddl advance ${state.taskId} --artifact ${state.artifacts.runDir}/policy-compliance.json`;
    case "index":
      return `Inputs: delivery report and accepted ship artifacts\nOutput: src/memory/features/${state.featureId}/index.json\nAdvance after artifacts are indexed: pnpm -w exec ddl advance ${state.taskId} --artifact src/memory/features/${state.featureId}/index.json`;
    case "complete":
...
\`\`\`bash
pnpm -w exec ddl close-artifacts ${state.taskId}
\`\`\`
...
4. Run \`pnpm -w exec ddl status ${state.taskId}\` and confirm the status resolves from the archive.
5. Report the resulting \`git status --short\`.

Do not manually move files unless \`ddl close-artifacts\` fails and the failure is reported back to the operator.`;
}
...
function makeInterventionRecord(
  ...
  const attrs: Record<string, unknown> = {
    ...
    "gen_ai.operation.name": `daedaline.pipeline.intervention.${command}`,
    "gen_ai.provider.name": "local-cli",
    "gen_ai.request.model": "none",
    "daedaline.feature_id": state.featureId,
    "daedaline.state_file": state.artifacts.stateFile,
    "daedaline.intervention.command": command,
  };
  if (abortReason !== undefined) {
    attrs["daedaline.intervention.abort_reason"] = abortReason;
  }
  return {
    ts: rfc3339UtcMs(now),
    trace_id: newTraceId(),
    span_id: newSpanId(),
    name: `daedaline.pipeline.intervention.${command}`,
    kind: "event",
    status: { code: "OK" },
    attributes: attrs,
    resource: { "service.name": "daedaline", "service.version": "0.0.0" },
    daedaline: {
...
function makeInvocationRecord(state: FeatureDeliveryState, now: Date): RunLogRecord {
  return {
    ...
    trace_id: newTraceId(),
    span_id: newSpanId(),
    name: "daedaline.pipeline.invoke",
    kind: "event",
    status: { code: "OK" },
    attributes: {
      "openinference.span.kind": "CHAIN",
...
function makeAdvanceRecord(
  ...
  return makeStateRecord(state, now, "daedaline.pipeline.advance", fromStage, toStage, event, artifact);
}
...
function makeRepairRecord(
  ...
  const record = makeStateRecord(
    state,
    now,
    "daedaline.pipeline.repair_state",
    fromStage,
    toStage,
    "manual_repair",
    artifact,
  ...
  return {
    ...
    attributes: {
      ...record.attributes,
      "daedaline.repair.reason": reason,
    },
  };
}
...
function makeCloseRecord(
  ...
  const record = makeStateRecord(
    state,
    now,
    "daedaline.pipeline.close_artifacts",
    TERMINAL_STAGE,
    TERMINAL_STAGE,
    "artifacts_closed",
    archivedRunDir,
  ...
  return {
    ...
    attributes: {
      ...record.attributes,
      "daedaline.previous_run_dir": previousRunDir,
      "daedaline.archived_run_dir": archivedRunDir,
      "daedaline.inbox_source": inboxSource,
      "daedaline.archived_inbox_path": archivedInboxPath,
    },
  };
}
...
function makeStateRecord(
  ...
  return {
    ...
    attributes: {
      ...
      "daedaline.feature_id": state.featureId,
      "daedaline.state_file": state.artifacts.stateFile,
      "daedaline.from_stage": fromStage,
      "daedaline.to_stage": toStage,
      "daedaline.transition_event": event,
      "daedaline.artifact": artifact,
    },
    resource: { "service.name": "daedaline", "service.version": "0.0.0" },
    daedaline: {
      task_id: asTaskId(state.taskId),
  ...
  switch (stage) {
    ...
    case "complete":
      return state.status === "closed"
        ...
        : `${prefix}Delegate next-prompt.md to librarian for final artifact closure with ddl close-artifacts.`;
    default:
