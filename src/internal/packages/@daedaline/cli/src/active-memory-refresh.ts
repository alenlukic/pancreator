...
export const DDL_ACTIVE_MEMORY_CONFLICT_EXIT_CODE = 126;

const OPS_NOTES_AUTO_START = "<!-- ddl:active-memory:operator-notes:auto -->";
const OPS_NOTES_AUTO_END = "<!-- /ddl:active-memory:operator-notes:auto -->";
  ...
  for (const rel of extractActiveFeatureInboxPointers(sectionInner)) {
    if (!existsSync(path.join(repoRoot, rel))) {
      return (
        ...
        "(ddl close-artifacts clears matching pointers automatically during artifact closure)."
      );
    }
  }
  return null;
...
export async function rewriteActiveMemoryFile(opts: {
  ...
  if (staleActiveMsg !== null) {
    ...
    return DDL_ACTIVE_MEMORY_CONFLICT_EXIT_CODE;
  }
