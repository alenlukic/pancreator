...
import type { PipelineDefinition } from "@daedaline/pipeline";
...
function sampleLedger(overrides: Partial<FeatureDeliveryRunnerLedger> = {}): FeatureDeliveryRunnerLedger {
  return {
    ...
    artifacts: {
      runDir: "src/work/172996_05-10-26/38670_1315_demo-feature",
      stateFile: "src/work/172996_05-10-26/38670_1315_demo-feature/state.json",
      runLogFile: "src/work/172996_05-10-26/38670_1315_demo-feature/run.log.jsonl",
      nextPromptFile: "src/work/172996_05-10-26/38670_1315_demo-feature/next-prompt.md",
    },
...
describe("feature-delivery-runner automation", () => {
  it("applySdkRetrySideEffects halts when cumulative retries exceed 3", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "ddl-runner-halt-"));
  ...
  it("maybePauseForReportApproval writes an outbox artifact when delivery report exists", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "ddl-report-gate-"));
    const state = sampleLedger();
    const reportRel = path.join(root, "src", "memory", "features", state.featureId, "delivery-report.md");
    ...
    expect(outboxRel).toMatch(/^src\/inbox\/out\//u);
  ...
  it("invokeFeatureDeliveryEnteringStage invokes runner for the requested stage, not the pipeline entry", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "ddl-runner-stage-slice-"));
    const personaDir = path.join(root, "src", "personas");
    await mkdir(personaDir, { recursive: true });
    await copyFile(
      path.join(CANONICAL_REPO_ROOT, "src", "personas", "coder.md"),
    ...
    const state = sampleLedger({
      currentStage: "implement",
      artifacts: {
        runDir: "src/work/demo/run",
        stateFile: "src/work/demo/run/state.json",
        runLogFile: "src/work/demo/run/run.log.jsonl",
        nextPromptFile: "src/work/demo/run/next-prompt.md",
      },
    });
    ...
    const record = JSON.parse(lastLine!) as { daedaline?: { stage_id?: string } };
    expect(record.daedaline?.stage_id).toBe("implement");
  });

  it("trySdkAutoChainAfterStageWork routes plan-invalidating qa_fails to plan", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "ddl-runner-qa-plan-"));
  ...
  it("resolveTestStageAdvanceEvent maps qa_fails to qa_fails_plan_invalidating when verdict requires plan", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "ddl-runner-resolve-qa-"));
    const state = sampleLedger({ currentStage: "test" });
    const testRel = path.posix.join(state.artifacts.runDir, "test-report.md");
    const testAbs = path.join(root, testRel);
    await mkdir(path.dirname(testAbs), { recursive: true });
