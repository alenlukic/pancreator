...
async function tmpRepo(): Promise<{ repoRoot: string; cleanup: () => Promise<void> }> {
  ...
  await mkdir(path.join(repoRoot, ".ddl", "sandboxes"), { recursive: true });
  return {
    repoRoot,
    cleanup: async () => {
...
describe("PortRegistryEnvIsolation", () => {
  it("reserves contiguous ports and persists them", async () => {
    ...
    try {
      ...
      const raw = await readFile(path.join(repoRoot, ".ddl", "sandboxes", "port-registry.json"), "utf8");
  ...
  it("detects duplicate ports across tasks on load", async () => {
    const { repoRoot, cleanup } = await tmpRepo();
    const file = path.join(repoRoot, ".ddl", "sandboxes", "port-registry.json");
    try {
      const bad = {
        version: 1,
        minPort: 7000,
