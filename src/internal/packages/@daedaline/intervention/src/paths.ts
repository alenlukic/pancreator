...
/** Returns `.ddl/scheduler/interventions` under `repoRoot`. */
export function defaultInterventionsDir(repoRoot: string): string {
  return path.resolve(repoRoot, ".ddl", "scheduler", "interventions");
}
...
/** Ensures `journalFilePath` resolves inside `repoRoot/.ddl/scheduler/interventions`. */
