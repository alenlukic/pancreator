...
async function tmpRepo(): Promise<{ repoRoot: string; cleanup: () => Promise<void> }> {
  ...
  await mkdir(path.join(repoRoot, ".tess", "worktrees"), { recursive: true });
  return {
    repoRoot,
    cleanup: async () => {
...
describe("GitWorktreePool", () => {
  ...
  it("persists pool state for a new pool instance", async () => {
    ...
    try {
      ...
      const raw = await readFile(path.join(repoRoot, ".tess", "worktrees", "pool-state.json"), "utf8");
  ...
  it("rejects worktrees roots outside .tess/worktrees", async () => {
    const { repoRoot, cleanup } = await tmpRepo();
    try {
      expect(
        () =>
