...
/**
 * Emits a full `src/personas/<name>.md` (or `.cursor/agents/<name>.md` mirror) document that
...
export interface MdcShimOptions {
  /** Rule globs; default is one entry `src/personas/<name>.md`. */
  globs?: string[];
...
/**
 * Emits `.cursor/rules/<name>.mdc` per `/src/memory/handbook/persona-spec.md` §5.2: five
 * non-blank frontmatter lines plus a single `@src/personas/<name>.md` body line.
 */
export function emitMdcShim(spec: PersonaSpec, options: MdcShimOptions = {}): string {
  const globs = options.globs ?? [`src/personas/${spec.name}.md`];
