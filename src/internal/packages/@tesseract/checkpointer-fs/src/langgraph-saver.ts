...
export interface TesseractCheckpointMetadata extends CheckpointMetadata {
...
function envelopeFromPut(
  ...
  metadata: TesseractCheckpointMetadata,
...
function metadataFromEnvelope(envelope: CheckpointEnvelopeV1): TesseractCheckpointMetadata {
  const cv = envelope.channel_values;
  if (cv !== null && typeof cv === "object") {
    const stored = (cv as Record<string, unknown>).langgraph_metadata;
    if (stored !== null && typeof stored === "object") {
      return stored as TesseractCheckpointMetadata;
    }
  }
  return {
    source: "update",
...
export class FsLangGraphCheckpointSaver extends BaseCheckpointSaver<number> {
  ...
  async put(
    ...
    const tessMeta = metadata as TesseractCheckpointMetadata;
    const envelope = envelopeFromPut(taskId, seq, copyCheckpoint(checkpoint), tessMeta);
    const target = envelopePath(this.root, taskId, checkpointId);
