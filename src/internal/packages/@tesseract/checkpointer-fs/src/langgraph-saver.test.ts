...
import {
  ...
  type TesseractCheckpointMetadata,
...
describe("FsLangGraphCheckpointSaver BaseCheckpointSaver conformance", () => {
  it("put, getTuple, and list round-trip checkpoint metadata", async () => {
    ...
    const metadata: TesseractCheckpointMetadata = {
      source: "input",
      step: -1,
      parents: {},
      run_log_offset: 42,
    ...
    const stored = tuple?.metadata as TesseractCheckpointMetadata | undefined;
  ...
  it("putWrites stores pending writes retrievable via getTuple", async () => {
    ...
    const writeMetadata: TesseractCheckpointMetadata = {
      source: "loop",
      step: 0,
      parents: {},
      run_log_offset: 0,
  ...
  it("deleteThread removes all checkpoints for a thread", async () => {
    ...
    const deleteMetadata: TesseractCheckpointMetadata = {
      source: "input",
      step: -1,
      parents: {},
      run_log_offset: 0,
