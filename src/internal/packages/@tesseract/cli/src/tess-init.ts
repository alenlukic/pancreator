...
export interface TessInitInput {
  repoRoot: string;
  dryRun?: boolean;
  apply?: boolean;
  force?: boolean;
...
export interface TessInitFileDiff {
  path: string;
  action: "create" | "skip" | "conflict";
}

export interface TessInitResult {
  command: "init";
  status: "ok";
  dryRun: boolean;
  applied: boolean;
  diffs: TessInitFileDiff[];
...
export async function runTessInit(input: TessInitInput): Promise<TessInitResult> {
  const repoRoot = path.resolve(input.repoRoot);
  const dryRun = input.dryRun ?? !input.apply;
  const force = input.force ?? false;
  const diffs: TessInitFileDiff[] = [];
  ...
  if (conflicts.length > 0 && !force) {
    const err = new Error(
      `tess init refused: ${conflicts.length} conflicting file(s). Re-run with --force to overwrite.`,
...
export interface CreateTesseractInput {
  targetDir: string;
  projectName: string;
  clock?: () => Date;
}

export interface CreateTesseractResult {
...
export async function runCreateTesseract(input: CreateTesseractInput): Promise<CreateTesseractResult> {
  const targetDir = path.resolve(input.targetDir, input.projectName);
  if (existsSync(targetDir)) {
    const entries = await readdir(targetDir);
    if (entries.length > 0) {
      throw new Error(`create-tesseract: target directory is not empty: ${targetDir}`);
    }
  } else {
    await mkdir(targetDir, { recursive: true });
  }
  const init = await runTessInit({
    repoRoot: targetDir,
    apply: true,
    clock: input.clock,
  });
...
/** Reads `runner.cursor.invocation` from tesseract.yaml when present. */
export async function readCursorInvocationMode(repoRoot: string): Promise<"manual" | "sdk"> {
  const cfgPath = path.join(repoRoot, "tesseract.yaml");
