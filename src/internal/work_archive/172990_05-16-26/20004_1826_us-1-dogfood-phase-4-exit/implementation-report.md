...
| --- | --- | --- | --- |
| End-to-end proof index | `src/inbox/in/phase-4-dogfood-proof-bundle-index.md` | _pending_ | Operator starts `pan run`, advances stages, preserves `run.log.jsonl`. |
...
Conditional updates to `AGENTS.md`, `docs/M1.index.md`, `pancreator.yaml`, and `src/memory/active/current.md` **deferred** until accepted proof bundle and human ratification (see `policy-compliance.json` deferred item `PHASE4-RATIFICATION-DOCS`).
...
5. After reviewing this artifact, run exactly one advance command if accepting implementation stage output:
   ...
   `pnpm -w exec pan advance 20004_1826_us-1-dogfood-phase-4-exit --artifact src/work/172990_05-16-26/20004_1826_us-1-dogfood-phase-4-exit/implementation-report.md`
