...
- [ ] **n-01 (Note)** — No `pnpm test` script defined. Proposal P-01 deferred to `tesseract-engineer`.
...
| m-02 | `librarian` | Bulk contentHash refresh pass against citation-rot-scan inventory; run `tess lint contracts` when available. |
| n-01 / P-01 | `tesseract-engineer` | Add unified `pnpm test` script to `package.json`. |
