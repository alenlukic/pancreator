...
- [ ] **n-01 (Note)** — No `pnpm test` script defined. Proposal P-01 deferred to `daedaline-engineer`.
...
| m-02 | `librarian` | Bulk contentHash refresh pass against citation-rot-scan inventory; run `ddl lint contracts` when available. |
| n-01 / P-01 | `daedaline-engineer` | Add unified `pnpm test` script to `package.json`. |
