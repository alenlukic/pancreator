...
- `assertWithinRoot` in `client/src/services/repo-paths.ts` still uses synchronous realpath checks in a server request path; this is acceptable at current scale but can be revisited if API latency becomes a concern.
