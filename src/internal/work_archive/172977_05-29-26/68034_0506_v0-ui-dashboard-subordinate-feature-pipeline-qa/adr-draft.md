...
When the dashboard renders relationships, the UI SHALL model five repository domains as first-class navigation nodes: `src/inbox/`, `src/memory/`, `src/personas/`, `src/work/`, and `src/internal/packages/`.
...
- Neutral: The decision does not change `src/pipelines/feature-delivery.yaml`, persona semantics, or parent harness code.
