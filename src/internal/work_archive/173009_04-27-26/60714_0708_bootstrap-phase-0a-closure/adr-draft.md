...
references:
  - kind: lines
    path: src/memory/features/bootstrap-phase-0a-closure/spec.md
    range: [49, 80]
...
When `coder` reconciles package inventory, existing `src/internal/packages/@pancreator/*` directories MUST remain in place. The unscoped `src/internal/packages/pancreator/` meta package MUST be added because PRD §11 names the `pancreator` meta package.
