...
This slice closes the Phase 0a scaffold gap by turning the existing repository shell into a runnable TypeScript monorepo baseline. The live repo already contains `AGENTS.md`, the always-apply Cursor rule, the canonical top-level directories, `pancreator.yaml`, `pancreator-defaults.yaml`, handbook seeds, personas, skills, and package README placeholders. The live repo lacks root workspace/tooling manifests, package manifests, package `src/index.ts` stubs, declaration-build wiring, a horizontal primitive dependency lint rule, a CI conformance check, and a Phase 0a verification record. The implementation MUST add only those scaffold files and MUST NOT implement Phase 0b handbook content, Phase 0c persona or skill content, Phase 2 contract clauses, or Phase 3 runtime behavior.
...
- Package skeleton files under every existing `src/internal/packages/@pancreator/*/` directory: `package.json`, `README.md`, `src/index.ts`.
- Meta package skeleton files under `src/internal/packages/pancreator/`: `package.json`, `README.md`, `src/index.ts`.

## Execution Steps

1. Reconcile package inventory. `coder` MUST preserve the existing package directories and MUST add only the missing meta package directory `src/internal/packages/pancreator/` for the unscoped `pancreator` package.
...
- The existing `src/internal/packages/@pancreator/contract*` directories remain skeleton-only. This plan includes manifests for them because the directories already exist and the dependency-order bootstrap needs stable package identities, but `coder` MUST NOT add contract runner behavior.
...
- Acceptance criteria: `{kind: "lines", path: "src/memory/features/bootstrap-phase-0a-closure/spec.md", range: [49, 71], contentHash: "31994149904e507e81029d40dea33a7d93dde9654ec5f58c0217e1f0a3e97b9c"}`
- Out of scope: `{kind: "lines", path: "src/memory/features/bootstrap-phase-0a-closure/spec.md", range: [73, 80], contentHash: "31994149904e507e81029d40dea33a7d93dde9654ec5f58c0217e1f0a3e97b9c"}`
