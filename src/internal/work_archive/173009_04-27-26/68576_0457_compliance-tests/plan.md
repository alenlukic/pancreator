...
approval_artifact: src/inbox/threads/compliance-tests/68576_0457_round-05-human-approve.md
next_owner: coder
next_stage: implement
references:
  - kind: lines
    path: src/memory/features/compliance-tests/spec.md
    range: [55, 60]
...
The implement slice SHALL keep compliance descriptors and schemas in the canonical `src/internal/tests/compliance/` tree, SHALL remove rejected duplicate files under `src/memory/features/compliance-tests/contracts/`, SHALL preserve operator and agent manual invocation for slice one, SHALL keep scheduler automation deferred to backlog, and SHALL preserve AGENTS trigger guidance for structure-change compliance runs.

## Implementation Tasks

1. Verify `src/internal/tests/compliance/schemas/latest.yaml` and `src/internal/tests/compliance/schemas/v1.yaml`; each schema MUST validate descriptor fields `schema_ref`, `id`, `severity`, `trigger_modes`, `scope`, and `assertion`.
2. Verify the three canonical seed descriptors in `src/internal/tests/compliance/`; each descriptor MUST use `schema_ref: "src/internal/tests/compliance/schemas/latest.yaml"` and one accepted severity value.
3. Remove or leave unstaged the rejected duplicate descriptor and schema files under `src/memory/features/compliance-tests/contracts/`; implement output MUST NOT store compliance tests or schemas in that tree.
4. Verify `src/memory/features/compliance-tests/manual-runbook.md`; the runbook MUST name `scheduled-cadence`, `structure-change`, and `operator-on-demand` and MUST declare manual operator and agent invocation as the executable first-slice path.
...
6. Verify `src/memory/features/compliance-tests/severity-routing.md` and `src/memory/features/compliance-tests/run-template.json`; routing MUST encode high, medium, and low obligations, and run records MUST persist timestamp, trigger mode, test identifiers, and pass/fail outcomes.
...
8. Verify `src/memory/backlog/index.yaml`; it MUST keep one deferred automation item for 4-hour scheduler enforcement and automatic structure-change execution.
...
- Stay inside `src/internal/work_archive/173009_04-27-26/68576_0457_compliance-tests/touch-set.json`.
- Treat the `src/internal/tests/compliance/` files as the canonical assets.
- Delete or exclude the `src/memory/features/compliance-tests/contracts/` duplicate files before handoff.
...
- Verify canonical descriptors and schemas live only under `src/internal/tests/compliance/`.
