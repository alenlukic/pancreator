...
Pancreator SHALL use `src/internal/tests/compliance/` for compliance descriptors and `src/internal/tests/compliance/schemas/` for descriptor schemas.
...
When severity is `low`, routing policy MUST create a backlog item and MUST emit warnings to `console` and `src/inbox/out`.
