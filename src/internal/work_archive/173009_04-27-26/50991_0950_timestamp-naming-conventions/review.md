...
write-mode behind `DAEDALINE_MIGRATION_GO=1`. Three `warn`-class findings
...
| ----------------------------------------------------------------------------------------------------- | -- | ------------- |
| `src/internal/tests/compliance/timestamp-naming-conventions.yaml`                                                  | A  | yes           |
| `src/memory/features/timestamp-naming-conventions/contracts/timestamp-naming.policy-descriptor.yaml`      | A  | yes           |
| `src/memory/features/timestamp-naming-conventions/contracts/timestamp-naming.migration-precedence.yaml`   | A  | yes           |
| `src/memory/features/timestamp-naming-conventions/contracts/timestamp-naming.reference-update-audit.yaml` | A  | yes           |
| `src/internal/tools/migrate-timestamp-naming.mjs`                                                                  | A  | yes           |
| `src/memory/handbook/inbox-lifecycle.md`                                                                  | M  | yes           |
| `src/memory/features/timestamp-naming-conventions/index.json`                                             | M  | yes           |
| `src/internal/tools/migrate-timestamp-naming.test.mjs`                                                             | A  | parent-ratified add #1 |
| `src/internal/work_archive/173009_04-27-26/50991_0950_timestamp-naming-conventions/migration-manifest.dry-run.json`                                   | A  | parent-ratified add #2 |
| `src/memory/adr/0005-timestamp-naming-conventions.md`                                                     | A  | parent-ratified add #3 |
| `package.json`                                                                                        | M  | parent-ratified add #4 |
| `src/internal/work_archive/173009_04-27-26/50991_0950_timestamp-naming-conventions/adr-draft.md`                                                      | M  | annotated promotion stub (in feature workspace) |
| `src/internal/work_archive/173009_04-27-26/50991_0950_timestamp-naming-conventions/policy-compliance.json`                                            | M  | required policy-compliance gate artifact (AGENTS.md §5) |

The four parent-ratified additions match the coder's return message exactly.
Citation: `{kind: lines, path: src/internal/work_archive/173009_04-27-26/50991_0950_timestamp-naming-conventions/touch-set.json, range: [1, 130], contentHash: TBD-on-commit}`.
...
`{kind: lines, path: src/memory/adr/0005-timestamp-naming-conventions.md, range: [54, 60], contentHash: TBD-on-commit}`.
...
| `src/work/*/run.log.jsonl` excluded (65-66) | `isExcludedFromMigration` check on `/run.log.jsonl`                         |
...
| Agent prefixes src/inbox/in human items (85-87) | `src/memory/handbook/inbox-lifecycle.md` §3c                               |
| Agent prefixes src/inbox/threads human items (88-90) | `src/memory/handbook/inbox-lifecycle.md` §3c                          |
...
| Migrate threads round files (104-105) | `listInboxFiles` walks `src/inbox/threads/<feature>/`; manifest contains them    |
...
| Update inbox-lifecycle.md (110-112) | `src/memory/handbook/inbox-lifecycle.md` §3c (new section)                         |

Every acceptance clause maps to at least one executable surface or
descriptor. Citation:
`{kind: lines, path: src/memory/features/timestamp-naming-conventions/spec.md, range: [54, 112], contentHash: TBD-on-commit}`.

## Layer-1 lint findings

| # | Severity | Citation                                                                                                                                     | Issue                                                                                                                                                   | Suggested fix                                                                                                              |
| - | -------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| L1 | warn    | `{kind: lines, path: src/memory/features/timestamp-naming-conventions/contracts/timestamp-naming.migration-precedence.yaml, range: [16, 20], contentHash: TBD-on-commit}` | The reference cites `src/internal/tools/migrate-timestamp-naming.mjs` line range `[1, 120]` while the `chooseTimestamp` symbol resides at lines `173-193`. The cited range MUST cover the named symbol per PRD §4.6 dual-anchor rule. | Update the range to `[164, 195]` so the citation covers `chooseTimestamp`'s declaration and body.                          |
| L2 | warn    | `{kind: lines, path: src/memory/handbook/inbox-lifecycle.md, range: [48, 52], contentHash: TBD-on-commit}` | The new §3c reference entry uses `contentHash: TBD-on-commit` while the surrounding entries already carry sha256 hashes; mixed discipline in one frontmatter block weakens Layer-1 audit. | Refresh `contentHash` for the §3c reference during the post-commit hash-refresh pass, or hold the dogfood pattern by tagging all entries `TBD-on-commit` until librarian indexing. |
| L3 | info    | `{kind: lines, path: src/internal/tools/migrate-timestamp-naming.mjs, range: [305, 409], contentHash: TBD-on-commit}` | `inventoryReferences` scans `src/work/` including the dry-run manifest itself, so re-running with `--strict-references` inflates `referenceUpdateRows` (committed: 451; second run: 1700) because the manifest text contains every legacy path token it plans to rewrite. | Either exclude `src/internal/work_archive/173009_04-27-26/50991_0950_timestamp-naming-conventions/migration-manifest*.json` from `inventoryReferences` scan roots, or regenerate the dry-run manifest immediately before the write-mode commit so counters track reality. |
| L4 | info    | `{kind: lines, path: src/memory/features/timestamp-naming-conventions/spec.md, range: [65, 66], contentHash: TBD-on-commit}` | The spec keeps `src/work/*/run.log.jsonl` out of naming-policy scope; the migration preserves its basename but moves the file when the parent task directory renames. The behaviour matches the spec ("not renamed" reads as "basename preserved"), but the delivery report SHOULD make the path-vs-basename distinction explicit. | Tech-writer documents the basename-preservation guarantee in the delivery report.                                          |
...
| --------------------------------------------- | ----- | -------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `daedaline.features.timestamp_naming.policy_descriptor`     | tests | block    | deferred-runner-not-wired (structural pass) | Compliance descriptor parses and lists `structure-change` + `operator-on-demand`; no rego/llm-judge runner panel wired in this slice. Citation: `{kind: lines, path: src/internal/tests/compliance/timestamp-naming-conventions.yaml, range: [1, 18], contentHash: TBD-on-commit}`. |
| `daedaline.features.timestamp_naming.migration_precedence` | tests | block    | deferred-runner-not-wired (test pass)        | `pnpm migration:test` exercises four `chooseTimestamp` precedence cases (git, frontmatter, mtime, override) and exits 18/18 green. Citation: `{kind: lines, path: src/internal/tools/migrate-timestamp-naming.test.mjs, range: [115, 152], contentHash: TBD-on-commit}`. |
| `daedaline.features.timestamp_naming.reference_update_audit` | tests | block    | deferred-runner-not-wired (script pass)      | `node src/internal/tools/migrate-timestamp-naming.mjs --dry-run --strict-references` returns exit 0 with `strictReferenceAudit.staleRows: []`. Citation: `{kind: lines, path: src/internal/tools/migrate-timestamp-naming.mjs, range: [789, 818], contentHash: TBD-on-commit}`. |

The contract-runner panel (`@daedaline/contract-runner-llm-judge` and
`@daedaline/contract-runner-rego`) is not wired against per-feature
...
| - | ----------------------------------------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------- |
| P1 | The intake directive and operator response NEVER appear as rename candidates.                        | pass   | `{kind: lines, path: src/internal/tools/migrate-timestamp-naming.mjs, range: [433, 457], contentHash: TBD-on-commit}` (`EXCLUDED_SOURCES` set + `isExcludedFromMigration`)                                                                                                                                                                                                                                                                                       |
| P2 | `src/work/*/run.log.jsonl` NEVER receives a rename plan; basename SHALL stay `run.log.jsonl`.            | pass   | `{kind: lines, path: src/internal/tools/migrate-timestamp-naming.mjs, range: [449, 456], contentHash: TBD-on-commit}` (`/run.log.jsonl` short-circuits inclusion); manifest spot-check confirms zero file-level rename rows for `run.log.jsonl`.                                                                                                                                                                                                                  |
| P3 | Write mode requires `DAEDALINE_MIGRATION_GO=1` AND `--write`.                                        | pass   | `{kind: lines, path: src/internal/tools/migrate-timestamp-naming.mjs, range: [896, 921], contentHash: TBD-on-commit}` (env-var guard with stderr message and non-zero exit when missing)                                                                                                                                                                                                                                                                          |
| P4 | `--strict-references` audit fails loudly (non-zero exit) when uncovered legacy hits exist.           | pass   | `{kind: lines, path: src/internal/tools/migrate-timestamp-naming.mjs, range: [789, 938], contentHash: TBD-on-commit}` (`stale.length > 0` -> `console.error` + `process.exitCode = 1`)                                                                                                                                                                                                                                                                            |
| P5 | Existing day-directory paths matching `^\d{6}_\d{2}-\d{2}-\d{2}$` are skipped on re-run.             | pass   | `{kind: lines, path: src/internal/tools/migrate-timestamp-naming.mjs, range: [463, 483], contentHash: TBD-on-commit}` (`isDayDirectoryName` guard inside `listFlatWorkTasks`)                                                                                                                                                                                                                                                                                     |
...
| - | ----------------------------------------------------------------- | -------------------------- | -------- | --------- | ---------- | ---- | ------------------------------------------------- | ------------------------------------------------------------------------------------------ | ------ |
| 1 | `src/internal/work_archive/173010_04-26-26/23777_1723_policy-compliance-gate-2026-04-26/`                         | 2026-04-26T17:23:43Z      | 173010   | 04-26-26  | 23777      | 1723 | `policy-compliance-gate-2026-04-26`               | `src/internal/work_archive/173010_04-26-26/23777_1723_policy-compliance-gate-2026-04-26/`                       | pass   |
| 2 | `src/internal/work_archive/173009_04-27-26/51356_0944_bootstrap-phase-4-pipeline-roster/`                         | 2026-04-27T09:44:04Z      | 173009   | 04-27-26  | 51356      | 0944 | `bootstrap-phase-4-pipeline-roster`               | `src/internal/work_archive/173009_04-27-26/51356_0944_bootstrap-phase-4-pipeline-roster/`                       | pass   |
| 3 | `src/internal/work_archive/173009_04-27-26/55075_0842_daedaline-worktree-env-isolation-impl/`                     | 2026-04-27T08:42:05Z      | 173009   | 04-27-26  | 55075      | 0842 | `daedaline-worktree-env-isolation-impl`           | `src/internal/work_archive/173009_04-27-26/55075_0842_daedaline-worktree-env-isolation-impl/`                   | pass   |
| 4 | `src/internal/work_archive/173009_04-27-26/55309_0838_daedaline-persona-pipeline-runner-impl/`                    | 2026-04-27T08:38:11Z      | 173009   | 04-27-26  | 55309      | 0838 | `daedaline-persona-pipeline-runner-impl`          | `src/internal/work_archive/173009_04-27-26/55309_0838_daedaline-persona-pipeline-runner-impl/`                  | pass   |
| 5 | `src/internal/work_archive/173009_04-27-26/55884_0828_daedaline-observability-impl/`                              | 2026-04-27T08:28:36Z      | 173009   | 04-27-26  | 55884      | 0828 | `daedaline-observability-impl`                    | `src/internal/work_archive/173009_04-27-26/55884_0828_daedaline-observability-impl/`                            | pass   |
...
target matches `^src/work/\d{6}_\d{2}-\d{2}-\d{2}/\d+_\d{4}_[a-z0-9-]+/$`; every
...
`{kind: lines, path: src/internal/work_archive/173009_04-27-26/50991_0950_timestamp-naming-conventions/migration-manifest.dry-run.json, range: [1, 11], contentHash: TBD-on-commit}`.

## Reference-update audit

`node src/internal/tools/migrate-timestamp-naming.mjs --dry-run --strict-references` exits
...
| phase-0a scaffold          | `pnpm run check:phase0a`                                             | pass   | `node src/internal/tools/check-phase-0a-scaffold.mjs` exits 0.                                                                                                                |
| migration tests            | `pnpm migration:test`                                                | pass   | `node --test src/internal/tools/migrate-timestamp-naming.test.mjs` reports `# pass 18 / # fail 0`.                                                                            |
| package tests              | `pnpm -r run test`                                                   | pass   | All workspaces (vitest + node:test) report green; no regressions vs implement-stage commit.                                                                      |
| migration dry-run + strict | `node src/internal/tools/migrate-timestamp-naming.mjs --dry-run --strict-references` | pass   | Exit 0; `staleRows: []`; basename derivations match independent re-derivation for all 5 sampled rows and the 35-row structural sweep.                            |

Coverage delta: the implement slice introduces 18 new node:test cases under
`src/internal/tools/migrate-timestamp-naming.test.mjs` covering FDS rollover, SID
...
1. `[warn]` Citation range mismatch in
   `src/memory/features/timestamp-naming-conventions/contracts/timestamp-naming.migration-precedence.yaml`
   (see L1).
2. `[warn]` Mixed `contentHash` discipline in
   `src/memory/handbook/inbox-lifecycle.md` references frontmatter (see L2).
...
5. `[info]` Migration write-mode (`--write` under `DAEDALINE_MIGRATION_GO=1`)
