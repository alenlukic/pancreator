...
| `src/internal/packages/@pancreator/cli/src/feature-delivery-run.ts` | `state.json` writes via **`stringifyCliJson`** (abbreviate + canonical print). |
| `src/internal/packages/@pancreator/cli/src/run.ts` | All **`emit`** paths stringify via canonical helper (stub + envelopes). |
| `src/internal/packages/@pancreator/cli/src/canonical-json-io.ts` | Thin bridge to **`canonical-json-format.mjs`** (runtime single source). |
| `src/internal/packages/@pancreator/cli/tsconfig.json` | **`allowJs`** + **`include`** canonical `.mjs` so `pnpm --filter @pancreator/cli exec tsc -p tsconfig.json` passes without duplicating formatter TypeScript. |
| `src/internal/packages/@pancreator/cli/src/run.test.ts` | **`PAN_JSON_FORMAT_ABBREV_LEN=7`** in **beforeEach/afterEach** for temp repos without `.git`; JSON layout expectations unchanged aside from deterministic abbrev. |
...
| `PANCREATOR_MIGRATION_GO=1 node src/internal/tools/migrate-json-formatting.mjs --write` (pass 1) | **0** | `wouldRewrite=0`, `files rewritten=0` |
| `node src/internal/tools/migrate-json-formatting.mjs --dry-run` (post-write) | **0** | `abbrevLen=7`, `candidates=144`, `wouldRewrite=0` |
| `PANCREATOR_MIGRATION_GO=1 node src/internal/tools/migrate-json-formatting.mjs --write` (pass 2) | **0** | `wouldRewrite=0`, `files rewritten=0` |
