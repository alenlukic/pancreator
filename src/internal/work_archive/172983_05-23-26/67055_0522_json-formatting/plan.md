...
7. Update `src/memory/handbook/glossary.md` entry for `Content hash` / `contentHash` so canonical stored values are abbreviated SHA-256 prefixes whose length equals `git rev-parse --short HEAD` at write time.
...
- Slice B (`tesseract-engineer`): run bulk `.json` migration and capture evidence.
...
- Bulk migration executor persona: `tesseract-engineer`.
