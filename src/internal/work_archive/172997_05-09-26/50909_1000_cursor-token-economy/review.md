...
`src/memory/features/cursor-token-economy/contracts/` does not exist; `contracts:from_feature` therefore pulls in zero clauses for this Feature. The empty bundle is valid per the directive in this review request and `src/personas/reviewer.md`. The `tess lint contracts:*` CLI is not wired in this repository, so no runner executed; the gate MUST NOT fail solely on missing CLI.
