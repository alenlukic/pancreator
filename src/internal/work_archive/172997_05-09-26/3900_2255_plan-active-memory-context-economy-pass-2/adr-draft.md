...
When routine orientation starts, Pancreator SHALL load `src/memory/active/**` as the only memory tier intended for default orientation.
...
When Pancreator classifies `src/work/**`, `src/inbox/out/**`, and `src/inbox/threads/**`, Pancreator SHALL treat those paths as archival memory and explicit-read surfaces.

When Pancreator classifies `src/memory/features/**`, `src/memory/adr/**`, and `src/memory/backlog/**`, Pancreator SHALL treat those paths as durable memory.

When Pancreator classifies `src/memory/handbook/**`, `src/personas/**`, `src/skills/**`, `.cursor/rules/**`, and `.cursor/agents/**`, Pancreator SHALL treat those paths as internal operating content loaded by route.
...
When Pancreator evaluates physical migration of `src/work/**` or existing `src/memory/**` trees, Pancreator SHALL defer migration until one slice ships reference updates, compatibility shims, tests, a migration manifest, and rollback notes.
