...
- `src/internal/packages/@pancreator/intervention/`
...
- `src/internal/packages/@pancreator/cli/src/` (run.ts, pan-init.ts, active-memory-refresh.ts)
...
{
  "kind": "lines",
  "path": "src/internal/packages/@pancreator/runner-cursor/src/cursor-runner.ts",
  "range": [27, 76],
  "contentHash": "5ff594a"
}
```
...
{
  "kind": "lines",
  "path": "src/internal/packages/@pancreator/cli/src/pan-init.ts",
  "range": [65, 142],
  "contentHash": "stub-cleared"
}
```

- Library-mode proof is present: `examples/library-script/index.mjs` imports only `@pancreator/persona`, emits `.cursor/agents/<name>.md` and `.cursor/rules/<name>.mdc` into a temp directory, and does not read `src/memory/`, `src/inbox/`, or `pancreator.yaml`. `docs/PRD.summary.md` §Library-mode proof references it as the US-8 proof.
...
The `pancreator-inbox` row in `src/memory/active/current.md` contained multi-line command output embedded inside the timestamp cell: `2026-05-26T09:34:14node --test tests/*.test.mjs\n...\n.000Z`. This caused the `refresh-active-memory` tool to report divergence and halt with exit code 126.
...
{
  "kind": "lines",
  "path": "src/internal/packages/@pancreator/cli/src/active-memory-refresh.ts",
  "range": [73, 139],
...
`src/memory/active/current.md` §Risks and blockers carried: _"Phoenix OTLP import remains open on the `pancreator-engineer` backlog."_ WP-D Option A shipped and the Phoenix smoke test passes (3/3 tests in Docker, C-07). The spec acceptance criterion states this row SHALL be removed when Option A smoke tests pass.
...
|---|---|---|---|
| R-01 | Fixed `pancreator-inbox` row corruption in active memory | `src/memory/active/current.md` | Low — restores correct timestamp |
...
| R-04 | Patched `parseShippedAtMs` to read `indexed_at` at root level | `src/internal/packages/@pancreator/cli/src/active-memory-refresh.ts` | Low — additive fallback path; 28 tests pass |
| R-05 | Patched `readDeliveryReportPath` to read `feature_artifacts[].kind === "delivery-report"` | `src/internal/packages/@pancreator/cli/src/active-memory-refresh.ts` | Low — additive fallback path; 28 tests pass |
...
- `src/internal/packages/@pancreator/cli/src/active-memory-refresh.ts`: implementation fix; no public API surface change; no doc update required beyond this audit report.
