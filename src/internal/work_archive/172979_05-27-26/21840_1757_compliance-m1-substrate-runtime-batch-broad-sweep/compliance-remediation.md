...
| File | Change | Finding |
|---|---|---|
| `src/memory/active/current.md` | Fixed `pancreator-inbox` row data corruption (embedded command output in timestamp cell) | M-01 |
...
| `src/internal/packages/@pancreator/cli/src/active-memory-refresh.ts` | `parseShippedAtMs`: added root-level `indexed_at` read before legacy `index.completed_at` path | M-02 |
| `src/internal/packages/@pancreator/cli/src/active-memory-refresh.ts` | `readDeliveryReportPath`: added `feature_artifacts[].kind === "delivery-report"` fallback | M-02 |
