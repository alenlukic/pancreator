...
| `docs-vs-handbook` | `ddl advance` command table | `README.md` | `OPERATION.md` § ddl CLI verbs | Moved |
| `docs-vs-handbook` | CLI `pnpm -w exec ddl` prefix | `AGENTS.md`, `daedaline-config.md`, `operator-output-contract.md` | Unchanged — AGENTS states rule; handbook holds examples; OPERATION holds operator tables | Mirror only; no collapse |
...
| `cli-help-vs-handbook` | CLI help strings in `run.ts` | Package source | `OPERATION.md` + `daedaline-config.md` | No drift found requiring `run.ts` edit |
