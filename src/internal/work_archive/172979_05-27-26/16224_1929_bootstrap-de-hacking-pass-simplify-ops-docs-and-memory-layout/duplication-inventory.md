...
| `docs-vs-handbook` | `tess advance` command table | `README.md` | `OPERATION.md` § tess CLI verbs | Moved |
| `docs-vs-handbook` | CLI `pnpm -w exec tess` prefix | `AGENTS.md`, `tesseract-config.md`, `operator-output-contract.md` | Unchanged — AGENTS states rule; handbook holds examples; OPERATION holds operator tables | Mirror only; no collapse |
...
| `cli-help-vs-handbook` | CLI help strings in `run.ts` | Package source | `OPERATION.md` + `tesseract-config.md` | No drift found requiring `run.ts` edit |
