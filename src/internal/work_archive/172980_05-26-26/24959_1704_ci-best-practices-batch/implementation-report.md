...
- Added noop `test` scripts to `@tesseract/core` and meta `tesseract` packages so turbo resolves every workspace participant.
...
- Corrected `tsconfig.base.json` extends depth in 11 `@tesseract/*` packages (`../../../../` → `../../../../../`).
- Fixed stale repo-root resolution in `@tesseract/memory` and `@tesseract/persona` vitest fixtures.
...
- Added `src/internal/packages/@tesseract/mcp-server/src/tess-read-handlers.ts` with typed handlers for:
  - `tess.feature` (`action: list|show`) → `feature.list` / `feature.show` envelopes
  - `tess.status` → workspace bootstrap + active task summary (optional `taskId`)
  - `tess.memory` (`query`) → `memory.query` via `MemoryRouter` + active-memory scan
- Updated `tess-execute.ts`, `definitions.ts` input schemas, and `README.md` (wired read tools + deferred write tools table).
- Extended `tess-execute.test.ts` with direct handler tests and stdio MCP transport coverage asserting no `{"status":"stub"}` responses.
- Added `yaml` dependency to `@tesseract/mcp-server` for `tesseract.yaml` status reads.
...
| `src/internal/packages/@tesseract/core/package.json` | A |
| `src/internal/packages/tesseract/package.json` | A |
| `src/internal/packages/@tesseract/*/tsconfig.json` (11 packages) | A blocker |
| `src/internal/packages/@tesseract/memory/src/memory-router.test.ts` | A blocker |
| `src/internal/packages/@tesseract/persona/src/tech-writer.roundtrip.test.ts` | A blocker |
...
| `src/internal/packages/@tesseract/mcp-server/src/tess-read-handlers.ts` | D |
| `src/internal/packages/@tesseract/mcp-server/src/tess-execute.ts` | D |
| `src/internal/packages/@tesseract/mcp-server/src/definitions.ts` | D |
| `src/internal/packages/@tesseract/mcp-server/src/tess-execute.test.ts` | D |
| `src/internal/packages/@tesseract/mcp-server/src/definitions.test.ts` | D |
| `src/internal/packages/@tesseract/mcp-server/package.json` | D |
| `src/internal/packages/@tesseract/mcp-server/README.md` | D |
...
| **MF-02** — incomplete touch-set | `touch-set.json` stage set to `implement`; added 11 blocker `tsconfig.json` paths, `@tesseract/persona/tsconfig.json`, vitest fixture files, `pnpm-lock.yaml`, `tests/repo-structure.test.mjs`, `src/memory/features/ci-best-practices-batch/index.json`, and `tess-read-handlers.ts` (39 paths total). |
...
| `pnpm --filter @tesseract/mcp-server test` | **PASS** | includes stdio transport read-tool suite |
...
| Surface | Action |
|---|---|
| `@tesseract/mcp-server/README.md` | **Updated** — wired read tools and deferred write-tool table |
...
- **`tess compliance run` CLI verb** remains deferred per spec out-of-scope.
- **MCP write tools** remain on structured deferral envelopes (`tess.init`, `tess.run`, `tess.approve`, `tess.contracts`, `tess.lint`).
...
```bash
cd /Users/alen/Dev/tesseract
pnpm -w exec tess advance 24959_1704_ci-best-practices-batch --artifact src/work/172980_05-26-26/24959_1704_ci-best-practices-batch/implementation-report.md
```
