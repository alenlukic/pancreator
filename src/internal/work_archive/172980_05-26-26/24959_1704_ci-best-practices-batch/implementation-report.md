...
- Added noop `test` scripts to `@daedaline/core` and meta `daedaline` packages so turbo resolves every workspace participant.
...
- Corrected `tsconfig.base.json` extends depth in 11 `@daedaline/*` packages (`../../../../` → `../../../../../`).
- Fixed stale repo-root resolution in `@daedaline/memory` and `@daedaline/persona` vitest fixtures.
...
- Added `src/internal/packages/@daedaline/mcp-server/src/ddl-read-handlers.ts` with typed handlers for:
  - `ddl.feature` (`action: list|show`) → `feature.list` / `feature.show` envelopes
  - `ddl.status` → workspace bootstrap + active task summary (optional `taskId`)
  - `ddl.memory` (`query`) → `memory.query` via `MemoryRouter` + active-memory scan
- Updated `ddl-execute.ts`, `definitions.ts` input schemas, and `README.md` (wired read tools + deferred write tools table).
- Extended `ddl-execute.test.ts` with direct handler tests and stdio MCP transport coverage asserting no `{"status":"stub"}` responses.
- Added `yaml` dependency to `@daedaline/mcp-server` for `daedaline.yaml` status reads.
...
| `src/internal/packages/@daedaline/core/package.json` | A |
| `src/internal/packages/daedaline/package.json` | A |
| `src/internal/packages/@daedaline/*/tsconfig.json` (11 packages) | A blocker |
| `src/internal/packages/@daedaline/memory/src/memory-router.test.ts` | A blocker |
| `src/internal/packages/@daedaline/persona/src/tech-writer.roundtrip.test.ts` | A blocker |
...
| `src/internal/packages/@daedaline/mcp-server/src/ddl-read-handlers.ts` | D |
| `src/internal/packages/@daedaline/mcp-server/src/ddl-execute.ts` | D |
| `src/internal/packages/@daedaline/mcp-server/src/definitions.ts` | D |
| `src/internal/packages/@daedaline/mcp-server/src/ddl-execute.test.ts` | D |
| `src/internal/packages/@daedaline/mcp-server/src/definitions.test.ts` | D |
| `src/internal/packages/@daedaline/mcp-server/package.json` | D |
| `src/internal/packages/@daedaline/mcp-server/README.md` | D |
...
| **MF-02** — incomplete touch-set | `touch-set.json` stage set to `implement`; added 11 blocker `tsconfig.json` paths, `@daedaline/persona/tsconfig.json`, vitest fixture files, `pnpm-lock.yaml`, `tests/repo-structure.test.mjs`, `src/memory/features/ci-best-practices-batch/index.json`, and `ddl-read-handlers.ts` (39 paths total). |
...
| `pnpm --filter @daedaline/mcp-server test` | **PASS** | includes stdio transport read-tool suite |
...
| Surface | Action |
|---|---|
| `@daedaline/mcp-server/README.md` | **Updated** — wired read tools and deferred write-tool table |
...
- **`ddl compliance run` CLI verb** remains deferred per spec out-of-scope.
- **MCP write tools** remain on structured deferral envelopes (`ddl.init`, `ddl.run`, `ddl.approve`, `ddl.contracts`, `ddl.lint`).
...
```bash
cd /Users/alen/Dev/daedaline
pnpm -w exec ddl advance 24959_1704_ci-best-practices-batch --artifact src/work/172980_05-26-26/24959_1704_ci-best-practices-batch/implementation-report.md
```
