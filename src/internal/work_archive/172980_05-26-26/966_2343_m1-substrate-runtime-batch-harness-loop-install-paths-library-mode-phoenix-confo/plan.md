...
1. Extend `@tesseract/checkpointer-fs` checkpoint primitives so metadata and persistence shapes can map to LangGraph saver semantics.
...
6. Update `src/internal/packages/@tesseract/run-logger/README.md` to cite `tests/run-logger-conformance/` as conformance authority for Phoenix import.
...
1. Add `tests/run-logger-conformance/fixtures/sample-run-log.jsonl` with OpenInference + OTel GenAI attributes aligned to `src/memory/handbook/run-log-schema.md`.
...
1. Add `.github/workflows/run-logger-conformance.yml` with path filters on `src/internal/packages/@tesseract/run-logger/**` and `tests/run-logger-conformance/**`.
...
2. Option B SHALL author `src/memory/adr/0007-run-logger-phoenix-conformance-deferral.md` only after explicit tech-lead ratification.
...
3. Add smoke tests that run outside the monorepo and prove no reads under `src/memory/`, `src/inbox/`, or `tesseract.yaml`.

### WP-F — install paths (`tess init`, `create-tesseract`)

1. Replace deferred `tess init` behavior with dry-run default, apply mode, conflict checks, adoption scan report emission, and inbox ratification item creation.
2. Add `create-tesseract` scaffolding flow and ensure generated projects run `tess inbox` + `tess run feature-delivery`.
...
- `pnpm --filter @tesseract/cli test -- --runInBand feature-delivery`
...
- `pnpm --filter @tesseract/cli test -- --runInBand run.ts`
- `pnpm test`

### Batch integration and compliance gate

- `pnpm -w exec tess status 966_2343_m1-substrate-runtime-batch-harness-loop-install-paths-library-mode-phoenix-confo`
...
documentation_impact:
  ...
  changed-surfaces:
    ...
    - package.json
    - src/internal/packages/@tesseract/cli/src/run.ts
  deferred-items:
...
- {kind: lines, path: "src/work/172980_05-26-26/966_2343_m1-substrate-runtime-batch-harness-loop-install-paths-library-mode-phoenix-confo/state.json", range: [3, 19], contentHash: "6353af4"}
- {kind: lines, path: "src/memory/features/m1-substrate-runtime-batch-harness-loop-install-paths-library-mode-phoenix-confo/spec.md", range: [56, 73], contentHash: "5009d5a"}
...
- {kind: symbol, path: "src/internal/packages/@tesseract/pipeline/src/execute.ts", symbol: "executePipeline", contentHash: "cca08db"}
- {kind: symbol, path: "src/internal/packages/@tesseract/cli/src/feature-delivery-run.ts", symbol: "advanceFeatureDelivery", contentHash: "1131dfc"}
