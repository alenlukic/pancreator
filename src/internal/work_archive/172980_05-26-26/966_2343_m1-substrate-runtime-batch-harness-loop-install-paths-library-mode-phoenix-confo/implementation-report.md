...
Must_fix re-entry closed four blocking review gaps: live Cursor SDK transport behind `runner.cursor.invocation: sdk`, LangGraph `StateGraph` compile/execute in `@tesseract/pipeline`, canonical inbox path normalization for `tess run feature-delivery`, and repository JSON formatting for touched manifests. WP-A, WP-D, WP-E, and WP-F behavior from the prior implement pass remain in place.
...
| **JSON formatting suite** | Canonical two-space formatting applied to `touch-set.json`, `@tesseract/pipeline/package.json`, `@tesseract/checkpointer-fs/package.json`, and `@tesseract/runner-cursor/package.json`. `tests/repo-structure.test.mjs` deferral matrix no longer expects `tess init` to exit `125` (WP-F implements init). |
...
- `manual` default (dry-run, no SDK). `sdk` invokes transport; `readCursorInvocationMode` reads `tesseract.yaml`.
...
- `tess init` (dry-run/apply/force) and `create-tesseract`; non-deferred CLI behavior.
...
| `pnpm -w exec tess run feature-delivery src/inbox/in/172980_05-26-26/2597_2316_m1-substrate-runtime-batch.md` | 0 | pass | Full inbox path accepted after strip/join fix |
...
| `pnpm -w exec tess status 966_2343_m1-substrate-runtime-batch-harness-loop-install-paths-library-mode-phoenix-confo` | 0 | pass | `currentStage`: `implement` |
| `node src/internal/tools/run-compliance.mjs` | 0 | pass | `review_passes: true`, `blockFindings: 0` |

**Build note:** Run `pnpm --filter @tesseract/{pipeline,runner-cursor,checkpointer-fs,cli} build` before `pnpm -w exec tess …` or live SDK smoke if workspace `dist/` artifacts are stale.
...
pnpm --filter @tesseract/runner-cursor build
...
const { CursorRunner } = await import('./src/internal/packages/@tesseract/runner-cursor/dist/index.js');
...
2. **`tess run` default invocation** — Repo `tesseract.yaml` has no `runner.cursor.invocation: sdk`; `tess run feature-delivery` still uses `manual` unless that key is set. Live SDK proof uses `CursorRunner({ invocation: 'sdk' })` directly.
...
```sh
cd /Users/alen/Dev/tesseract
pnpm -w exec tess advance 966_2343_m1-substrate-runtime-batch-harness-loop-install-paths-library-mode-phoenix-confo --artifact src/work/172980_05-26-26/966_2343_m1-substrate-runtime-batch-harness-loop-install-paths-library-mode-phoenix-confo/implementation-report.md
```
...
pnpm --filter @tesseract/cli test
pnpm -w exec tess run feature-delivery src/inbox/in/172980_05-26-26/2597_2316_m1-substrate-runtime-batch.md
...
- **Impact:** Read-only except `tess run feature-delivery`, which creates a new active-work directory under `src/work/` (use a throwaway inbox entry if you want to avoid clutter).
