...
Must_fix re-entry closed four blocking review gaps: live Cursor SDK transport behind `runner.cursor.invocation: sdk`, LangGraph `StateGraph` compile/execute in `@daedaline/pipeline`, canonical inbox path normalization for `ddl run feature-delivery`, and repository JSON formatting for touched manifests. WP-A, WP-D, WP-E, and WP-F behavior from the prior implement pass remain in place.
...
| **Inbox path double-prefix** | `assertInboxInRelativePath` strips a leading `src/inbox/in/` so both bucket-relative and full inbox paths resolve. CLI test added. |
| **JSON formatting suite** | Canonical two-space formatting applied to `touch-set.json`, `@daedaline/pipeline/package.json`, `@daedaline/checkpointer-fs/package.json`, and `@daedaline/runner-cursor/package.json`. `tests/repo-structure.test.mjs` deferral matrix no longer expects `ddl init` to exit `125` (WP-F implements init). |
...
- `manual` default (dry-run, no SDK). `sdk` invokes transport; `readCursorInvocationMode` reads `daedaline.yaml`.
...
- `ddl init` (dry-run/apply/force) and `create-daedaline`; non-deferred CLI behavior.
...
| `pnpm --filter @daedaline/checkpointer-fs test` | 0 | pass | 7 tests |
| `pnpm --filter @daedaline/runner-cursor test` | 0 | pass | 4 tests |
| `pnpm --filter @daedaline/intervention test` | 0 | pass | 15 tests |
| `pnpm --filter @daedaline/pipeline test` | 0 | pass | 11 tests |
| `pnpm --filter @daedaline/cli test` | 0 | pass | 28 tests |
| `pnpm -w exec ddl run feature-delivery src/inbox/in/172980_05-26-26/2597_2316_m1-substrate-runtime-batch.md` | 0 | pass | Full inbox path accepted after strip/join fix |
| `pnpm --filter @daedaline/run-logger test` | 0 | pass | 6 tests |
| `pnpm test:run-logger-conformance` | 0 | pass | Phoenix boot + 3 smoke subtests |
| `pnpm --filter @daedaline/persona test` | 0 | pass | 5 tests |
...
| `pnpm -w exec ddl status 966_2343_m1-substrate-runtime-batch-harness-loop-install-paths-library-mode-phoenix-confo` | 0 | pass | `currentStage`: `implement` |
| `node src/internal/tools/run-compliance.mjs` | 0 | pass | `review_passes: true`, `blockFindings: 0` |

**Build note:** Run `pnpm --filter @daedaline/{pipeline,runner-cursor,checkpointer-fs,cli} build` before `pnpm -w exec ddl …` or live SDK smoke if workspace `dist/` artifacts are stale.
...
pnpm --filter @daedaline/runner-cursor build
...
const { CursorRunner } = await import('./src/internal/packages/@daedaline/runner-cursor/dist/index.js');
...
2. **`ddl run` default invocation** — Repo `daedaline.yaml` has no `runner.cursor.invocation: sdk`; `ddl run feature-delivery` still uses `manual` unless that key is set. Live SDK proof uses `CursorRunner({ invocation: 'sdk' })` directly.
...
```sh
cd /Users/alen/Dev/daedaline
pnpm -w exec ddl advance 966_2343_m1-substrate-runtime-batch-harness-loop-install-paths-library-mode-phoenix-confo --artifact src/work/172980_05-26-26/966_2343_m1-substrate-runtime-batch-harness-loop-install-paths-library-mode-phoenix-confo/implementation-report.md
```
...
- **Impact:** Read-only except `ddl run feature-delivery`, which creates a new active-work directory under `src/work/` (use a throwaway inbox entry if you want to avoid clutter).
