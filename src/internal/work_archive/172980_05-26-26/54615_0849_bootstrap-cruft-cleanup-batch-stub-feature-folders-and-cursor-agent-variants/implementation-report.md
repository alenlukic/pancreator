...
## Work package A — tesseract-* feature folders

For each of **20** folders under `src/memory/features/tesseract-*/` the change set:

| Folder | `spec.md` | `delivery-report.md` | `index.json` |
| --- | --- | --- | --- |
| `tesseract-adopter-scan` | Prepended YAML (`title`, `feature_id`, `lifecycle_stage`) | Created stub report | Created stub index aligned to Phase-2-ish shape |
| `tesseract-checkpointer-fs` | same pattern | same | same |
| `tesseract-cli` | same pattern | same | same |
| `tesseract-contract` | same pattern | same | same |
| `tesseract-contract-runner-llm-judge` | same pattern | same | same |
| `tesseract-contract-runner-rego` | same pattern | same | same |
| `tesseract-contract-style` | same pattern | same | same |
| `tesseract-core` | same pattern | same | same |
| `tesseract-env-isolation` | same pattern | same | same |
| `tesseract-inbox` | same pattern | same | same |
| `tesseract-intervention` | same pattern | same | same |
| `tesseract-mcp-server` | same pattern | same | same |
| `tesseract-memory` | same pattern | same | same |
| `tesseract-notifier` | same pattern | same | same |
| `tesseract-persona` | same pattern | same | same |
| `tesseract-pipeline` | same pattern | same | same |
| `tesseract-policy` | same pattern | same | same |
| `tesseract-run-logger` | same pattern | same | same |
| `tesseract-runner-cursor` | same pattern | same | same |
| `tesseract-worktree` | same pattern | same | same |
...
- **`src/memory/features/index.json`**: each tesseract row now points at paired `index.json`, drops `per_feature_index_missing`, and `coverage_gaps` is cleared (empty).
...
| `.cursor/agents/tesseract-engineer-standard.md` |
| `.cursor/agents/tesseract-engineer-complex.md` |
...
- `src/internal/packages/@tesseract/persona/src/emit.ts` — JSDoc states single-file Cursor mirror policy; cites: `{ "kind": "lines", "path": "src/internal/packages/@tesseract/persona/src/emit.ts", "range": [1, 46], "contentHash": "35b1382" }`
...
- `src/memory/features/bootstrap-cruft-cleanup-batch-stub-feature-folders-and-cursor-agent-variants/spec.md` — references refreshed for `subagent-model-tiers` and representative `tesseract-cli/spec.md`; work-package prose aligned with preserved contract surfaces.

## JSON corpus maintenance

Ran `migrate-json-formatting --write` with `TESSERACT_MIGRATION_GO=1` so new `*.json` match repository canonical JSON policy (fixes `repository JSON files use two-space formatting` gate).
...
Work package A (20 `tesseract-*` folder normalization) is unchanged.
