...
export function resolveAbbrevLen(repoRoot = process.cwd()) {
  const o = process.env.TESS_JSON_FORMAT_ABBREV_LEN;
  if (o !== undefined && o !== "") {
    if (!/^[0-9]+$/.test(o)) {
      throw new Error(
        "TESS_JSON_FORMAT_ABBREV_LEN must contain decimal digits only when set.",
      );
    }
    const n = Number(o);
    if (n < 4 || n > 255) {
      throw new Error("TESS_JSON_FORMAT_ABBREV_LEN must be between 4 and 255.");
    }
    return n;
  }
  try {
    ...
    throw new Error(
      `canonical-json-format: cannot derive abbreviation length (${msg}); export TESS_JSON_FORMAT_ABBREV_LEN for sandboxed repos.`,
    );
  }
}
