 ...
 *
 * Usage:
 *   node src/internal/tools/run-compliance.mjs [descriptor-id]
 *   node src/internal/tools/run-compliance.mjs --run-id <task-id>
 */
...
export function resolveRunOutputDir(runId) {
  ...
  if (!existsSync(workRoot)) {
    throw new Error(`Missing src/work root for --run-id ${runId}.`);
  }
  ...
  throw new Error(`No src/work/<day>/${runId} directory found for --run-id.`);
}
