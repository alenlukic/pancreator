...
export function matcherForIndexingPatternLine(line) {
  ...
  if (idx !== -1) {
    ...
    /* Nested JSON globs rooted at src/memory/. */
    if (rest === "*.json") {
    ...
    // src/memory/** slash delivery-report.md and memory slash index.json literals
...
export function classifyExclusiveTier(rel) {
  ...
  if (/^src\/memory\/(.+\/)?index\.json$/i.test(r)) return "generated_machine";
  if (/^src\/memory\/(.+\/)?delivery-report\.md$/i.test(r)) return "generated_machine";
  if (/^src\/memory\/.+\.json$/i.test(r)) return "generated_machine";

  const activeWork = /^src\/work(?:\/|$)/.test(r);
  if (activeWork) return "active_work";

  const archival =
    /^src\/internal\/work_archive(?:\/|$)/.test(r) ||
    /^src\/inbox\/out(?:\/|$)/.test(r) ||
    /^src\/inbox\/archive(?:\/|$)/.test(r) ||
    /^src\/inbox\/threads(?:\/|$)/.test(r);
  if (archival) return "archival_memory";

  if (/^src\/memory\/active(?:\/|$)/.test(r)) return "active_memory";

  const durable =
    /^src\/memory\/features(?:\/|$)/.test(r) ||
    /^src\/memory\/adr(?:\/|$)/.test(r) ||
    /^src\/memory\/backlog(?:\/|$)/.test(r);
  if (durable) return "durable_memory";

  const internal =
    /^src\/memory\/handbook(?:\/|$)/.test(r) ||
    /^src\/personas(?:\/|$)/.test(r) ||
    /^src\/skills(?:\/|$)/.test(r) ||
    /^\.cursor\/rules(?:\/|$)/.test(r) ||
...
const TIER_META = /** @type {const} */ ({
  active_memory: { label: "1 active memory", dir: "`src/memory/active/**`" },
  active_work: { label: "2 active work", dir: "`src/work/**`" },
  durable_memory: {
    label: "3 durable memory",
    dir: "`src/memory/features/**`, `src/memory/adr/**`, `src/memory/backlog/**`",
  },
  archival_memory: {
    label: "4 archival memory",
    dir: "`src/internal/work_archive/**`, `src/inbox/out/**`, `src/inbox/archive/**`, `src/inbox/threads/**`",
  },
  internal_operating: {
    label: "5 internal operating content",
    dir: "`src/memory/handbook/**`, `src/personas/**`, `src/skills/**`, `.cursor/rules/**`, `.cursor/agents/**`",
  },
  product_context: {
  ...
  other: { label: "(unclassified)", dir: "paths outside the eight groups" },
});
...
function printReport() {
  ...
  const legacyScopes = [
    ...
    { id: "active_work", label: "Active work slice: src/work/**", pred: (rel) => /^src\/work(?:\/|$)/.test(posixRel(rel)) },
    { id: "archived_work", label: "Archived work slice: src/internal/work_archive/**", pred: (rel) => /^src\/internal\/work_archive(?:\/|$)/.test(posixRel(rel)) },
    { id: "memory", label: "Legacy slice: src/memory/**", pred: (rel) => /^src\/memory(?:\/|$)/.test(posixRel(rel)) },
  ];
