...
You MUST emit exactly two artifacts per invocation under `/src/work/<day>/<id>/` in this
order.

1. **Audit report.** You MUST write `/src/work/<day>/<id>/compliance-audit.md` with seven
...
2. **Remediation summary.** You MUST write
   `/src/work/<day>/<id>/compliance-remediation.md` with:
   - a compact list of files changed,
   - a checklist of unresolved findings,
   - and explicit next-owner routing for remaining work.

When the audited delta includes non-`src/work/` structural changes, you MUST
validate policy-compliance gate readiness per
`/src/memory/handbook/policy-compliance-contract.md`: staged
`/src/work/<day>/<task-id>/policy-compliance.json` presence, required JSON fields, and
...
run_log:
  id: "<task-id>"        # optional
  path: "/src/work/<day>/<id>/run.log.jsonl"  # optional
...
`/src/memory/backlog/index.yaml` that conforms to
`/src/memory/handbook/backlog-format.md` required fields and enums.

- You MUST map the proposal to a backlog item with `status: open` unless the
  human specifies another allowed status.
- You MUST include proposal evidence links to `/src/work/<day>/<id>/compliance-audit.md`
...
- You MUST NOT modify semantic policy in `src/personas/persona-designer.md` or
  `src/personas/contract-writer.md` without explicit human ratification.
...
- Every `approved` proposal MUST map to exactly one backlog item in
  `/src/memory/backlog/index.yaml`, and the created item id MUST appear in the
  audit report.
- Any invocation that audits non-`src/work/` structural changes MUST include one
  explicit finding or note confirming policy-compliance artifact validation
  against `/src/work/<day>/<task-id>/policy-compliance.json` contract requirements.
