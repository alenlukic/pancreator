...
metadata:
  ...
  tesseract-emits:
    ...
    - /memory/features/<id>/contracts.index.json entry
references:
  - kind: lines
    path: PRD.md
    range: [317, 480]
...
is `personas/contract-writer.md`; `intake-analyst`, `tech-lead`, `reviewer`,
`appsec`, `sdet`, `design-engineer`, and `groomer` MAY invoke it directly when their
own work product needs a contract gate.

## Prerequisites

- `/memory/handbook/contract-format.md` SHALL exist and define the §4.5 wrapper
  schema reference.
- `/memory/handbook/contract-style.md` SHALL exist and capture Layers 1–3 of §4.6.
- `/memory/handbook/contract-templates/` SHALL contain at minimum the 6 MVP
...
- The contract's `owner` persona SHALL exist in `personas/`.
...
Load the matching template from `/memory/handbook/contract-templates/<kind>.md` or
...
an RFC under `/memory/rfc/draft/` proposing a template revision per Layer 5.
...
checklist in `/memory/handbook/contract-style.md`.
...
1. Place the clause inline (in the artifact's frontmatter under `contract:`) or as
   a sidecar file under `/memory/features/<id>/contracts/<clause-id>.{rego,spec.ts,...}`
...
2. Auto-register the `clause.id` in `/memory/features/<id>/contracts.index.json`
...
3. Verify the wrapper schema (Zod) and the per-kind payload schema both validate
   ...
   `/memory/handbook/contract-format.md`.
4. Confirm the `owner` persona exists in `personas/`; if not, halt and open an
