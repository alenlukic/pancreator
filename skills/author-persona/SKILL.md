...
canonical caller is `personas/persona-designer.md`; the Librarian MAY invoke it
directly when proposing an SME draft (M4+).

## Prerequisites

- `/memory/handbook/persona-spec.md` SHALL exist and define the Anthropic Claude
  Agent SDK 16-field YAML reference plus the Cursor `.mdc` shim recipe.
- `/memory/handbook/glossary.md` SHALL exist; the persona's `description:` and body
...
Load `/memory/handbook/persona-spec.md` into context. The 16 fields are: `name`,
...
Load `/memory/handbook/glossary.md` so domain nouns resolve at lint time without a
re-read.
...
- `name` MUST be lowercase-kebab-case and unique across `personas/`.
...
- `color` is a UX hint only; pick from the unused palette in `/memory/handbook/persona-colors.md`.
...
Write `.cursor/rules/<name>.mdc` per the recipe in `/memory/handbook/persona-spec.md`.
...
`@`-include of the canonical persona file (`@personas/<name>.md`).
