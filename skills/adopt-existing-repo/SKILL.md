...
metadata:
  ...
  tesseract-emits:
    ...
    - inbox/in/<timestamp>-adopter-sme-<name>.md
    - inbox/in/<timestamp>-adopter-thresholds.md
references:
  - kind: lines
    path: PRD.md
    range: [504, 504]
...
caller is `personas/adopter.md`. The skill MUST stay non-destructive: every
...
- `/memory/adoption/` SHALL exist as a writable directory; the skill
...
declared at PRD §3.5 lines 215 through 222 is `/memory/`, `/personas/`,
`/skills/`, `/pipelines/`, `/inbox/`, `/.tess/`, and `tesseract.yaml`.

When any target path conflicts with an existing file, the skill MUST exit
non-zero and post one inbox item at
`inbox/in/<timestamp>-adopter-conflicts.md` carrying the per-file diff per
...
Overwrite `/memory/adoption/scan-<date>.md` with the eight `##` sections
declared in `personas/adopter.md` in this order. Every fact in every
...
`inbox/in/<timestamp>-adopter-sme-<name>.md` and one threshold-policy
proposal at `inbox/in/<timestamp>-adopter-thresholds.md`. Each inbox file
MUST link the relevant section of the scan report.

Apply the Layer 1 lint discipline declared in
`/memory/handbook/contract-style.md` to the scan report. Each rule MUST
hold:
...
- Halt when `/memory/adoption/scan-*.md` already exists; treat the run as
...
- If the human runs `npx tesseract init` against a repository that already
  carries `/memory/adoption/scan-*.md`, the skill MUST treat the run as a
  re-scan and emit a delta report at `/memory/adoption/scan-<date>.md`
  for the new date per `personas/adopter.md`.
