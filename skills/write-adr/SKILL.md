---
name: write-adr
description: Authors one immutable Architecture Decision Record in the Nygard format. Picks the next sequence number, fills the four-section template (context, decision, status, consequences), wires `supersedes:` and dual-anchor citations, runs Layer 1 lint, then files under `/memory/adr/<seq>-<slug>.md` for human ratification.
license: Apache-2.0
metadata:
...
references:
  ...
  - kind: lines
    ...
    range: [649, 658]
    contentHash: TBD-on-commit
    note: "PRD §7 — feature-delivery `plan` stage YAML declaring `/work/<id>/adr-draft.md` as a required output of the plan stage."
  - kind: lines
    path: PRD.md
    range: [915, 915]
    contentHash: TBD-on-commit
    note: "PRD §8 — Memory architecture: `/memory/adr/` holds immutable architecture decision records in Nygard format."
  - kind: lines
    path: PRD.md
    range: [1004, 1010]
    contentHash: TBD-on-commit
...
canonical callers are `personas/tech-lead.md` (drafts the ADR at the `plan`
stage), `personas/reviewer.md` (drafts a corrective ADR when the `review`
stage uncovers an undocumented decision), and `personas/librarian.md` (drafts
...
- `/memory/adr/` SHALL exist and contain the prior ADR sequence; the next ADR
  sequence number is `max(seq) + 1` zero-padded to 4 digits.
- `/memory/handbook/glossary.md` SHALL exist; every domain noun in the ADR
  body resolves through this file.
- `/memory/handbook/contract-style.md` SHALL exist; the ADR body satisfies
...
Read `/memory/adr/` and pick `<seq> = max(existing seq) + 1`, zero-padded to
4 digits. The slug MUST be lowercase-kebab-case at most 6 words, drawn from
the decision title. The full filename is
`/memory/adr/<seq>-<slug>.md`. When the ADR drafts under a Feature, the
`plan` stage MUST also stage `/work/<id>/adr-draft.md` per PRD §7
lines 655 through 658; the file copies forward into `/memory/adr/` only on
human ratification.
...
`/memory/handbook/contract-style.md` to every normative clause in the ADR
...
1. Stage the new file at `/memory/adr/<seq>-<slug>.md` with `status:
   proposed`. You MUST NOT commit; you MUST NOT push.
2. When the ADR drafts under the `feature-delivery` `plan` stage, also stage
   `/work/<id>/adr-draft.md` for the downstream `implement` and `review`
   stages per PRD §7 lines 655 through 658.
3. Open one inbox item at `inbox/in/<timestamp>-adr-<seq>-ratification.md`
...
5. Register the new ADR in `/memory/features/<id>/index.json` when the ADR
