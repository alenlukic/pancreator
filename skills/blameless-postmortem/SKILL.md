...
metadata:
  ...
  tesseract-emits:
    - /memory/postmortems/<task-id>-stub.md
references:
  - kind: lines
    path: PRD.md
    range: [503, 503]
  ...
  - kind: lines
    ...
    range: [949, 949]
    contentHash: TBD-on-commit
    note: "PRD §8 — Memory architecture: `/memory/postmortems/` holds blameless RCAs, never modified after publication."
---
...
`personas/supervisor.md` (invokes the skill on every `tess abort` per
PRD §3.5 US-10 line 236 and on every circuit-breaker trip that ends a
run). M4+ adds `personas/postmortem-author.md` as the long-form caller
...
- `/work/<task-id>/run.log.jsonl` SHALL exist and SHALL carry the OpenInference
  spans plus the OTel GenAI semconv parallel layer declared at PRD §7
  line 838; an empty or truncated run log fails the gate.
- `/work/<task-id>/run-summary.md` SHALL exist when the trigger is
  `tess abort` per PRD §3.5 US-10 line 244; the supervisor emits the
  summary on abort per PRD §7 line 890.
- `/memory/postmortems/` SHALL exist as a writable directory; the skill
  scaffolds it on first invocation.
- `/memory/handbook/glossary.md` and `/memory/handbook/contract-style.md`
...
Read `/work/<task-id>/run.log.jsonl` and parse every span emitted between
...
Overwrite `/memory/postmortems/<task-id>-stub.md` with exactly seven `##`
...
1. Stage `/memory/postmortems/<task-id>-stub.md`. You MUST NOT commit;
   you MUST NOT push. The published postmortem is immutable per PRD §8
   line 949 and lands at `/memory/postmortems/<task-id>.md` only after
   human ratification.
2. Open one inbox item at
   `inbox/in/<timestamp>-postmortem-<task-id>-ratification.md`
   summarizing the second story, the safety-invariant verdict, and the
   open questions.
3. Schedule the action items in `/memory/backlog/items/` through the
...
- Halt when `/work/<task-id>/run.log.jsonl` is missing or empty; the
  postmortem cannot rest on a synthesized timeline. Route an inbox item
  rather than fabricate spans.
- Halt when `/work/<task-id>/run-summary.md` is missing on an abort
