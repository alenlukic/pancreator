...
canonical caller is `personas/reviewer.md`. M2+ adds `appsec` (security
...
- The `review` stage's inputs MUST be present at the paths declared in PRD
  §7 lines 671 through 676: code, tests, `/work/<id>/plan.md`,
  `/work/<id>/adr-draft.md`, and the contracts pulled in by
  `contracts:from_feature`.
- `/work/<id>/test-report.md` SHALL exist (emitted by the upstream `test`
  stage) and SHALL carry a coverage delta the review cites.
- `/memory/handbook/glossary.md`, `/memory/handbook/contract-style.md`, and
  `/memory/handbook/contract-format.md` SHALL exist; the review body
...
Read `/memory/features/<id>/spec.md`, `/work/<id>/plan.md`, and
`/work/<id>/adr-draft.md` first; do NOT open the diff until the design
...
`/work/<id>/touch-set.json`. For each defect, classify under exactly one
...
Enumerate every clause in `/memory/features/<id>/contracts/`. For each
clause, run the matching `ContractRunner` per
`/memory/handbook/contract-format.md` and record the row below in the
...
| `runner output path` | dual-anchor citation to the runner's stdout/stderr capture under `/work/<id>/contract-runs/<clause.id>.log` |
...
Read `/work/<id>/test-report.md` and record the statement and branch
...
declared test paths per `personas/coder.md`. A missing test routes a
...
Write `/work/<id>/review.md` with exactly four `##` sections in the order
declared in `personas/reviewer.md`:
...
4. **Coverage delta.** The figures captured in Step 4 plus a dual-anchor
   citation into `/work/<id>/test-report.md`.
...
`/memory/handbook/contract-style.md` to every normative clause in the
...
- If `/work/<id>/plan.md`, `/work/<id>/adr-draft.md`, or
  `/work/<id>/test-report.md` is missing, the review MUST halt and open
  one inbox item at `inbox/in/<timestamp>-reviewer-missing-input.md`
...
- The contract runner aggregate cap defaults to
  ...
  `/memory/handbook/contract-format.md`. On soft-overage at 80%, the
