...
- `scope.no_unapproved_changes` MUST treat a tracked change in the harness root as contamination when the run works in another directory, and it carries no exception for a release landing. A worker declaration attributes only the run's own workspace, so landing a self-development release on the harness root's local default branch MUST be sequenced after submit as an operator step rather than excepted here. The ship contract owns that sequencing; a ship stage that follows it presents no harness-root delta, and a harness-root delta that is not that landing MUST still fail and MUST name the paths.
