...
2. Read internal `AGENTS.md` only when the bounded prompt omits the Pancreator self-development operating contract the task needs.
3. Read `lib/personas/contract-writer.md` only when the bounded prompt omits persona role semantics required for the task.
