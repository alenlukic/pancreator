...
## Citation conformance

- Each claim MUST carry a dual-anchor citation as fenced `json` with double-quoted keys.
- Use `lib/memory/handbook/contract-templates/delivery-report.template.md` for section layout and citation shape.
- JS-literal `{kind: lines, path: ...}` inline form is forbidden.
- Compact single-line `{"kind":"lines",...}` citation blobs are forbidden.
- `contentHash` MUST use the abbreviated git blob prefix from json-formatting policy.
