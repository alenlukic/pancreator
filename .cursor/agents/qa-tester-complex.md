...
`src/personas/qa-tester.md`. It intentionally avoids duplicating persona prose,
...
2. Read `src/personas/qa-tester.md` for role semantics before performing persona-owned work.
...
- `qa-tester-standard` is the default for bounded or routine work; its model is selected by the current frontmatter policy.
- `qa-tester-complex` preserves the prior fixed model selection for reasoning-heavy work.
- `qa-tester` is a backward-compatible standard alias unless an operator explicitly asks for the complex tier.
