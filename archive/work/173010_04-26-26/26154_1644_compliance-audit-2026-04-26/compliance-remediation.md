...
- [ ] `block` - Backfill immutable `contentHash` values across `severity: block` contract wrappers in `/lib/memory/features/**/contracts/*.yaml`.
- [ ] `major` - Backfill unresolved `references[].contentHash` placeholders in `/lib/memory/handbook/run-log-schema.md`.
