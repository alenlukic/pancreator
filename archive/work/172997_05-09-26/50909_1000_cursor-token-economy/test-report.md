...
- `lib/internal/tools/context-budget-report.mjs` — exercised by `node --test lib/internal/tools/context-budget-report.test.mjs` (spawns script, asserts stdout markers).
...
`lib/internal/tools/context-budget-report.test.mjs` (lines 1–18), which covers the CLI
