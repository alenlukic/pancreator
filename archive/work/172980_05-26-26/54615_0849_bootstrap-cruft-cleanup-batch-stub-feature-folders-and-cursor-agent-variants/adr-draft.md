...
1. `lib/memory/features/pancreator-*/` folders that must be dispositioned and brought to a maintainable Phase-2 shape.
...
1. **Feature-folder normalization without destructive deletion.**  
   Classify all 20 `pancreator-*` folders as `author`; preserve current contract and planning artifacts; add missing `index.json` and `delivery-report.md`; then update `lib/memory/features/index.json` in lockstep.
...
- `{kind: lines, path: lib/inbox/in/172981_05-25-26/71700_0612_bootstrap-cruft-cleanup-batch.md, range: [34, 137], contentHash: ec6a02d}`
- `{kind: lines, path: lib/memory/features/bootstrap-cruft-cleanup-batch-stub-feature-folders-and-cursor-agent-variants/spec.md, range: [61, 135], contentHash: 0158529}`
- `{kind: lines, path: lib/memory/features/json-formatting/spec.md, range: [1, 179], contentHash: 697b305}`
- `{kind: lines, path: lib/memory/handbook/subagent-model-tiers.md, range: [1, 93], contentHash: 6d90e18}`
- `{kind: lines, path: lib/memory/adr/0004-documentation-impact-contract.md, range: [66, 100], contentHash: 1fa849a}`
