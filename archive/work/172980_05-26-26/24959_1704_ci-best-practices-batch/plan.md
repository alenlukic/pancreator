...
references:
  ...
  - kind: lines
    path: work/172980_05-26-26/24959_1704_ci-best-practices-batch/state.json
    range: [13, 19]
    contentHash: 3969501
...
1. **WP-A - Add root test aggregation and CI invocation.**
   ...
   - Traceability citation: `{kind: lines, path:
     "lib/memory/features/ci-best-practices-batch/spec.md", range: [123, 138],
     contentHash: "22be0fb"}`.

2. **WP-B - Implement descriptor-runner compliance gate.**
   - When `node lib/internal/tools/run-compliance.mjs` is invoked, the tool
   ...
   - When `--run-id <id>` is provided, the runner SHALL emit
     `work/<day>/<id>/compliance-result.json`.
   - Traceability citation: `{kind: lines, path:
     "lib/memory/features/ci-best-practices-batch/spec.md", range: [139, 158],
     contentHash: "22be0fb"}`.

3. **WP-C - Implement deterministic citation refresh tooling.**
   - When `node lib/internal/tools/refresh-citations.mjs` runs, it SHALL patch
   ...
   - The tool SHALL be idempotent, SHALL support `--dry-run`, and SHALL refuse
     writes under `lib/inbox/notes/` while skipping
     `lib/inbox/{in,out,threads}/` with warnings.
   - A dedicated node test suite SHALL cover the three surfaces plus refusal and
     idempotency behavior.
   - Traceability citation: `{kind: lines, path:
     "lib/memory/features/ci-best-practices-batch/spec.md", range: [159, 182],
     contentHash: "22be0fb"}`.
...
4. **WP-D - Wire MCP read-only handlers to typed responses.**
   ...
   - Traceability citation: `{kind: lines, path:
     "lib/memory/features/ci-best-practices-batch/spec.md", range: [183, 205],
     contentHash: "22be0fb"}`.
...
5. **Cross-cutting validation and runtime budget control.**
   ...
   - Traceability citation: `{kind: lines, path:
     "lib/memory/features/ci-best-practices-batch/spec.md", range: [207, 226],
     contentHash: "22be0fb"}`.
