...
- **Fix:** `defaultDeferredTrackingIntake(cfg.verb)` routes **`pan init`** to `lib/inbox/in/172981_05-25-26/64500_0605_pan-init-and-create-pancreator-install-paths.md`; other deferred CLI verbs keep the batch inbox path. MCP: exported `deferredToolTrackingIntake` mirrors the same paths (`pan.init` → `64500…`, others → `64488…`).
...
- `lib/internal/packages/@pancreator/cli/lib/run.test.ts`
- `lib/internal/packages/@pancreator/mcp-server/lib/pan-execute.ts`
- `lib/internal/packages/@pancreator/mcp-server/lib/pan-execute.test.ts`
- `AGENTS.md`
- Optional: `lib/memory/active/current.md` refreshed when live `pan refresh-active-memory` apply runs (managed timestamp / derived slices only).
