...
| File | Change | Finding |
|---|---|---|
| `lib/memory/features/json-formatting/index.json` | `deferred_backlog_ids` primitive string array reformatted from multi-line to inline canonical layout | B-01 (resolved) |
| `lib/personas/compliance-auditor.md` | `references[2].range` updated `[1, 260]` → `[1, 115]`; `contentHash` refreshed. `references[4].range` updated `[1, 221]` → `[1, 220]`; `contentHash` refreshed. | M-02 (resolved) |

## Unresolved findings checklist

- [x] **M-01 (Major)** — `lib/memory/active/current.md` stale: resolved by archiving `lib/inbox/in/172983_05-23-26/74280_0322_intake-json-formatting-ratification.md` to `archive/inbox/in/172983_05-23-26/74280_0322_intake-json-formatting-ratification.md` and reconciling active-memory state.
- [x] **m-01 (Minor)** — `lib/memory/active/current.md` lines 15, 20 `TBD-on-commit` placeholders resolved to concrete abbreviated hashes during active-memory rotation.
...
- [x] **m-03 (Minor)** — `lib/memory/active/current.md` shipped-feature table rotated to include `json-formatting`.
...
|---|---|---|
| M-01, m-01, m-03 | Completed | Archived stale ratification request and updated `lib/memory/active/current.md` (Active Feature, hashes, shipped-feature table). |
