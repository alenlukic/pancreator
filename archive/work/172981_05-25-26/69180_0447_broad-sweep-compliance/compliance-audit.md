...
audited_path_set:
  - lib/personas/*.md            # all 12 persona specs
  - .cursor/agents/*.md          # all 37 Cursor projections
  - lib/personas/skills/*/SKILL.md        # all 8 skill packs
  - lib/memory/handbook/*.md     # handbook anchors
  - lib/memory/active/current.md # active-memory orientation
  - lib/memory/features/json-formatting/  # closed and indexed feature
  - archive/inbox/in/172983_05-23-26/ # archived ratification evidence
  ...
  - lib/memory/features/json-formatting/index.json
  - lib/personas/compliance-auditor.md   # self-audit references
```
...
| `check-phase-0a` | `node lib/internal/tools/check-phase-0a-scaffold.mjs` | PASS — no output |
...
| `dual-anchor-citation-check` | Citation range validation on `lib/personas/compliance-auditor.md` | STALE ranges detected and fixed |
...
| `active-memory-staleness` | Compare `lib/memory/active/current.md` claim against live `lib/inbox/in/` | RESOLVED — stale ratification inbox item archived; active-memory now matches queue state |
| `json-formatting-spec-conformance` | Review `lib/memory/features/json-formatting/spec.md` citation and Q2 policy tension | CLOSED — operator confirmed glossary alignment and feature closure |
...
**B-01 — JSON formatting violation in `lib/memory/features/json-formatting/index.json`**
- **Status:** AUTO-REMEDIATED. See section 4.
- **Evidence:**
  - `tests/repo-structure.test.mjs` test "repository JSON files use two-space formatting" failed pre-remediation with offender `lib/memory/features/json-formatting/index.json`.
...
**M-01 — `lib/memory/active/current.md` was stale: active inbox item not reflected**
- **Status:** RESOLVED.
- **Description:** The stale ratification inbox item `172983_05-23-26/74280_0322_intake-json-formatting-ratification.md` was archived to `archive/inbox/in/172983_05-23-26/74280_0322_intake-json-formatting-ratification.md`, and `lib/memory/active/current.md` now accurately reflects an empty markdown inbox queue.
...
- **Evidence:**
  ...
  {
    "kind": "lines",
    "path": "archive/inbox/in/172983_05-23-26/74280_0322_intake-json-formatting-ratification.md",
    "range": [1, 10],
    "contentHash": "TBD-on-commit"
  }
  ```

**M-02 — `lib/personas/compliance-auditor.md` had stale `references[]` line ranges**
- **Status:** AUTO-REMEDIATED. See section 4.
...
- **Description:** 501 `TBD-on-commit` placeholders are known per `lib/memory/features/timestamp-naming-conventions/citation-rot-scan.md`. Affected files include ADRs 0001–0005, feature specs for `cursor-token-economy`, `timestamp-naming-conventions`, `compliance-tests`, and handbook files (`inbox-lifecycle.md`, `contract-format.md`, `contract-templates/llm-judge.template.md`, `backlog-format.md`). This is pre-existing documented debt, backlog-tracked.
- **Existing mitigation:** Deferred to backlog per `lib/memory/features/json-formatting/delivery-report.md` (deferred items `json-formatting-citation-verifier-prefix`, `json-formatting-markdown-corpus-scan`). The citation-rot-scan at `lib/memory/features/timestamp-naming-conventions/citation-rot-scan.md` documents scope.
...
**m-03 — `lib/memory/active/current.md` rotation lagged shipped features**
- **Status:** RESOLVED.
...
### R-01 — Canonical JSON formatting: `lib/memory/features/json-formatting/index.json`

- **Rationale:** `deferred_backlog_ids` array used multi-line primitive string layout. The canonical formatter (`lib/internal/tools/migrate-json-formatting.mjs`, function `rewriteJsonText`) requires inline layout for all-primitive arrays. The fix is deterministic and fully reversible via `git diff`.
- **Changed paths:** `lib/memory/features/json-formatting/index.json`
...
### R-02 — Citation range realignment: `lib/personas/compliance-auditor.md`
...
- **Changed paths:** `lib/personas/compliance-auditor.md`
- **Changes:**
...
documentation_impact:
  applies: true
  rationale: >
    Two files were modified by auto-remediation (lib/memory/features/json-formatting/index.json
    and lib/personas/compliance-auditor.md). The index.json change is a formatting
...
- `status`: `deferred`
- `problem_statement`: `lib/memory/active/current.md` does not reflect the live inbox state; the rotation is manual and has no automated owner, causing agents to read stale pickup state.
- `evidence_anchors`:
  ```json
  {
    "kind": "lines",
  ...
  ```
- `proposed_change`: Define an automated `knowledge-curation` cron pipeline step that refreshes `current.md` whenever `lib/inbox/in/` contents change. Add a `supervisor` invariant check that blocks pipeline start when `current.md` Active Feature section does not match live inbox.
