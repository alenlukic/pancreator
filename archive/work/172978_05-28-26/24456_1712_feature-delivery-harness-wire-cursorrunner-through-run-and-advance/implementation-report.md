...
| Path | Change |
|------|--------|
| `lib/internal/packages/@pancreator/runner-cursor/lib/cursor-sdk-prereqs.ts` | **New** — resolve bundled `rg`, set `CURSOR_RIPGREP_PATH` |
| `lib/internal/packages/@pancreator/runner-cursor/lib/cursor-sdk-prereqs.test.ts` | **New** — unit tests for resolution |
| `lib/internal/packages/@pancreator/runner-cursor/lib/sdk-transport.ts` | Prereq call before SDK import; fail-fast error |
| `lib/internal/packages/@pancreator/runner-cursor/lib/index.ts` | Export prereq helpers |
| `lib/internal/packages/@pancreator/runner-cursor/package.json` | Optional platform binary deps |
| `lib/internal/packages/@pancreator/cli/lib/repo-env.ts` | `configureCursorSdkTransportPrereqs` |
| `lib/internal/packages/@pancreator/cli/lib/feature-delivery-runner.ts` | Invoke prereqs for live SDK runner |
...
**Expected:** `work/<day>/<task-id>/state.json` is written and `run.log.jsonl` contains a `cursor.runner.sdk` event (no ripgrep configuration error).
...
```bash
pnpm -w exec pan advance 24456_1712_feature-delivery-harness-wire-cursorrunner-through-run-and-advance --artifact work/172978_05-28-26/24456_1712_feature-delivery-harness-wire-cursorrunner-through-run-and-advance/implementation-report.md
```

## Out of scope (unchanged)

- Subordinate `client/` dashboard implementation
- `lib/pipelines/feature-delivery.yaml` stage inventory
