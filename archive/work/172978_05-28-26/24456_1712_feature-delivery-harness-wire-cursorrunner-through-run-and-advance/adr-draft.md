...
ADR-0002 separates current repository substrate from planned runtime behavior. This decision upgrades one planned runtime edge to an executable, opt-in path while preserving the manual substrate. Citation: `{kind: lines, path: "lib/memory/adr/0002-system-architecture-map.md", range: [102, 135], contentHash: "3574b43"}`.

ADR-0003 and ADR-0005 define inbox lifecycle and timestamp naming. The retry-limit halt and report approval gate therefore MUST write system-produced outbox files that follow the existing inbox and timestamp rules. Citations: `{kind: lines, path: "lib/memory/adr/0003-inbox-lifecycle-and-archival.md", range: [52, 109], contentHash: "9187a27"}` and `{kind: lines, path: "lib/memory/adr/0005-timestamp-naming-conventions.md", range: [35, 60], contentHash: "8b59da5"}`.
...
The CLI SHALL resolve persona specs from `lib/personas/<name>.md`; no live path SHALL use synthetic persona data.
...
The runtime SHALL pause at the report approval gate and retry-limit halt through timestamp-prefixed `lib/inbox/out/<day-bucket>/` artifacts.
...
- `{kind: lines, path: "lib/memory/adr/0002-system-architecture-map.md", range: [102, 135], contentHash: "3574b43"}`
- `{kind: lines, path: "lib/memory/adr/0003-inbox-lifecycle-and-archival.md", range: [52, 109], contentHash: "9187a27"}`
- `{kind: lines, path: "lib/memory/adr/0005-timestamp-naming-conventions.md", range: [35, 60], contentHash: "8b59da5"}`
- `{kind: symbol, path: "lib/internal/packages/@pancreator/cli/lib/feature-delivery-run.ts", symbol: "startFeatureDelivery", contentHash: "7c5e4d7"}`
