...
| `validation.phase-0a-scaffold` | command | block | pass | terminal run (`node lib/internal/tools/check-phase-0a-scaffold.mjs`, exit `0`) |
| `validation.context-budget` | command | block | pass | terminal run (`node lib/internal/tools/context-budget-report.mjs`, exit `0`) |
...
| `validation.operator-output` | command | block | pass | terminal run (`node lib/internal/tools/check-operator-output.mjs`, exit `0`) |
...
1. **What:** Accept the review artifact and advance to the report stage.  
   **How:** Run:
   ...
   ```bash
   pnpm -w exec pan advance 16224_1929_bootstrap-de-hacking-pass-simplify-ops-docs-and-memory-layout --artifact work/172979_05-27-26/16224_1929_bootstrap-de-hacking-pass-simplify-ops-docs-and-memory-layout/review.md
   ```

2. **What:** Delegate the regenerated report prompt to `tech-writer`.  
   **How:** Read-only: open `work/172979_05-27-26/16224_1929_bootstrap-de-hacking-pass-simplify-ops-docs-and-memory-layout/next-prompt.md` after `advance` succeeds.
