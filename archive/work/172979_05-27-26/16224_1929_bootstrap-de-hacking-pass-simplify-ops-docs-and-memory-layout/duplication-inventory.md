...
| `parallel-agent-projections` | Persona canon vs Cursor projections | `lib/personas/<name>.md`, `.cursor/agents/<name>-{standard,complex}.md`, `.cursor/rules/<name>.mdc` | Persona file + handbook `persona-spec.md` | No edit — compact projections are retrieval stubs; operator examples live in persona canon |
| `overlapping-feature-specs` | Superseded `m1-substrate-runtime-batch` intake skeleton | `lib/memory/features/m1-substrate-runtime-batch/spec.md`, shipped `m1-substrate-runtime-batch-harness-loop-…` feature folder | `lib/memory/backlog/drafts/m1-substrate-runtime-batch.md` + harness feature index | Relocated draft to backlog; deleted duplicate feature folder (WP-1) |
