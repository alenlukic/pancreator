...
1. Fix standalone inbox migration write safety.
   - In lib/internal/tools/migrate-inbox-convention.mjs, standalone write mode must not recompute a fresh plan and directly apply it.
...
2. Fix or justify legacy thread discovery coverage.
   - Current concern: discovery may only cover lib/inbox/threads/<feature>/<file>.
   - Make discovery safely recursive for legacy thread directories if needed.
   - Do not traverse lib/inbox/notes.
