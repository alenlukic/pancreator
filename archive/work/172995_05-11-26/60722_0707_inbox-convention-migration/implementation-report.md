...
- `lib/internal/tools/migrate-inbox-convention.mjs`
- `tests/migrate-inbox-convention.test.mjs`
- `work/172995_05-11-26/60722_0707_inbox-convention-migration/implementation-report.md` (this file)
...
- **`--write` requires `--manifest <path>`** to a persisted JSON document with schema `pancreator.inbox-convention-migration-manifest.v1` (the normal dry-run output).
...
- **`PANCREATOR_MIGRATION_GO=1`** is still required for `--write`.
...
- Under each first-level legacy feature directory in `lib/inbox/threads/<feature>/`, discovery **recurses** into subdirectories for files.
...
- **Notes:** paths containing `/notes/` are never traversed (defensive; canonical threads live under `lib/inbox/threads`).
