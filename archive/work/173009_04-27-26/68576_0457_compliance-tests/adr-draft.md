...
Pancreator SHALL use `lib/internal/tests/compliance/` for compliance descriptors and `lib/internal/tests/compliance/schemas/` for descriptor schemas.

When duplicate descriptors or schemas exist under `lib/memory/features/compliance-tests/contracts/`, the implement stage MUST remove them or exclude them from staged output.
...
When severity is `low`, routing policy MUST create a backlog item and MUST emit warnings to `console` and `lib/inbox/out`.
