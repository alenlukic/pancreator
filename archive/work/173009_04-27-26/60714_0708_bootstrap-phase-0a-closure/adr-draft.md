...
references:
  - kind: lines
    path: lib/memory/features/bootstrap-phase-0a-closure/spec.md
    range: [49, 80]
...
When Phase 0a closure implements the scaffold, `coder` MUST use `pnpm` workspaces with catalogs, Turborepo, Changesets linked releases, `tsup --dts`, `@arethetypeswrong/cli`, `publint`, ESLint, and a local `@pancreator/no-horizontal-primitive-deps` rule.

When `coder` creates package skeletons, each package MUST receive only `package.json`, `README.md`, and `lib/index.ts` scaffold files. The scaffold MAY export versioned placeholder symbols, but it MUST NOT implement package runtime behavior.

When `coder` reconciles package inventory, existing `lib/internal/packages/@pancreator/*` directories MUST remain in place. The unscoped `lib/internal/packages/pancreator/` meta package MUST be added because PRD §11 names the `pancreator` meta package.
