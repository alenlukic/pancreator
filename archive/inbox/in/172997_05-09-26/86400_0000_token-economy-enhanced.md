...
references:
  ...
  - kind: path
    path: lib/memory/
    note: Existing durable memory tree; needs active/lib/internal/durable/archive separation.
---
...
The repo currently has useful context controls, including `.cursorindexingignore`, `PRD.summary.md`, `PRD.index.md`, `lib/memory/handbook/context-economy.md`, and `lib/internal/tools/context-budget-report.mjs`.

However, the underlying memory model is still too implicit.

In practice:

- `work/**` contains historical/generated execution artifacts but lives as a prominent top-level corpus.
- `lib/memory/**` mixes durable feature memory, ADRs, backlog, handbook pages, generated indexes, delivery reports, and current operating knowledge.
...
This task SHALL NOT break existing Pancreator pipeline behavior, run logging, inbox processing, Cursor custom agent discovery, or policy-compliance gates.

This task SHALL NOT perform risky large-scale path migrations without either compatibility shims or explicit deferral.

This task SHALL NOT make `PRD.md`, `BOOTSTRAP.md`, `lib/memory/**`, `work/**`, `lib/personas/**`, or `.cursor/agents/**` unavailable for explicit reads.
...
`lib/memory/active/README.md` explains what belongs in active memory and what does not.

`lib/memory/active/current.md` is a compact operator-facing current-state file. It should contain only short summaries and pointers.

`lib/memory/active/runs.md` tracks active/recent run pointers without embedding full run artifacts.
...
- the whole `lib/memory/active/` tier SHOULD stay under approximately 5,000 words unless deliberately ratified.
...
- Create the new `lib/memory/active/` tier.
- Document `work/**` as archival memory and explicit-read only.
- Document `lib/memory/features/**`, `lib/memory/adr/**`, and `lib/memory/backlog/**` as durable memory.
- Document `lib/memory/handbook/**`, `lib/personas/**`, `lib/personas/skills/**`, `.cursor/rules/**`, and `.cursor/agents/**` as internal operating content.
...
- `lib/memory/adr/000X-active-vs-archival-memory.md`
...
- why `work/**` is archival/explicit-read by default,
- why `lib/memory/active/**` exists,
...
- Route active-memory questions to `lib/memory/active/current.md`.
- Route memory-tier questions to `lib/memory/handbook/memory-tiers.md`.
- State that `lib/memory/active/**` is the only memory tier intended for routine default orientation.
- State that `work/**` is archival and explicit-read only.
...
This can live in `AGENTS.md`, `lib/memory/handbook/context-economy.md`, or a new concise rule/doc if that is cleaner.
...
- do not traverse `lib/memory/**`,
- do not traverse `work/**`,
...
- Move verbose detail into `lib/memory/handbook/**`.
...
- Remove `lib/personas/**/*.md` from `pancreator-engineer` unless the task is persona work.
- Avoid broad `lib/memory/handbook/**/*.md` triggers where a narrower route is sufficient.
- Keep `work/**` out of default rule activation except for rules that specifically operate on run artifacts.
...
Update `lib/internal/tools/context-budget-report.mjs` so it reports approximate footprint by memory tier.

Minimum required groups:

- active memory: `lib/memory/active/**`
- durable memory: `lib/memory/features/**`, `lib/memory/adr/**`, `lib/memory/backlog/**`
- archival memory: `work/**`, `lib/inbox/out/**`, `lib/inbox/threads/**`
- internal operating content: `lib/memory/handbook/**`, `lib/personas/**`, `lib/personas/skills/**`, `.cursor/rules/**`, `.cursor/agents/**`
...
- `lib/inbox/out/**` remains excluded.
- `lib/inbox/threads/**` remains excluded.
...
- `lib/memory/active/**` should generally remain indexable unless it becomes too large.
...
Add guidance somewhere appropriate, likely `lib/memory/handbook/context-economy.md` or `AGENTS.md`.
...
Record deferred migration work in `lib/memory/backlog/index.yaml`.

## Acceptance criteria

This task is complete when all of the following are true:

1. `lib/memory/handbook/memory-tiers.md` exists and defines active, durable, archival, internal, and generated-machine tiers.
2. `lib/memory/active/README.md` exists.
3. `lib/memory/active/current.md` exists and is intentionally small.
4. `lib/memory/active/runs.md` exists and stores pointers rather than copied artifacts.
5. A new ADR records the active-versus-archival memory decision.
6. `work/**` is explicitly documented as archival memory and explicit-read only by default.
7. Internal operating content is documented separately from active memory.
8. `lib/memory/handbook/context-economy.md` reflects the new tier model.
9. `lib/memory/handbook/index.md` routes to active memory and memory-tier guidance.
...
13. `pancreator-engineer` no longer broadly activates persona/handbook context unless justified.
14. `lib/internal/tools/context-budget-report.mjs` reports approximate footprint by tier.
