...
Implement a comprehensive context-economy pass that reduces default Cursor context exposure while preserving the Pancreator repo’s core operating model.
...
This task SHALL NOT hide `PRD.md`, `BOOTSTRAP.md`, `lib/memory/`, or `work/` from explicit human or agent access. The desired behavior is “not default indexed / not default loaded,” not “unavailable.”
...
lib/memory/**/*.json
lib/memory/**/delivery-report.md
lib/memory/**/index.json
...
The `pancreator-engineer` rule currently appears too broad. Replace broad corpus globs such as:

```yaml
globs: ["AGENTS.md", "BOOTSTRAP.md", "lib/memory/**/*", "lib/personas/**/*", "lib/personas/skills/**/*", "work/**/*", ".cursor/agents/**/*", ".cursor/rules/**/*"]
```
...
globs:
  - "lib/inbox/in/**/*.md"
  - "lib/internal/packages/@pancreator/**"
  - "lib/internal/packages/pancreator/**"
  - "tools/**/*.mjs"
  - "tests/**/*.ts"
  - "tests/**/*.mjs"
  ...
  - "lib/memory/handbook/**/*.md"
  - "lib/personas/skills/**/SKILL.md"
  - ".cursor/rules/*.mdc"
```

Do not include `work/**/*` in default engineering rule activation unless a rule specifically handles run logs, plans, reviews, or delivery reports.
...
- when to read `lib/memory/handbook/index.md`,
- when to read `lib/memory/features/**`,
- when to read `work/**`,
...
Do not delete existing `work/**` or generated artifacts.

Instead:

- ensure bulky generated files are excluded from Cursor semantic indexing,
- document that `work/**` is explicit-read only by default,
...
- record any deferred migration or architecture changes in `/lib/memory/backlog/index.yaml`.
...
```text
lib/internal/tools/context-budget-report.mjs
```
...
4. `work/**` is no longer part of default Cursor semantic indexing.
...
10. `lib/memory/handbook/context-economy.md` exists and is routed from `lib/memory/handbook/index.md`.
...
4. Confirm whether explicit references such as `@PRD.md`, `@BOOTSTRAP.md`, and selected `work/**` artifacts remain accessible when intentionally requested.
