...
references:
  ...
  - kind: path
    path: AGENTS.md
    note: `lib/inbox/in/` is the canonical incoming queue and phase boundaries require human ratification.
---
...
- no real US-1 dogfood run has been completed end-to-end from `lib/inbox/in/`
...
Place a real intake item in `lib/inbox/in/` and run the `feature-delivery`
...
- a delivery report staged under `lib/inbox/out/`;
...
1. A real inbox item under `lib/inbox/in/` has been processed end-to-end
...
6. A delivery report for the dogfood run exists under `lib/inbox/out/`.
...
2. inspect the delivery report under `lib/inbox/out/`;
