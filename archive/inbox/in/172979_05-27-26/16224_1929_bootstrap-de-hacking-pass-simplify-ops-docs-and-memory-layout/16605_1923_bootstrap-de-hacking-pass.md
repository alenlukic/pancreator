...
- Making `lib/memory/features/` truthfully represent implemented capabilities.
...
- Refactor or delete redundancies where a single canonical source exists per
  `lib/memory/handbook/context-economy.md` and `documentation-impact-contract.md`.
...
- Audit persona projections and handbook contracts for **Next operator steps**
  conformance per `lib/memory/handbook/operator-output-contract.md`.
...
- Merge **`lib/memory/debt/`** into backlog-only tracking; debt items are backlog
  items (tag or `category: debt` in backlog schema if useful).
- Remove or redirect empty `lib/memory/debt/` tree; update AGENTS workspace map,
...
- A feature spec (`lib/memory/features/bootstrap-de-hacking-pass/spec.md`) exists
...
- Rewriting historical `archive/work/` content.
- Reading or ingesting `lib/inbox/notes/` (human-only sandbox).
...
- Complements: `lib/inbox/in/172979_05-27-26/72021_0359_feature-delivery-cursor-runner-harness-wiring.md`
