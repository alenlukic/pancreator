...
4. **MCP read tools.** `@pancreator/mcp-server` wires four tools; eight return
...
2. Each `@pancreator/*` package declares a `test` script so `turbo run test`
...
1. `node lib/internal/tools/run-compliance.mjs [<descriptor>]` runs one or
...
5. With `--run-id`, the runner emits a JSON result under
   `work/<day>/<task-id>/` for dual-anchor audit evidence.

### Work package C — `bulk-contenthash-refresh`

1. `node lib/internal/tools/refresh-citations.mjs [--dry-run] [<glob>...]`
...
4. The tool is idempotent and refuses `lib/inbox/notes/`; it respects
   semantic immutability for `lib/inbox/{in,out,threads}/`.
5. A node-test suite under `tests/` covers all three citation surfaces.

### Work package D — `mcp-server-readonly-tool-wireup`

1. `pan.feature.list`, `pan.feature.show`, `pan.status`, and
   `pan.memory.query` return typed results (not stub envelopes).
...
4. `@pancreator/mcp-server` README documents the wired surface and Q18
...
- `node lib/internal/tools/run-compliance.mjs` runs all five descriptors
...
- `pan compliance run` CLI verb (node script is sufficient until M2).
...
|---|---|
| A — CI test aggregation | `pancreator-engineer` (aggregator + package scripts); `coder` (workflow); `reviewer` (runtime budget) |
| B — Compliance runner | `pancreator-engineer` (runner + CI); `compliance-auditor` (adapter contract); `reviewer` (exit codes) |
| C — Citation refresh | `pancreator-engineer` (tool + tests); `librarian` (post-merge sweep); `compliance-auditor` (m-02 closure) |
| D — MCP read tools | `tech-lead` (result envelope); `pancreator-engineer` (handlers + vitest); `reviewer` (no-write audit) |
