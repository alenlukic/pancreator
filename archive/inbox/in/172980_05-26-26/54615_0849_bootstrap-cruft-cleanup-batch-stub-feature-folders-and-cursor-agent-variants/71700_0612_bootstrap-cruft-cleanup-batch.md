...
references:
  - kind: path
    ...
    note: 17-line Phase-2 placeholder; identical pattern across 16 sibling pancreator-* feature folders.
  - kind: path
    path: .cursor/agents/
  ...
  - kind: path
    path: lib/memory/features/json-formatting/
...
|---|---|
| Resolve stub `@pancreator/*` feature folders | `resolve-package-stub-feature-folders` |
...
1. **Stub feature folders.** `lib/memory/features/` carries 17 folders of the
   form `pancreator-<pkg>/` where each `spec.md` is a 17-line shell pointing
   ...
   pollute listings that `librarian` and `pan feature` operate against.
...
2. **Triplicated Cursor agent files.** Every MVP persona has three Cursor
   ...
   canonical `lib/personas/<name>.md`. The variants encode a single
   standard-vs-complex distinction already defined in
   `lib/memory/handbook/subagent-model-tiers.md`. Operators must remember
...
1. A tech-lead-authored disposition table classifies each of the 17
   `pancreator-<pkg>/` folders into one of three buckets:
...
5. `lib/memory/features/index.json` is updated in lockstep.
...
2. Standard-vs-complex selection is expressed in persona metadata (for
   example `metadata.pancreator-subagent-tiers`) and resolved at invocation
...
5. `lib/memory/handbook/subagent-model-tiers.md` records the migration with
...
- A compliance-auditor broad sweep reports zero `block` findings tied to
  ...
  active-memory.
- The disposition table is referenced from `lib/memory/active/current.md`
