...
File-backed checkpoint envelopes under `<checkpoints-root>/<task_id>/<seq>.json` with a required `metadata.run_log_offset` that ties to the JSONL run log (see `/memory/handbook/run-log-schema.md`).
