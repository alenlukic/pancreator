...
export function defaultRegistryFilePath(repoRoot: string): string {
  return path.resolve(repoRoot, ".tess", "sandboxes", "port-registry.json");
}

export function defaultSandboxesDir(repoRoot: string): string {
  return path.resolve(repoRoot, ".tess", "sandboxes");
}

/** Ensures `registryFilePath` resolves inside `repoRoot/.tess/sandboxes`. */
