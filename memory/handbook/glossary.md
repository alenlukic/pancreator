...
`/memory/adr/draft/0003-glossary-divergence.md`.

## 1 — Personas, cohorts, and authoring roles

- **Persona** — a named subagent specification. One Markdown file under
  `personas/<name>.md` whose YAML frontmatter conforms to the Anthropic Claude
  ...
  (SMEs). See `/memory/handbook/persona-spec.md`.
- **Persona Spec Format** — the 16-field YAML frontmatter plus a Tesseract
  `metadata` map. Authored once at `personas/<name>.md`; a build step emits a
...
- **SME (Subject-Matter Expert)** — a long-lived persona that owns a knowledge
  domain and accumulates a private memory under `/memory/smes/<name>/`. SMEs
...
- **Skill** — a reusable procedure under `skills/<name>/SKILL.md` conforming to
...
- **Pipeline** — a declarative workflow under `/pipelines/<name>.yaml` that
...
- **Checkpoint** — a serialized snapshot of pipeline state at a stage boundary.
  ...
  `/memory/checkpoints/<task-id>/<seq>.json`.
...
- **Layer 3 template** — slot-driven scaffold under
  `/memory/handbook/contract-templates/`. Authors fill slots; they do not
...
- **Weasel word** — a hedging adverb or adjective that defeats RFC 2119
  ...
  *and so on*, *user-friendly*, *modern*, *robust*, *seamless*. The list is
  versioned in `/memory/handbook/contract-style.md`.
...
- **Memory** — the union of `/memory/` (semantic and procedural),
  `/memory/smes/` (per-SME), `/memory/features/` (per-feature),
  `/memory/backlog/`, `/inbox/` (episodic), and the codebase itself.
- **Memory tiers** — *procedural* (`/memory/handbook/`, `/personas/`,
...
- **Feature** — a first-class entity under `/memory/features/<id>/` binding
  every artifact for a single product capability. Has a unique slug and a
  generated `index.json`.
- **Backlog** — `/memory/backlog/`. The live, ranked product roadmap. Owned
...
- **Rename a term.** Open an inbox item to `librarian` proposing the rename.
  ...
  records the change under `/memory/adr/<seq>-glossary-rename-<term>.md`.
