...
related:
  ...
  - /skills/author-persona/SKILL.md
external:
...
A persona spec is one Markdown file under `personas/<name>.md`. Frontmatter is
...
|---|---|---|---|---|
| 1 | `name` | string | required | Lowercase-kebab-case, unique across `personas/`. |
...
| 9 | `skills` | string[] | `[]` | Each entry MUST resolve to an existing `skills/<name>/SKILL.md`. |
| 10 | `isolation` | enum | `worktree` | `none` only for read-only personas. |
| 11 | `memory` | enum | `project` | `private` for SMEs writing to `/memory/smes/<name>/`. |
...
`personas/tech-writer.md` against this spec. Human reviews:
