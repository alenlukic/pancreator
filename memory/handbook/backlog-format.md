...
purpose: |
  The canonical schema for `/memory/backlog/index.yaml`, including required
...
references:
  - kind: lines
    path: memory/adr/0001-backlog-tracking.md
    range: [76, 81]
    contentHash: TBD-on-commit
    note: "ADR decision establishes `/memory/backlog/index.yaml` and this handbook schema."
  - kind: lines
    path: AGENTS.md
    range: [77, 83]
    contentHash: TBD-on-commit
...
related:
  - /memory/adr/0001-backlog-tracking.md
...
metadata:
  ...
  owners: [tech-lead, librarian]
  source_of_truth: /memory/backlog/index.yaml

items:
  - id: bootstrap-prd-hash-refresh
    title: Refresh TBD-on-commit content hashes
    source: memory/adr/0001-backlog-tracking.md
    owner: librarian
    milestone: phase-1
    status: open
    priority: high
