...
related:
  ...
  - /skills/author-contract/SKILL.md
external:
...
`/memory/handbook/contract-style.md`. This file is the structural reference.
...
owner: <persona-name>                          # required; MUST exist in personas/
...
applies_to:
  ...
  path: /memory/features/checkout/spec.md
...
`/memory/handbook/contract-style.md` Layer 2.
...
runtime:
  ...
  spec: /memory/features/<id>/contracts/<id>.spec.ts
```
...
schemas are sketched under `/memory/handbook/contract-templates/` when their
...
spec: /memory/features/tesseract-core/contracts/no-horizontal-deps.rego
runtime:
    ...
    examples:
      good:
        - text: |
            ...
            ```
            Then create `personas/me.md` with the 16 frontmatter fields and
  ...
  "clauses": [
    {
      ...
      "path": "/memory/features/checkout/contracts/401-on-missing-token.yaml",
      "owner": "appsec",
      "appliesTo": {
        "kind": "artifact-symbol",
...
M1; new fields require an RFC under `/memory/rfc/draft/` and a major version
