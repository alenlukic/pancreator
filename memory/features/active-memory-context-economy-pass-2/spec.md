...
source_inbox_item: inbox/in/token-economy-enhanced.md
...
intake_notes:
  ...
  - The intake-analyst MUST NOT modify the inbox source item per the
    ...
    at `inbox/in/token-economy-enhanced.md`.
...
references:
  ...
  - kind: lines
    ...
    note: "Operator-sandbox immunity rule excludes /inbox/notes/ from agent traversal across every pass 2 edit."
  - kind: lines
    ...
    note: "Policy-compliance contract requires `/work/<task-id>/policy-compliance.json` for non-`work/` structural changes."
  - kind: lines
...
`{kind: lines, path: memory/features/cursor-token-economy/spec.md, range: [121, 224], contentHash: f44cc8ebe621e6c9aadc207abde8026773fe9fdc27012957c43a7c861f6f3449}`
with an explicit memory-tier taxonomy, a new `memory/active/` operator surface,
...
the directive at
`{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [99, 478], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
...
This Feature SHALL keep `PRD.md`, `BOOTSTRAP.md`, the `memory/` tree, the
`work/` tree, the `personas/` tree, and the `.cursor/agents/` tree reachable
for explicit human and agent reads per the non-goals at
`{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [89, 97], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
...
- When the Feature lands the memory-tier model, the Feature MUST create one
  new handbook page at `memory/handbook/memory-tiers.md`.
- When the Feature authors `memory/handbook/memory-tiers.md`, the page MUST
  ...
  directive at
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [101, 229], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
...
- When the Feature creates the active-memory tier, the Feature MUST create the
  directory `memory/active/`.
- When the Feature creates the active-memory tier, the Feature MUST add one
  file at `memory/active/README.md` that explains tier scope and exclusions.
- When the Feature creates the active-memory tier, the Feature MUST add one
  file at `memory/active/current.md` that contains short summaries and
  pointers without copied artifacts.
- When the Feature creates the active-memory tier, the Feature MUST add one
  file at `memory/active/runs.md` that stores active-run pointers without
...
- When the Feature documents the active-memory soft budgets, the page MUST
  state that the entire `memory/active/` tier SHOULD remain at most 5000
...
- When the Feature classifies existing paths, the Feature MUST classify
  `work/**`, `inbox/out/**`, and `inbox/threads/**` as archival memory.
- When the Feature classifies existing paths, the Feature MUST classify
  `memory/features/**`, `memory/adr/**`, and `memory/backlog/**` as durable
  memory.
- When the Feature classifies existing paths, the Feature MUST classify
  `memory/handbook/**`, `personas/**`, `skills/**`, `.cursor/rules/**`, and
...
- When the Feature evaluates physical migration of existing directory trees,
  ...
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [254, 276], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
...
- When the Feature ratifies the memory-tier model, the Feature MUST author one
  ...
  `memory/adr/000X-active-vs-archival-memory.md` where `X` is the next
...
- When the Feature authors the new ADR, the ADR MUST record the problem
  ...
  by default, the reason `memory/active/**` exists, the active-to-durable
  promotion rule, the archival accessibility rule, and the deferred migrations
  per the directive at
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [278, 294], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.

### Context-economy and handbook index updates

- When the Feature updates the context-economy handbook page, the Feature
  MUST extend `memory/handbook/context-economy.md` with the memory-tier
  routing rule.
- When the Feature updates the context-economy page, the page MUST route
  active-memory questions to `memory/active/current.md`.
- When the Feature updates the context-economy page, the page MUST route
  memory-tier questions to `memory/handbook/memory-tiers.md`.
- When the Feature updates the context-economy page, the page MUST state that
  `memory/active/**` is the only memory tier intended for routine default
  orientation.
- When the Feature updates the context-economy page, the page MUST state that
...
- When the Feature updates the handbook routing index, the Feature MUST add
  one row to `memory/handbook/index.md` that routes active-memory questions
  to `memory/active/current.md`.
- When the Feature updates the handbook routing index, the Feature MUST add
  one row to `memory/handbook/index.md` that routes memory-tier questions to
...
- When the Feature defines `simple task mode`, the Feature MUST publish one
  ...
  `memory/handbook/context-economy.md`, or in a new dedicated handbook page
  per the directive at
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [313, 352], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
...
- When the Feature defines `simple task mode` default behavior, the page MUST
  instruct agents not to traverse `memory/**` while `simple task mode`
  applies.
- When the Feature defines `simple task mode` default behavior, the page MUST
...
- When the Feature defines `simple task mode` escalation, the page MUST list
  ...
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [344, 351], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
...
- When the Feature audits `.cursor/rules/*.mdc`, the audit MUST cover at least
  ...
  at
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [378, 402], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
- When the Feature narrows `.cursor/rules/tesseract-engineer.mdc`, the
  Feature MUST remove `personas/**/*.md` from the default activation set
...
- When the Feature audits the rule files, the Feature MUST avoid broad
  `memory/handbook/**/*.md` triggers when a narrower route resolves the rule
  intent.
...
- When the Feature extends the context-budget report, the report MUST emit a
  per-tier footprint group for durable memory covering `memory/features/**`,
  `memory/adr/**`, and `memory/backlog/**`.
- When the Feature extends the context-budget report, the report MUST emit a
  per-tier footprint group for archival memory covering `work/**`,
  `inbox/out/**`, and `inbox/threads/**`.
- When the Feature extends the context-budget report, the report MUST emit a
...
- When the Feature audits `.cursorindexingignore`, the policy MUST keep
  `work/**`, `inbox/out/**`, and `inbox/threads/**` excluded from default
...
- When the Feature audits `.cursorindexingignore`, the policy MUST keep
  `memory/active/**` indexable by default.
...
- When the Feature documents escalation guidance, the Feature MUST publish
  one section in either `memory/handbook/context-economy.md` or `AGENTS.md`
  per the directive at
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [446, 457], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
...
- When the Feature applies the documentation-impact contract, the Feature
  ...
  `.cursorindexingignore`, `memory/handbook/index.md`,
  `memory/handbook/context-economy.md`, `memory/handbook/memory-tiers.md`,
  the three new `memory/active/*.md` files, the new active-versus-archival
  ADR, `README.md`, and `memory/backlog/index.yaml`.
- When the Feature defers a documentation-impact update, the Feature MUST
  ...
  `{kind: lines, path: memory/handbook/documentation-impact-contract.md, range: [42, 102], contentHash: 38ed8213e11f7aa1f53d46588cc55cad7da746016c52aa1747f03d8b97d16248}`.
- When the Feature stages every governed change, the Feature MUST stage one
  policy-compliance artifact at `/work/<task-id>/policy-compliance.json` per
  the policy-compliance contract cited at
  `{kind: lines, path: memory/handbook/policy-compliance-contract.md, range: [47, 98], contentHash: 971f594cd9fb1c1cbc9da86b6104d6fff81b9c723cd9266766687b63277a3d67}`.
...
- When the Feature reads, traverses, or cites repository content, the
  Feature MUST NOT touch any path under `/inbox/notes/` per the operator
  sandbox rule cited at
  `{kind: lines, path: memory/handbook/inbox-lifecycle.md, range: [79, 100], contentHash: bb2a660a67aaa86d36890a0c3c8dcc2d995b68334a14b42546c44c7dae496f29}`.
...
- When the Feature stages the delivery report, the report MUST request the
  ...
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [505, 515], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
- When the Feature stages the delivery report, the report MUST cite the
  directional cache-read token bands declared at
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [517, 535], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`
...
- This Feature SHALL NOT delete historical artifacts under `work/**`,
  `memory/**`, `inbox/out/**`, or `inbox/threads/**`.
- This Feature SHALL NOT weaken policy-compliance, documentation-impact,
  inbox-lifecycle, or human-ratification requirements.
- This Feature SHALL NOT hide `PRD.md`, `BOOTSTRAP.md`, the `memory/` tree,
  the `work/` tree, the `personas/` tree, or the `.cursor/agents/` tree from
...
- This Feature SHALL NOT redesign the canonical inbox lifecycle states
  defined at
  `{kind: lines, path: memory/handbook/inbox-lifecycle.md, range: [102, 113], contentHash: bb2a660a67aaa86d36890a0c3c8dcc2d995b68334a14b42546c44c7dae496f29}`.
...
- D1. Choose the single canonical surface that publishes `simple task mode`:
  an `AGENTS.md` section, a new `memory/handbook/context-economy.md` section,
  or a new dedicated handbook page. The directive authorizes the
  implementation to choose at
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [315, 318], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
- D2. Choose the next available ADR sequence number for
  `memory/adr/000X-active-vs-archival-memory.md`. The directive authorizes the
  implementation to choose at
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [282, 284], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
- D3. Choose whether to ship a lightweight active-memory budget warning
  ...
  choose at
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [248, 252], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
- D4. Choose the canonical surface for model-and-context escalation guidance:
  `memory/handbook/context-economy.md` or `AGENTS.md`. The directive
  authorizes the implementation to choose at
  `{kind: lines, path: inbox/in/token-economy-enhanced.md, range: [446, 449], contentHash: fb1ac76d5d8075ee087808b7efbab96242534524d161f26db68b75f21ae37051}`.
...
- The Feature MAY defer the memory-tier addition to
  `memory/handbook/glossary.md` to a documentation-impact backlog item when
...
- This Feature extends `memory/features/cursor-token-economy/`. The
