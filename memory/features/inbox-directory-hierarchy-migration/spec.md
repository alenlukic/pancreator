...
intake_round: 1
intake_thread: inbox/threads/inbox-directory-hierarchy-migration/
...
references:
  ...
  - kind: lines
    ...
    note: "Operator-sandbox immunity rule excludes inbox/notes/ from agent traversal and migration scope."
  - kind: lines
    path: memory/handbook/inbox-lifecycle.md
    range: [142, 158]
...
day-bucketed directory hierarchy that mirrors the `work/` layout already
ratified in
`{kind: lines, path: memory/adr/0005-timestamp-naming-conventions.md, range: [36, 51], contentHash: 64d620e338d077214f03804c2b0fe6f9272be895a9d56e551f2d89cfd6915851}`,
SHALL preserve every basename token defined in
`{kind: lines, path: memory/features/timestamp-naming-conventions/spec.md, range: [114, 129], contentHash: 0c47ddf6df2f9236d65509cdba0d225dfd86a7a3494d6572b23520f504864d17}`,
...
- When the Feature evaluates directory-hierarchy scope, the Feature MUST
  include `inbox/in/`, `inbox/out/`, `inbox/threads/`, and
  `inbox/archive/in/`.
- When the Feature evaluates directory-hierarchy scope, the Feature MUST
  exclude `inbox/notes/` because `/inbox/notes/` is an operator-only
  sandbox per the rule cited at
  `{kind: lines, path: memory/handbook/inbox-lifecycle.md, range: [79, 100], contentHash: 6f313f6aeb223acb8a8b67563b501b47fa1c43f68de09f083baa401d258914fb}`.
- When the Feature evaluates directory-hierarchy scope, the Feature MUST
  exclude every path outside `inbox/` because the directive named only the
  inbox subtree.
...
- When the Feature derives `days-to-FDS`, the Feature MUST use the
  ...
  `{kind: lines, path: memory/features/timestamp-naming-conventions/spec.md, range: [50, 70], contentHash: 0c47ddf6df2f9236d65509cdba0d225dfd86a7a3494d6572b23520f504864d17}`.
...
- When the Feature places a single-file artifact inside a day bucket, the
  ...
  `{kind: lines, path: memory/features/timestamp-naming-conventions/spec.md, range: [77, 90], contentHash: 0c47ddf6df2f9236d65509cdba0d225dfd86a7a3494d6572b23520f504864d17}`.
- When the Feature encounters a non-conforming human-generated artifact in
  `inbox/in/` or `inbox/threads/`, the agent performing the move MUST
  apply the time-prefix actor-ownership rule cited at
  `{kind: lines, path: memory/handbook/inbox-lifecycle.md, range: [160, 174], contentHash: c96a49ed17a85976ed44098a74adbf83a1a73c240d83c4ba9343eed05d469903}`
...
- When the Feature migrates `inbox/threads/<feature-slug>/`, the Feature
...
- When the Feature places day buckets inside the threads subtree, the
  ...
  `inbox/threads/<feature-slug>/{days-to-FDS}_{MM-DD-YY}/<round-file>.md`
...
- When the Feature migrates an existing artifact, the Feature MUST
  ...
  cited at
  `{kind: lines, path: memory/handbook/inbox-lifecycle.md, range: [142, 158], contentHash: 1a108e8636f13b89cc4fa570ee7bf806c6ce2ea20e05f64af438a6fe5fa3f953}`.
...
- When the Feature encodes the policy, the Feature MUST author or extend
  ...
  `memory/features/inbox-directory-hierarchy-migration/contracts/`
  covering policy assertion, migration precedence, and reference-update
  audit per the contract format cited at
  `{kind: lines, path: memory/handbook/inbox-lifecycle.md, range: [67, 77], contentHash: 15aa6412f0028b7f96f214977adfe329cc397b95c1c19d47613e474080eaafeb}`.
...
- When the Feature lands the new layout, the Feature MUST update
  `memory/handbook/inbox-lifecycle.md` Section 1 canonical-locations
  enumeration to show the day-bucket pattern.
- When the Feature lands the new layout, the Feature MUST update the
  glossary Inbox entry at
  `{kind: lines, path: memory/handbook/glossary.md, range: [231, 234], contentHash: 32977d62644e45572bce949eac7078f0ce59595e2ec65f2e11613828ad4c31cb}`
  to reference the hierarchical layout.
- When the Feature lands the new layout, the Feature MUST update the
  `Out of scope` section of
  `memory/features/timestamp-naming-conventions/spec.md` with a
...
- When the Feature lands the new layout, the Feature MUST author one new
  ADR `memory/adr/0006-inbox-directory-hierarchy.md` that extends ADR-0005
...
- This Feature does not modify, traverse, or migrate any path under
  `inbox/notes/` because that subtree remains operator-only per the rule
  cited at
  `{kind: lines, path: memory/handbook/inbox-lifecycle.md, range: [79, 100], contentHash: 6f313f6aeb223acb8a8b67563b501b47fa1c43f68de09f083baa401d258914fb}`.
- This Feature does not modify the `work/` directory layout or the
...
- A1. Single-file artifacts sit directly inside a day bucket with the
  ...
  items emit one file each, while `work/` task directories emit several.
  Tied to Q1.
- A2. Day buckets nest inside `inbox/threads/<feature-slug>/` so the
...
- A7. The Feature applies the day-bucket hierarchy retroactively to every
  artifact already in `inbox/archive/in/` because reverse-chronological
...
`inbox/threads/inbox-directory-hierarchy-migration/11700_2045_round-01-clarify.md`.
...
- Q1. Single-file leaf placement versus per-artifact `HHMM` subdirectory.
  ...
  require strict folder-per-item parity with `work/`?
...
- Q6. Migration ordering: one atomic migration manifest covering
  `inbox/in/`, `inbox/out/`, `inbox/threads/`, and `inbox/archive/in/`
...
- Q7. Archive subtree handling: apply the day-bucket hierarchy
  retroactively to every artifact already in `inbox/archive/in/` (A7),
...
- This Feature extends `memory/features/timestamp-naming-conventions/`.
  The relationship is `extends-deferred-scope`, recorded in
  `index.json`.
- The directive at
  `inbox/in/inbox_convention_migration.md` cites
  `personas/tesseract-engineer.md` for downstream implementation
