...
- First-slice action: operator or agent manually runs descriptors in
  `internal/tests/compliance/` immediately after the structure change lands.
...
2. Select trigger mode: `structure-change` or `operator-on-demand`; keep
   ...
   lands.
3. Evaluate each descriptor in `internal/tests/compliance/*.yaml` against
