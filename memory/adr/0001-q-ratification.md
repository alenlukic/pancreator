...
at `memory/adr/0001-q-ratification.md`.
...
| Q10 | `/memory/backlog/` is canonical; M3 adds sync adapters | ratified |
...
| Q17 | `personas/*.md` is canonical; emit `.cursor/agents` mirror; keep `.mdc` only where rule-loading still requires it | ratified |
