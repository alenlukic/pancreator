...
references:
  ...
  - kind: lines
    ...
    range: [101, 103]
    contentHash: TBD-on-commit
    note: "AGENTS defines `/inbox/in/` as the canonical incoming work queue and `/inbox/out/` as staged responses."
  - kind: lines
    path: AGENTS.md
    range: [130, 130]
    contentHash: TBD-on-commit
  ...
  - kind: lines
    ...
    range: [49, 53]
    contentHash: TBD-on-commit
    note: "Bootstrap scaffold includes `inbox/{in,out,threads}/` as a required repository substrate."
  - kind: lines
...
artifact that enters `/inbox/in/`:

- `new`: the item exists in `/inbox/in/` and no operator has claimed active
  execution.
- `in_progress`: an operator has claimed active execution and has begun
  producing the response artifact.
- `responded`: the required response artifact is present in `/inbox/out/`, and
  the inbound item is complete for the current cycle.
- `archived`: the inbound item has been moved from `/inbox/in/` to
  `/inbox/archive/in/` after the response is produced.
...
- Current mechanism (bootstrap and pre-runtime): operators MUST perform archive
  transitions manually by moving responded inbound files from `/inbox/in/` to
  `/inbox/archive/in/`.
...
`/inbox/in/` as the active queue and `/inbox/archive/in/` as the canonical
...
- negative: Existing tools that only read `/inbox/in/` MUST be updated before
