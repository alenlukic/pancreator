...
references:
  ...
  - kind: path
    path: memory/
    note: Existing durable memory tree; needs active/internal/durable/archive separation.
---
...
- `memory/**` mixes durable feature memory, ADRs, backlog, handbook pages, generated indexes, delivery reports, and current operating knowledge.
...
This task SHALL NOT make `PRD.md`, `BOOTSTRAP.md`, `memory/**`, `work/**`, `personas/**`, or `.cursor/agents/**` unavailable for explicit reads.
...
`memory/active/README.md` explains what belongs in active memory and what does not.

`memory/active/current.md` is a compact operator-facing current-state file. It should contain only short summaries and pointers.

`memory/active/runs.md` tracks active/recent run pointers without embedding full run artifacts.
...
- the whole `memory/active/` tier SHOULD stay under approximately 5,000 words unless deliberately ratified.
...
- Create the new `memory/active/` tier.
- Document `work/**` as archival memory and explicit-read only.
- Document `memory/features/**`, `memory/adr/**`, and `memory/backlog/**` as durable memory.
- Document `memory/handbook/**`, `personas/**`, `skills/**`, `.cursor/rules/**`, and `.cursor/agents/**` as internal operating content.
...
- `memory/adr/000X-active-vs-archival-memory.md`
...
- Route active-memory questions to `memory/active/current.md`.
- Route memory-tier questions to `memory/handbook/memory-tiers.md`.
- State that `memory/active/**` is the only memory tier intended for routine default orientation.
...
This can live in `AGENTS.md`, `memory/handbook/context-economy.md`, or a new concise rule/doc if that is cleaner.
...
- Move verbose detail into `memory/handbook/**`.
...
- Remove `personas/**/*.md` from `tesseract-engineer` unless the task is persona work.
- Avoid broad `memory/handbook/**/*.md` triggers where a narrower route is sufficient.
...
- active memory: `memory/active/**`
- durable memory: `memory/features/**`, `memory/adr/**`, `memory/backlog/**`
- archival memory: `work/**`, `inbox/out/**`, `inbox/threads/**`
- internal operating content: `memory/handbook/**`, `personas/**`, `skills/**`, `.cursor/rules/**`, `.cursor/agents/**`
...
- `inbox/out/**` remains excluded.
- `inbox/threads/**` remains excluded.
...
- `memory/active/**` should generally remain indexable unless it becomes too large.
...
Add guidance somewhere appropriate, likely `memory/handbook/context-economy.md` or `AGENTS.md`.
...
Record deferred migration work in `memory/backlog/index.yaml`.

## Acceptance criteria

This task is complete when all of the following are true:

1. `memory/handbook/memory-tiers.md` exists and defines active, durable, archival, internal, and generated-machine tiers.
2. `memory/active/README.md` exists.
3. `memory/active/current.md` exists and is intentionally small.
4. `memory/active/runs.md` exists and stores pointers rather than copied artifacts.
...
8. `memory/handbook/context-economy.md` reflects the new tier model.
9. `memory/handbook/index.md` routes to active memory and memory-tier guidance.
